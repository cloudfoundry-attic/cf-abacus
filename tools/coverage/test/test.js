...
describe('cf-abacus-coverage', () => {
    it('reports overall code coverage', (done) => {
        ...
        process.exit = (code) => {
            ...
            expect(reporters[0].addAll.args[0][0]).to.deep.equal(['lcov', 'json']);
            ...
            expect(files[0]).to.equal('index.js');
        ...
        fs.mkdir('.coverage', () => fs.readFile('test/coverage.json', (err, val) => fs.writeFile('.coverage/coverage.json', val, () => {
