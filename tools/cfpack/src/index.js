    ...
    makeSymlink(path.join(root, 'lib'), '.cfpack/lib', () => {
      if (err) noop();
        ...
        fs.access(additionalDirPath, (err) => {
          if (err) cb();
          else makeSymlink(additionalDirPath, path.join('.cfpack', additionalDir), cb);
        });
      }
    });
  });
...
const local = (root, additional, d) => {
  if (!/file:/.test(d[1])) return d;
  const vendoredPath = path.resolve(d[1].substr(5)).replace(additional, additionalDir);
  const dependencyPath = path.join('file:.cfpack', path.relative(root, vendoredPath));
...
const fixShwinkwrapDeps = (deps, loc) => {
  map(pairs(deps), (dep) => {
    ...
    if (/file:/.test(newPath)) deps[name].from = newPath.substr(5);
    if (val.dependencies) fixShwinkwrapDeps(val.dependencies, loc);
  });
};
...
const repackage = (root, additional, cb) => {
  ...
  fs.writeFile(path.join('.cfpack', 'package.json'), JSON.stringify(rmod, undefined, 2), (err) => {
    const shWrpPath = path.join(process.cwd(), 'npm-shrinkwrap.json');
    if (err || !fs.existsSync(shWrpPath)) cb(err);
    else {
      const shrinkwrap = require(shWrpPath);
      fixShwinkwrapDeps(shrinkwrap.dependencies, loc);
      fs.writeFile(path.join('.cfpack', 'npm-shrinkwrap.json'), JSON.stringify(shrinkwrap, undefined, 2), cb);
    }
  });
};
...
const executeZip = (directories, ignore, cb) => {
  ...
  const files =
    '-type f -not -regex "\\./\\.cfpack/package\\.json" ' +
...
const zip = (ignore, include, cb) => {
  fs.unlink(path.resolve('.cfpack', 'app.zip'), (err) => {
    ...
    fs.access(include, (err) => {
      if (include && include.length !== 0) directories += err ? '' : ` ${include}/*`;
...
const rootDir = (dir) => {
  if (dir === '/') return dir;
  try {
    if (JSON.parse(fs.readFileSync(path.resolve(dir, 'package.json')).toString()).name === 'cf-abacus') return dir;
...
const runCLI = () => {
  ...
  commander
    ...
    .option('-r, --root <dir>', 'root local dependencies directory', process.env.ABACUS_ROOT || rootDir(process.cwd()))
    .option('-a, --additional <dir>', 'additional directory that will be packed', process.env.ADDITIONAL_PACK_DIR)
    .parse(process.argv);
