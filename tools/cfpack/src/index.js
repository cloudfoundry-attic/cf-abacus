...
var local = function(root, d) {
    return !/file:/.test(d[1]) ? d : [d[0], path.join('file:.cfpack', path.relative(root, path.resolve(d[1].substr(5))))];
};
...
var repackage = function(root, cb) {
    ...
    fs.writeFile(path.join('.cfpack', 'package.json'), JSON.stringify(rmod, undefined, 2), cb);
};
...
var zip = function(ignore, cb) {
    fs.unlink(path.resolve('.cfpack', 'app.zip'), function(err) {
        ...
        var files = '-type f -not -regex "\\./\\.cfpack/package\\.json" -not -regex ".*/\\.git"';
        var ex = cp.exec('(find . .cfpack/lib/* ' + files + ' | zip -q -x@' + ignore + ' -@ .cfpack/app.zip) && ' + '(zip -q -j .cfpack/app.zip .cfpack/package.json)', { cwd: process.cwd() });
...
var rootDir = function(dir) {
    if(dir === '/') return dir;
    try {
        if(JSON.parse(fs.readFileSync(path.resolve(dir, 'package.json')).toString()).name === 'cf-abacus')
            return dir;
