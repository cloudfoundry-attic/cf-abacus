...
var runCLI = function(stdin, stdout) {
    ...
    fs.readFile(path.resolve(__dirname, '../html/index.html'), function(err, template) {
        ...
        sin.on('end', function() {
            ...
            var html = hogan.compile(template.toString()).render({ __dirname: path.relative(process.cwd(), path.resolve(__dirname, '..')), markdown: md.render(input.join()) });

            // Inline all resources into a single self contained HTML page
            var s = new stream.Readable();
            s.push(html);
            s.push(null);
            fs.createReadStream = wrap(fs.createReadStream, function(fscrs, file) {
                // Get external resources using the request module
                var r = /.*\/(.*):\/(.*)/.exec(file);
                return r ? request.get(r.slice(1, 3).join('://')) : fscrs(file);
            });
            s.pipe(inliner({ basedir: process.cwd() })).
