...
var mkdirs = function(pubdir, cb) {
    fs.mkdir(pubdir, function() {
        fs.mkdir(path.resolve(pubdir, 'package'), function() { cb(); });
    });
};
...
var repackage = function(mod, pubdir, cb) {
    ...
    fs.unlink(pkg, function() {
        var rmod = extend(clone(mod), { private: false, dependencies: publicize(mod.dependencies), devDependencies: publicize(mod.devDependencies) });
...
var publish = function(tgz, pubdir, cb) {
    ...
    var ex = cp.exec('gunzip ' + tgz + ' && tar -uf ' + tar + ' package && gzip -c ' + tar + ' > ' + tgz + ' && rm ' + tar + ' && npm publish ./' + tgz, { cwd: pubdir });
    ex.stdout.on('data', function(data) { process.stdout.write(data); });
    ex.stderr.on('data', function(data) { process.stderr.write(data); });
