...
const verifyManifest = (manifest) => {
  if (!manifest.applications) throw new Error('Invalid application manifest yaml. No applications.', manifest);

  if (!manifest.applications[0].path) throw new Error('Invalid application manifest yaml. No path.', manifest);
};
...
const adjustManifest = (manifest, properties) => {
  ...
  if (!properties || Object.keys(properties).length == 0) return manifest;
