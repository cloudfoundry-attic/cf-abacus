...
const inside = (pos, spans) => reduce(spans, (i, span) => i ||
  !(pos.line < span.start.line || pos.line > span.end.line ||
  pos.line === span.start.line && pos.column < span.start.column ||
  pos.line === span.end.line && pos.column >= span.end.column), false);

// Return the next position in the source document given a character
const nextpos = (pos, c) => c === '\n' ? {
  line: pos.line + 1,
  column: 0
} : {
  line: pos.line,
  ...
  const markedSource = (accum, c) => {
    ...
    return {
      source: accum.source.concat(
        mark !== accum.mark ? [accum.mark.end, mark.start, c] : [c]
      ),
...
const printCoverage = (coverage, sources, opt) => {
  map(filter(values(coverage), inThisModule), (cov) => {
    ...
    if (!fullcov) {
      ...
      const branches = zip(flatten(values(cov.b)),
        flatten(map(values(cov.branchMap), (b) => {
          return b.locations;
        })));
