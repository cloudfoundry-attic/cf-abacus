...
var inAbacusModule = function(file) {
  return inThisModule(file) || /\/abacus-[^\/]*\/lib\/[^\/]*\.js$/.test(file) ||
    /\/abacus-[^\/]*\/lib\/[^\/]*\/[^\/]*\.js$/.test(file);
};
...
var transformer = function(sources, maps) {
  ...
  var instrument = function(code, file) {
    if (!noinstrument && inAbacusModule(file) && !inTestDir(file)) {
      process.stdout.write(util.format(
        'Running Istanbul instrumentation on %s\n', path.relative(
          process.cwd(), file)));
  ...
  return function(code, file) {
    ...
    sources[file] = fs.readFileSync(lib.slice(0, l).concat(['src']).concat(
      lib.slice(l + 1)).join('/')).toString();
...
var remap = function(coverage, maps) {
  _.map(_.values(coverage), function(cov) {
    ...
    _.map(_.values(cov.statementMap), function(s) {
      reloc(s);
    });
...
var runCLI = function() {
  ...
  var mocha = new Mocha(process.env.MOCHA_COLORS ? {
    useColors: true,
    timeout: 20000
  } : {});
  ...
  mocha.run(function(failures) {
    ...
    reporter.write(collector, false, function() {
      fs.writeFileSync('.coverage/coverage.json', JSON.stringify(
        coverage));
