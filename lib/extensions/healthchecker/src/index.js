...
const createHealthcheckRoute = (router, route, applicationsFilter, yHealthchecker) => {
  router.get(route, throttle(function*() {
    const healthchecker = yield yHealthchecker();
    return {
...
const startApp = () => {
  ...
  const healthchecker = createHealthchecker(applicationGroups, 
    applicationHealthClient, 
    applicationsUrisBuilder
  );

  const yhealthcheck = yieldable(healthchecker);

  const healthCheckFilter = createHealthcheckFilter(internalApplications);
  const app = healthcheckerApplication(yhealthcheck, healthCheckFilter);
