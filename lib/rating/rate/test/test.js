...
describe('cf-abacus-usage-rate', () => {
    describe('validate usage rating', () => {
        it('rate a usage', function(done) {
            ...
            const u = { organization_guid: 'a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf27', start: 1435968000000, end: 1436054400000,
            ...
            const post = (done) => {
                ...
                // Post the usage doc
                request.post('http://localhost::p/v1/rating/usage', { p: server.address().port, body: u }, (err, val) => {
            ...
            const get = (done) => {
                ...
                request.get(locations[u.id], {}, (err, val) => {
                    ...
                    expect(omit(val.body, 'id')).to.deep.equal(rated);
                    cb();
                });
            };
