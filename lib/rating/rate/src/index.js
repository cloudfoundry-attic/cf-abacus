...
const rateusage = function *(u) {
  const pricingCountry = yield accountPricingCountry(u.organization_guid);
      ...
      return {
        ...
        aggregated_usage: map(service.aggregated_usage, (a) => ({
            ...
            cost: ratefn(a.unit)(getprice(
              service.id, service.plan_id, a.unit, pricingCountry), a.quantity)
        }))
      };
    })
  });
...
const rateUsage = function *(u) {
  ...
  const rid = dbclient.kturi(u.organization_guid, day(u.end));
  const rlogid = dbclient.kturi(u.organization_guid, [
