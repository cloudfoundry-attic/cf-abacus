...
const ratedb = yieldable(batch(retry(breaker(db(uris.couchdb,
  'abacus-rated-usage')))));
...
const rateusage = (u) => {
  return extend(clone(u), {
    services: map(u.services, (service) => {
      ...
      return {
        ...
        aggregated_usage: map(service.aggregated_usage, (a) => ({
          unit: a.unit,
          quantity: a.quantity,
          cost: ratefn(a.unit)(getprice(service.id, service.plan_id,
            a.unit, 'USA'), a.quantity)
        }))
      };
    })
  });
...
const store = function *(r) {
  ...
  const rdoc = extend(clone(r), {
    id: rid
  });
  yield ratedb.put(extend(clone(rdoc), {
    _id: rid
  }));
