    ...
    it('rate a usage', function(done) {
      ...
      const post = (done) => {
        ...
        map(usage, (u) => request.post(
          'http://localhost::p/v1/rating/usage', {
            ...
            expect(val.headers.location).to.not.equal(
              undefined);
      ...
      const get = (done) => {
        ...
        map(usage, (u) => request.get(locations[u.id], {}, (err, val) => {
          ...
          if (val.body.end === 1436054400000) expect(omit(
            val.body, 'id')).to.deep.equal(rated[0]);
          if (val.body.end === 1436140800000) expect(omit(
            val.body, 'id')).to.deep.equal(rated[1]);
          cb();
        }));
      };
