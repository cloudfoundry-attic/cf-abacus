...
const ratingdb = yieldable(retry(breaker(batch(
  ...
  dbclient.dburi(uris.couchdb, 'abacus-rating-plans'))))));

// Configure pricing plan db
const pricingdb = yieldable(retry(breaker(batch(
  dbclient(partition.singleton,
  dbclient.dburi(uris.couchdb, 'abacus-pricing-plans'))))));

// Create an express router
const routes = router();

// Store a new pricing plan
const newPricing = function *(ppid, conf) {
  schemas.pricingPlan.validate(conf);
  debug('Storing new pricing plan with id %s', ppid);
...
const newRating = function *(rpid, conf) {
  schemas.ratingPlan.validate(conf);
