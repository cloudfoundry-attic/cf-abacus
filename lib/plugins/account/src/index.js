...
const sampleRatings = mappings.sampleRating;
...
const samplePricings = mappings.samplePricing;
...
routes.get('/v1/organizations/:org_id/account/:time', function*(req) {
  ...
  if (testOrgs[req.params.org_id])
    return testOrgs[req.params.org_id];
...
routes.get(
  ...
  function*(req) {
    ...
    const id = yield mpid(yield rtype(req.params.resource_type), req.params.plan_id);
    if (!id)
      return {
        status: 404
      };
