...
// The fake test account returned by the plugin
const fake = {
  account_id: '1234',
  organizations: [
...
routes.get('/v1/accounts/:account_id', function *(req) {
  ...
  // This is a plugin here so we always succeed and return our fake account
  return {
    status: 200,
...
const pricingId = (rt, pid) => {
  // In this plugin, the pricing_plan_id is obtained by a simple object mapping
...
const pconfig = function *(ppid) {
  ...
  try {
    ...
    try {
      return pcache(ppid, schemas.pricingConfig.validate(
...
routes.get(
  ...
  // Get the pricing_plan_id. This is a plugin so we just do a simple mapping
...
const ratingId = (rt, pid) => {
  // In this plugin, the rating plan id is obtained by a simple object mapping
...
const rconfig = function *(rpid) {
  ...
  try {
    ...
    try {
      return rcache(rpid, schemas.ratingConfig.validate(
...
routes.get(
  ...
  // Get the rating_plan_id. This is a plugin so we just do a simple mapping
  // to find the rating_plan_id.
...
routes.get(
  '/v1/rating/plans/:rating_plan_id/config', function *(req) {
    ...
    // This is a plugin here so we just return our sample resource price
    // configs
