...
const db = yieldable(
  retry(breaker(batch(dbclient(partition.singleton, dbclient.dburi(uris[dbalias], 'abacus-app-instances')))))
);
...
routes.get('/eureka/v2/apps/:app/:instance', function*(req) {
  debug('Retrieving app %s instance %s', req.params.app, req.params.instance);

  const idoc = yield db.get(dbclient.kturi([req.params.app, req.params.instance].join('/'), 0));
  if (!idoc)
    return {
      status: 404
    };
...
routes.delete('/eureka/v2/apps/:app/:instance', function*(req) {
  ...
  const id = dbclient.kturi([req.params.app, req.params.instance].join('/'), 0);
  const idoc = yield db.get(id);
  if (!idoc)
    return {
      status: 404
...
const eureka = () => {
  ...
  // bearer access token
  if (secured)
    app.use(/^\/eureka\/v2/, oauth.basicStrategy(process.env.API, scopes, process.env.JWTKEY, process.env.JWTALGO));
