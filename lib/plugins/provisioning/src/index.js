...
const planDb = require('./lib/plan-db.js');
...
const upsertPlan = function*(plan) {
  if (yield planDb.read(plan.plan_id, { cache: false }))
    yield planDb.update(plan.plan_id, plan);
  else
    yield planDb.create(plan);
};

const storeAllPlans = (cb) => {
  ...
  yieldable.functioncb(function*() {
    for(let file in planFiles) {
      ...
      yield upsertPlan(defaultPlan);
    }
  })((error) => {
    if(error)
      throw new Error('Failed to store default plan: ' + error);

    debug('Default metering plan mappings created');
    cb();
  });
};
...
routes.get(
  ...
  function*(req) {
    ...
    const meteringPlan = yield planDb.read(id, { cache: false });
    if(!meteringPlan) {
...
routes.post('/v1/metering/plans', function*(req) {
  ...
  yield planDb.create(req.body);
  return {
    status: 201
  };
});

routes.put('/v1/metering/plan/:plan_id', function*(req) {
  ...
  yield planDb.update(bodyPlanId, req.body);
  return {
    status: 200
  };
});

routes.get('/v1/metering/plans/:plan_id', function*(req) {
  ...
  const plan = yield planDb.read(req.params.plan_id, { cache: cacheOpt });

  if(!plan) {
    debug('Plan with id %s of type %s not found', req.params.plan_id, req.params.plan_type);
    return {
      status: 404
    };
  }
...
routes.post(
  '/v1/provisioning/mappings/services/resource/:resource/plan/:plan',
  (req, res) => {
...
const provisioning = () => {
  ...
  // bearer access token
  if(secured())
    app.use(/^\/v1\/(provisioning|metering)|^\/batch$/,
...
module.exports.storeAllDefaultPlans = storeAllPlans;
