...
// Contains mapping of metering_plan_id.
// meteringPlans[resource_type][plan_id]
const meteringPlans = {
  analytics: {
...
const mpid = function *(rtype, pid) {
  ...
  return meteringPlans[rtype] ? meteringPlans[rtype][pid] : undefined;
};
...
const config = function *(mpid) {
  ...
  try {
    ...
    if(cc) {
      debug('Metering plan config %s found in cache',
        mpid);
      return cc;
    }
...
routes.get(
  ...
  const id = yield mpid(yield rtype(req.params.resource_id),
    req.params.plan_id);
...
// Validate the metering configuration given the organization_id,
...
routes.get(
  ...
  debug('Retrieving metering config %s at time %d',
  ...
  const id = yield mpid(yield rtype(req.params.resource_type),
    req.params.plan_id);
