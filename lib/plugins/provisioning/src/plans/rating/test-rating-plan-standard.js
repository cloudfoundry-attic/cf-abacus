...
module.exports = {
  plan_id: 'test-rating-plan-standard',
  metrics: [
    {
      name: 'storage',
      rate: (price, qty) => new BigNumber(price || 0).mul(qty).toNumber()
    },
    {
      name: 'thousand_light_api_calls',
      charge: (t, cost) => cost ? cost : 0
    },
    {
      name: 'heavy_api_calls',
      rate: (price, qty) => new BigNumber(qty).mul(price || 0).toNumber()
    },
    {
      name: 'memory',
