...
describe('abacus-provisioning-plugin', () => {
  before((done) => {
    ...
    dbclient.drop(process.env.DB, /^abacus-rating-plans|^abacus-pricing-plans|^abacus-metering-plans/, done);
  });

  it('returns resource instance info by ' + 'org/space/app/resource/plan/instance', (done) => {
    const verify = (secured, done) => {
      ...
      request.get(
        'http://localhost::p/v1/provisioning/organizations/:org_id/' +
          ...
          'plans/:plan_id/instances/:resource_instance_id/:time',
        extend(
          {
  ...
  it('returns available metering, rating and pricing plans', (done) => {
    ...
    request.get(
      ...
      (err, val) => {
        expect(err).to.equal(undefined);
        expect(val.statusCode).to.equal(200);
        expect(val.body).to.deep.equal(require('../plans/metering/basic-object-storage'));
        done1();
      }
    );
    ...
    request.get(
      ...
      (err, val) => {
        expect(err).to.equal(undefined);
        expect(val.statusCode).to.equal(200);
        expect(omit(val.body, 'id')).to.deep.equal(require('../plans/rating/object-rating-plan'));
        done1();
      }
    );
    ...
    request.get(
      ...
      (err, val) => {
        expect(err).to.equal(undefined);
        expect(val.statusCode).to.equal(200);
        expect(omit(val.body, 'id')).to.deep.equal(require('../plans/pricing/object-pricing-basic'));
        done1();
      }
    );
  });
  ...
  it('returns the type of a resource', (done) => {
    ...
    request.get(
      'http://localhost::p/v1/provisioning/resources' + '/:resource_id/type',
      {
        p: server.address().port,
        resource_id: 'object-storage'
      },
  ...
  it('validates all test rating plans', () => {
    map(
      ...
      (name) => {
        ...
        const conf = require('../plans/rating/' + name);
  ...
  it('validates all test pricing plans', () => {
    map(
      ...
      (name) => {
        ...
        const conf = require('../plans/pricing/' + name);
  ...
  it('validates creation of new rating plans', (done) => {
    ...
    const rating = {
      ...
      metrics: [
        {
          name: 'classifier',
          rate: ((price, qty) => new BigNumber(price || 0).mul(qty).toNumber()).toString()
        }
      ]
    };
