...
describe('abacus-provisioning-plugin updating', () => {
  ...
  context('secured', () => {
    ...
    context('when system scope is provided', () => {
      testUpdatePlan('metering', meteringPlan, utils.getSystemWriteAuthorization());
    });

    context('when resource specific scope is provided', () => {
      testUpdatePlan('metering', meteringPlan, utils.getResourceWriteAuthorization(resourceId));
