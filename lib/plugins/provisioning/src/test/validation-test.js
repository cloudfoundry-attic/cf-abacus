...
describe('abacus-provisioning-plugin validation', () => {
  ...
  beforeEach((done) => {
    ...
    dbclient.drop(process.env.DB, /^abacus-metering-plan/, () => provisioning.storeAllDefaultPlans(done));
  });
  ...
  const provisioningPath = path.dirname(
    require.resolve('abacus-provisioning-plugin'));

  const authHeader = (secured) => secured ?
  ...
  it('returns available metering plans', (done) => {
    ...
    const check = (port, secured, done) => {
      request.get('http://localhost::p/v1/metering/plans/:metering_plan_id', {
        ...
        expect(err).to.equal(undefined);
        expect(val.statusCode).to.equal(200);
        expect(val.body.metrics.length).to.equal(3);
  ...
  it('validates all test metering plans', () => {
    map([
      ...
      const conf = require(`${provisioningPath}/plans/${name}`);
