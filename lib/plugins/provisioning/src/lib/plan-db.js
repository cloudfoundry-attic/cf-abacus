...
const findPlan = function*(pid, lock, cache) {
  ...
  try {
    ...
    const doc = yield meteringdb.get(['k', pid].join('/'));
    if(doc) {
...
const update = function*(validator, planId, planBody) {
  validator.validate(planBody);

  const id = ['k', planId].join('/');
  const doc = yield meteringdb.get(id);
  if(doc) {
...
module.exports = {
  read: function*(id, opts) {
    return yield findPlan(id, meteringLock, opts.cache ? meteringCache : noCache);
  },
  create: function*(planBody) {
    return yield create(schemas.meteringPlan, planBody);
  },
  update: function*(planId, planBody) {
    return yield update(schemas.meteringPlan, planId, planBody);
  }
};
