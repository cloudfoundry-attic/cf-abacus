    ...
    collect: async(usageDoc) => {
      ...
      try {
        ...
        if (error instanceof InvalidSchemaError || error instanceof InvalidPlanError)
          metrics.bulletin('usage.collect.error').post('Invalid Plan');
        else if (error instanceof UnsupportedLicenseTypeError)
          metrics.bulletin('usage.collect.error').post('Unsupported License Type');
        else
          metrics.bulletin('usage.collect.error').post('Collector error');
