  ...
  return extend({}, original, {
    collect: async(usageDoc) => {
      ...
      try {
        ...
        else if (error instanceof InvalidResourceInstance)
          metrics.bulletin(errorBulletin).post('Invalid resource instance');
        else if (error instanceof InvalidAcountError)
          metrics.bulletin(errorBulletin).post('Invalid account');
