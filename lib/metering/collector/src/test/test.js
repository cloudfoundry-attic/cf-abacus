...
describe('abacus-usage-collector', () => {
  it('stores and retrieves batches of service usage data', function(done) {
    ...
    request.post(
      'http://localhost::p/v1/metering/services/:service_id/usage',
      {
        ...
        request.get(val.headers.location, {}, (err, val) => {
          ...
          expect(reqmock.get.args[0][0]).to.equal(
            ...
            ':organization_guid/spaces/:space_guid/consumers/:consumer_id/' +
            'services/:service_id/plans/:plan_id/instances/:service_instance_id'
          );
  ...
  it('stores and retrieves batches of service instance usage data',
    function(done) {
      this.timeout(60000);
      ...
      request.post(
        'http://localhost::p/v1/metering/service_instances/' +
        ':service_instance_id/usage',
        {
          p: server.address().port,
          service_instance_id: 5678,
          body: batch
        }, (err, val) => {
          expect(err).to.equal(undefined);
          expect(val.statusCode).to.equal(201);
          ...
          request.get(val.headers.location, {}, (err, val) => {
            ...
            expect(reqmock.get.args[1][0]).to.equal(
              'http://localhost:9380/v1/provisioning/regions/:region/orgs/' +
              ':organization_guid/spaces/:space_guid/consumers/:consumer_id/' +
              'services/:service_id/plans/:plan_id/instances/' +
              ':service_instance_id'
            );
