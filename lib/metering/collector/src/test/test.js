  ...
  it('stores and retrieves resource usage data', (done) => {
    ...
    const verify = (secured, done) => {
      ...
      getspy = (reqs, cb) => {
        ...
        // Expect a batch call to account service
        if(reqs[0][0] === 'http://localhost:9881/v1/orgs/' +
          ':org_id/account/:time') {
          expect(reqs[0][1]).to.deep.equal(extend({}, {
            org_id: measured.usage[0].organization_id,
            time: measured.usage[0].end
          }));
          expect(reqs[1][0]).to.equal('http://localhost:9881/v1/rating/' +
            'orgs/:organization_id/resources/:resource_id/types/' +
            ':resource_type/plans/:plan_id/:time');
          expect(reqs[1][1]).to.deep.equal(extend({}, measured.usage[0], {
            time: measured.usage[0].end,
            resource_type: 'test-resource'
          }));
          expect(reqs[2][0]).to.equal('http://localhost:9881/v1/pricing/' +
            'orgs/:organization_id/resources/:resource_id/types/' +
            ':resource_type/plans/:plan_id/:time');
          expect(reqs[2][1]).to.deep.equal(extend({}, measured.usage[0], {
            time: measured.usage[0].end,
            resource_type: 'test-resource'
          }));
          cb(undefined, [[undefined, {
        ...
        // Expect a batch call to account service, but this time without
        // the get account call since it is cached.
        if(reqs[0][0] === 'http://localhost:9881/v1/rating/' +
            'orgs/:organization_id/resources/:resource_id/types/' +
            ':resource_type/plans/:plan_id/:time') {
          expect(reqs[0][1]).to.deep.equal(extend({}, measured.usage[0], {
            time: measured.usage[0].end,
          ...
          expect(reqs[1][0]).to.equal('http://localhost:9881/v1/pricing/' +
            'orgs/:organization_id/resources/:resource_id/types/' +
            ':resource_type/plans/:plan_id/:time');
          expect(reqs[1][1]).to.deep.equal(extend({}, measured.usage[0], {
            time: measured.usage[0].end,
