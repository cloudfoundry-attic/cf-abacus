...
describe('UsageValidator', () => {
  ...
  context('when usage with invalid resource instance is passed', () => {
    ...
    it('should throw InvalidResourceInstance', async () => {
      await expect(usageValidator.validate(usage)).to.be.rejectedWith(InvalidResourceInstance);
    });

  });

  context('when usage with unsupported license type is passed', () => {
    const unsupportedLicense = 'unsupported';

    beforeEach(() => {
      accountClientStub.validateAccount.withArgs(usage).callsFake(async () => {
        const error = new Error('some error');
        error.unsupportedLicense = unsupportedLicense;
        throw error;
      });
    });

    it('should throw UnsupportedLicenseTypeError', async () => {
      await expect(usageValidator.validate(usage))
        .to.be.rejectedWith(UnsupportedLicenseTypeError, `Unsupported license type "${unsupportedLicense}"`);
    });

  });

  context('when usage invalid account is passed', () => {
    ...
    it('should throw InvalidAcountError', async () => {
      await expect(usageValidator.validate(usage)).to.be.rejectedWith(InvalidAcountError);
    });
