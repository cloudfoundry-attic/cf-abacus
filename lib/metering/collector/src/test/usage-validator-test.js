...
describe('usage validator tests', () => {
    ...
    context('with invalid schema', () => {
      it('should throw with valid error', async() => {
        try {
          ...
          expect(err).to.have.property('badRequest', true);
          expect(err).to.have.property('error', 'Invalid schema');
        }
      });
    });
  });
  ...
  context('validate plan', () => {
    ...
    context('with invalid plan', () => {
      it('should throw with valid error', async() => {
        requestMock.setSpy(provisioningUri, { statusCode: 404, body: { error: 'Not valid plan' } });
        try {
          await validator.validate(usage, undefined);
        } catch (err) {
          expect(err).to.be.an('Error');
          expect(err).to.have.property('badRequest', true);
          expect(err).to.have.property('error', 'Invalid plan');
        }
      });
    });
    ...
    context('with provisioning down', () => {
      it('should throw with valid error', async() => {
        requestMock.setSpy(provisioningUri, { statusCode: 500 });
        try {
          await validator.validate(usage, undefined);
        } catch (err) {
          expect(err).to.be.an('Error');
          expect(err).to.have.property('badRequest', false);
          expect(err).to.have.property('error', 'Unable to retrieve metering plan');
        }
      });
    });
  ...
  context('validate account', () => {
    ...
    context('with unsupported account', () => {
      it('should throw unsupported license type error', async() => {
        ...
        const validator = createValidator([unsupportedType]);
      ...
      it('should throw with valid error', async() => {
        ...
        try {
          ...
          expect(err).to.have.property('error',
            `Unable to retrieve account info for ${usage.organization_id} at ${usage.end}`);
        }
      });
    });
  });
