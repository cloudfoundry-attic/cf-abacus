...
describe('test collector', () => {
  ...
  context('when posting usage', () => {
    ...
    it('fails when schema is invalid', async() => {
      validatorStub.throws(new InvalidSchemaError()) ;
      const response = await collector.collect(usage);
      expect(response.status).to.equal(httpStatus.BAD_REQUEST);
    });

    it('fails when plan is invalid', async() => {
      validatorStub.throws(new InvalidPlanError()) ;
      const response = await collector.collect(usage);
      expect(response.status).to.equal(httpStatus.BAD_REQUEST);
    });
    ...
    it('fails on account with unsupported license', async() => {
      validatorStub.throws(new UnsupportedLicenseTypeError());
      const response = await collector.collect(usage);
