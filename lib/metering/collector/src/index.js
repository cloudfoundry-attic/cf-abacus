...
const getResourceType = function *(o, u) {
  debug('Retrieving resource type');
...
const getAccount = function *(o, u) {
  debug('Retrieving account information');

  // Look in our cache first
  const ac = accounts.get(u.organization_id);

  if (ac)
    return ac;
  // Get account information
  const res = yield brequest.get(uris.account +
    '/v1/orgs/:org_id/account/:time',
    extend({}, o, {
  ...
  accounts.set(u.organization_id,
    pick(res.body, 'account_id', 'pricing_country'));
  return accounts.get(u.organization_id);
};

// Return the rating_plan_id
const getRating = function *(o, u) {
  debug('Retrieving rating information');

  // Get rating_plan_id
  const res = yield brequest.get(uris.account +
    '/v1/rating/orgs/:organization_id/resources/:resource_id/types/' +
    ':resource_type/plans/:plan_id/:time',
    extend({}, o, u, {
      time: u.end
    }));

  // Unable to retrieve rating_plan_id
  if (res.statusCode !== 200) {
    edebug('Unable to retrieve rating_plan_id, %o', res);
    debug('Unable to retrieve rating_plan_id, %o', res);
  ...
  // Return the rating_plan_id
  return pick(res.body, 'rating_plan_id');
};

// Return the prices and the pricing_plan_id
const getPricing = function *(o, u) {
  debug('Retrieving pricing information');
  ...
  if (res.statusCode !== 200) {
    edebug('Unable to retrieve price information, %o', res);
    debug('Unable to retrieve price information, %o', res);
  ...
  // Return the rating_plan_id
  return pick(res.body, 'pricing_metrics', 'pricing_plan_id');
};

// Return the metering_plan_id
const getMetering = function *(o, u) {
  debug('Retrieving metering information');

  // Get metering_plan_id. For now this is a stub until implemented
  const res = {
    statusCode: 200,
    body: {
  ...
  // Unable to retrieve account information
  if (res.statusCode !== 200) {
    edebug('Unable to retrieve metering_plan_id, %o', res);
    debug('Unable to retrieve metering_plan_id, %o', res);

    // Throw response objecet as an exception to stop further processing
    throw res;
  }

  // return the metering_plan_id
  return pick(res.body, 'metering_plan_id');
};
...
const normalizeUsage = function *(udoc, auth) {
  ...
  yield tmap(udoc.usage, function *(u) {
    const res = yield brequest.get(uris.provisioning +
      '/v1/provisioning/orgs/:organization_id/spaces/' +
    ...
    const rt = yield getResourceType(o, u);
    const rtusage = extend({}, u, rt);

    // Validates and enriches the individual usage with additional information
    // used for metering and rating.
    let [metering, account, rating, pricing] = yield [
      getMetering(o, rtusage),
      getAccount(o, u),
      getRating(o, rtusage),
      getPricing(o, rtusage)
    ];
