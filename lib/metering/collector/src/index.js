...
const submitdb = yieldable(retry(breaker(batch(db(uris.couchdb,
  'abacus-submitted-usage')))));
const normdb = yieldable(retry(breaker(batch(db(uris.couchdb,
  'abacus-normalized-usage')))));

// Return a doc location given a route template and params
const loc = (req, template, parms) => req.protocol + '://' + req.headers.host +
  request.route(template, parms);
...
const storeServiceUsage = function *(sid, ubatch, region) {
  ...
  map(ubatch.service_instances, (si) => map(si.usage, (u) => extend(u, {
    region: u.region ? u.region : 'us'
  })));
  ...
  yield tmap(ubatch.service_instances, function *(si) {
    yield tmap(si.usage, function *(v) {
      return yield get(uris.provisioning +
        '/v1/provisioning/regions/:region/orgs/:organization_guid/spaces/' +
        ':space_guid/consumers/:consumer_id/services/:service_id/plans/' +
        ':plan_id/instances/:service_instance_id',
        extend(clone(v), {
          cache: true,
  ...
  yield submitdb.put(extend(clone(bdoc), {
    _id: bid
  }));
  ...
  const ids = yield tmap(ulist, function *(u) {
    ...
    yield normdb.put(extend(clone(doc), {
      _id: id
    }));
    debug('Logged normalized service usage %o', doc);

    // Post to the meter service
    yield post(uris.meter + '/v1/metering/usage', {
      body: doc
    });
    return id;
  });
...
const storeServiceInstanceUsage = function *(siid, ubatch, region) {
  debug('Received service instance usage batch for service instance %s %o',
    siid, ubatch);
  // Validate the input doc
  schemas.serviceInstanceUsage.validate(ubatch);

  // Add optional region to the usage elements
  map(ubatch.usage, (u) => extend(u, {
    region: u.region ? u.region : 'us'
  }));
...
// Store a batch of usage data for the given service
routes.post('/v1/metering/services/:service_id/usage',
  throttle(function *(req) {
    const ids = yield storeServiceUsage(req.params.service_id, req.body,
      req.query.region);
    return {
      statusCode: 201,
      header: {
        Location: loc(req, '/v1/metering/services/:service_id/usage/:id',
          extend({
            id: ids.batch_id
          }, req.params))
      },
...
// Store a batch of usage data for the given service instance
routes.post('/v1/metering/service_instances/:service_instance_id/usage',
  throttle(function *(req) {
    const ids = yield storeServiceInstanceUsage(req.params.service_instance_id,
      req.body, req.query.region);
    return {
      statusCode: 201,
      header: {
        Location: loc(req,
          '/v1/metering/service_instances/:service_instance_id/usage/:id',
          extend({
            id: ids.batch_id
          }, req.params))
      },
...
routes.get('/v1/metering/services/:service_id/usage/:id', throttle(function *(
  req) {
  debug('Retrieving usage batch for service %s id %s', req.params.service_id,
    req.params.id);
  const doc = omit(yield submitdb.get(req.params.id), ['_id', '_rev',
    'service_id'
  ]);
  return {
    body: doc
...
routes.get('/v1/metering/service_instances/:service_instance_id/usage/:id',
  throttle(function *(req) {
    debug('Retrieving usage batch for service instance %s id %s', req.params
      .service_instance_id, req.params.id);
    const doc = omit(yield submitdb.get(req.params.id), ['_id', '_rev',
      'service_instance_id'
    ]);
    return {
