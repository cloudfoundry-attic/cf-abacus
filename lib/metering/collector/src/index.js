...
const okeysV2 = (udoc, ikey) =>
  map(udoc.usage, (u) => [
    u.organization_id, u.resource_instance_id,
    u.consumer_id || 'UNKNOWN', u.metering_plan_id].join('/'));

// Map submitted resource usage doc to normalized usage docs
const normalizeUsageV2 = function *(udoc, auth) {
  debug('Normalizing usage %o', udoc);
  ...
  yield tmap(udoc.usage, function *(u) {
    const res = yield brequest.get(uris.provisioning +
      '/v2/provisioning/orgs/:organization_id/spaces/' +
      ':space_id/consumers/:consumer_id/resources/:resource_id/plans/' +
      ':plan_id/instances/:resource_instance_id/:time',
      extend(o, u, {
        cache: true,
    ...
    // Enrich the usage document with additional org information
    extend(u, pick(res.body, 'resource_type', 'account_id', 'metering_plan_id',
      'price_id', 'rating_plan_id'));
  });
...
const collector = () => {
  ...
  const mapperV2 = dataflow.mapper(normalizeUsageV2, {
    input: {
      type: 'collected_usage_v2',
      schema: schemas.resourceUsage,
      post: '/v2/metering/collected/usage',
      get: '/v2/metering/collected/usage/t/:tseq/k/:kprovider',
      dbname: 'abacus-collector-collected-usage-v2',
      wscope: iwscope,
      rscope: irscope,
      key: ikey,
      time: itime,
    ...
    output: {
      ...
      get: '/v2/metering/normalized/usage' + 
        ...
        '/t/:tend/:tstart/:tseq',
      dbname: 'abacus-collector-normalized-usage-v2',
      rscope: orscope,
      keys: okeysV2,
      times: otimes
    },
...
// For testing
module.exports.normalize = normalizeUsageV2;
