...
const { ProvisioningClient, BearerAuthHeaderProvider, VoidAuthHeaderProvider, RequestStrategy } =
  require('abacus-api-clients');
...
const baseUrl = (req) => req ? req.protocol + '://' + req.headers.host + (req.baseUrl || '') : uris().meter;
...
const createProducer = async() => {
  const connManager = new ConnectionManager(collectorConfig.rabbitUris);
...
const startApp = async() => {
  ...
  const badRequestResponse = (errorMessage) => ({
  ...
  routes.post('/v1/metering/collected/usage', async(request) => {
    ...
    try {
      ...
      edebug('Error while collecting usage. Usage: "%o", error: "%o"', usageDoc, error);
      if (error instanceof InvalidSchemaError)
        return badRequestResponse('Invalid usage: invalid schema');

      if (error instanceof InvalidResourceInstance)
        return badRequestResponse('Invalid usage: invalid resource instance');

      if (error instanceof InvalidAcountError)
        return badRequestResponse('Invalid usage: not existing account');
