...
const ratingId = yieldable(rconfig.configId);

// Return the prices and the pricing_plan_id
const pricingId = yieldable(pconfig.configId);
...
const normalizeUsage = function *(udoc, auth) {
  ...
  yield tmap(udoc.usage, function *(u) {
    ...
    console.log("i dont know whats blocking");
    const rate = yield ratingId(extend({}, rt, pick(u, 'organization_id', 'plan_id'),
        { time: u.end }, o));
    ...
    const [metering, rating, pricing] = yield [
      getMetering(o, u),
      ratingId(extend({}, rt, pick(u, 'organization_id', 'plan_id'),
        { time: u.end }, o)),
      pricingId(extend({}, rt, pick(u, 'organization_id', 'plan_id'),
        { time: u.end }, pick(account, 'pricing_country')), o)
    ];
    extend(u, rt, account, metering, rating, pricing);
    console.log(u);
  });
  console.log(udoc.usage);
