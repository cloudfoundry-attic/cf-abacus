...
const storeUsage = function *(ubatch) {
  ...
  yield tmap(ubatch.usage, function *(u) {
    return yield get(uris.provisioning +
      ...
      cache: true,
      consumer_id: u.consumer ? u.consumer.value : 'all'
    }));
  });

  // Log the usage batch
  // TODO use the service provider id as a key
  const bid = dbclient.tkuri('usage', seqid());
...
routes.get(
  '/v1/metering/resource/usage/:id', throttle(function *(req) {
    debug('Retrieving usage batch for id %s', req.params.id);
