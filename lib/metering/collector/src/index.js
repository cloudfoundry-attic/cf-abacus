...
const { extend, find, pick, map } = require('underscore');
...
const unsupportedLicenses =
  process.env.UNSUPOORTED_LICENSES ? process.env.UNSUPOORTED_LICENSES.split(',') : [];
...
const otimes = (udoc, itime) => [
  map([udoc.end, udoc.start], seqid.pad16)
    .concat([itime])
    .join('/')
];
...
const getResourceType = function*(rid, auth) {
  ...
  if (res.statusCode !== 200) {
    ...
    return error('ertnotfound', 'Unable to retrieve resource type for resource id ' + rid, res);
  }
...
const getAccount = function*(oid, time, auth) {
  ...
  if (res.statusCode !== 200) {
    ...
    // Return with the reason of the error
    return error('eorgnotfound', 'Unable to retrieve account info for ' + oid + ' at ' + time, res);
  }

  // cache and return only properties we use
  return cacheAccount(k, pick(res.body, 'account_id', 'pricing_country', 'license_type'));
};
...
const validateUsage = function*(udoc, auth) {
  const o = auth ? { headers: { authorization: auth } } : {};
  const response = yield brequest.get(
    uris.provisioning +
  ...
  if (response.statusCode !== 200) {
    // Validation failed. Unable to retrieve provisioning information for the given resource instance
    const sanitizedResponse = pick(response, 'statusCode', 'headers', 'body');
...
const normalizeUsage = function*(udoc, systemToken) {
  debug('Normalizing usage %o', udoc);

  // Forward authorization header field to provisioning
  const auth = systemToken();
  yield validateUsage(udoc, auth);
  ...
  // Verify if the account license type is supported
  ...
  const [mpid, rpid, ppid] = yield [
    meteringId(udoc.organization_id, rt.resource_type, udoc.plan_id, udoc.end, auth),
    ratingId(udoc.organization_id, rt.resource_type, udoc.plan_id, udoc.end, auth),
    pricingId(udoc.organization_id, rt.resource_type, udoc.plan_id, udoc.end, auth)
  ];
  ...
  const prices = yield pricingPlan(ppid.pricing_plan_id, account.pricing_country, auth);
...
const collector = (systemToken) => {
  ...
  const authFn = () => secured() ? systemToken : () => {};

  const normalizeUsageFn = function*(doc) {
    return yield normalizeUsage(doc, authFn());
  };

  const mapper = dataflow.mapper(normalizeUsageFn, {
    input: {
    ...
    error: {
      ...
      delete:
        '/v1/metering/collected/usage/t/:tend/:tstart/:tseq' +
        '/k/:kprovider/:kresource_id/:kplan_id' +
