...
const getPathName = (time, key) => `/v1/metering/collected/usage/t/${time}/k/${key}`;

const getDocumentLocation = (req, usageDoc) => {
  ...
  return (req ? req.protocol + '://' + req.headers.host + (req.baseUrl || '') : 'http://localhost:9080') +
    getPathName(usageDoc.processed, key);
};
...
const startApp = async() => {
  ...
  routes.post('/v1/metering/collected/usage', async(request, response) => {
    const usageDoc = extend({}, request.body, { processed: seqid() });
    ...
    const res = await collector.collect(usageDoc, token());
    if (res.status === HttpStatus.CREATED)
      return extend({}, res, { header : { Location: getDocumentLocation(request, usageDoc) } });
    return res;
