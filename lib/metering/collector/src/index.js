...
const submitdb = yieldable(
    retry(breaker(batch(db(uris.couchdb, 'abacus-submitted-usage')))));
const normdb = yieldable(
    retry(breaker(batch(db(uris.couchdb, 'abacus-normalized-usage')))));
  ...
  yield tmap(ubatch.service_instances, function *(si) {
    yield tmap(si.usage, function *(v) {
      return yield get(uris.provisioning +
          ...
          ':plan_id/instances/:service_instance_id', extend(clone(v), {
            cache: true,
...
// Store a batch of usage data for the given service
routes.post(
  '/v1/metering/services/:service_id/usage', throttle(function *(req) {
    const ids = yield storeServiceUsage(
      req.params.service_id, req.body, req.query.region);
    return {
      statusCode: 201,
      header: {
...
// Store a batch of usage data for the given service instance
routes.post(
  '/v1/metering/service_instances/:service_instance_id/usage',
  throttle(function *(req) {
    const ids = yield storeServiceInstanceUsage(
      req.params.service_instance_id, req.body, req.query.region);
    return {
      statusCode: 201,
      header: {
        Location: loc(req,
...
routes.get(
  '/v1/metering/services/:service_id/usage/:id', throttle(function *(req) {
    debug('Retrieving usage batch for service %s id %s',
      req.params.service_id, req.params.id);
...
routes.get(
    '/v1/metering/service_instances/:service_instance_id/usage/:id',
    throttle(function *(req) {
    debug('Retrieving usage batch for service instance %s id %s',
      req.params.service_instance_id, req.params.id);
    const doc = omit(yield submitdb.get(req.params.id), ['_id', '_rev',
      'service_instance_id'
    ]);
    return {
