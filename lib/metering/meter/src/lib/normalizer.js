...
const functionFormatName = 'function';

const requiredFunction = {
  required: true,
  format: functionFormatName
};

const formatsConfig = {
  formats: {
    [functionFormatName]: (value) => typeof value === 'function'
  }
};

const validateProvisioningClient = validator({
  required: true,
  type: 'object',
  properties: {
    getResourceType: requiredFunction,
    getPricingPlan: requiredFunction
  }
}, formatsConfig);

const validateAccountClient = validator({
  required: true,
  type: 'object',
  properties: {
    getMeteringId: requiredFunction,
    getRatingId: requiredFunction,
    getPricingId: requiredFunction,
    getAccount: requiredFunction
  }
}, formatsConfig);
...
class Normalizer {
  ...
  constructor(provisioningPluginClient, accountPluginClient) {
    if (!validateProvisioningClient(provisioningPluginClient))
      throw new Error('Provided provisioning plugin client is not valid.');

    if(!validateAccountClient(accountPluginClient))
      throw new Error('Provided account plugin client is not valid.');
