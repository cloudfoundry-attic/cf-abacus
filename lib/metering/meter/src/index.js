...
// Return usage transforms for a given resource id
const transforms = (rid) => (config(rid) || {
    transforms: []
  }).transforms;

// Converts usage metrics an object in the format expected by the metering
// functions.
...
const metrics = (u) => {
  return reduce(map(u.metrics, (m) => {
    let o = {};
    if(m.name) {
      o[m.name] = {};
      o[m.name][m.unit] = m.quantity;
      return o;
    }
    o[m.unit] = m.quantity;
    return o;
...
// Transform usage by applying the meter function configured on the given
// transform
const transform = (t) => {
  return (u) => {
    // Translate u.metrics to the metric object expected by the meter function
    const m = metrics(u);

    // Apply the meter function to the metric
    return t.meterfn(m);
  };
};
...
const meter = (u) => {
  ...
  // Apply all the configured transforms to the usage
  const mu = extend(clone(u), {
    metered_usage: map(transforms(u.resource_id), (t) => ({
        unit: t.id,
        quantity: transform(t)(u)
    }))
  });
