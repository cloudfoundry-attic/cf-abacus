...
const startApp = async() => {
  ...
  const auth = async() => {
    ...
    const asyncTokenStart = util.promisify(token.start);
    await asyncTokenStart();
    return token();
  };
  ...
  const provisioningClient = new ProvisioningClient(uris().provisioning, authHeader);
  ...
  const provisioningPluginClient = {
    ...
    getPricingPlan: (planId, pricingCountry) =>
      getPricingPlan(planId, pricingCountry, authHeader)
  };

  const accountPluginClient = {
    getAccount: (usageDoc) => accountClient.getAccount(usageDoc, authHeader),
    getMeteringId: (organizationId, resourceType, planId, timestamp) =>
      getMeteringId(organizationId, resourceType, planId, timestamp, authHeader),
    getRatingId: (organizationId, resourceType, planId, timestamp) =>
      getRatingId(organizationId, resourceType, planId, timestamp, authHeader),
    getPricingId: (organizationId, resourceType, planId, timestamp) =>
      getPricingId(organizationId, resourceType, planId, timestamp, authHeader)
  };
  ...
  const meter = new Meter({
    getPlan: (planId) => getMeteringPlan(planId, authHeader)
  });

  debug('Creating accumulator client with url %s', uris().accumulator);

  const accumulatorClient = new AccumulatorClient(uris().accumulator, httpClient, authHeader);
  /*   END   */
