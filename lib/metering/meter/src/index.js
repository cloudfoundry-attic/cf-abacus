...
const createDocumentRetriever = (partitioner, meterDbClient) => {
  ...
  const outputDbDefaultPartitioner = dbClientWrapper(
    dbClient(defaultPartitioner, dbClient.dburi(uris().db_uri, 'abacus-meter')),
    unsupportedPutFn
  );
  const errorDbDefaultPartitioner = dbClientWrapper(
    dbClient(defaultPartitioner, dbClient.dburi(uris().db_uri, 'abacus-business-errors')),
    unsupportedPutFn
  );
  const collectorInputDb = dbClient(partitioner, dbClient.dburi(uris().db_uri, 'abacus-collector-collected-usage'));
...
const createMeterDbClient = (partitioner) => {
  const errorDb = dbClient(partitioner, dbClient.dburi(uris().db_uri, 'abacus-business-errors'));
  const outputDb = dbClient(partitioner, dbClient.dburi(uris().db_uri, 'abacus-meter'));

  return {
    output: dbClientWrapper(outputDb, keyBuilder.createMeterDocId),
    error: dbClientWrapper(errorDb, keyBuilder.createMeterDocId)
  };
};
...
const startApp = async() => {
  ...
  debug('Creating duplicate detector');
  const duplicateDetector = new DuplicateDetector(meterDbClient.output, dedupe);
  ...
  const messageHandler =
    createMessageHandler(duplicateDetector, normalizer, meter, accumulatorClient, meterDbClient, maxUsageFails);
