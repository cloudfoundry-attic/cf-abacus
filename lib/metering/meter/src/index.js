...
// [['BYTE', '/'], ['12', '*'], ['Memory.INSTANCE', '*'], ['HOUR', '']]
const dslpairs = (f) => map(rest(f.match(
  /\s*{\s*[\w.]*\s*}[*//+-]?|\s*\w+\s*[*//+-]?/g)),
...
// Converts usage metrics into an object that can be used by DSL formula
...
const consume = (a) => {
  return (u) => {
    // Translate u.metrics to an object expected by the consumer function
    const r = metrics(u);

    // If there is a consumption function then use it, else parse the formula,
    // construct consumption function and then evaluate
    return typeof a.meter === 'function' ?
      a.meter(r) : eval(dslformula(a.meter));
  };
};
