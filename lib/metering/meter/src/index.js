...
const meterdb = yieldable(retry(breaker(batch(db(uris.couchdb,
  'abacus-metered-usage')))));

// Return a doc location given a route template and params
const loc = (req, template, params) => req.protocol + '://' + req.headers.host +
  request.route(template, params);
...
const dslpairs = (f) => map(rest(f.match(
  /\s*{\s*[\w.]*\s*}[*//+-]?|\s*\w+\s*[*//+-]?/g)), (s) => rest(s.match(
  /\s*{?\s*([\w.]*)\s*}?([*//+-]?)/)));
...
// Converts usage resources into an object that can be used by DSL formula
// using javascript property access syntax
// ex: {resources: [{unit: 'unit1', quantity: 1}, {name: 'resource2', unit:
// 'unit2', quantity: 2}, {name: 'resource2', unit: 'unit3', quantity: 3},
// {unit: 'unit4', quantity: 4}]} is mapped into  [{unit1: 1}, {resource2:
// {unit2: 2}}, {resource2: {unit3: 3}}, {unit4: 4}]
// [{unit1: 1}, {resource2: {unit2: 2}}, {resource2: {unit3: 3}}, {unit4: 4}]
// is reduced to {unit1: 1, resource2: {unit2: 2, unit3: 3}, {unit4: 4}}
const resources = (u) => {
...
const consumers = (sid) => (config(sid) || {
  aggregations: []
}).aggregations;
...
const consume = (a) => {
  return (u) => {
    ...
    return typeof a.formula === 'function' ? a.formula(r) : eval(dslformula(
      a.formula));
  };
};
...
const meter = (u) => {
  ...
  const mu = extend(clone(u), {
    metered_usage: map(consumers(sid(u)), (a) => ({
      unit: a.id,
...
routes.post('/v1/metering/usage', throttle(function *(req) {
  ...
  yield post((yield accumuri(u.organization_guid, t)) +
