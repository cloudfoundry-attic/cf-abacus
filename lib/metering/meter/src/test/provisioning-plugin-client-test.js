...
describe('provisioning-plugin-client tests', () => {
  ...
  describe('getResourceType', () => {
    ...
    context('when oauthToken is provided', () => {
      ...
      beforeEach(() => {
        provisioningPluginClient = new ProvisioningPluginClient(rootUrl, { createHeader: () => auth });
      });
      ...
      context('when provisioning responds with an error', () => {
        ...
        it('an exception is thrown', async() => {
          ...
          expect(error.message).to.include('Failed to get resource type for resource');
        });
      });
    });
