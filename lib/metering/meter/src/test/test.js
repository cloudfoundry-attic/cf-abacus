...
describe('abacus-usage-meter', () => {
  ...
  it('meters usage docs in V2', (done) => {
    ...
    const verify = (secured, done) => {
      ...
      postspy = (reqs, cb) => {
        // Expect metered usage to be posted to the accumulator service
        expect(reqs[0][0])
          .to.equal('http://localhost:9200/v2/metering/metered/usage');

        expect(omit(
          reqs[0][1].body, 'id', 'processed', 'normalized_usage_v2_id'))
          .to.deep.equal(
            extend(omit(usage, 'id', 'processed'), {
              metered_usage: [{
                metric: 'storage',
      ...
      // Post normalized usage to the meter service
      request.post('http://localhost::p/v2/metering/normalized/usage', {
        p: server.address().port,
        body: usage
      }, (err, val) => {
        expect(err).to.equal(undefined);
