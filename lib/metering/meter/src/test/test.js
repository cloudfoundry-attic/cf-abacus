...
describe('abacus-usage-meter', () => {
  describe('validate usage consumption formulas', () => {
    it('validate a formula with a unit', () => {
      expect(meterapp.dslformula('SUM({LIGHT_API_CALL})'))
        .to.equal('r.LIGHT_API_CALL');
    });
    it('validate a formula with a unit and a division', () => {
      expect(meterapp.dslformula('MAX({BYTE}/1073741824)'))
        .to.equal('r.BYTE / 1073741824');
    });
    ...
    it('validate a formula with multiple units and a multiplication', () => {
      expect(meterapp.dslformula('SUM({GIGABYTE}*{INSTANCE}*{HOUR})'))
        .to.equal('r.GIGABYTE * r.INSTANCE * r.HOUR');
    });
    it('validate a formula with multiple metric units and a multiplication',
      () => {
        expect(meterapp.dslformula(
          'SUM({Memory.GIGABYTE}*{Memory.INSTANCE}*{Memory.HOUR})'))
          .to.equal('r.Memory.GIGABYTE * r.Memory.INSTANCE * r.Memory.HOUR');
      });
  });

  describe('convert metric to a javascript object', () => {
    it('validate metrics without metric name', () => {
      expect(meterapp.metrics({
        metrics: [{
          unit: 'BYTE',
          quantity: 1
        }, {
    ...
    it('validate metrics with metric name', () => {
      expect(meterapp.metrics({
        metrics: [{
          name: 'Memory',
          unit: 'GIGABYTE',
      ...
      request.post('http://localhost::p/v1/metering/usage', {
        ...
        request.get(val.headers.location, {}, (err, val) => {
          ...
          expect(reqmock.noWaitPost.args[0][0])
            .to.equal('http://localhost:9102/v1/metering/metered/usage');
          done();
        });
      });
    });
