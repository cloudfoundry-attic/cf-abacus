...
describe('abacus-usage-meter', () => {
  ...
  it('meters usage docs', (done) => {
    const verify = (secured, done) => {
      ...
      const app = meterapp(systemToken);
      ...
      postspy = (reqs, cb) => {
        ...
        expect(reqs[0][0]).to.equal('http://localhost:9200/v1/metering/metered/usage');

        expect(omit(reqs[0][1].body, 'id', 'processed', 'processed_id', 'normalized_usage_id')).to.deep.equal(
    ...
    const verify = (secured, done) => {
          ...
          [
            undefined,
            {
              ...
              body: {
                ...
                reason: 'Rating plan with id test-rating-plan not found',
                cause: {
                  statusCode: 404
                }
              }
      ...
      request.post('http://localhost::p/v1/metering/normalized/usage',
        ...
        (err, val) => {
          ...
          expect(sanitizedError).to.deep.equal({
            error: 'esink',
            reason:[{
              error: 'erplannotfound',
              reason: 'Rating plan with id test-rating-plan not found',
