...
describe('Accumulator client tests', () => {
  ...
  describe('postUsage', () => {
    ...
    context('when oauthToken is provided', () => {
      ...
      beforeEach(() => {
        accumulatorClient = new AccumulatorClient(urlBuilderStub, postClientStub, () => token);
      });
      ...
      context('when accumulator responds with no error', () => {
        ...
        it('usage doc has processed time attached', async() => {
          const doc = await accumulatorClient.postUsage(usageDoc);
          expect(doc).to.include.keys('processed');
        });
      });
