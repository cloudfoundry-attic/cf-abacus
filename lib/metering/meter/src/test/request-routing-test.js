...
describe('Request Routing tests', () => {
  ...
  context('when using multiple apps', () => {
    ...
    context('when app url contains port', () => {
      ...
      beforeEach(async() => {
        ...
        requestRouting = createRequestRouting(partitionFake, 10, appUrl);
    ...
    context('when app url does not contain port', () => {
      ...
      beforeEach(async() => {
        ...
        requestRouting = createRequestRouting(partitionFake, 10, url);
        routeUri = await requestRouting.getUri(doc);
      });
  ...
  context('when using single app', () => {
    ...
    beforeEach(async() => {
      ...
      requestRouting = createRequestRouting(partitionFake, 1, url);
      routeUri = await requestRouting.getUri(doc);
    });
