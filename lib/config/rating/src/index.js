...
const id = (organizationId, resourceType, provisioningPlanId, time, auth, cb) => {
  ...
  lock(key, (err, unlock) => {
    ...
    return brequest.get(
      ...
      (err, val) => {
        if (err) {
          edebug(`Error retrieving rating plan id for resource type ${resourceType}` +
        ...
        if (val.statusCode !== 200) {
          ...
          return unlock(
            cb(
              ...
              error(
                'erpidnotfound',
                `Rating plan id for resource type ${resourceType} and plan id ${provisioningPlanId} not found`,
                val
              )
            )
          );
