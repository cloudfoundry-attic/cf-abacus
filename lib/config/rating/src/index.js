    ...
    return brequest.get(
      ...
      (err, val) => {
        ...
        if (val.statusCode !== 200) {
          edebug('Unable to retrieve rating plan, %d - %o', val.statusCode, val.body || '');
          debug('Unable to retrieve rating plan, %d - %o', val.statusCode, val.body || '');

          // Throw response object as an exception to stop further processing.
          return unlock(cb(undefined, error('erplannotfound', `Rating plan with id ${rpid} not found`, val)));
        }
  ...
  lock(key, (err, unlock) => {
    ...
    return brequest.get(
      ...
      (err, val) => {
        ...
        if (val.statusCode !== 200) {
          edebug('Unable to retrieve rating plan id, %d - %o', val.statusCode, val.body || '');
          debug('Unable to retrieve rating plan id, %d - %o', val.statusCode, val.body || '');
