...
describe('abacus-resource-config', () => {
  ...
  describe('evaluate metering formulas', () => {
    it('evaluates a formula with a unit', () => {
      expect(formula.meterfn('SUM({LIGHT_API_CALL})').source)
        .to.equal('m.LIGHT_API_CALL');
    });
    it('evaluates a formula with a unit and a division', () => {
      expect(formula.meterfn('MAX({BYTE}/1073741824)').source)
        .to.equal('m.BYTE / 1073741824');
    });
    ...
    it('evaluates a formula with multiple units and a multiplication', () => {
      expect(formula.meterfn('SUM({GIGABYTE}*{INSTANCE}*{HOUR})').source)
        .to.equal('m.GIGABYTE * m.INSTANCE * m.HOUR');
    });
    it('evaluates a formula with multiple metric units and a multiplication',
      () => {
        expect(formula.meterfn(
          'SUM({Memory.GIGABYTE}*{Memory.INSTANCE}*{Memory.HOUR})').source)
          .to.equal('m.Memory.GIGABYTE * m.Memory.INSTANCE * m.Memory.HOUR');
      });
  });
});
