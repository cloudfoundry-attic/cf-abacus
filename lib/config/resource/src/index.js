...
const cache = (resource) => {
  configs[[resource.source.resource_id].join('/')] = resource;
  return resource;
};
...
// Retrieve the configuration for the specified resource
const config = (rid, time, cb) => {
  lock(rid, (err, unlock) => {
    if(err)
      return unlock(cb(err));

    debug('Retrieving config for resource %s effective %d', rid, time);
    ...
    if(rid === 'test-resource')
      return unlock(
        cb(undefined, cache(compile(require('./test/test-resource')))));

    // Get the requested resource config from the config service
    return brequest.get(uris.provisioning +
      '/v1/provisioning/resource/config/:resource_id/:time', {
        resource_id: rid,
        time: time
      }, (err, val) => {
        if(err)
        ...
        return unlock(
          cb(undefined, cache(compile(val.body))));
      });
  });
};
