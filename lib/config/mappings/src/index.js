...
const meteringMappingsDb = yieldable(
  retry(breaker(batch(dbclient(partition.singleton, dbclient.dburi(uris.db_uri, 'abacus-metering-plan-mappings')))))
);

// Configure rating plan mappings db
const ratingMappingsDb = yieldable(
  retry(breaker(batch(dbclient(partition.singleton, dbclient.dburi(uris.db_uri, 'abacus-rating-plan-mappings')))))
);

// Configure pricing plan mappings db
const pricingMappingsDb = yieldable(
  retry(breaker(batch(dbclient(partition.singleton, dbclient.dburi(uris.db_uri, 'abacus-pricing-plan-mappings')))))
);
