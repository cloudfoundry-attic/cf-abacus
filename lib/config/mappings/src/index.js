...
const metadata = (id, doc) => {
  let metadata = { _id: id };
  if (doc)
    metadata = extend(metadata, { _rev: doc._rev });
  return metadata;
};
...
const mappedRatingPlanDoc = function *(resourceType, planName) {
  ...
  try {
    ...
    if (doc)
      debug('Rating plan %s found in db', id);
    ...
    for(let resourceType in mapping)
      for (let planName in mapping[resourceType]) {
        ...
        yield createFn(resourceType, planName,
          mapping[resourceType][planName], doc);
      }
  })((error) => {
    if(error) {
...
const storeDefaultMappings = (cb) => {
  ...
  storeMapping('metering', meteringMappingsDb, mappedMeteringPlanDoc,
    newMeteringMapping, defaultMeteringMapping, countCb);
  storeMapping('rating', ratingMappingsDb, mappedRatingPlanDoc,
    newRatingMapping, defaultRatingMapping, countCb);
  storeMapping('pricing', pricingMappingsDb, mappedPricingPlanDoc,
