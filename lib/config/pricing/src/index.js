    ...
    return brequest.get(
      ...
      (err, val) => {
        ...
        if (val.statusCode !== 200) {
          edebug('Unable to retrieve pricing plan, %d - %o', val.statusCode, val.body || '');
          debug('Unable to retrieve pricing plan, %d - %o', val.statusCode, val.body || '');

          // Unlock and throw response object as an exception
          // to stop further processing
          return unlock(cb(undefined, error('epplannotfound', `Pricing plan with id ${ppid} not found`, val)));
        }
  ...
  return lock(key, (err, unlock) => {
    ...
    return brequest.get(
      ...
      (err, val) => {
        ...
        if (val.statusCode !== 200) {
          edebug('Unable to retrieve pricing plan id, %d - %o', val.statusCode, val.body || '');
          debug('Unable to retrieve pricing plan id, %d - %o', val.statusCode, val.body || '');
          ...
          return unlock(
            cb(
              ...
              error(
                'eppidnotfound',
                `Pricing plan id for resource type ${rtype} ` + `and plan id ${ppid} not found`,
                val
              )
            )
          );
