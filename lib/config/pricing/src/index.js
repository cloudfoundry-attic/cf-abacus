...
const id = (oid, rtype, ppid, time, auth, cb) => {
  ...
  return lock(key, (err, unlock) => {
    ...
    return brequest.get(
      ...
      (err, val) => {
        if (err) {
          edebug(`Error retrieving pricing plan id for resource type ${rtype} and plan id ${ppid}, error: %o`, err);
          debug(`Error retrieving pricing plan id for resource type ${rtype} and plan id ${ppid}, error: %o`, err);
          return unlock(cb(err));
        }

        // Authorization failed. Unable to retrieve pricing configuration
        if (val.statusCode !== 200) {
          edebug(`Error retrieving pricing plan id for resource type ${rtype} and plan id ${ppid}, %d - %o`,
            val.statusCode, val.body || '');
          debug(`Error retrieving pricing plan id for resource type ${rtype} and plan id ${ppid}, %d - %o`,
            val.statusCode, val.body || '');
