...
const value = process.env.VALUE || 5;
...
describe('Plan tester', () => {
  ...
  const checkPlanFlow = (measure, metric, opts) => {
    ...
    nextValue.aggregated = {
      initial: aggregateOutput,
      previous: opts.aggregated.initial
    };
  ...
  context('on continuous accumulation/aggregation', () => {
    ...
    before(() => {
      ...
      previousStep.noLogs = true;
      previousStep.accumulated = {
        initial: 0
      };
      previousStep.aggregated = {
        initial: undefined,
        previous: undefined
      };
      previousStep.summarized = {
        total: undefined
      };
    });
