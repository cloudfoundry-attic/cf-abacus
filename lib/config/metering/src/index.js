    ...
    return brequest.get(
      ...
      (err, val) => {
        ...
        if (val.statusCode !== 200) {
          edebug('Unable to retrieve metering plan id, %d - %o', val.statusCode, val.body || '');
          debug('Unable to retrieve metering plan id, %d - %o', val.statusCode, val.body || '');
  ...
  lock(mpid, (err, unlock) => {
    ...
    return brequest.get(
      ...
      (err, val) => {
        ...
        if (val.statusCode !== 200) {
          edebug('Unable to retrieve metering plan, %d - %o', val.statusCode, val.body || '');
          debug('Unable to retrieve metering plan, %d - %o', val.statusCode, val.body || '');

          // Throw response object as an exception to stop further processing.
          return unlock(cb(undefined, error('emplannotfound', `Metering plan with id ${mpid} not found`, val)));
        }
