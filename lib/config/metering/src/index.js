...
const lock = require('abacus-lock').locker('mconfig');
...
const meterfn = (t) => t.meter ? xeval(t.meter, context) : t.formula ? formula.meterfn(t.formula) : meter(t.name);

// Return the accumulation function for a given metric
const accumulatefn = (t) =>
  t.accumulate ? xeval(t.accumulate, context) : t.formula ? formula.accumfn(t.formula) : accumulate;

// Return the aggregation function for a given metric
const aggregatefn = (t) => t.aggregate ? xeval(t.aggregate, context) : aggregate;

// Return the summary function for a given metric
const summarizefn = (t) => t.summarize ? xeval(t.summarize, context) : summarize;
...
const id = (oid, rtype, ppid, time, auth, cb) => {
  ...
  debug('Retrieving metering plan id for %s', key);
  lock(key, (err, unlock) => {
    if (err) {
      edebug('Error locking metering id %s: %o', key, err);
    ...
    if (c) {
      debug('Metering plan id for %s found in cache', key);
      return unlock(
        cb(undefined, {
          metering_plan_id: c
        })
    ...
    if (rtype === 'test-resource')
      return unlock(
        cb(undefined, {
          metering_plan_id: cacheId(key, require('./test/test-metering-plan').plan_id)
        })
      );
    ...
    return brequest.get(
      ...
      (err, val) => {
        if (err) {
          edebug(`Error retrieving metering plan id for resource type ${rtype} and plan id ${ppid}, error: %o`, err);
          debug(`Error retrieving metering plan id for resource type ${rtype} and plan id ${ppid}, error: %o`, err);
          return unlock(cb(err));
        }

        // Unable to retrieve metering configuration
        if (val.statusCode !== 200) {
          edebug(`Error retrieving metering plan id for resource type ${rtype} and plan id ${ppid}, %d - %o`,
            val.statusCode, val.body || '');
          debug(`Error retrieving metering plan id for resource type ${rtype} and plan id ${ppid}, %d - %o`,
            val.statusCode, val.body || '');
          ...
          return unlock(
            cb(
              ...
              error(
                'empidnotfound',
                `Metering plan id for resource type ${rtype} and plan id ${ppid} not found`,
                val
              )
            )
          );
        ...
        debug(`Retrieved metering plan id ${ppid}`);
        return unlock(
          cb(undefined, {
            metering_plan_id: cacheId(key, val.body)
          })
        );
      }
    );
...
const plan = (mpid, auth, cb) => {
  lock(mpid, (err, unlock) => {
    ...
    return brequest.get(
      uris.provisioning + '/v1/metering/plans/:metering_plan_id',
      extend(o, {
        metering_plan_id: mpid
      }),
      (err, val) => {
        if (err) {
          edebug(`Error retrieving metering plan ${mpid}, error: %o`, err);
          debug(`Error retrieving metering plan ${mpid}, error: %o`, err);
          return unlock(cb(err));
        }

        // Authorization failed. Unable to retrieve metering plan
        if (val.statusCode !== 200) {
          edebug(`Error retrieving metering plan ${mpid}, %d - %o`, val.statusCode, val.body || '');
          debug(`Error retrieving metering plan ${mpid}, %d - %o`, val.statusCode, val.body || '');
          ...
          return unlock(
            cb(
              ...
              error(
                'emplannotfound',
                `Metering plan with id ${mpid} not found`,
                val
              )
            )
          );
        ...
        debug(`Retrieved metering plan ${mpid}`);
        return unlock(
          cb(undefined, {
