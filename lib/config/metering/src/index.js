...
const accumulate = (a, qty, start, end, from, to, twCell) =>
    ...
    new BigNumber(a || 0).add(qty || 0).toNumber();
...
const maxAge = process.env.PLAN_CACHE_MAX_AGE ?
  parseInt(process.env.PLAN_CACHE_MAX_AGE) : 1200000;
  ...
  lock(key, (err, unlock) => {
    ...
    extend({}, o, {
      ...
      if (val.statusCode !== 200) {
        ...
        return unlock(cb(undefined, error('empidnotfound',
          `Metering plan id for resource type ${rtype} ` +
          `and plan id ${ppid} not found`, val
        )));
      }
    ...
    return brequest.get(uris.provisioning +
      ...
      if (val.statusCode !== 200) {
        ...
        return unlock(cb(undefined, error('emplannotfound',
          `Metering plan with id ${mpid} not found`, val
        )));
      }
