...
const meter = (name) => (measures) => measures[name];
...
const id = (organizationId, resourceType, provisioningPlanId, time, auth, cb) => {
  ...
  lock(key, (err, unlock) => {
    ...
    return brequest.get(
      ...
      (err, val) => {
        if (err) {
          edebug(`Error retrieving metering plan id for resource type ${resourceType}` +
        ...
        if (val.statusCode !== 200) {
          ...
          return unlock(
            cb(
              ...
              error(
                'empidnotfound',
                `Metering plan id for resource type ${resourceType} and plan id ${provisioningPlanId} not found`,
                val
              )
            )
          );
        ...
        debug(`Retrieved metering plan id ${provisioningPlanId}`);
        return unlock(
          cb(undefined, {
