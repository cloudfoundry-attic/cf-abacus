...
// Provides access to metering configuration.
...
const id = (oid, rtype, ppid, time, auth, cb) => {
  debug('Retrieving metering_plan_id');
  ...
  const c = cachedId(k);
  if(c) {
    debug('Metering_plan_id for %s found in cache', k);
  ...
  return brequest.get(uris.provisioning +
    ...
    extend({}, o, {
      ...
      if (val.statusCode !== 200) {
        edebug('Unable to retrieve metering_plan_id, %d - %o',
          val.statusCode, val.body || '');
        debug('Unable to retrieve metering_plan_id, %d - %o',
          val.statusCode, val.body || '');
...
// Retrieve the configuration for the specified metering plan id
