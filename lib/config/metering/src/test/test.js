...
describe('abacus-metering-config', () => {
  it('returns metering plan id for an org, resource type, plan and time',
    (done) => {
      ...
      config.id(
        'a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf27', 'object-storage',
  ...
  it('returns metering config for a metering plan id', (done) => {
    let cbs = 0;
    const cb = () => {
      if (++cbs === 2) done();
    };

    // Retrieve a metering config
    config('basic-test-metering-plan', undefined, (err, val) => {
      expect(err).to.equal(undefined);
