...
const { APIError, UnavailableForLegalReasonsError } = require('abacus-api-clients');
    ...
    describe('#processSpans', () => {
      ...
      context('when findIncompleteSpans fails', () => {
        ...
        it('rethrows the error', async () => {
          await expect(controller.processSpans()).to.be.rejectedWith(findErr);
        });
      });
    });
    ...
    describe('#cleanupSpans', () => {
      ...
      context('when findCompleteSpans fails', () => {
        ...
        it('it rethrows the error', async () => {
          await expect(controller.cleanupSpans()).to.be.rejectedWith(findErr);
        });
      });
      ...
      context('when deleteSpansByIDs fails', () => {
        ...
        it('it rethrows the error', async () => {
          await expect(controller.cleanupSpans()).to.be.rejectedWith(deleteErr);
        });
      });
    });
  });
    ...
    describe('#processSpan', () => {
      context('when span is planned', () => {
        ...
        it('executes the planned interval of the span', async () => {
          ...
          assert.calledWithExactly(abacusClient.postUsage, {
            dedup_id: plannedSpan.processing.planned_interval.usage_guid,
        ...
        context('when abacus client throws an unavailable due to legal reasons error', () => {
          beforeEach(() => {
            abacusClient.postUsage.callsFake(async () => {
              throw new UnavailableForLegalReasonsError();
            });
          });

          it('ignore the error', async () => {
            await processor.processSpan(plannedSpan);
        ...
        context('when abacus client throws an unknown error', () => {
          beforeEach(() => {
            abacusClient.postUsage.callsFake(async () => {
              throw new APIError();
            });
          ...
          it('ignore the error', async () => {
            await expect(processor.processSpan(plannedSpan)).to.be.rejectedWith(APIError);
          });
        });
        ...
        context('when updateSpanProcessedInterval fails', () => {
          ...
          it('rethrows the error', async () => {
            await expect(processor.processSpan(plannedSpan)).to.be.rejectedWith(updateErr);
          });
        });
      });
      ...
      context('when span is unplanned', () => {
        ...
        context('when updateSpanPlannedInterval fails', () => {
          ...
          it('rethrows the error', async () => {
            await expect(processor.processSpan(unplannedSpan)).to.be.rejectedWith(updateErr);
          });
        });
      });
    });
