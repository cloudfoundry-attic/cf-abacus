...
class Controller {
  ...
  async handleMappings(resourceID, planID, meteringPlan, ratingPlan, pricingPlan) {
    const meteringPlanMapped = await this._handleMapping(this.provisioningClient.mapMeteringPlan,
      resourceID, planID, meteringPlan);
    const ratingPlanMapped = await this._handleMapping(this.provisioningClient.mapRatingPlan,
      resourceID, planID, ratingPlan);
    const pricingPlanMapped = await this._handleMapping(this.provisioningClient.mapPricingPlan,
