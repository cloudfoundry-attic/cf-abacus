...
class Controller {
  ...
  async handleMappings(resourceID, planID, meteringPlan, ratingPlan, pricingPlan) {
    try {
      await Promise.all([
        this.provisioningClient.mapMeteringPlan(resourceID, planID, meteringPlan),
        this.provisioningClient.mapRatingPlan(resourceID, planID, ratingPlan),
        this.provisioningClient.mapPricingPlan(resourceID, planID, pricingPlan)
      ]);
    } catch (e) {
