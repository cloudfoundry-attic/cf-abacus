...
const { EventSchemaValidator, InvalidEventError } = require('../lib/event-schema-validator');
...
describe('EventSchemaValidator', () => {
  ...
  beforeEach(() => {
    eventValidator = new EventSchemaValidator();
  });
  ...
  const itNoTimestamp = (createValidEventFn, validateFn) =>
    it('should throw if "timestamp" is missing', () => {
      ...
      expect(() => validateFn(event)).to.throw(InvalidEventError);
    });
  ...
  describe('#validateStopEvent', () => {
    ...
    context('when stop event is not valid', () => {
      ...
      it('should throw if "measured_usage" is present', () => {
        ...
        expect(() => eventValidator.validateStopEvent(stopEvent)).to.throw(InvalidEventError);
      });
