...
describe('Presenter', () => {
  ...
  beforeEach(() => {
    ...
    onStartSuccess = sinon.stub();
    presenter.on(Presenter.Events.start.success, onStartSuccess);
    onStartInvalidEventError = sinon.stub();
    presenter.on(Presenter.Events.start.invalidEvent, onStartInvalidEventError);
    onStartOutOfSlackError = sinon.stub();
    presenter.on(Presenter.Events.start.outOfSlack, onStartOutOfSlackError);
    onStartConflictError = sinon.stub();
    presenter.on(Presenter.Events.start.conflict, onStartConflictError);
    onStartFailure = sinon.stub();
    presenter.on(Presenter.Events.start.failure, onStartFailure);

    onStopSuccess = sinon.stub();
    presenter.on(Presenter.Events.stop.success, onStopSuccess);
    onStopInvalidEventError = sinon.stub();
    presenter.on(Presenter.Events.stop.invalidEvent, onStopInvalidEventError);
    onStopOutOfSlackError = sinon.stub();
    presenter.on(Presenter.Events.stop.outOfSlack, onStopOutOfSlackError);
    onStopConflictError = sinon.stub();
    presenter.on(Presenter.Events.stop.conflict, onStopConflictError);
    onStopMissingSpanError = sinon.stub();
    presenter.on(Presenter.Events.stop.missingSpan, onStopMissingSpanError);
    onStopOutOfOrderError = sinon.stub();
    presenter.on(Presenter.Events.stop.outOfOrder, onStopOutOfOrderError);
    onStopFailure = sinon.stub();
    presenter.on(Presenter.Events.stop.failure, onStopFailure);

    onMappingsSuccess = sinon.stub();
    presenter.on(Presenter.Events.mappings.success, onMappingsSuccess);
    onMappingsInvalidError = sinon.stub();
    presenter.on(Presenter.Events.mappings.invalidMappings, onMappingsInvalidError);
    onMappingsConflictError = sinon.stub();
    presenter.on(Presenter.Events.mappings.conflict, onMappingsConflictError);
    onMappingsFailure = sinon.stub();
    presenter.on(Presenter.Events.mappings.failure, onMappingsFailure);
  });
  ...
  describe('#handleStart', () => {
    ...
    context('when no error occurs', () => {
      ...
      it('"start.success" event is emitted', async () => {
        assert.calledOnce(onStartSuccess);
      });
    ...
    context('when schema validator throws validation error', () => {
      ...
      it('"start.error.invalid-event" event is emitted', () => {
        assert.notCalled(onStartSuccess);
  ...
  describe('#handleStop', () => {
    ...
    context('when no error occurs', () => {
      ...
      it('"stop.success" event is emitted', async () => {
        assert.calledOnce(onStopSuccess);
      });
    ...
    context('when schema validator throws validation error', () => {
      ...
      it('"stop.error.invalid-event" event is emitted', () => {
        assert.notCalled(onStopSuccess);
    ...
    context('when slack validator throws validation error', () => {
      ...
      it('"stop.error.out-of-slack" event is emitted', () => {
        assert.notCalled(onStopSuccess);
        assert.notCalled(onStopFailure);
        assert.calledOnce(onStopOutOfSlackError);
      });
    });
  ...
  describe('#handleMappings', () => {
    ...
    context('when mappings creation is success', () => {
      beforeEach(async () => {
      ...
      it('"mappings.success" event is emitted', async () => {
        assert.calledOnce(onMappingsSuccess);
      });
    });
    ...
    context('when schema validator throws validation error', () => {
      ...
      it('"start.error.invalid-mappings" event is emitted', () => {
        assert.notCalled(onMappingsSuccess);
