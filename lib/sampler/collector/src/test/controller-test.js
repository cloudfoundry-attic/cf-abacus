...
describe('Controller', () => {
  ...
  describe('#handleMappings', () => {
    ...
    const contextWhenTheErrorIsConflict = () => {
      context('when the error is conflict', () => {
        beforeEach(() => {
          mapErr = conflictErr;
        });

        it('throws a conflict plan error', async () => {
          await expect(handleMappings()).to.be.rejectedWith(ConflictMappingError);
        });
      });
    };

    const contextWhenTheErrorIsUnknown = () => {
      context('when the error is unknown', () => {
        beforeEach(() => {
          mapErr = unknownErr;
        });

        it('forwards the error', async () => {
          await expect(handleMappings()).to.be.rejectedWith(mapErr);
        });
      });
    };

    context('when metering plan mapping errors', () => {
      beforeEach(() => {
      ...
      contextWhenTheErrorIsConflict();

      contextWhenTheErrorIsUnknown();
    });

    context('when rating plan mapping errors', () => {
      beforeEach(() => {
    ...
    context('when pricing plan mapping errors', () => {
      beforeEach(() => {
