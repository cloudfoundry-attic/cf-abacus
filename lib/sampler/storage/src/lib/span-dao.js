...
class SpanDAO {
  ...
  async findIncompleteSpans(before, offset, limit) {
    const cursor = this.collection.find({
      $or: [
        { 'processing.complete': false, 'processing.last_interval.end': { '$lt': before } },
        { 'processing.complete': false, 'end_is_set': true }
      ]
...
const createEndPartialIndex = async (collection) => {
  const definition = {
    'processing.complete': 1
  };
  const options = {
    name: 'search_end_is_set',
    partialFilterExpression: { end_is_set: { $exists: true } }
  };
  await collection.createIndex(definition, options);
};
...
const createSpanDAO = async (db, collectionName) => {
  ...
  await createEndPartialIndex(collection);
