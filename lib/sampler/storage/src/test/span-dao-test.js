...
describe('span-dao', () => {
  ...
  describe('#findIncompleteSpans', () => {
    ...
    beforeEach(async () => {
      ...
      const firstInterval = {
        start: startTimestamp,
        end: 1534161600000 // 2018-08-13 12:00:00
      };
      ...
      const secondInterval = {
        start: startTimestamp,
        end: 1534168800000 // 2018-08-13 14:00:00
      };
      ...
      const thirdInterval = {
        start: startTimestamp,
        end: 1534176000000 // 2018-08-13 16:00:00
      };
    ...
    it('returns only spans that are sampled earlier than specified', async () => {
      const spans = await dao.findIncompleteSpans(1534161600001, 0, 3); // 2018-08-13 12:00:00.001
      expect(spans.length).to.equal(1);
      expect(spans[0].target).to.deep.equal(firstTarget);
    });

    it('returns multiple spans, when applicable', async () => {
      const spans = await dao.findIncompleteSpans(1534168800001, 0, 3); // 2018-08-13 14:00:00.001
    ...
    it('returns only spans that are not yet complete', async () => {
      const spans = await dao.findIncompleteSpans(1534176000001, 0, 3); // 2018-08-13 16:00:00.001
      expect(spans.length).to.equal(2);
      expect(spans[0].target).to.deep.equal(firstTarget);
      expect(spans[1].target).to.deep.equal(secondTarget);
    });
    ...
    it('returns no candiates, if there are no matching', async () => {
      const spans = await dao.findIncompleteSpans(1534161600000, 0, 3); // 2018-08-13 12:00:00.000
      expect(spans.length).to.equal(0);
    });

    it('returns spans according to the limit setting', async () => {
      const spans = await dao.findIncompleteSpans(1534168800001, 0, 1); // 2018-08-13 14:00:00.001
      expect(spans.length).to.equal(1);
      expect(spans[0].target).to.deep.equal(firstTarget);
    });

    it('returns spans according to the offset setting', async () => {
      const spans = await dao.findIncompleteSpans(1534168800001, 1, 3); // 2018-08-13 14:00:00.001
      expect(spans.length).to.equal(1);
  ...
  describe('#findCompleteSpans', () => {
    ...
    it('returns only spans that are sampled earlier than specified', async () => {
      const spans = await dao.findCompleteSpans(1534161600001, 0, 3); // 2018-08-13 12:00:00.001
      expect(spans.length).to.equal(1);
      expect(spans[0].target).to.deep.equal(firstTarget);
    });

    it('returns multiple spans, when applicable', async () => {
      const spans = await dao.findCompleteSpans(1534168800001, 0, 3); // 2018-08-13 14:00:00.001
      expect(spans.length).to.equal(2);
      expect(spans[0].target).to.deep.equal(firstTarget);
      expect(spans[1].target).to.deep.equal(secondTarget);
    });
    ...
    it('returns only spans that are not yet complete', async () => {
      const spans = await dao.findCompleteSpans(1534176000001, 0, 3); // 2018-08-13 16:00:00.001
      expect(spans.length).to.equal(2);
      expect(spans[0].target).to.deep.equal(firstTarget);
      expect(spans[1].target).to.deep.equal(secondTarget);
    });
    ...
    it('returns no candiates, if there are no matching', async () => {
      const spans = await dao.findCompleteSpans(1534161600000, 0, 3); // 2018-08-13 12:00:00.000
      expect(spans.length).to.equal(0);
    });

    it('returns spans according to the limit setting', async () => {
      const spans = await dao.findCompleteSpans(1534168800001, 0, 1); // 2018-08-13 14:00:00.001
      expect(spans.length).to.equal(1);
      expect(spans[0].target).to.deep.equal(firstTarget);
    });

    it('returns spans according to the offset setting', async () => {
      const spans = await dao.findCompleteSpans(1534168800001, 1, 3); // 2018-08-13 14:00:00.001
      expect(spans.length).to.equal(1);
      expect(spans[0].target).to.deep.equal(secondTarget);
    });
  });
