...
describe('span-dao', () => {
  ...
  describe('#endSpan', () => {
    it('is successful when the span exists', async () => {
      ...
      expect(renamedSpan.end_is_set).to.equal(true);
    });
  ...
  describe('#findIncompleteSpans', () => {
    ...
    beforeEach(async () => {
      ...
      firstSpanPO = spanPageObject(dao, firstTarget);
      await firstSpanPO.startSpan(startTimestamp, measures).verify();
      const firstInterval = {
        start: startTimestamp,
      ...
      secondSpanPO = spanPageObject(dao, secondTarget);
      await secondSpanPO.startSpan(startTimestamp, measures).verify();
    ...
    it('returns only spans that are sampled earlier than specified and end is not set', async () => {
    ...
    it('returns multiple when end is set or spans are sampled earlier than specified', async () => {
      const before = moment.utcTimestamp(2018, Aug, 13, 12, 0, 0, 1);
      const newCorrelationID = uuid.v4();
      await secondSpanPO.endSpan(endTimestamp, newCorrelationID).verify();
      const spans = await dao.findIncompleteSpans(before, 0, 3);
      expect(spans.length).to.equal(2);
      expect(spans[0].target).to.deep.equal(firstTarget);
      expect(spans[1].target).to.deep.equal(extend({}, secondTarget, {
        correlation_id: newCorrelationID
      }));
    });
    ...
    it('returns only spans that are sampled later than specified and end is set', async () => {
      const before = moment.utcTimestamp(2018, Aug, 13, 12, 0, 0, 0);
      const newCorrelationID = uuid.v4();
      await firstSpanPO.endSpan(endTimestamp, newCorrelationID).verify();
      const spans = await dao.findIncompleteSpans(before, 0, 3);
      expect(spans.length).to.equal(1);
      expect(spans[0].target).to.deep.equal(extend({}, firstTarget, {
        correlation_id: newCorrelationID
      }));
    });
