...
const dateToTimestamp = (year, month, day, hour, minute, second, millis) => {
  // in moment.js the month starts from 0 (i.e. Jan = 0), hence the subtraction
  return moment.utc([year, month - 1, day, hour, minute, second, millis]).valueOf();
};
...
describe('span-dao', () => {
  ...
  beforeEach(async () => {
    dao = await createSpanDAO(db, collectionName);
  ...
  it('is possible to create dao twice', async () => {
    await createSpanDAO(db, collectionName);
  });
  ...
  describe('#findIncompleteSpans', () => {
    ...
    beforeEach(async () => {
      ...
      const firstInterval = {
        start: startTimestamp,
        end: dateToTimestamp(2018, 8, 13, 12, 0, 0, 0)
      };
      ...
      const secondInterval = {
        start: startTimestamp,
        end: dateToTimestamp(2018, 8, 13, 14, 0, 0, 0)
      };
      ...
      const thirdInterval = {
        start: startTimestamp,
        end: dateToTimestamp(2018, 8, 13, 16, 0, 0, 0)
      };
    ...
    it('returns only spans that are sampled earlier than specified', async () => {
      const before = dateToTimestamp(2018, 8, 13, 12, 0, 0, 1);
    ...
    it('returns multiple spans, when applicable', async () => {
      const before = dateToTimestamp(2018, 8, 13, 14, 0, 0, 1);
      const spans = await dao.findIncompleteSpans(before, 0, 3);
    ...
    it('returns only spans that are not yet complete', async () => {
      const before = dateToTimestamp(2018, 8, 13, 16, 0, 0, 1);
      const spans = await dao.findIncompleteSpans(before, 0, 3);
      expect(spans.length).to.equal(2);
      expect(spans[0].target).to.deep.equal(firstTarget);
      expect(spans[1].target).to.deep.equal(secondTarget);
    ...
    it('returns no candiates, if there are no matching', async () => {
      const before = dateToTimestamp(2018, 8, 13, 12, 0, 0, 0);
      const spans = await dao.findIncompleteSpans(before, 0, 3);
