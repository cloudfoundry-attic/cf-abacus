...
describe('themeController', () => {
  ...
  describe('saveThemePreference', () => {
    ...
    beforeEach(function() {
      ...
      fetchRecordStub = sinon.stub(dbUtils.prototype, 'fetchRecords').callsFake((col, opt, cb) => {
        return cb(null, [{
          'themePreference': 'test'
        }]);
      });
      upsertStub = sinon.stub(dbUtils.prototype, 'upsert').callsFake((col, par1, par2, cb) => {
  ...
  describe('getThemePreference', () => {
    ...
    describe('gets saved theme, with response othertype theme', () => {
      ...
      before(() => {
        ...
        anotherStub = sinon.stub(themeCtrl, '_getThemeMapingJson').callsFake(() => {
          return {
            'themes': [{
              'displayname': 'test',
              'filename': 'test.css',
  ...
  describe('_checkUserPref, response has custom themeType', () => {
    ...
    beforeEach(function() {
      ...
      dbUtilsStub = sinon.stub(dbUtils.prototype, 'getCSSFromDB').callsFake((id, cb) => {
        return cb(null, '');
      });
    });
  ...
  describe('getDefaultThemeFile', () => {
    ...
    describe('_checkUserPref returns not null', () => {
      before(() => {
        checkUserPrefStb = sinon.stub(themeCtrl, '_checkUserPref').callsFake((req, cb) => {
          return cb('success');
        });
      });
    ...
    describe('_checkUserPref returns null', () => {
      ...
      before(() => {
        ...
        _getThemeMapingJsonStb = sinon.stub(themeCtrl, '_getThemeMapingJson').callsFake(() => {
          return {};
        });
        _getThemeSrcFromMappingStb = sinon.stub(themeCtrl, '_getThemeSrcFromMapping').callsFake(() => {
          let readSrc = path
