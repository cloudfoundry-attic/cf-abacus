...
describe('userProfile Routes', () => {
  ...
  describe('/cssupoad route with success', () => {
    ...
    beforeEach(function() {
      uploadFormDBStb = sinon.stub(dbUtils.prototype, 'saveCSSToDB').callsFake((path, name, opt, cb) => {
  ...
  describe('/cssupload route with error', () => {
    let uploadFormStb;

    describe('with error code', () => {
      before(() => {
        uploadFormStb = sinon.stub(themeCtrl, 'processIncommingForm').throws({
          'status': 400,
          'message': 'Bad Request'
        });
      });
  ...
  describe('/getThemeMetadata route with success', () => {
    ...
    beforeEach(function() {
      ...
      fetchRecordsStb = sinon.stub(dbUtils.prototype, 'fetchRecords').callsFake((coll, opt, cb) => {
        return cb(null, [2]);
      });
    });
  ...
  describe('/saveThemePreference route with success', () => {
    ...
    before(() => {
      saveThemePreferenceStb = sinon.stub(themeCtrl, 'saveThemePreference').callsFake((req, cb) => {
        return cb(true);
      });
    });
  ...
  describe('/getThemePreference route with success', () => {
    ...
    before(() => {
      getThemePreferenceStb = sinon.stub(themeCtrl, 'getThemePreference').callsFake((req, cb) => {
        return cb(null, true);
      });
    });
  ...
  describe('/getDefaultThemeFile route', () => {
    ...
    before(() => {
      getDefaultThemeFileStb = sinon.stub(themeCtrl, 'getDefaultThemeFile').callsFake((req, cb) => {
        return cb(true);
      });
    });
  ...
  describe('/removePrefAndLoadPreDefTheme route', () => {
    ...
    beforeEach(function() {
      removeRecordsDBStb = sinon.stub(dbUtils.prototype, 'removeRecords').callsFake((col, opt, cb) => {
        return cb(null, 'saved');
      });
    });
