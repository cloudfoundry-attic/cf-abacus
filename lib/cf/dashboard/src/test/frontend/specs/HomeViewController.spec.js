...
describe('controller:: HomeViewModuleController', function() {
  var $controller, scope, $httpBackend, controller, $stateParams, $window;
  beforeEach(function() {
  ...
  beforeEach(inject(function(_$rootScope_, _$controller_, _$httpBackend_,_$translate_,_$window_) {
  ...
  describe('should load data successfully', function() {
    ...
    describe('download plan dialog test', function() {
      var openLoadingSpinnerSpy, closeLoadingSpinnerSpy, getMessageSpy;

      beforeEach(function() {
        openLoadingSpinnerSpy = spyOn(ResourceProviderFactory, 'openLoadingSpinner');
        closeLoadingSpinnerSpy = spyOn(ResourceProviderFactory, 'closeLoadingSpinner');
        getMessageSpy = spyOn(ResourceProviderFactory, 'getMessage').and.returnValue('');
      });

      afterEach(function() {
        openLoadingSpinnerSpy.calls.reset();
        closeLoadingSpinnerSpy.calls.reset();
        getMessageSpy.calls.reset();
      });

      it('should call get meteringplan success', function() {
        $httpBackend.expectGET('v1/metering/plans/test-metering-plan')
          .respond(200, readJSON('mock/plan.json'));
        $httpBackend.expectGET('components/home/templates/viewMeteringPlanDialog.html').respond(200);
        controller.onDownloadPlanClick();
        $httpBackend.flush();
        expect(openLoadingSpinnerSpy).toHaveBeenCalled();
      });

      it('should call get meteringplan failure', function() {
        $httpBackend.expectGET('v1/metering/plans/test-metering-plan')
          .respond(400, {});
        $httpBackend.expectGET('components/partials/ErrorBox.html')
          .respond(200);
        ...
        $httpBackend.flush();
        expect(openLoadingSpinnerSpy).toHaveBeenCalled();
        expect(closeLoadingSpinnerSpy).toHaveBeenCalled();
        expect(getMessageSpy).toHaveBeenCalledWith('ResourceProvider_ErrorBox_viewPlanDialog_XMSG');
      });

      describe('downloadMeteringPlanController', function() {
        var modalScope;
        var mockData = readJSON('mock/plan.json');
        var modalInstance = {
          close: jasmine.createSpy('modalInstance.close'),
        ...
        beforeEach(function() {
          ...
          blobSpy = spyOn($window, 'Blob');
          blobObj = 'some-data';
          blobSpy.and.callFake(function() {
            return blobObj;
          });
        });

        it('should check metering plan', function() {
          expect(modalScope.doc.metering).toBe(
            JSON.stringify(mockData, undefined, 2)
          );
        });

        it('should dissmiss on ok click', function() {
          modalScope.onOk();
          expect(modalInstance.dismiss).toHaveBeenCalledWith('cancel');
        });

        it('should call download for IE',function() {
          var msSaveBlobSpy = jasmine.createSpy();
          $window.navigator = {};
          $window.navigator.msSaveOrOpenBlob = msSaveBlobSpy;
          modalScope.onDownload();
          expect(msSaveBlobSpy.calls.mostRecent().args[0]).toEqual(Object({}));
          expect(msSaveBlobSpy.calls.mostRecent().args[1]).toBe(fileName);
          $window.navigator.msSaveOrOpenBlob = null;
        });

        it('should call download for non IE', function() {
          var createObjectURLSpy = spyOn($window.URL, 'createObjectURL');
          var uriObject = 'uriobject';
          createObjectURLSpy.and.callFake(function() {
            return uriObject;
          });
          blobSpy.and.returnValue('blobObject');
          var atag = angular.element('<a>Download</a>');
          var angularSpy = spyOn(angular, 'element');
          angularSpy.and.callFake(function() {
            return atag;
          });
          var cssSpy = spyOn(atag, 'css');
          var attrSpy = spyOn(atag, 'attr');
          var linkSpy = spyOn(atag[0], 'click');
          var removeSpy = spyOn(atag, 'remove');
          var appendSpy = spyOn(atag, 'append');
          linkSpy.and.callFake(function() { });
          var revokeURLSpy = spyOn($window.URL, 'revokeObjectURL');
          revokeURLSpy.and.callFake(function() { });

          modalScope.onDownload();
          expect(blobSpy.calls.mostRecent().args[1]).toEqual({ type: 'text/json' });
          expect(createObjectURLSpy).toHaveBeenCalled();
          expect(cssSpy).toHaveBeenCalledWith('display', 'none');
          expect(attrSpy).toHaveBeenCalledWith('href', 'uriobject');
          expect(attrSpy).toHaveBeenCalledWith('download', fileName);
          expect(linkSpy).toHaveBeenCalled();
          expect(removeSpy).toHaveBeenCalled();
          expect(appendSpy).toHaveBeenCalledWith(atag);
          expect(revokeURLSpy).toHaveBeenCalledWith(uriObject);
        });
      });
