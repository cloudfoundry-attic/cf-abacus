...
describe('lib', () => {
  ...
  describe('middleware', () => {
    ...
    describe('authmiddleware unauthenticated/accesstokenexpired', () => {
      ...
      it('should not authenticate /healthcheck call', (done) => {
        req.originalUrl = 'http://service.dashboard.com/healthcheck';
        req.baseUrl = '/healthcheck';
        auth.ensureAuthenticated(req,res,next);
        sinon.assert.neverCalledWith(passportAuthStub,req,res,next);
        done();
      });
    });
  });
