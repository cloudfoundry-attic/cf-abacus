/*
 abacus-service-dashboard 2018-03-27 
*/
"user strict";
...
angular.module("Resource-Provider.userProfile", [ "MessageBoxService" ]), angular.module("httpInterceptor", []).factory("interceptorFactory", [ "$injector", "$q", "$stateParams", function(a, b, c) {
    ...
    }, i.onDownloadPlanClick = function() {
        d.openLoadingSpinner(), d.getMeteringPlan(e.planId).then(function(a) {
            d.closeLoadingSpinner(), e.modalInstance = h.open({
            ...
            d.closeLoadingSpinner();
            var b = d.constructErrorMessage("ResourceProvider_ErrorBox_viewPlanDialog_XMSG", "", a.statusText);
            g.openErrorBox(b);
        });
    }, i.initController();
...
} ]).controller("downloadMeteringPlanController", [ "$scope", "$window", "$uibModalInstance", "data", function(a, b, c, d) {
    a.doc = {}, a.doc.metering = JSON.stringify(d, void 0, 2), a.onDownload = function() {
        var c = new b.Blob([ a.doc.metering ], {
            type: "text/json"
        }), d = "metering-plan.json";
        if (b.navigator && b.navigator.msSaveOrOpenBlob) b.navigator.msSaveOrOpenBlob(c, d); else {
            var e = b.URL.createObjectURL(c), f = angular.element("<a>Download</a>");
            f.css("display", "none"), f.attr("href", e), f.attr("download", d), angular.element(b.document.body).append(f), 
            f[0].click(), f.remove(), b.URL.revokeObjectURL(e);
        }
    }, a.onOk = function() {
        c.dismiss("cancel");
    };
