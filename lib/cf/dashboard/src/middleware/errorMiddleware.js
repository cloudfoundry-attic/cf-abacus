...
const edebug = require('abacus-debug')('e-abacus-dashboard');

exports.notFound = () => (req, res, next) => {
  // exclude /healthcheck from notfound middleware
  if(req.path === '/healthcheck')
    return next();
  return next(new NotFound(`Unable to find any resource matching the requested path '${req.path}'`));
};
...
exports.error = (opts) => {
  ...
  return function(err, req, res, next) {
    ...
    debug('default format is', defaultFormat);
