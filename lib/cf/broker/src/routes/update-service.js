...
module.exports = (serviceMappingClient, provisioningClient, config, planBuilder) => {
  ...
  const updateServiceMapping = async (requestParameters, resourceProvider, planId) => {
  ...
  const updateService = async (request, response) => {
    ...
    try {
      if (userProvidedPlan && userProvidedPlan.resource_provider)
        await updateServiceMapping(requestParameters, userProvidedPlan.resource_provider, planId);
    } catch (error) {
