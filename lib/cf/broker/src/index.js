...
const updateService = require('./routes/update-service.js');
...
routes.put('/v2/service_instances/:instance_id',
  basicAuth, createService(
    (mappingApi) => new ServiceMappingClient(mappingApi, authHeaderProvider, skipSslValidation),
    provisioningClient, config, planBuilder).createService);
...
routes.patch('/v2/service_instances/:instance_id',
  basicAuth, updateService(
    (mappingApi) => new ServiceMappingClient(mappingApi, authHeaderProvider, skipSslValidation),
    provisioningClient, config, planBuilder).updateService);;
