...
describe('Create Service', () => {
  ...
  context('when createService is called', () => {
    ...
    context('when service mapping validation is successful', () => {
      ...
      context('when request contains resource provider', () => {
        ...
        context('when create service mapping is successful', () => {
          ...
          // it('should configure plan builder', async () => {
          //   assert.calledOnce(planBuilderStub.withUserProvidedPlan);
          //   assert.calledWith(planBuilderStub.withUserProvidedPlan,
          //     requestWithResourceProvider.body.parameters.plans[0]);
          //   assert.calledOnce(planBuilderStub.withPlanId);
          //   assert.calledWith(planBuilderStub.withPlanId, generatedPlanId);
          // });
        ...
        const createBadRequestErrorContext = (stub) =>
          context('when fails with "bad request" error', () => {
            ...
            it('should return error description', () => {
              assert.calledWithExactly(response.send, { description: 'Provided plan is not valid.' });
            });
          });
    ...
    context('when service mapping validation is NOT successful', () => {
