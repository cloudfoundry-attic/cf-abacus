...
const loadServiceGuidsConfiguration = (retryFn, cb) => {
  paging.readPage('/v2/services?=label IN ' +
    serviceLabelsWithMissingGuid(serviceConfigs).join(','),
    cfAdminToken, perf, statistics, { processResourceFn: (event, done) => {
      serviceConfigs[event.entity.label].guid = event.metadata.guid;
      done();
    },
      failure: (error, response) => {
