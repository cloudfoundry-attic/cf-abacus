...
const getResourceIdFromLabel = (serviceLabel) => {
...
const buildServiceUsage = (event, cb) => {
  const resourceId = getResourceIdFromLabel(event.entity.service_label);
  if (!resourceId || !supportedState(event.entity.state)) {
    cb(undefined, undefined);
    return;
  }

  const planId = getServiceSize(resourceId, event.entity.service_plan_guid);
  if (!planId) {
    cb(undefined, undefined);
    return;
  }
...
const scheduleUsageReporting = () => {
  ...
  if (secured) {
    ...
    linuxContainerToken = oauth.cache(uris().auth_server,
  ...
  module.usageReporter = setTimeout(() => {
    ...
    reportServiceUsage(cfAdminToken, linuxContainerToken, {
      failure: (error, response) => {},
      success: () => {}
    });
