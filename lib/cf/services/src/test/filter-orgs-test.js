...
process.env.SERVICES = '[map[PLANS:map[4fd1a379-2738-408e-9020-c5238a47a004' +
  ':medium] NAME:mongodb GUID:bc3690b2-cc50-4475-b2cf-44d68c51f9d3]]';
...
describe('Filtering usage', () => {
  ...
  const generateUsageReport = (serviceId, orgId, instances) => {
    return {
      ...
      resource_id: 'mongodb',
      plan_id: 'medium',
      resource_instance_id: 'service:medium:' + serviceId,
      measured_usage: [
        {
  ...
  const reportServiceUsage = (done) => {
    ...
    bridge.fetchServiceConfigurations();
  ...
  context('with last recorded guid', () => {
    ...
    it('uses the guid to get app usage events from API', () => {
      ...
      checkGetRequest('http://api', '/v2/service_usage_events?' +
        'order-direction=asc&results-per-page=50&' +
        'q=service_guid IN bc3690b2-cc50-4475-b2cf-44d68c51f9d3&' +
        'after_guid=1', args[0]);
      checkGetRequest('http://api', '/page2', args[1]);
    });
