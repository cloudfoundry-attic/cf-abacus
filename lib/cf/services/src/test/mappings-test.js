...
const extmappings = require('abacus-plan-mappings');
...
describe('mappings', () => {
  describe('storeServiceMappings', () => {
    ...
    beforeEach(() => {
      newMeteringMappingStub = sandbox.stub(extmappings, 'newMeteringMapping').returns({});
      newPricingMappingStub = sandbox.stub(extmappings, 'newPricingMapping').returns({});
      newRatingMappingStub = sandbox.stub(extmappings, 'newRatingMapping').returns({});
    });
    ...
    it('should store mappings based on the configuration', (done) => {
      const verifyCalls = (stub, planId) => {
        ...
        expect(stub.firstCall.args).to.deep.equal(['service1', 'plan1', planId, rev]);
        expect(stub.secondCall.args).to.deep.equal(['service1', 'plan2', planId, rev]);
        expect(stub.thirdCall.args).to.deep.equal(['service2', 'plan2', planId, rev]);
      };
