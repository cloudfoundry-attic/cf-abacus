...
describe('Test server initialization', () => {
  ...
  const setupAbacusOauth = (tokenStartFn = (cb) => cb()) => {
    delete require.cache[require.resolve('abacus-oauth')];
    const oauth = require('abacus-oauth');
    cacheStub = sandbox.stub(oauth, 'cache').callsFake(() => {
      const f = () => 'Bearer abc';
      f.start = tokenStartFn;

      return f;
    });
  };

  const startServiceBridge = (tokenStartFn) => {
    delete require.cache[require.resolve('..')];
    setupAbacusOauth(tokenStartFn);
    service = require('..');
    service();
  };
  ...
  context('when service plans and labels are provided', () => {
    ...
    it('should retry retrieving admin token in case of error', (done) => {
      const tokenStartStub = sandbox.stub();

      tokenStartStub.onFirstCall().callsFake((cb) => {
        expect(typeof cb).to.equal('function');
        cb(new Error('some_error'));
      });

      tokenStartStub.onSecondCall().callsFake(() => {
        done();
      });

      startServiceBridge(tokenStartStub);
    }).timeout(5000);;
      ...
      it('should read service usage events based on them', (done) => {
        ...
        getStub.onSecondCall().callsFake((uri, opts, cb) => {
          assert.calledWith(getStub.firstCall, sinon.match.any,
            sinon.match({ page: '/v2/services?q=label IN service1,service2' }));
          expect(opts.page)
    ...
    it('should read only missing service guids',
      (done) => {
        ...
        getStub.onSecondCall().callsFake((uri, opts, cb) => {
          assert.calledWith(getStub.firstCall, sinon.match.any,
            sinon.match({ page: '/v2/services?q=label IN service2' }));
          expect(opts.page)
            .to.include(`service_guid IN ${serviceGuid1},${serviceGuid2}`);
          done();
        });
