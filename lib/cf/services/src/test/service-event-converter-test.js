...
describe('service event converter', () => {
  ...
  const createUsage = (currentInstances, previousInstances, state) => ({
    end: 1000,
    start: 1000,
    eventState: state,
    space_id: spaceGuid,
  ...
  context('when event is not mapped', () => {
    ...
    beforeEach(yieldable.functioncb(function*() {
      mapperStub.toMultipleEvents.callsFake(function*(fakeEvent) { 
        return undefined; 
      });
  ...
  context('when event is mapped to single event', () => {
    ...
    singleEventContext(states.CREATED, [createUsage(1, 0, states.CREATED)]);
    singleEventContext(states.DELETED, [createUsage(0, 1, states.DELETED)]);
  });
  ...
  context('when event is mapped to multiple events', () => {
    ...
    it('should convert to correct usage', () => {        
      const expectedUsages = [createUsage(0, 1, states.DELETED), createUsage(1, 0, states.CREATED)];    
      expect(usages).to.deep.equal(expectedUsages);
    });
