  ...
  yieldable.functioncb(function*() {
    for (let service in servicesConfig)
      for (let plan of servicesConfig[service].plans) {
        yield mappings.newMeteringMapping(service, plan, mappings.sampleMetering.service.standard, rev);
        yield mappings.newPricingMapping(service, plan, mappings.samplePricing.service.standard, rev);
        yield mappings.newRatingMapping(service, plan, mappings.sampleRating.service.standard, rev);
      }
  })((err) => {
    if (err) {
