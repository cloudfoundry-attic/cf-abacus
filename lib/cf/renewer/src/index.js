...
const filterTimeBasedMetrics = function *(usageDocs) {
  debug('Filtering time-based measures in %d usage docs', usageDocs.length);
  ...
  for (let doc of usageDocs) {
    ...
    try {
      ...
      for (let metric of plan.metrics)
        if (metric.type === 'time-based') {
...
const renewUsage = (systemToken, { failure, success }) => {
  ...
  yieldable.functioncb(function *() {
    ...
    const timeBasedDocs = yield filterTimeBasedMetrics(usageDocs);
...
module.exports.filterTimeBasedMetrics = filterTimeBasedMetrics;
