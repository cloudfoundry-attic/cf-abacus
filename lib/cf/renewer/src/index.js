...
const registerError = (message, error, response, name, time) => {
  ...
  errors.lastErrorTimestamp = moment().toISOString();
...
const processUsage = (reference, systemToken, failure, cb) => {
  ...
  getUsageDoc(id, systemToken, (doc, error, response) => {
    ...
    const rawStopUsage = zeroUsage(refreshUsage(doc, 0), 'current');
    const stopUsage = sanitizeUsageDoc(rawStopUsage);
    debug('Reporting stop usage for doc with id %s ...', doc.id);
    reportUsage(stopUsage, systemToken, (error, response) => {
      if (error) {
        failure(buildError('stop report', stopUsage, error, response),
          response);
        return;
      }
      if (response.statusCode == 409)
        debug('Skipped conflicting stop usage with id %s', doc.id);

      const rawStartUsage = zeroUsage(refreshUsage(doc, 1), 'previous');
      const startUsage = sanitizeUsageDoc(rawStartUsage);
      debug('Reporting start usage for doc with id %s ...', doc.id);
      reportUsage(startUsage, systemToken, (error, response) => {
        if (error) {
        ...
        carryOver.write(startUsage, response, (error) => {
          if (error) {
...
const renewUsagePage = (systemToken, startId, endId, skip,
  ...
  carryOver.readPage(startId, endId, pageSize, skip, (error, usage) => {
    ...
    let numReportedDocs = 0;
    const countReportedDocs = () => {
      if (++numReportedDocs === usage.length) {
    ...
    for (let ref of references)
      processUsage(ref, systemToken, failure, countReportedDocs);
  });
};
...
const renewUsage = (systemToken, { failure, success }) => {
  ...
  debug('Scheduling next execution on %s',
    moment().add(retryConfig.retryInterval, 'milliseconds').utc().toDate());
  setRenewTimeout(() => renewUsage(systemToken, { failure, success }),
