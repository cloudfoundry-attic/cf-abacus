...
const reliableRequest = yieldable(throttle(retry(breaker(batch(request))),
...
const postError = (usage, response) => {
  edebug('Failed reporting\n\tusage: %j\n\tresponse: %j', usage, response);
  return extend({},
    { statusCode: response.statusCode },
    response.body || {});
};

const reportUsage = function *(usage, token) {
  const t0 = Date.now();
  try {
    const response = yield reliableRequest.post(
    ...
    throw postError(usage, response);
  }
  catch(exc) {
    statistics.usage.reportFailures++;
    perf.report('report', t0, undefined, exc);
    throw exc;
  }
};
...
const reportUsageDocuments = function *(docs, systemToken) {
  ...
  yield reportUsage(refreshUsage(usage), systemToken);
  yield reportUsageDocuments(tail(docs), systemToken);
};
...
const sanitizeUsageDocs = (usageDocs) => map(usageDocs,
  (doc) => sanitizeUsageDoc(doc));

const getError = (id, response) => {
  edebug('Failed to get usage for\n\tdocument id %s\n\tresponse %j',
    id, response);
  return extend({}, {
    id: id,
...
const getUsageDocs = function *(references, systemToken) {
  ...
  for (let id of references) {
    const t0 = Date.now();
    try {
      const response = yield reliableRequest.get(
...
const renewUsagePage = function *(systemToken, startId, endId) {
  ...
  const usageDocs = yield getUsageDocs(references, systemToken);
  const carryOverDocs = map(usageDocs, (doc, index) =>
    extend(doc, { carry_over_id: appKeys[index] })
  );

  yield reportUsageDocuments(sanitizeUsageDocs(carryOverDocs), systemToken);

  if (carryOverDocs.length < pageSize) {
    debug('Read the last %d DB documents.', carryOverDocs.length);
    return;
  }

  const lastId = last(carryOverDocs).carry_over_id;
  yield renewUsagePage(systemToken, lastId, endId);
};
...
const renewUsage = (systemToken, { failure, success }) => {
  ...
  yieldable.functioncb(function *() {
    return yield renewUsagePage(systemToken, startId, endId);
  })((error, response) => {
    if (error || response) {
...
const startRenewer = () => {
  ...
  setRenewTimeout(() => renewUsage(systemToken, {
    ...
    failure: (err) => {
      edebug('Renewer failed due to %j', err);
    }
  }), 0);
