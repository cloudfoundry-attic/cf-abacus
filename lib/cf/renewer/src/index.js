...
const processUsage = (reference, systemToken, failure, cb) => {
  const key = dbclient.k(reference._id);
  carryOver.findInCurrentMonth(key, (error, doc) => {
    if (error) {
      failure(`Error while searching for matching documents for ${reference}. Error: ${error}`);
      cb();
      return;
    }
    ...
    getUsageDoc(id, systemToken, (doc, error, response) => {
      ...
      reporter.reportUsage(startUsage, systemToken, (error, response) => {
        ...
        carryOver.write(startUsage, response, reference.event_guid, reference.state, (error) => {
          if (error) {
