...
const processUsage = (reference, systemToken, failure, cb) => {
  ...
  getUsageDoc(id, systemToken, (doc, error, response) => {
    if (error || response) {
      const err = buildError('get', { collector_id: id }, error, response);
      debug('Failed to get usage for id %s due to %o', id, err);
    ...
    reporter.reportUsage(startUsage, systemToken, (error, response) => {
      if (error) {
        const err = buildError('start report', startUsage, error, response);
        debug('Failed to report usage for doc with id %s due to %o',
          doc.id, err);
        failure(err, response);
        cb();
        return;
      ...
      carryOver.write(startUsage, response, (error) => {
        if (error) {
          debug('Failed to store carry-over usage for doc with id %s due to %o',
            doc.id, error);
