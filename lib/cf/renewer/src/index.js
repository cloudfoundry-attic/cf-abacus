...
const buildKey = (doc) => util.format('%s/%s/%s/%s/%s/%s',
  doc.organization_id, doc.space_id, doc.consumer_id,
...
const renewUsage = (systemToken, { failure, success }) => {
  ...
  yieldable.functioncb(function *() {
    ...
    const descendingDocs = sortBy(usageSchemaDocs, (doc) => -1 * doc.start);
    return removeStalledUsage(descendingDocs);
  })((error, docs) => {
    if (error) {
      failure(error);
      return;
