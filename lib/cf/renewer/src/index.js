...
const extractDocs = (usageDocs) =>
  map(usageDocs, (usageDoc) => usageDoc.doc);

const extractSupportedUsageDocs = (usageDocs) => filter(
  extractDocs(usageDocs), (doc) => supportedStates.includes(doc.state));
...
const renewUsagePage = (systemToken, startId, endId, { failure, success }) => {
  ...
  const processPage = (usage, cb) => {
    ...
    async.forEachOfSeries(
      extractSupportedUsageDocs(usage), (reference, key, done) =>
        processUsage(reference, systemToken, failure, done), cb);
  };
