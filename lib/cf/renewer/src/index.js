...
const storeCarryOverUsage = (usage, response, cb) => {
  ...
  carryOverDB.get(key, (error, doc) => {
    if (error) {
      ...
      registerError('Error getting carry-over usage', error, undefined,
        'carryOver', t0);
      cb(error, response);
      return;
    }
    ...
    if (findWhere(usage.measured_usage, stoppedAppCriteria)) {
      ...
      carryOverDB.remove(updatedDoc, (error) => {
        ...
        if (opError) {
          ...
          registerError('Error removing carry-over usage', opError, undefined,
            'carryOver', t0);
        }
        else {
      ...
      getUsageDoc(id, systemToken, (doc, error, response) => {
        ...
        reportUsage(stopUsage, systemToken, (error, response) => {
          ...
          reportUsage(startUsage, systemToken, (error, response) => {
            ...
            storeCarryOverUsage(startUsage, response, (error, response) => {
              if (error || response) {
                failure(buildError('carry over', startUsage, error, response),
                  response);
                return;
              }
