...
const docRequest = retry(breaker(request));
...
// Default page size
// Each page is processed in parallel, so by default we limit the size to 100
...
const systemToken = secured
  ? oauth.cache(
    uris().auth_server,
    process.env.ABACUS_CLIENT_ID,
    process.env.ABACUS_CLIENT_SECRET,
...
const getUsageDoc = (id, systemToken, cb) => {
  ...
  docRequest.get(
    ...
    (error, response) => {
      ...
      statistics.usage.get.failures++;
      errors.consecutiveGetFailures++;
      registerError(util.format('Failed getting usage: %j', doc), error, response, 'get', t0);
...
const zeroUsage = (usage, usageType) => {
  ...
  for (let measureObject of clonedUsage.measured_usage)
    if (measureObject.measure.startsWith(usageType)) measureObject.quantity = 0;
...
const extractDocs = (usageDocs) => map(usageDocs, (usageDoc) => usageDoc.doc);

const extractSupportedUsageDocs = (usageDocs) =>
  filter(extractDocs(usageDocs), (doc) => supportedStates.includes(doc.state));

const resourceUsageSchemaProperties = map(pairs(usageSchemas.resourceUsage.json().properties), (p) => p[0]);

const sanitizeUsageDoc = (doc) => pick(doc, resourceUsageSchemaProperties);

const setRenewTimeout = (fn, interval) => {
  clearTimeout(module.usageRenewer);
  module.usageRenewer = setTimeout(fn, interval);
  debug('Retry interval set to %d ms after %d retries', interval, statistics.retries.count);
...
const processUsage = (reference, systemToken, failure, cb) => {
  ...
  getUsageDoc(id, systemToken, (doc, error, response) => {
    if (error || response) {
      failure(buildError('get', { collector_id: id }, error, response), response);
      cb();
      return;
    }
    ...
    debug('Reporting start usage for doc with id %s ...', id);
    reporter.reportUsage(startUsage, systemToken, (error, response) => {
      if (error) {
        failure(buildError('start report', startUsage, error, response), response);
        statistics.usage.report.failures++;
        cb();
        return;
      }
      if (response.statusCode === UNAVAILABLE_FOR_LEGAL_REASONS) {
        debug('Skipped trial account usage with id %s', id);
        statistics.usage.report.skipped.legal_reasons++;
        cb();
        return;
      }
      if (response.statusCode === 409) {
        debug('Skipped conflicting start usage with id %s', id);
        statistics.usage.report.skipped.conflicts++;
        cb();
        return;
      }

      debug('Successfully reported usage for doc with id %s', id);
      statistics.usage.report.success++;

      carryOver.insert(startUsage, response, reference.event_guid, reference.state, (error) => {
        if (error) {
          if (error.existingDocument) {
            debug('Document for usage with id "%s" already exists in carryover.', id);
            cb();
            return;
          }
        ...
        debug('Successfully inserted document in carryover for usage with id "%s"', id);
        cb();
      });
    });
  });
...
const renewUsagePage = (systemToken, startId, endId, { failure, success }) => {
  debug('Reading usage data from %s to %s with limit %d', startId, endId, pageSize);

  const processPage = (usage, cb) => {
    debug('Processing %d DB documents ...', usage.length);
    async.forEachOfSeries(
      extractSupportedUsageDocs(usage),
      (reference, key, done) => processUsage(reference, systemToken, failure, done),
      cb
    );
  };

  carryOver.readAllPages(
    {
      startId: startId,
      endId: endId,
      pageSize: pageSize
...
const renewUsage = (systemToken, { failure, success }) => {
  ...
  if (secured && !systemToken()) {
    setRenewTimeout(() => renewUsage(systemToken, { failure, success }), 5000);
    statistics.usage.get.missingToken++;
  ...
  debug(
    'Scheduling next execution on %s',
    moment
      .utc()
      .add(retryConfig.retryInterval, 'milliseconds')
      .toDate()
  );
  setRenewTimeout(() => renewUsage(systemToken, { failure, success }), retryConfig.retryInterval);
  ...
  const endTime = moment
    ...
    .endOf('month');
  debug(
    'Will scan for usage between %s and %s with slack window %d ms',
  ...
  const endId = 't/' + dbclient.pad16(endTime.valueOf()) + 'ZZZ';
...
const startRenewer = () => {
  ...
  if (secured) systemToken.start();

  setRenewTimeout(
    () =>
      renewUsage(systemToken, {
        success: () => debug('Renewer completed successfully'),
        failure: (e) => edebug('Renewer failed due to: %o', e)
      }),
    5000
  );
...
const createStatsRoute = () => {
  const route = router();
  if (secured) route.use(oauth.authorizer(process.env.JWTKEY, process.env.JWTALGO, ['abacus.usage.read']));
...
const renew = () => {
  ...
  const app = webapp();
  app.use('/v1/stats', createStatsRoute());
