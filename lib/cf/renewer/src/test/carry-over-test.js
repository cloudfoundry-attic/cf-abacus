...
const tests = (secured) => {
  ...
  context('on success', () => {
    ...
    it('inserts in carry-over db', () => {
      ...
      assert.alwaysCalledWith(writeSpy, generateUsage(appUsage, 0, 'previous'));
    });
  });
  ...
  context('on failure storing carry-over', () => {
    ...
    it('does not store in carry-over db', () => {
      expect(writeSpy.callCount).to.equal(2);
      assert.calledWith(writeSpy, changeOrgId(generateUsage(appUsage, 0, 'previous'), '1'));
      assert.calledWith(writeSpy, changeOrgId(generateUsage(appUsage, 0, 'previous'), '2'));
    });
  });
};
