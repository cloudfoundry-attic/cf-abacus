...
describe('Stalled usage', () => {
  ...
  const changeOrgId = (usage, guid) => extend({}, usage, {
    organization_id: 'us-south:' + guid
  });

  const changeAppId = (usage, guid) => extend({}, usage, {
    consumer_id: 'app:' + guid,
    resource_instance_id: 'memory:' + guid
  });
  ...
  context('single application', () => {
    ...
    context('stopped then started application', () => {
      const appUsage = [
        runningAppUsage, stoppedAppUsage, runningAppUsage
      ].reverse();
    ...
    context('restarted then scaled application', () => {
      const appUsage = [
        runningAppUsage, stoppedAppUsage, runningAppUsage, scaledAppUsage
      ].reverse();
  ...
  context('multiple applications in different orgs', () => {
    ...
    context('app1 is restarted and scaled, app2 is started and scaled', () => {
      const app1Usage = [
        runningAppUsage, stoppedAppUsage, runningAppUsage, scaledAppUsage
      ].reverse();
      const app2Usage = [
        runningApp2Usage, scaledApp2Usage
      ].reverse();
  ...
  context('multiple applications in the same org', () => {
    const runningApp2Usage = changeAppId(runningAppUsage, 2);
    const stoppedApp2Usage = changeAppId(stoppedAppUsage, 2);
    const scaledApp2Usage = changeAppId(scaledAppUsage, 2);
