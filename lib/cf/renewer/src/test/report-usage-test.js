  ...
  context('with usage in db', () => {
    ...
    context(`on error getting ${usageType.name} usage`, () => {
      ...
      beforeEach(() => {
        ...
        reqmock = extend({}, request, {
          get: spy((uri, opts, cb) => {
            cb(opts.usage_id === collectorIdToError ? 'error' : undefined, buildResponse(200, {}, usageType.events));
          }),
          post: spy((uri, opts, cb) => {
        ...
        beforeEach((done) => {
          ...
          renewer.renewUsage(systemToken, {
            failure: (error, response) => {
              ...
              expect(error.response).to.deep.equal(buildResponse(200, {}, usageType.events));
              expect(response).to.deep.equal(buildResponse(200, {}, usageType.events));
            },
            success: () => {
              done();
            }
      ...
      beforeEach(() => {
        ...
        reqmock = extend({}, request, {
          ...
          post: spy((uri, opts, cb) => {
            cb(
              undefined,
              buildResponse(202, { location: 'some location' }, changeOrgId(usageType.events, opts.usage_id))
            );
          })
        });
        require.cache[require.resolve('abacus-request')].exports = reqmock;
