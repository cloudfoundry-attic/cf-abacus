...
const tests = (usageType, secured) => {
  ...
  const renewUsage = (done) => {
    renewer.renewUsage(systemToken, {
      failure: (error, response) => {
        done(new Error(util.format('Unexpected call of failure with ' + 'error %j and response %j', error, response)));
      },
      success: () => {
        done();
      }
  ...
  const checkGetRequest = (request, collectorId) => {
    expect(request[0]).to.equal(':collector/v1/metering/collected/usage/:usage_id');
    expect(request[1]).to.contain.all.keys('collector', 'usage_id', 'headers');
  ...
  context('with usage in db', () => {
    context('with multiple events', () => {
      beforeEach((done) => {
        ...
        reqmock = extend({}, request, {
          get: spy((uri, opts, cb) => {
            cb(undefined, { statusCode: 200, body: { usageDoc: usageType.events } });
          }),
      ...
      it('reports refreshed usage to COLLECTOR', () => {
        ...
        expect(args.length).to.equal(2);
        checkPostRequest(args[0], generateUsage(usageType.events, 0, 'previous'));
        checkPostRequest(args[1], generateUsage(usageType.events, 0, 'previous'));
      });
      ...
      it('counts the reported usage', () => {
        ...
        expect(renewer.statistics.usage.report.skipped.conflicts).to.equal(0);
        expect(renewer.statistics.usage.report.skipped.legal_reasons).to.equal(0);
    ...
    context(`on error getting ${usageType.name} usage`, () => {
        ...
        reqmock = extend({}, request, {
          get: spy((uri, opts, cb) => {
            cb(opts.usage_id === collectorIdToError ? 'error' : undefined,
              buildResponse(200, {}, { usageDoc: usageType.events }));
          }),
          post: spy((uri, opts, cb) => {
            cb(undefined, buildResponse(202, { location: 'some location' }));
          })
        });
        require.cache[require.resolve('abacus-request')].exports = reqmock;
      ...
      context('on the first org usage', () => {
        ...
        beforeEach((done) => {
          ...
          renewer.renewUsage(systemToken, {
            failure: (error, response) => {
              ...
              expect(error.response).to.deep.equal(buildResponse(200, {}, { usageDoc: usageType.events }));
              expect(response).to.deep.equal(buildResponse(200, {}, { usageDoc: usageType.events }));
            },
            success: () => {
              done();
            }
        ...
        it('raises correct reporting errors', () => {
          ...
          const errorTime = moment.utc(renewer.errors.lastErrorTimestamp).valueOf();
      ...
      beforeEach(() => {
        ...
        reqmock = extend({}, request, {
          ...
          post: spy((uri, opts, cb) => {
            cb(
              undefined,
              buildResponse(202, { location: 'some location' },
                { usageDoc: changeOrgId(usageType.events, opts.usage_id) })
            );
          })
        });
        require.cache[require.resolve('abacus-request')].exports = reqmock;
        ...
        beforeEach((done) => {
          ...
          renewer.renewUsage(systemToken, {
            failure: (error, response) => {
              ...
              expect(error.response).to.deep.equal(buildResponse(errorResponseCode, {}));
              expect(response).to.deep.equal(buildResponse(errorResponseCode, {}));
            },
            success: () => {
              done();
            }
    ...
    context('on error during reporting', () => {
      const mockedResponse = buildResponse(202, { location: 'some location' });
      ...
      beforeEach(() => {
        ...
        reqmock = extend({}, request, {
          get: spy((uri, opts, cb) => {
            cb(undefined, {
              statusCode: 200,
              body: { usageDoc: changeOrgId(usageType.events, opts.usage_id) }
            });
          }),
          post: spy((uri, opts, cb) => {
            cb(++numPostRequests === numPostRequestToError ? 'error' : undefined, mockedResponse);
          })
        });
        require.cache[require.resolve('abacus-request')].exports = reqmock;
      ...
      context('on the first org usage', () => {
        ...
        it('reports refreshed usage to COLLECTOR', () => {
          const args = reqmock.post.args;
          expect(args.length).to.equal(2);
          checkPostRequest(args[0], changeOrgId(generateUsage(usageType.events, 0, 'previous'), '1'));
        });
      ...
      context('on the last org usage', () => {
        ...
        it('reports refreshed usage to COLLECTOR', () => {
          ...
          expect(args.length).to.equal(2);
          checkPostRequest(args[0], changeOrgId(generateUsage(usageType.events, 0, 'previous'), '1'));
          checkPostRequest(args[1], changeOrgId(generateUsage(usageType.events, 0, 'previous'), '2'));
        });
    ...
    context('on bad response during reporting', () => {
        ...
        beforeEach((done) => {
          ...
          renewer.renewUsage(systemToken, {
            failure: (error, response) => {
              ...
              expect(response).to.deep.equal(buildResponse(errorResponseCode, {}, errorResponseBody));
            },
            success: () => {
              done();
            }
      ...
      context('on the last org usage', () => {
        ...
        it('raises correct reporting errors', () => {
          ...
          expect(renewer.errors.lastError).to.match(/^Failed reporting usage/);
          const errorTime = new moment(renewer.errors.lastErrorTimestamp).valueOf();
          expect(errorTime).to.be.at.least(startTime);
...
for (const secured of [true, false])
  for (const usageType of usageList)
    describe(`Report ${usageType.name} usage ` + `${secured ? 'with' : 'without'} security`, () =>
