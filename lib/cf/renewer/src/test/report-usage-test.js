...
const tests = (secured) => {
  ...
  const checkPostRequest = (req, usage) => {
    ...
    const usageToCheck = extend(usage, {
      start: moment(usage.start).add(1, 'month').valueOf(),
      end: moment(usage.end).add(1, 'month').valueOf()
    });
  ...
  context('on non-empty usage event stream', () => {
    ...
    context('with single apps', () => {
      ...
      it('reports resource usage to COLLECTOR', (done) => {
        ...
        checkPostRequest(args[0], scaledAppUsage);
    ...
    context('with multiple apps', () => {
      ...
      it('reports resource usage to COLLECTOR', (done) => {
        ...
        checkPostRequest(args[0], scaledAppUsage);
        checkPostRequest(args[1], runningApp3Usage);
    ...
    context('on error during reporting', () => {
      ...
      beforeEach(() => {
        ...
        reqmock = extend({}, request, {
          post: spy((uri, opts, cb) => {
            cb(opts.body.organization_id === orgIdToError ? 'error' : undefined,
              mockedResponse);
          })
        });
        require.cache[require.resolve('abacus-request')].exports = reqmock;
      ...
      context('on the last usage', () => {
        ...
        it('tried to report all usage to COLLECTOR', (done) => {
          const args = reqmock.post.args;
          expect(args.length).to.equal(2);
          checkPostRequest(args[0], scaledAppUsage);
          checkPostRequest(args[1], runningApp3Usage);
      ...
      context('on the first usage', () => {
        ...
        it('reports only one usage to COLLECTOR', (done) => {
          const args = reqmock.post.args;
          expect(args.length).to.equal(1);
          checkPostRequest(args[0], scaledAppUsage);
    ...
    context('on bad response during reporting', () => {
      ...
      beforeEach(() => {
        ...
        reqmock = extend({}, request, {
          post: spy((uri, opts, cb) => {
            cb(undefined, opts.body.organization_id === orgIdToError ?
  ...
  context('on empty usage event stream', () => {
    ...
    it('does not report any usage to COLLECTOR', (done) => {
      const args = reqmock.post.args;
    ...
    runWithSecurity('calls back with error', (done) => {
      renewer.renewUsage(() => undefined, {
        failure: (error, response) => {
          ...
          expect(error).to.equal('Missing resource provider token');
