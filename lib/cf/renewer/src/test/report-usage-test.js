...
const tests = (secured) => {
  ...
  const checkGetRequest = (request, appId, collectorId) => {
    ...
    expect(request[1].usage_id).to.deep.equal({
      _id: appId,
  ...
  context('on non-empty usage event stream', () => {
    ...
    context('with multiple apps', () => {
      ...
      it('gets the real usage from the COLLECTOR', () => {
        ...
        checkGetRequest(args[0], 'app1', '1');
        checkGetRequest(args[1], 'app2', '2');
      });
    ...
    context('on error getting usage', () => {
      ...
      beforeEach(() => {
        ...
        reqmock = extend({}, request, {
          get: spy((uri, opts, cb) => {
            cb(opts.usage_id.collector_id === collectorIdToError ?
              'error' : undefined,
              { statusCode: 200, body: appUsage });
          }),
          post: spy((uri, opts, cb) => {
    ...
    context('on bad response getting usage', () => {
      ...
      beforeEach(() => {
        ...
        reqmock = extend({}, request, {
          get: spy((uri, opts, cb) => {
            cb(undefined,
              opts.usage_id.collector_id === collectorIdToError ?
                getResponse(errorResponseCode) : getResponse(200));
          }),
          post: spy((uri, opts, cb) => {
            cb(undefined, {
              statusCode: 201,
        ...
        beforeEach((done) => {
          ...
          renewer.renewUsage(systemToken, {
            failure: (error, response) => {
              ...
              expect(error.status).to.equal(errorResponseCode);
              expect(response).to.equal(undefined);
              done();
            },
            success: () => {
        ...
        beforeEach((done) => {
          ...
          renewer.renewUsage(systemToken, {
            failure: (error, response) => {
              ...
              expect(error).to.deep.equal(getResponse(errorResponseCode));
              expect(response).to.equal(undefined);
              done();
            },
            success: () => {
