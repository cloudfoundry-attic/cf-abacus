...
const tests = (secured) => {
  ...
  const buildDocs = (usageDocs) => {
    // Fix usage docs start/end times
    const shiftedDocs = map(usageDocs, (doc, key) => ({
      doc: extend({}, doc, { start: key, end: key })
    }));
    // Return docs in descending order
    return { rows: sortBy(shiftedDocs, (doc) => doc.doc.id).reverse() };
  };
  ...
  context('on non-empty usage event stream', () => {
    ...
    context('with single apps', () => {
      beforeEach((done) => {
        ...
        docs = buildDocs([runningAppUsage, stoppedAppUsage, scaledAppUsage]);
    ...
    context('with multiple apps in different orgs', () => {
      ...
      beforeEach((done) => {
        ...
        docs = buildDocs([runningAppUsage, stoppedAppUsage, scaledAppUsage,
          stoppedApp2Usage,
          scaledApp3Usage, runningApp3Usage]);
    ...
    context('with multiple apps in the same org', () => {
      const stoppedApp2Usage = changeAppId(stoppedAppUsage, 2);

      const runningApp3Usage = changeAppId(runningAppUsage, 3);
      const scaledApp3Usage = changeAppId(scaledAppUsage, 3);
