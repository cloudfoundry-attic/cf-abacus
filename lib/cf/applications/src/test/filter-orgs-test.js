...
describe('Filtering usage', () => {
  ...
  const cleanDB = (cb) => {
    ...
    dbclient.drop(process.env.DB, /^abacus-cf-bridge-/, () => {
      dbclient.drop(process.env.DB, /^abacus-carry-over-/, cb);
    });
  };
  ...
  const prepareTestEnvironment = (cb) => {
  ...
  beforeEach((done) => {
    cleanDB(() => deleteModules(() => prepareTestEnvironment(done)));
  });
  ...
  const wrapGet = (request, fn) => {
    return spy((uri, opts, cb) => {
      if (opts.page)
        fn(uri, opts, cb);
      else
        request.get(uri, opts, cb);
    });
  };

  const wrapPost = (request, fn) => {
    return spy((uri, opts, cb) => {
      if (fn)
        fn(uri, opts, cb);
      else
        request.post(uri, opts, cb);
    });
  };

  const defaultGetFn = (uri, opts, cb) => {
    if (opts.page.indexOf('page2') > -1)
      cb(null, { statusCode: 200, body: appUsagePageTwo });
    else
      cb(null, { statusCode: 200, body: appUsagePageOne });
  };
  const defaultPostFn = (uri, opts, cb) => {
    cb(null, {
      statusCode: 201,
      body: {},
  ...
  const mockRequestModule = ({ getFn, postFn }) => {
    const request = require('abacus-request');
    reqmock = extend({}, request, {
      get: wrapGet(request, getFn),
      post: wrapPost(request, postFn)
    });
  ...
  context('with empty list of orgs to report', () => {
    ...
    it('gets all app usage events from API', () => {
      ...
      checkGetRequest('http://api', '/v2/app_usage_events?' +
        'order-direction=asc&results-per-page=100', args[0]);
  ...
  context('with last recorded guid', () => {
    ...
    it('uses the guid to get app usage events from API', () => {
      ...
      expect(args.length).to.equal(2);
      checkGetRequest('http://api', '/v2/app_usage_events?' +
        'order-direction=asc&results-per-page=100&after_guid=1', args[0]);
      checkGetRequest('http://api', '/page2', args[1]);
    });
