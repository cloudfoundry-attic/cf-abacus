...
describe('app-event-converter', () => {
  ...
  const createEvent = (opts) => ({
    ...
    entity: {
      ...
      previous_instance_count: opts.previousInstanceCount,
  ...
  context('when supported event is provided', () => {
    ...
    context('when STARTED event is provided', () => {
      ...
      context('when previous state was STARTED', () => {
        beforeEach(() => {
          event = createEvent({
            state: states.STARTED,
            previousState: states.STARTED,
            instanceCount: 1,
            previousInstanceCount: null,
            previousMemoryPerInstance: 4096,
            memoryPerInstance: 4096
          });
        });
    
        it('it should convert previous instance count to correct usage', () => {
          const usage = convert(event);
          expect(usage).to.deep.equal(
            createUsage({
              current_instance_memory: 4294967296,
              current_running_instances: 1,
              previous_instance_memory: 4294967296,
