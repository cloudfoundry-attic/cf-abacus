...
const isPurgeEvent = (event) => {
  ...
  const instanceCountMatches = entity.previous_instance_count === entity.instance_count;
  const memoryPerInstanceMatches = entity.previous_memory_in_mb_per_instance === entity.memory_in_mb_per_instance;
  return instanceCountMatches && memoryPerInstanceMatches;
};

const createDefaultMeasures = (event) => {
  return {
    previousRunningInstances: event.entity.previous_instance_count || event.entity.instance_count,
    previousInstanceMemory: event.entity.previous_memory_in_mb_per_instance * 1048576,
    currentRunningInstances: event.entity.instance_count,
    currentInstanceMemory: event.entity.memory_in_mb_per_instance * 1048576
  };
};
