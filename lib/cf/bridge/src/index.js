...
const carryOverKey = require('abacus-carryover').buildKey;
...
const reportUsage = (usage, resourceInfo, token, cb = () => {}) => {
  reporter.reportUsage(usage, token, (error, response) => {
    console.log('In reported CB');
    console.log('%j', error);
    if (!error && response && response.statusCode === 201)
      carryOver.write(usage, resourceInfo, response, (error) => {
        cb(error, response);
      });
    else
      cb(error, response);
...
const buildAppUsage = function *(event, stopped = false) {
  ...
  return yield adjustTimestamp(usageDoc);
};
...
const buildResourceData = (resource) => {
  return {
    ...
    timestamp: resource.metadata.created_at
  };
};
...
const reportAppUsage = (cfToken, abacusToken, { failure, success }) => {
  ...
  paging.readPage(uri, cfToken, perf, statistics, {
    processResourceFn: (resource, done) => {
      const t0 = moment.now();
      const buildAppUsageCb = yieldable.functioncb(buildAppUsage);

      buildAppUsageCb(resource, (error, usage) => {
        if (error) {
        ...
        if (usage && isElder(resource) && isRequested(resource)) {
          ...
          reportUsage(usage, buildResourceData(resource), abacusToken,
            (error, response) => {
    ...
    processResourceFn: (resource, done) => {
      ...
      if (resource.entity.state === 'STARTED') {
        if (cache.apps.indexOf(guid) < 0) {
          ...
          buildAppUsageCb(resource, true, (error, usage) => {
            if (error) {
