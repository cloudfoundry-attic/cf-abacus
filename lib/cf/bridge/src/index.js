...
const buildAppUsage = (event, stopped = false) => {
  ...
  const eventTime = moment(event.metadata.created_at).valueOf();
  return {
    start: eventTime,
    end: eventTime,
...
const isElder = (resource) => {
  ...
  const resourceDate = moment(resource.metadata.created_at).valueOf();
...
const storeGuid = (resource) => {
  const resourceDate = moment(resource.metadata.created_at);
