...
const registerError = (message, error, name, time) => {
  errors.lastError = message;
  errors.lastErrorTimestamp = new Date().toISOString();

  edebug('%s error: %j', message, error);

  if (name && time && error)
    perf.report(name, time, undefined, new Error(error));
};
...
const storeCarryOverUsage = (usage, response, cb) => {
  ...
  carryOverDB.get(key, (error, doc) => {
    if (error) {
      ...
      registerError('Error getting carry-over usage:', error, 'carryOver',
        t0);
      cb(error, response);
      return;
    }
    ...
    if (findWhere(usage.measured_usage, stoppedAppCriteria)) {
      ...
      carryOverDB.remove(updatedDoc, (error) => {
        ...
        if (opError) {
          ...
          registerError('Error removing carry-over usage:', opError,
            'carryOver', t0);
        }
        else {
...
const reportUsage = (usage, token, config, cb = () => {}) => {
  ...
  brequest.post(':collector/v1/metering/collected/usage', {
    ...
    registerError(message, error, 'report', t0);
      ...
      if (usage && isElder(resource) && isRequested(resource)) {
        ...
        reportUsage(usage, abacusToken, reportingConfig, (error, response) => {
          ...
          if (error || !response || response.statusCode !== 201) {
            ...
            registerError('Error reporting usage', error, 'usage', t0);
            done(error, response);
            return;
          }
...
const fetchCFApps = (cfToken, { failure, success }) => {
  ...
  paging.readPage(uri, cfToken, perf, statistics, {
    ...
    failure: (error, response) => {
      ...
      registerError('Failed to fetch CF apps due to error', error, 'fetch', t0);
      failure(error, response);
    },
    success: () => {
            ...
            reportUsage(usage, abacusToken, compensationConfig,
              (error, response) => {
                ...
                if (error || responseCode !== 201) {
                  ...
                  const message = 'Error reporting compensating usage';
                  registerError(message, message, 'compensation', t0);
                  done(error, response);
                  return;
                }
