...
const reportUsage = (usage, state, token, cb = () => {}) => {
  reporter.reportUsage(usage, token, (error, response) => {
    if (!error && response && response.statusCode === 201)
      carryOver.write(usage, response, state, (error) => {
        cb(error, response);
      });
    else
      cb(error, response);
...
const buildAppUsage = (event, stopped = false, cb) => {
  ...
  carryOver.adjustTimestamp(usageDoc, cb);
};
...
const reportAppUsage = (cfToken, abacusToken, { failure, success }) => {
  ...
  paging.readPage(uri, cfToken, perf, statistics, {
    processResourceFn: (resource, done) => {
      ...
      buildAppUsage(resource, false, (error, usage) => {
        if (error) {
        ...
        if (usage && isElder(resource) && isOrgEnabled(resource)) {
          debug('Reporting usage event %j', usage);
          reportUsage(usage, resource.entity.state, abacusToken,
            (error, response) => {
...
const purgeCompensation = (cfToken, abacusToken, { failure, success }) => {
  ...
  paging.readPage(uri, cfToken, perf, statistics, {
    processResourceFn: (resource, done) => {
      const t0 = moment.now();
      const guid = resource.entity.app_guid;
      const state = resource.entity.state;

      if (state === 'STARTED') {
        if (cache.apps.indexOf(guid) < 0)
          buildAppUsage(resource, true, (error, usage) => {
            if (error) {
            ...
            if (usage) {
              ...
              reportUsage(usage, state, abacusToken,
                (error, response) => {
