...
const reportUsage = (usage, config, cb = () => {}) => {
  const req = config.requestBatching ? brequest : request;

  throttle(req.post(':collector/v1/metering/collected/usage', {
    ...
    if (!error && response && response.statusCode === 409) {
      debug('Detected duplicate usage %j', usage);
      cb(new Error('Detected duplicate usage'),
        { statusCode: 409, body: usage });
      return;
    }
      ...
      reportUsage(usage, reportingConfig, (error, response) => {
        if (error || !response || response.statusCode !== 201) {
          ...
          if (response && response.statusCode === 409) {
            reportingStats.duplicates++;

            if (!reportingConfig.requestBatching) {
              storeElderGuid(resource);
              debug('Skipping report for app usage %j', usage);
              done();
              return;
            }

            reportingConfig.requestBatching = false;
            debug('Request batching for app usage disabled');
          }
          else
        ...
        reportingConfig.requestBatching = true;
        debug('Request batching for app usage active');
...
const shouldRetryDuplicateSubmission = (usage, guid, done) => {
  compensationStats.duplicates++;

  if (!compensationConfig.requestBatching) {
    cache.lastCompensatedGUID = guid;
    debug('Skipping report for compensating usage %j', usage);
    done();
    return false;
  }

  compensationConfig.requestBatching = false;
  debug('Request batching for compensating usage disabled');
  return true;
};
      ...
      if (resource.entity.state === 'STARTED') {
        if (cache.apps.indexOf(guid) < 0) {
          ...
          reportUsage(usage, compensationConfig, (error, response) => {
            ...
            if (responseCode === 409) {
              if (shouldRetryDuplicateSubmission(usage, guid, done, onError)) {
                schedulePurgeCompensation(purgeCompensation,
                  getRetryInterval(compensationConfig));
            ...
            compensationConfig.requestBatching = true;
            debug('Request batching for purge compensation active');
