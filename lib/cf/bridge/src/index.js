...
const modifyStartEventUsage = (usage, previousState) => {
  // First/initial start with staging
...
const modifyUsageOnStagingStopped = (usage, eventEntity) => {
  debug('Detected STAGING_STOPPED event. Setting memory and instances to 0');
  usage.app_guid = eventEntity.parent_app_guid;
  usage.currentInstanceMemory = 0;
  usage.currentRunningInstances = 0;
};

const modifyBuildpackSetEventUsage = (usage) => {
  debug('Buildpack set event detected. ' +
    'Setting previous memory and instances to 0');
  usage.currentRunningInstances = usage.currentRunningInstances - 1;
  usage.previousInstanceMemory = 0;
  usage.previousRunningInstances = 0;
};
...
const modifyUsage = (usage, event) => {
  ...
  switch (eventState) {
    ...
    case 'STAGING_STARTED':
      debug('Skipping STAGING_STARTED event');
      return undefined;
    case 'STAGING_STOPPED':
      modifyUsageOnStagingStopped(usage, event.entity);
      break;
    case'BUILDPACK_SET':
      modifyBuildpackSetEventUsage(usage);
      break;
    case 'STOPPED':
      modifyStopEventUsage(usage);
      break;
    default:
      debug('Found unknown event state %s for %j', eventState, event);
      return undefined;
  }
...
const buildAppUsage = (event, stopped = false) => {
  const modifiableUsageProperties = {
    app_guid: event.entity.app_guid,
  ...
  return {
    ...
    consumer_id: 'app:' + usage.app_guid,
