...
const storeCarryOverUsage = (usage, response, cb) => {
  ...
  const key = buildKey(usage.usage);
  ...
  carryOverDB.get(key, (error, doc) => {
    ...
    if (findWhere(usage.usage.measured_usage, stoppedAppCriteria)) {
    ...
    if (!error && response) {
      if (response.statusCode === 201) {
        if (!response.headers || !response.headers.Location) {
          edebug('No Location header found in response %j', response);
          cb(new Error('No Location header found'), response);
          return;
        }
