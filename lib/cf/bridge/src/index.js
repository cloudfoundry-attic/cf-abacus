...
const reportAppUsage = (cfToken, abacusToken, cb = () => {}) => {
  ...
  paging.readPage(uri, cfToken, perf, {
...
const saveStartedApps = (resource, done) => {
  ...
  if (resource.entity.state === 'STARTED' && cache.apps.indexOf(appGuid) < 0) {
    ...
    perf.report('apps.save.started', t0);
  }

  perf.report('apps.save', t0);
  done();
};

const fetchCFApps = (token, callback = () => {}) => {
  const t0 = Date.now();
  const uri = '/v2/apps?order-direction=asc&results-per-page=50';
  paging.readPage(uri, token, perf, {
    processResourceFn: saveStartedApps,
    callback: (error, response) => {
      // retry?
      if (error || !response || response.statusCode !== 200) {
        edebug('Failed to fetch CF apps due to error %j and response %j',
          error, response);
        module.appsReader = setTimeout(() => fetchCFApps(token), 1000);
        perf.report('apps.failure', t0);
      }
      else
        perf.report('apps.success', t0);
  ...
  paging.readPage(uri, cfToken, perf, {
    processResourceFn: (resource, done) => {
      ...
      if (resource.entity.state === 'STARTED') {
          ...
          reportUsage(usage, abacusToken, compensationConfig,
            (error, response) => {
              ...
              if (error || responseCode !== 201) {
                edebug('Failed to compensate purge call due to error %j ' +
                ...
                perf.report('compensation.failure', t0);
                return done(error, response);
              }

              cache.lastCompensatedGUID = guid;
              compensationConfig.currentRetries = 0;
              perf.report('compensation.success', t0);
              done();
            });
        }
      }
        ...
        cache.lastCompensatedGUID = guid;
        perf.report('compensation.skip', t0);
        done();
      }
    },
    callback: cb
  ...
  return {
    body: {
      bridge: {
        ...
        statistics: {
          cache: {
            reads: perf.stats('cache.read'),
            writes: perf.stats('cache.write')
          },
          paging: {
            success: perf.stats('paging.success'),
            failure: perf.stats('paging.failure'),
            resources: {
              end: perf.stats('paging.resources.end'),
              success: perf.stats('paging.resources.success'),
              failure: perf.stats('paging.resources.failure')
            }
          }
        }
      },
      reporting: {
        ...
        statistics: {
          usage: {
            report: perf.stats('usage.report'),
            success: perf.stats('usage.success'),
            failure: perf.stats('usage.failure')
          }
        }
      },
      purgeCompensation: {
        ...
        statistics: {
          apps: {
            success: perf.stats('apps.success'),
            failure: perf.stats('apps.failure'),
            save: perf.stats('apps.save'),
            started: perf.stats('apps.save.started')
          },
          compensation: {
            success: perf.stats('compensation.success'),
            failure: perf.stats('compensation.failure'),
            skip: perf.stats('compensation.skip')
          }
        }
      }
    }
