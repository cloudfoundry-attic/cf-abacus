...
describe('CF apps read', () => {
  ...
  context('on non-empty apps list', () => {
    ...
    it('generates correct list request', function(done) {
      bridge.readAllApps((error, response) => {
        expect(error).to.equal(null);
        expect(response.statusCode).to.equal(200);
      });

      setTimeout(() => {
        const expectedURLPage1 = ':api/v2/apps?' +
          'order-direction=asc&results-per-page=50';
        const args = reqmock.get.args;
        expect(args.length).to.equal(2);
        checkRequest('http://api', expectedURLPage1, args[0]);
        checkRequest('http://api', ':api/page2', args[1]);

        done();
      }, 100);
    });

    it('receives correct list response', function(done) {
      bridge.readAllApps((err, response) => {
