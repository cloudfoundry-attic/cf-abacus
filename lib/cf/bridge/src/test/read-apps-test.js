...
describe('CF apps read', () => {
  ...
  context('on non-empty apps list', () => {
    ...
    it('generates correct list request', (done) => {
      bridge.fetchCFApps(token, (error, response) => {
  ...
  context('when listing apps fails', () => {
    ...
    context('when there is an error', () => {
      ...
      const verifyError = (err, response) => {
      ...
      const verifyProcessed = (err, response) => {
        expect(err).to.equal(undefined);
        expect(response).to.equal(undefined);
        expect(bridge.cache.apps).to.contain('a8bbf5d0');
      };

      const verify = (err, response, done) => {
        if (returnError)
          return verifyError(err, response);

        verifyProcessed(err, response);
        done();
      };

      it('retries', (done) => {
        bridge.fetchCFApps(token,
          (err, response) => verify(err, response, done));
      });
    });
