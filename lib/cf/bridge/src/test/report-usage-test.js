...
describe('Report app usage', () => {
  ...
  context('on failure', () => {
    ...
    context('posting usage errors', () => {
      ...
      context('when reporting duplicate usage', () => {
        context('in batch request', () => {
          beforeEach(() => {
            expectedError = new Error('Detected duplicate usage');
          ...
          it('fails with duplicate usage', (done) => {
            bridge.reportAppUsage((err, response) => {
          ...
          it('turns request batching off', (done) => {
            bridge.reportAppUsage(() => {
              expect(bridge.reportingConfig.requestBatching).to.equal(false);
              done();
            });
          });
        });

        context('and batching is off', () => {
          beforeEach(() => {
            // Mock the request module
            const request = require('abacus-request');
            reqmock = extend({}, request, {
            ...
            bridge.reportingConfig.requestBatching = false;
          });

          it('skips usage report', (done) => {
            bridge.reportAppUsage();
        ...
        context('and duplicate usage was skipped', () => {
          beforeEach(() => {
            // Mock the request module
            const request = require('abacus-request');
            reqmock = extend({}, request, {
            ...
            bridge.reportingConfig.requestBatching = false;
            bridge.reportAppUsage((error, response) => {
              throw new Error(util.format('Unexpected error %s and ' +
                'response code %s', error, response));
            });
          });

          it('reactivates request batching on successful report', (done) => {
            setTimeout(() => {
              expect(bridge.reportingConfig.requestBatching).to.equal(true);
              done();
            }, 50);
          });
        });
