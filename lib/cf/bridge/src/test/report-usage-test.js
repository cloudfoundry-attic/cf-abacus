  ...
  context('on non-empty usage event stream', () => {
    context('with multiple pages', () => {
      ...
      it('reports resource usage to COLLECTOR', (done) => {
        ...
        checkPostRequest(args[0], 'f3a07a06', 1073741824, 1, 0, 0);
        checkPostRequest(args[1], 'f3a07a06', 0, 0, 1073741824, 1);
        checkPostRequest(args[2], 'f3a07a06', 1073741824, 1, 0, 0);
        checkPostRequest(args[3], '35c4ff2f', 536870912, 1, 268435456, 2);
      ...
      bridge.reportAppUsage(cfToken, abacusToken, {
        ...
        success: () => {
          if (returnEmptyPage) {
            ...
            bridge.stopReporting();
            expect(reqmock.post.args.length).to.equal(4);
            done();
          }
        }
      });
