    ...
    context('with multiple pages', () => {
      ...
      const checkPostRequest = (req, appId,
        ...
        expect(req[1].body).to.deep.equal(
          generateUsageReport(appId, 536870912, 1,
            previousMemory, previousInstances));
      };
      ...
      it('reports resource usage to COLLECTOR', (done) => {
        const args = reqmock.post.args;
        expect(args.length).to.equal(2);
        checkPostRequest(args[0], '35c4ff0f', 0, 0);
        checkPostRequest(args[1], '35c4ff2f', 268435456, 2);
  ...
  context('on failure', () => {
    ...
    context('posting usage to Abacus', () => {
      ...
      context('with missing Location header', () => {
        ...
        it('calls back with error and response code', (done) => {
          bridge.reportAppUsage(cfToken, abacusToken,
            expectError(bridge, 'No Location header found', {
              statusCode: 201,
              body: {}
            }, done));
        });
