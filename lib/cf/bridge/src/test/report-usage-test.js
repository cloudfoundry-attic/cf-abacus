...
const tests = (secured) => {
  ...
  const appUsagePageOneWithEqualCreatedAt = {
    total_results: 2,
    total_pages: 1,
    prev_url: null,
    next_url: null,
  ...
  context('on non-empty usage event stream', () => {
    context('with multiple pages', () => {
      ...
      it.only('reports resource usage to COLLECTOR', () => {
        const args = reqmock.post.args;
        expect(args.length).to.equal(2);
      ...
      context('with missing Location header', () => {
        ...
        it('throws error', (done) => {
          bridge.reportAppUsage(cfToken, abacusToken, {
            ...
            failure: () => {
              // console.log(bridge);
              expect(bridge.statistics.usage.loopFailures).to.eql(1);
              bridge.stopReporting();
              done();
            }
          });
  ...
  context('when two consecutive events have the same created_at time',
    () => {
      let readPageStub;
      let request; // FIXME use as global const

      beforeEach((done) => {
        const carryover = require('abacus-carryover');
        readPageStub = sandbox.stub();
        readPageStub.onFirstCall().yields(undefined, []);
        readPageStub.onSecondCall().yields(undefined, [carryOverDbDocs[0]]);

        sinon.stub(carryover, 'carryover', () => {
          return {
            write: sandbox.stub().yields(),
            readPage: readPageStub
          };
        });
        ...
        sandbox.stub(request, 'get').yields(undefined, {
          statusCode: 200,
          body: appUsagePageOneWithEqualCreatedAt
        });
      ...
      it('populates paging statistics', () => {
        ...
        expect(request.post.firstCall.args[1].body).include({
          start: 1439897300000,
          end:   1439897300000,
...
describe.skip('Report app usage with security', () => tests(true));
