...
describe('Carry-over', () => {
  ...
  context('on success', () => {
    ...
    it('tries to get the documents revisions from the DB', () => {
      ...
      checkId(getArgs[0], '35c4ff0f', 1439897300000);
      checkId(getArgs[1], '35c4ff0f', 1439897302000);
      checkId(getArgs[2], '35c4ff1f', 1439897303000);
      checkId(getArgs[3], '35c4ff1f', 1439897305000);
      checkId(getArgs[4], '35c4ff2f', 1439897306000);
      checkId(getArgs[5], '35c4ff0f', 1439897307000);
    });

    it('stores references to the documents in the DB', () => {
      const putArgs = putMock.args;
      expect(putArgs.length).to.equal(4);
      checkPutDocumentRef(putArgs[0], '35c4ff0f', 1439897300000);
      checkPutDocumentRef(putArgs[1], '35c4ff0f', 1439897302000);
      checkPutDocumentRef(putArgs[2], '35c4ff1f', 1439897303000);
      checkPutDocumentRef(putArgs[3], '35c4ff2f', 1439897306000);
    });

    it('removes stopped apps from the DB', () => {
      const removeArgs = removeMock.args;
      expect(removeArgs.length).to.equal(2);
      checkId(removeArgs[0], '35c4ff1f', 1439897305000);
      checkId(removeArgs[1], '35c4ff0f', 1439897307000);
    });
  });
  ...
  context('on failure', () => {
    context('when new app cannot be read', () => {
      beforeEach((done) => {
        ...
        getMock = spy((key, cb) => {
          cb(key.endsWith('1439897303000') ? testError : undefined, {});
        });
        putMock = spy((doc, cb) => {
          cb(undefined, {});
        });
    ...
    context('when new app cannot be stored', () => {
      beforeEach((done) => {
        ...
        putMock = spy((doc, cb) => {
          cb(doc._id.endsWith('1439897303000') ? testError : undefined, {});
        });
    ...
    context('when an app cannot be removed', () => {
      beforeEach((done) => {
        ...
        const testError = new Error('test error');
        removeMock = spy((doc, cb) => {
          cb(doc._id.endsWith('1439897305000') ? testError : undefined, {});
        });
        getMock = spy((doc, cb) => {
          cb(undefined, {});
        });
