...
describe('Purge compensation', () => {
  ...
  const checkCompensationRequest = (req, usage) => {
    expect(req[0]).to.equal(':collector/v1/metering/resource/usage');
  ...
  context('on success', () => {
    context('for STARTED app usage event', () => {
      ...
      beforeEach(() => {
        ...
        bridge = require('..');
        bridge.cache.oauthToken = 'token';
      });

      const checkPage = (req, expectedURLPage) => {
        expect(req[0]).to.equal(expectedURLPage);
      ...
      it('generates correct usage request', function(done) {
        ...
        setTimeout(() => {
          ...
          checkPage(args[0], expectedURLPage1);
          checkPage(args[1], ':api/page2?param=1&after_guid=35c4ff2f');

          done();
        }, 200);
      });

      it('receives correct usage response', function(done) {
      ...
      it('generates correct usage reporting request', function(done) {
        bridge.purgeCompensation((error, response) => {
          expect(error).to.equal(null);
          expect(response.statusCode).to.equal(200);
        });
      ...
      it('remembers the last processed guid', function(done) {
    ...
    context('for STOPPED app usage event', () => {
      ...
      it('generates no compensation usage reporting request', function(done) {
  ...
  context('on failure', () => {
    context('caused by error getting usage events', () => {
      ...
      it('generates no usage reporting request', function(done) {
        bridge.purgeCompensation(() => {
          const args = reqmock.post.args;
          expect(args.length).to.equal(0);
          done();
    ...
    context('caused by bad response when getting usage events', () => {
      ...
      it('returns the bad response', function(done) {
        bridge.purgeCompensation((error, response) => {
          expect(error).to.equal(null);
    ...
    context('caused by error when reporting usage', () => {
      ...
      it('remembers the last processed guid', function(done) {
        bridge.purgeCompensation((error) => {
          ...
          expect(error).to.equal(null);
          expect(bridge.cache.lastCompensatedGUID).to.equal(null);
        });
      });
