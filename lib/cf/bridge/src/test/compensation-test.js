    ...
    context('when reporting duplicate usage', () => {
      ...
      context('in batch request', () => {
        ...
        it('fails with duplicate usage', (done) => {
          bridge.purgeCompensation((err, response) => {
            expect(err).to.eql(new Error('Detected duplicate usage'));
        ...
        it('turns request batching off', (done) => {
          bridge.purgeCompensation(() => {
            expect(bridge.compensationConfig.requestBatching).to.equal(false);
            done();
          });
        });
      });
      ...
      context('and batching is off', () => {
        beforeEach(() => {
          ...
          bridge.compensationConfig.requestBatching = false;
        });
      ...
      context('and duplicate usage was skipped', () => {
        ...
        it('reactivates request batching on successful report', (done) => {
          setTimeout(() => {
            expect(bridge.compensationConfig.requestBatching).to.equal(true);
            done();
          }, 50);
        });
      });
