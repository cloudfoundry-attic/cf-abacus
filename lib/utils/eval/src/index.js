...
const timeout = process.env.EVAL_TIMEOUT ?
  { timeout: parseInt(process.env.EVAL_TIMEOUT) } : {};
...
const xeval = (s, c) => {
  const context = c || {};
  debug('Evaluating expression %s with context %o and %o using %s',
  ...
  try {
    switch (vmtype) {
      case 'vm':
        ...
        return vm.runInContext(s, context, timeout);
      case 'vm2':
        const vm2 = new VM(extend({
