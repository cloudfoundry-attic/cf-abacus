  ...
  async.each([undefined, 100], (timeout, timeoutCb) => {
    describe(`abacus-eval (${vmtype}) and timeout ${timeout}`, () => {
      ...
      before(() => {
        ...
        if (timeout)
          process.env.EVAL_TIMEOUT = timeout;
      ...
      context('with invalid or malicious input', () => {
        it('rejects non-functions', () => {
          ...
          try {
            ...
            expect(err).to.satisfy(function(e) {
              return e.message === 'f is not a function' ||
                e.message === 'Function.prototype.apply was called on 42, ' +
        ...
        vm2it('prevents escaping via this.constructor.constructor', () => {
          const f = xeval('() => ' +
            'this.constructor.constructor("return process")().uptime()');
          expect(typeof f).to.equal('function');
          expect(f).to.throw('process is not defined');
        });
