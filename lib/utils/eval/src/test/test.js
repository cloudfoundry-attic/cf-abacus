...
async.each(['vm', 'vm2'], (vmtype, vmCb) => {
  async.each([undefined, 100], (timeout, timeoutCb) => {
    describe(`abacus-eval (${vmtype}) and timeout ${timeout}`, () => {
      const vm2it = vmtype === 'vm' ? it.skip : it;
      const timeoutIt = timeout ? it : it.skip;
      let xeval;

      before(() => {
        process.env.EVAL_VMTYPE = vmtype;
        if (timeout) process.env.EVAL_TIMEOUT = timeout;
      ...
      context('with invalid or malicious input', () => {
        it('rejects non-functions', () => {
          ...
          try {
            ...
            expect(err).to.satisfy(function(e) {
              return (
                e.message === 'f is not a function' ||
                e.message === 'Function.prototype.apply was called on 42, ' + 'which is a number and not a function'
              );
            });
          }
        });

        timeoutIt('prevents endless loops', () => {
          const f = xeval('() => { while (true) {} }');
          expect(typeof f).to.equal('function');
          expect(f).to.throw('Script execution timed out').with.property('timeoutError', true);
        });

        it('prevents invalid expressions executions', () => {
          const error = 'Error during execution';
          const f = xeval(`() => { throw new Error('${error}') }`);
          expect(typeof f).to.equal('function');
          expect(f).to.throw(error).with.property('expressionError', true);
        });
        ...
        it('prevents use of built-in modules and globals', () => {
          const f = xeval('() => process.exit()');
          expect(typeof f).to.equal('function');
          expect(f).to.throw('process is not defined');
        });
        ...
        vm2it('prevents escaping via this.constructor.constructor', () => {
          const f = xeval('() => ' + 'this.constructor.constructor("return process")().uptime()');
          expect(typeof f).to.equal('function');
          expect(f).to.throw('process is not defined');
        });
        ...
        it('prevents escaping via global.constructor.constructor', () => {
          ...
          expect(typeof f).to.equal('function');
          // we get not defined for process in vm & for global in vm2
        ...
        it('rejects arguments.callee', () => {
          ...
          expect(typeof f).to.equal('function');
          expect(f).to.throw('\'caller\', \'callee\', and \'arguments\' properties may not be accessed on strict ' +
        ...
        vm2it('prevents changes to the context', () => {
          ...
          const f = xeval('() => { util.add = (a, b) => a - b; }', { util: util });
          expect(typeof f).to.equal('function');
          expect(f).to.throw('\'set\' on proxy: trap returned falsish for property \'add\'');
        });
      });
    });
