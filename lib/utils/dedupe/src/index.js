...
const { each, extend, defaults, first, last, map } = require('underscore');
...
const deduper = (slack, max, cache) => {
  ...
  return {
    ...
    add: (val) => {
      ...
      if (filters[1].count >= opt.max) edebug('Dedupe filter capacity exceeded %d', filters[1].count);
    },
      ...
      return {
        filters: map(filters, (filter) => {
          return extend({}, filter, {
            filter: {
              bitfield: {
                buffer: zlib.deflateSync(filter.filter.bitfield.buffer).toString('base64')
              }
            }
          });
        }),
    ...
    deserialize: (sFilters) => {
      filters = map(sFilters.filters, (filter, i) => {
        const df = extend({}, filter, {
          filter: {
            bitfield: {
              buffer: zlib.inflateSync(new Buffer(filter.filter.bitfield.buffer, 'base64'))
            }
          }
        });
