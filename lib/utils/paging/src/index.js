...
// Setup debug log
const debug = require('abacus-debug')('abacus-paging');
const edebug = require('abacus-debug')('e-abacus-paging');
...
const readPage = (pageURI, cfAdminToken, perf, statistics, { processResourceFn, failure, success }) => {
  const t0 = moment.now();
    ...
    (error, response) => {
      if (error || response.statusCode !== 200) {
        const message = util.format(
          'Cannot fetch resource from %s; page: %s; ' + 'error %s; response %j',
          uris().api,
          pageURI,
          error,
          response
      ...
      if (resources) debug('Processing %d resources ...', resources.length);
      processResources(resources, processResourceFn, perf, statistics, (error, errorResponse) => {
