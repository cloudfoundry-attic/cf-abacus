    ...
    context('on non-empty page', () => {
      beforeEach(() => {
        ...
        reqmock = extend({}, request, {
          get: spy((uri, opts, cb) => {
            if (opts.page.indexOf('page2') > -1) cb(null, { statusCode: 200, body: pageTwo });
            else cb(null, { statusCode: 200, body: pageOne });
          })
        });
        ...
        paging.readPage(
          ...
          { paging: {} },
          {
            ...
            failure: (error, response) => {
                ...
                new Error(util.format('Unexpected call of failure with ' + 'error %o and response %j', error, response))
              );
            },
            success: () => {
  ...
  context('on failure', () => {
    ...
    context('when second page read errors', () => {
      beforeEach(() => {
        ...
        reqmock = extend({}, request, {
          get: spy((uri, opts, cb) => {
            if (opts.page.indexOf('page2') > -1) cb(null, { statusCode: 500, body: pageTwo });
            else cb(null, { statusCode: 200, body: pageOne });
          })
        });
        require.cache[require.resolve('abacus-request')].exports = reqmock;
      ...
      it('stops processing resources', (done) => {
        ...
        paging.readPage(
          ...
          { paging: {} },
          {
            processResourceFn: (resource, error) => {
              resources.push(resource);
              if (resources.length < 5) error(undefined);
              else error('too many resources');
            },
            failure: (error, response) => {
