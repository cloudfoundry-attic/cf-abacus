...
const { forEach, map, toArray } = require('abacus-iterable');
...
// encodings
const fitTransform = (encoder, X) => {
  fit(encoder, X);
  return transform(encoder, X);
};

// Transform labels back to original feature vectors
const inverseTransform = (encoder, Y) => {
  return toArray(map(Y, (y) => encoder.X[y]));
};

module.exports = { encoder, fit, transform, fitTransform, inverseTransform };
