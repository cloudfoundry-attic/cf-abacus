...
const { map, forEach, zip, toArray, shuffle } = require('abacus-iterable');
const { dense, toCSC, toDense } = require('abacus-matrix');
...
const model = (isize, osize, epochs = 5) => {
  const p = {
...
// Train a model = require(a featureset and the corresponding class labels
const fit = (model, X, Y) => {
  ...
  for(let e = 0; e < epochs; e++) {
    ...
    forEach(T, ([x, gold]) => {
      ...
      // Predict target label = require(features, using the raw weights
...
module.exports = { twmatrix, model, load, fit, predict, decisionFunction,
  sparsity, score };
