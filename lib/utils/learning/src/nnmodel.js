...
const { map, forEach, zip, toArray } = require('abacus-iterable');
const { pool, clear } = require('abacus-matrix');
const { graph, backward, softmax } = require('./compgraph');
const rmsprop = require('./rmsprop');
const { argmax, median } = require('./utils');
...
const fit = (model, X, Y) => {
  ...
  for(let e = 0, n = model.epochs; e < n; e++) {
    if((e + 1) % 10 === 0)
      log('Training epoch %f', ((e + 1) / tl).toFixed(2));
...
// of featuresets
const decisionFunction = (model, X, istate) => {
  // Clear the number pool
  clear(model.pool);
  ...
  // Compute the output probabilities P= require(input X and state
...
const predict = (model, X, istate) => {
  ...
  // Predict Y= require(input X and state
...
module.exports = { model, fit, decisionFunction, predict };
