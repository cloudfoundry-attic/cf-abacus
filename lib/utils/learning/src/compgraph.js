...
const { dwmatrix } = require ('abacus-matrix');

/* eslint camelcase: 0 */

const exp = (power) => Math.pow(Math.E, power);

// Return a computation graph
const graph = (pool, backprop = true) => {
  return {
    pool: pool,
...
module.exports = { exp, graph, backward, rowPluck, tanh, sigmoid, relu,
  leakyrelu, mul, add, dot, eltmul, softmax };
