...
const dbOpts = (uri, opt) => {
  ...
  return /:/.test(uri) ? defaults(opt || {}, defaultOpts) :
    defaults(opt || {}, defaultOpts, { adapter: 'memory' });
};
...
const couchclient = (part, uri, cons) => {
  ...
  const pool = (p, rw, cb) => {
    ...
    if(Array.isArray(p[0]))
      return transform.map(p, (v, i, p, mapCb) => {
        const u = dbopt.uri(v);
        ...
        if (partitions[dbKey])
          return mapCb(null, partitions[dbKey]);
        ...
        return dbopt.cons(u, {
          ...
          if(err) {
            mapCb(null, errdb('dbcons-err-' + u, err));
            return;
          }
  ...
  return {
    ...
    resetPool: () => {
      debug('Clearing %d pooled handles', Object.keys(partitions).length);
      partitions = [];
      debug('Pooled handles cleared');
    }
  };
};
...
const drop = (server, regex, cb) => {
  if(/:/.test(server)) {
    ...
    const dbNames = map(globalStoreLocations, (key) => key.slice(1));
    dropDBs(dbNames, regex, server, { adapter: 'memory' }, cb);
  }
};
