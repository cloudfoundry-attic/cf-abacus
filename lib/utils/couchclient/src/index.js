...
const dropDatabases = (server, regex, localOnly, cb) => {
  const lcb = (err, deletedDatabases) => {
    ...
    if (deletedDatabases.length > 0) debug('Deleted databases: %o', deletedDatabases);
    else debug('No databases were deleted');
    return cb(null, deletedDatabases);
  };

  if (/:/.test(server)) dropServerDatabases(server, regex, {}, localOnly, lcb);
  else dropInMemoryDatabases(server, regex, inMemoryOpt, lcb);
};
