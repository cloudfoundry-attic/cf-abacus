...
const dropDBs = (dbs, regex, server, opts, cb) => {
  // Find the dbs that match the given regex
  const names = filter(dbs, (name) => regex.exec(name));
  debug('Will delete DBs %o', names);
  transform.map(
    without(names, '_replicator', '_users'), (name, i, l, cb) => {
      // Delete each db
      const dbName = server ? [server, name].join('/') : name;
      const db = new PouchDB(dbName, opts);
      db.destroy((err) => {
        if (err)
          edebug('Failed to delete db %s due to %o', dbName, err);
        else
          debug('Deleted db %s', dbName);
        cb(err);
      });
    }, (err) => {
      cb(err);
...
const drop = (server, regex, cb) => {
  if(/:/.test(server)) {
    ...
    if(u.hostname !== 'localhost' && u.hostname != '127.0.0.1') {
      debug('Server not on localhost, not deleting dbs on %s', server);
      cb();
      return;
    }
    ...
    request.get(':server/_all_dbs', {
      ...
      if(!val.body)
        ...
        dropDBs(val.body, regex, server, {}, cb);
    });
  }
  else {
    debug('In-memory DB %s detected; deleting dbs', server);

    const globalStoreLocations = Object.keys(memdown('_$nonexistent$_')._store);
    const prefix = '$' + (server ? server + '/' : '');
    debug('Removing prefix %s from db names', prefix);
    const dbNames = map(globalStoreLocations, (key) => key.replace(prefix, ''));

    debug('Found in-memory DBs %o', dbNames);
    dropDBs(dbNames, regex, server, inMemoryOpt, cb);
  }
};
