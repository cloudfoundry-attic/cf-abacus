...
const inMemoryDBNames = new Set();
PouchDB.on('destroyed', (dbName) => inMemoryDBNames.delete(dbName));
...
const dropInMemoryDatabases = (server, regex, options, cb) => {
  ...
  dropListedDatabases(server, [...inMemoryDBNames], regex, options, cb);
};
