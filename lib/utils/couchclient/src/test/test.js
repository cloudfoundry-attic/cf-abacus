...
describe('abacus-couchclient', (d) => {
  const helloKey = couchclient.kturi('Hello', moment.utc(
    '2014-11-06 19:06:54').valueOf());
  const helloAgainKey = couchclient.kturi('Hello', moment.utc(
    '2014-11-06 19:07:54').valueOf());
  const heyKey = couchclient.tkuri('Hey', moment.utc(
    '2014-10-06 19:06:54').valueOf());
  const blahKey = couchclient.kturi('Blah', moment.utc(
    '2014-10-06 19:06:54').valueOf());
  const awwwwKey = couchclient.kturi('Awwww', moment.utc(
    '2014-11-06 19:06:54').valueOf());
  ...
  it('distributes bulk operations over 4 db partitions', (done) => {
    ...
    const getrangeerr = (done) => {
      ...
      db.allDocs({
        include_docs: true,
        startkey: couchclient.kturi('Blah',moment.utc(
          '2014-10-05 19:06:54').valueOf()),
        endkey: couchclient.kturi('Blah', moment.utc(
  ...
  context('when DBs are dropped', () => {
    ...
    // Drop the database
    const drop = (db, serverName, done) => {
      debug('drop: starting drop ...');
      couchclient.drop(serverName, /^abacus-couchclient-/, (err) => {
        expect(err).to.equal(undefined);
    ...
    it('can reuse the DB', (done) => {
      const db = couchclient(undefined,
        couchclient.dburi(dbserver(), 'abacus-couchclient-test-drop'));

      clean(db, () => put(db, () => get(db, () => drop(db, dbserver(), () =>
    ...
    it('can work with new DB', (done) => {
      ...
      clean(db, () => put(db, () => get(db, () => drop(db, dbserver(), () => {
        const newdb = couchclient(undefined,
          couchclient.dburi(dbserver(), 'abacus-couchclient-test-drop'));
        missingCheck(newdb, done);
      }))));
    });

    it('works with server name', (done) => {
      const db = couchclient(undefined,
        couchclient.dburi(dbserver() || 'test',
          'abacus-couchclient-test-drop'));

      clean(db, () => put(db, () => get(db, () =>
        drop(db, dbserver() || 'test', () => missingCheck(db, done)))));
    });
  });
