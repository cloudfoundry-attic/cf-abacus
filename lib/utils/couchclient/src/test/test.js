...
describe('abacus-couchclient', (d) => {
  const helloKey = dbCommons.kturi('Hello', moment.utc(
    '2014-11-06 19:06:54').valueOf());
  const helloAgainKey = dbCommons.kturi('Hello', moment.utc(
    '2014-11-06 19:07:54').valueOf());
  const heyKey = dbCommons.tkuri('Hey', moment.utc(
    '2014-10-06 19:06:54').valueOf());
  const blahKey = dbCommons.kturi('Blah', moment.utc(
    '2014-10-06 19:06:54').valueOf());
  const awwwwKey = dbCommons.kturi('Awwww', moment.utc(
    '2014-11-06 19:06:54').valueOf());
  ...
  it('distributes bulk operations over 4 db partitions', (done) => {
    ...
    const getrangeerr = (done) => {
      ...
      db.allDocs({
        include_docs: true,
        startkey: dbCommons.kturi('Blah',moment.utc(
          '2014-10-05 19:06:54').valueOf()),
        endkey: dbCommons.kturi('Blah', moment.utc(
  ...
  it('range test', (done) => {
    ...
    const docs = flatten(map(keys, (k) =>
      map(range(1417144014000, 1417576014000, 86400000), (t) =>
        dbCommons.tkuri(k, dbCommons.pad16(t))
      )));
  ...
  context('when dropping databases and partitions', () => {
    ...
    const getdb = (server) => couchclient(undefined,
      couchclient.dburi(server, dbname));

    it('drops all databases', (done) => {
      const db = getdb(dbserver());
      clean(db, () => put(db, () => get(db, () => drop(dbserver(), () =>
        check(db, [], done)))));
    });
    ...
    it('drops all databases on a named server', (done) => {
      ...
      clean(db, () => put(db, () => get(db, () =>
        drop(dbserver() || 'test', () => check(db, [], done)))));
    });

    it('deletes all partitions', (done) => {
      const db = getdb(dbserver());
      clean(db, () => put(db, () => get(db, () =>
        deletePartitions(dbserver(), () => check(db, [], done)))));
    });

    it('deletes only the filtered partitions', (done) => {
      const db = getdb(dbserver());
      clean(db, () => put(db, () => get(db, () =>
        couchclient.deletePartitions(dbserver(), /.*-201410/, () => check(db,
  ...
  it('supports readAllPages', (done) => {
    const part = partition.partitioner(partition.bucket, partition.period,
      partition.forward, partition.balance, true);
    const db = couchclient(part,
      couchclient.dburi(dbserver(), 'abacus-couchclient-test-read-all-pages'));
