...
describe('counter', () => {
  ...
  describe('summary', () => {
    ...
    it('returns average of previous and current minute rates', () => {
      // previous minute
      counter.inc(1);
      clock.tick(30 * 1000);
      counter.inc(1);
      clock.tick(30 * 1000);
      // rate = 2 / 60sec = 0.03(3) increments / sec
      ...
      counter.inc(1);
      clock.tick(15 * 1000);
      // estimated rate = 2 / 30 sec = 0.06(6) increments / sec

      const summary = counter.summary();
      expect(summary.rate).to.be.closeTo(0.04999995, epsilon);
    });
  });
