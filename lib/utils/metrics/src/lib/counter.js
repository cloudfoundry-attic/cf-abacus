...
class Counter {
  ...
  summary() {
    const currentTime = moment.now();
    const prevRate = this.minutesWindow.calculatePreviousRate(currentTime);
    const currRate = this.minutesWindow.estimateCurrentRate(currentTime);
    return {
      total: this.value,
      rate: (prevRate + currRate) / 2
    };
  }
