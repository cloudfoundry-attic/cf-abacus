...
const carryOverDB = throttle(
  retry(breaker(batch(dbClient(checkKeyPart, dbClient.dburi(uris()[dbalias], 'abacus-carry-over')))))
);
...
const carryover = (statistics, errorFn) => ({
    ...
    getDocument(resourceInfo._id, statistics, errorFn, (error, doc) => {
      ...
      carryOverDB.put(updatedDoc, (error) => {
        if (error) {
          ...
          errorFn('Error upserting carry-over usage', error, undefined, 'carryOver', start);
        } else {
  ...
  findInCurrentMonth: (key, cb) => {
    ...
    const id = dbClient.tkuri(key, startOfCurrentMonth);
    debug('Searching for document with id "%s"', id);
    carryOverDB.get(id, (error, doc) => {
      if (error) {
        cb(error);
        return;
      }
