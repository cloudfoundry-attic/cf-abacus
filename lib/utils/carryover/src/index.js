...
// Carry-over DB used to transfer usage to the next month
const db =
  dbClient(checkKeyPart, dbClient.dburi(uris()[dbalias], 'abacus-carry-over'));

// Omit wrapping readAllPages in breaker, because it leads to  timeout
// when reading multiple pages
const carryOverDB = throttle(retry(batch(
  extend(db, breaker(omit(db, 'readAllPages'))))));
...
const carryover = (statistics, errorFn) => ({
  ...
  readAllPages: (opts, processingFn, cb) => {
    ...
    carryOverDB.readAllPages(opts, pageProcessingFunction, (error) => {
      if (error)
        errorFn(util.format('Failed reading usage data from %s to %s ' +
          'with page size %d', opts.startId, opts.endId, opts.pageSize),
