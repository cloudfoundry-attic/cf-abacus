...
describe('Adjust carry-over usage', () => {
  ...
  const test = (storedDoc, error, expectedDoc, stats, errorFnCalled) => {
    let result;

    beforeEach((done) => {
      getStub.callsArgWith(1, error, storedDoc);
      carryOver = require('..')(statistics, errorFn);
      carryOver.adjustTimestamp(appUsage, (error, doc) => {
        result = { error, doc };
        done();
      });
    });
    ...
    it('populates statistics object', () => {
      expect(statistics.carryOver).to.deep.equal(extend({
        getSuccess   : 0,
        getNotFound  : 0,
        getFailure   : 0,
        removeSuccess: 0,
  ...
  context('when adjusting new doc', () => {
    test(undefined, undefined, appUsage, { getNotFound  : 1 }, false);
  });
  ...
  context('when db returns error', () => {
    ...
    test(undefined, error, undefined, { getFailure  : 1 }, true);
  });
  ...
  context('with document with the same timestamp', () => {
    ...
    test(storedDoc, undefined, expectedDoc, { getSuccess  : 1 }, false);
  });
  ...
  context('with older document', () => {
    ...
    test(storedDoc, undefined, appUsage, { getSuccess  : 1 }, false);
  });
  ...
  context('if stored document does not have state & timestamp', () => {
    const storedDoc = {
      _id: 123,
      collector_id: 123
    };
