...
describe('Adjust carry-over usage', () => {
  ...
  context('when db returns error', () => {
    ...
    test(undefined, error, undefined,
      undefined, { getFailure  : 1 }, true);
  });
  ...
  context('with document with the same timestamp', () => {
    ...
    test(storedDoc, undefined, undefined,
      expectedDoc, { getSuccess  : 1 }, false);
  });
  ...
  context('if stored document has the same GUID', () => {
    ...
    test(storedDoc, undefined, 'old_guid',
      appUsage, { getSuccess  : 1 }, false);
  });
