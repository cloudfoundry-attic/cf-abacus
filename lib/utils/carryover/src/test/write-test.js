...
const { each, extend, memoize } = require('underscore');
...
describe('Write carry-over usage', () => {
  ...
  const checkId = (doc, id, time) => {
    ...
    expect(content).to.equal(
      dbclient.tkuri(
        util.format(
          'e8139b76-e829-4af3-b332-87316b1c0a6c/' +
  ...
  const usageResponse = {
    ...
    body: {},
    headers: {
      location: uris().collector + '/v1/metering/collected/usage/t/20161010/k/anonymous'
    }
  };
  ...
  context('on success', () => {
    beforeEach((done) => {
      ...
      const countingCb = (error) => {
        if (error) done(error);
        if (++count === appUsage.length) done();
      };

      carryOver = require('..')(statistics, errorFn);

      each(appUsage, (usage) => {
        carryOver.write(usage, usageResponse, guid, state, (opError) => {
  ...
  context('on failure', () => {
    context('when new app cannot be read', () => {
      ...
      beforeEach((done) => {
        ...
        carryOver = require('..')(statistics, errorFn);
        each(appUsage, (usage) => {
          carryOver.write(usage, usageResponse, undefined, undefined, countingCb);
        });
      });
      ...
      it('calls error function', () => {
        ...
        assert.alwaysCalledWith(errorFn, 'Error getting carry-over usage', testError, undefined, 'carryOver');
      });
    });
    ...
    context('when new app cannot be stored', () => {
      ...
      it('calls error function', () => {
        expect(errorFn.callCount).to.equal(2);
        assert.alwaysCalledWith(errorFn, 'Error upserting carry-over usage', testError, undefined, 'carryOver');
      });
    });
  });
  ...
  context('with a event for a known app', () => {
    ...
    beforeEach((done) => {
      ...
      carryOver = require('..')(statistics, errorFn);
      carryOver.write(appUsage[0], usageResponse, undefined, newResourceState, (opError) => {
  ...
  context('when working with the DB', () => {
    ...
    const checkForTimeoutError = () => {
      ...
      expect(cbSpy.firstCall.args[0]).to.be.instanceOf(require('abacus-breaker').TimeoutError);
    };
    ...
    it('should timeout when getting a document', () => {
      getMock = stub().callsFake((key, cb) => setTimeout(() => cb(), 3 * timeout));
    ...
    it('should timeout when writing a document', () => {
      getMock = stub().yields();
      putMock = stub().callsFake((doc, cb) => setTimeout(() => cb(), 3 * timeout));
