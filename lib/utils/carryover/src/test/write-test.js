...
describe('Write carry-over usage', () => {
  ...
  const checkEventData = (doc, state, timestamp) => {
    expect(doc[0].state).to.equal(state);
    expect(doc[0].timestamp).to.equal(timestamp);
  };
  ...
  context('on success', () => {
    ...
    it('stores references to the documents in the DB', () => {
      ...
      checkPutDocumentRef(putArgs[3], '35c4ff1f', 1438387200000);
      checkPutDocumentRef(putArgs[4], '35c4ff2f', 1438387200000);
      checkPutDocumentRef(putArgs[5], '35c4ff0f', 1438387200000);
    });
    ...
    it('populates statistics object', () => {
      ...
      expect(statistics.carryOver.upsertSuccess).to.equal(6);
  ...
  context('on failure', () => {
    context('when new app cannot be read', () => {
      ...
      it('stores documents we could get in the DB', () => {
        ...
        checkPutDocumentRef(putArgs[3], '35c4ff0f', 1438387200000);
      });
    ...
    context('when new app cannot be stored', () => {
      ...
      it('populates statistics object', () => {
        ...
        expect(statistics.carryOver.getFailure).to.equal(0);
        expect(statistics.carryOver.upsertSuccess).to.equal(4);
        expect(statistics.carryOver.upsertFailure).to.equal(2);
      });
  ...
  context('when an event comes', () => {
    let resourceState = 'new_state';
    let resourceTimestamp = 'current_time';
    let resourceInfo = {
      state: resourceState,
    ...
    beforeEach((done) => {
      ...
      carryOver = require('..')(statistics, errorFn);
      carryOver.write(appUsage[0], resourceInfo, usageResponse, (opError) => {
        assert.notCalled(errorFn);
        done();
      });
    ...
    it('the event info is being updated in the DB', () => {
      const putArgs = putMock.args;
      expect(putArgs.length).to.equal(1);
      checkEventData(putArgs[0], resourceState, resourceTimestamp);
      expect(statistics.carryOver.upsertSuccess).to.equal(1);
    });
  });
});
