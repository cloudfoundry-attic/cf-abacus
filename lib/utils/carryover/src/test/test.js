...
describe('carryover', () => {
  ...
  context('on success', () => {
    ...
    beforeEach((done) => {
      ...
      map(appUsage, (usage) => {
        carryover(usage, usageResponse, statistics,
          (opError, response) => {
            sinon.assert.notCalled(errorFn);
            expect(response).to.deep.equal(usageResponse);
            done(opError);
          });
      });
    });
  ...
  context('on failure', () => {
    context('when new app cannot be read', () => {
      ...
      it('populates statistics object', () => {
        expect(statistics.carryOver.removeSuccess).to.equal(0);
        expect(statistics.carryOver.removeFailure).to.equal(0);
        expect(statistics.carryOver.upsertSuccess).to.equal(2);
        expect(statistics.carryOver.upsertFailure).to.equal(1);
      });

      it('calls error function', ()=> {
        sinon.assert.calledWith(errorFn,
          'Error removing carry-over usage', {}, undefined, 'carryOver', t0);
      });
    });
