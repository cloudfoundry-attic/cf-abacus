...
const iindex = () =>
  app() ? app().instance_index.toString() : process.env.INSTANCE_INDEX ? process.env.INSTANCE_INDEX : '0';

// Return the current app instance IP address
const iaddress = () => process.env.CF_INSTANCE_IP || process.env.HOST || 'localhost';

// Return the current app instance ports from the CF_INSTANCE_PORTS
const iports = memoize(
  () => {
    if (!process.env.CF_INSTANCE_PORTS) return undefined;
...
const iport = () => iports() ? iports()[0].external : process.env.PORT ? parseInt(process.env.PORT) : undefined;

// Return the VCAP service instances from the VCAP_SERVICES env variable
const services = memoize(
  () => {
    return process.env.VCAP_SERVICES ? JSON.parse(process.env.VCAP_SERVICES) : undefined;
  },
  () => {
...
const serviceInstances = (serviceInstanceName) => {
  const foundInstances = reduce(
    services() || {},
    (accum, services) =>
      accum.concat(filter(services, (svc) => svc.name.match(new RegExp(serviceInstanceName + '.*')))),
    []
  );
...
const vcapenv = () => {
  const uris = (app() || {}).application_uris;
  return uris !== undefined && uris.length !== 0;
};
...
module.exports.vcapenv = vcapenv;
