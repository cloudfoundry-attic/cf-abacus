...
describe('abacus-vcapenv', () => {
  it('returns the VCAP_APPLICATION env var', () => {
    ...
    process.env.VCAP_APPLICATION = '{ "name": "foo-1", "instance_id": "abcd", "instance_index": 12 }';

    // Expect the app info and instance id to be parsed from the VCAP_APPLICATION env variable
  ...
  it('returns a default empty VCAP_APPLICATION env var', () => {
    ...
    expect(vcapenv.appversion()).to.equal(require('../../package.json').version);
  ...
  it('returns the CF_INSTANCE address and ports', () => {
    ...
    process.env.CF_INSTANCE_PORTS = '[{ "external": 5678, "internal": 6789 }]';
  ...
  context('when no service instances exist', () => {
    ...
    it('returns a default empty service instance key', () => {
      expect(vcapenv.serviceInstancesCredentials('rediscloud-1', 'host')).to.deep.equal([]);
      expect(vcapenv.serviceInstancesCredentials('unknown', 'host')).to.deep.equal([]);
    });
  ...
  context('when a single service instance is bound', () => {
    ...
    it('returns the service instance', () => {
      ...
      // Expect the service instances to be parsed from VCAP_SERVICES
      expect(vcapenv.serviceInstances('rediscloud-1')).to.deep.equal([serviceInstance]);
    });
    ...
    it('returns a service instance key', () => {
      ...
      expect(vcapenv.serviceInstancesCredentials('rediscloud-1', 'host')).to.deep.equal([
        serviceInstance.credentials.host
      ]);
    });
  });
  ...
  context('when multiple service instances are bound', () => {
    ...
    const services = {
      rediscloud: [serviceInstance1, serviceInstance0]
    };
    ...
    it('returns the correct service instance', () => {
      ...
      // Expect the service instances to be parsed from VCAP_SERVICES
      expect(vcapenv.serviceInstances('rediscloud-1')).to.deep.equal([serviceInstance1]);
    });
    ...
    it('returns all matching service instances, sorted by name', () => {
      ...
      // Expect the service instances to be parsed from VCAP_SERVICES
      expect(vcapenv.serviceInstances('rediscloud')).to.deep.equal([serviceInstance0, serviceInstance1]);
    });
    ...
    it('returns all matching service instances keys', () => {
      ...
      // Expect the credentials key to be parsed from VCAP_SERVICES
      expect(vcapenv.serviceInstancesCredentials('rediscloud', 'host')).to.deep.equal([
        serviceInstance0.credentials.host,
        serviceInstance1.credentials.host
      ]);
    });
  });

  it('returns app and instance info in headers', () => {
    ...
    expect(res.header.args[8]).to.deep.equal(['X-Instance-Id', process.pid.toString()]);
    expect(res.header.args[9]).to.deep.equal(['X-Instance-Index', '1']);
    expect(next.args.length).to.equal(2);
  });

  describe('vcapenv', () => {
    context('in non Cloud Foundry environment', () => {
      before(() => {
        delete process.env.VCAP_APPLICATION;
      });

      it('returns true', () => {
        expect(vcapenv.vcapenv()).to.equal(false);
      });
    });

    context('in invalid Cloud Foundry environment', () => {
      before(() => {
        process.env.VCAP_APPLICATION = `{ 
          "name": "foo-1", 
          "instance_id": "abcd", 
    ...
    context('in valid Cloud Foundry environment', () => {
      before(() => {
        process.env.VCAP_APPLICATION = `{ 
          "name": "foo-1", 
          "instance_id": "abcd", 
          "instance_index": 12,
          "application_uris" : [ "domain" ] 
        }`;
      });

      it('returns true', () => {
        expect(vcapenv.vcapenv()).to.equal(true);
      });
    });
  });
});
