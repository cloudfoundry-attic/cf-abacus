...
describe('abacus-yieldable', () => {
  ...
  it('converts promises to functions with callbacks', (done) => {
    ...
    const sum = (x, y) =>
      new Promise((resolve, reject) => {
        cbget(x + '+' + y, (err, val) => err ? reject(err) : resolve(val.body.result));
      });
  ...
  it('converts functions with callbacks to promises', (done) => {
    ...
    const sum = (x, y, cb) => cbget(x + '+' + y, (err, val) => err ? cb(err) : cb(null, val.body.result));
  ...
  it('checks for promises', () => {
    expect(yieldable.isPromise(new Promise((resolve) => {
      resolve(2);
    }))).to.equal(true);
    expect(yieldable.isPromise(() => {
      return 2;
    })).to.equal(false);
  });
  ...
  it('checks for generators', () => {
    expect(yieldable.isGenerator(gen())).to.equal(true);
    expect(yieldable.isGenerator({})).to.equal(false);
  });

  it('checks for generator functions', () => {
    expect(yieldable.isGeneratorFunction(function*() {
      return 2;
    })).to.equal(true);
    expect(yieldable.isGeneratorFunction(() => {
      return 2;
    })).to.equal(false);
  });

  it('checks for async functions', () => {
    expect(yieldable.isAsyncFunction(async() => {
      return 2;
    })).to.equal(true);
    expect(yieldable.isAsyncFunction(() => {
      return 2;
    })).to.equal(false);
  });
