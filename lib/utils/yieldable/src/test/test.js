...
const cbget = (expr, cb) => IO(() => cb(undefined, { body: { result: eval(expr) }}), 5);

describe('cf-abacus-yieldable', () => {
    it('converts callback-based functions to yieldable generators', (done) => {
        ...
        const sum = function *(x, y) { return (yield get(x + '+' + y)).body.result; };
    ...
    it('converts functions in modules to generator-based flow functions', (done) => {
        // Convert a module exporting a function to a module containing a
        // generator-based flow function
        const mod = yieldable({ unit: yieldable.thunk(5), val: 3 });
