...
global.__shared_emitter = global.__shared_emitter || memoize((name) => emitter());
...
const iterator = (emitter) => {
    ...
    nstream.pipe(through((data) => {
        ...
        callback()(undefined, { value: { name: data[0], value: data[1] }, done: false });

    }, () => {

        // End of the stream, signalled by a null event, call the first
        // callback in the queue with done true
        callback()(undefined, { value: undefined, done: true });
    }));
    ...
    it.next = (cb) => {
        ...
        process.nextTick(() => nstream.resume());
    };
    return it;
};
