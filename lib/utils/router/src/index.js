...
// Small Express router that runs request handlers written as ES6 generators using Node co.
// On top of that the router supports batching

const { allKeys, each, extend, isEmpty, map, omit, pick, toArray, without } = require('underscore');
...
const callbackify = (m, trusted) => {
  ...
  return function() {
    ...
    const error = (err, type) => {
      // filter duplicate message errors
    ...
    try {
      mfunc.apply(
        ...
        params.concat([
          (err, value) => {
            if (err)
              error(err, 'generator error');
            else if (value)
              // Store the returned value in the response, it'll be sent by one of our Express middleware later
...
// Return an implementation of the router.use(path, middleware) function that supports middleware implemented as
// generators in addition to regular callbacks
...
// Return an implementation of the router.route() function that supports middleware implemented as generators
// in addition to regular callbacks
...
const batch = (routes) => {
  return (req, res, next) => {
    if (req.method !== 'POST' || req.url !== '/batch' || isEmpty(req.body)) {
      next();
      return;
    }
    debug('Handling %d batch requests %o', req.body.length, req.body);
