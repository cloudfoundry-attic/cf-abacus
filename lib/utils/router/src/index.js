...
const use = (original, trusted) => {
  return function(path, m) {
    return typeof path === 'function' ? original.call(this, callbackify(
      path, trusted)) : original.call(this, path, callbackify(m, trusted));
  };
};
...
const batch = (routes) => {
  return (req, res, next) => {
    ...
    transform.map(req.body, (r, i, reqs, rcb) => {
      ...
      const rreq = extend(clone(req), {
        method: r.method,
        url: url.resolve(req.url, r.uri),
        body: r.body
      });
