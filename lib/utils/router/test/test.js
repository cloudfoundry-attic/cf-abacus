...
const express = require('cf-abacus-express');
...
describe('cf-abacus-router', () => {
    ...
    it('handles HTTP requests', (done) => {
        ...
        // Send an HTTP request, expecting an OK response
        request.get('http://localhost::p/:v/generator/:r', { p: server.address().port, v: 'ok', r: 'request' }, (err, val) => {
            expect(err).to.equal(undefined);
            expect(val.statusCode).to.equal(200);
            expect(val.body).to.equal('okay');
            done1();
    ...
    it('handles exceptions', (done) => {
        ...
        // Send an HTTP request, expecting an error message
        request.get('http://localhost::p/:v/generator/:r/:m', { p: server.address().port, v: 'err', r: 'request', m: 'boo' }, (err, val) => {
            expect(err.message).to.equal('HTTP response status code 500');
            expect(val).to.equal(undefined);
    ...
    it('handles domain asynchronous exceptions', (done) => {
        process.exit = spy();
        ...
        // Send an HTTP request, expecting an error message
        request.get('http://localhost::p/:v/:r/:m', { p: server.address().port, v: 'err', r: 'request', m: 'boo' }, (err, val) => {
            expect(err.message).to.equal('HTTP response status code 500');
            expect(val).to.equal(undefined);
            expect(process.exit.args.length).to.equal(1);
            done();
    ...
    it.only('handles batched HTTP requests', (done) => {
        // Create a test Express app
        const app = express();
        ...
        // Send an HTTP request, expecting an OK response
        brequest.get('http://localhost::p/:v/:r', { p: server.address().port, v: 'ok', r: 'request' }, (err, val) => {
            expect(err).to.equal(undefined);
            expect(val.statusCode).to.equal(200);
            expect(val.body).to.equal('okay');
            done1();
        ...
        // Send an HTTP request, expecting an OK response
        brequest.get('http://localhost::p/:v/generator/:r', { p: server.address().port, v: 'ok', r: 'request' }, (err, val) => {
            expect(err).to.equal(undefined);
            expect(val.statusCode).to.equal(200);
            expect(val.body).to.equal('okay');
            done1();
        ...
        // Send an HTTP request, expecting a 500 status code
        // Here test the option to pass the URI as a field of the options object
        brequest.get({ uri: 'http://localhost::p/:v/:r', p: server.address().port, v: '500', r: 'request' }, (err, val) => {
            expect(err.message).to.equal('HTTP response status code 500');
            expect(val).to.equal(undefined);
            done1();
        });
        ...
        // Send an HTTP request, expecting an error message
        brequest.get('http://localhost::p/:v/:r/:m', { p: server.address().port, v: 'err', r: 'request', m: 'duh' }, (err, val) => {
            expect(err.message).to.equal('duh');
            expect(val).to.equal(undefined);
            done1();
        });
