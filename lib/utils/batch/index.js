...
// once. Unbatching takes a batch of calls, applies then individually to a
...
const batchifyfn = (fn, timeout) => {
    ...
    const wrapper = function() {
        ...
        debug('Recording function call %o', call);
        ...
        if(calls.length === 1)
            setTimeout(() => {
                ...
                debug('Calling batch function %s with %o', name, bcalls);
                fn(map(bcalls, (call) => initial(call)), (err, vals) => {
                    // Pass each call result to the corresponding callback
                    debug('Returning batch function results %o', vals);
...
const unbatchify = (fn) => {
    return (calls, cb) => {
        debug('Applying function call batch %o', calls);
        ...
        map(calls, (call, i) => {
            fn.apply(undefined, call.concat([(err, val) => {
                // Collect function call result
                debug('Collecting function result %o', [err, val]);
                ...
                if(count === calls.length) {
                    debug('Calling back with batch function results %o', vals);
                    cb(undefined, vals);
                }
            }]));
        });
