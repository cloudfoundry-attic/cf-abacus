  ...
  const wrapper = function() {
    ...
    if(calls.length === 1)
      setTimeout(() => {
        ...
        debug('Calling batch function %s with %d batched calls',
          name, bcalls.length);
...
// well.
const batchify = (fn, timeout) => extend(isFunction(fn) ?
    batchifyfn(fn, timeout) : {}, object(pairs(fn)),
      object(map(filter(functions(fn), (k) => /^batch_/.test(k)),
        (k) => [k.substr(6), batchifyfn(nbind(fn, k), timeout)])));
