...
// Warning: In case there is an error in one of the groups the whole group fails. This behaviur
// is due the fact that with the current implementation of transform.map we could not easily distinguish where
// the error has happened. This logic needs to be revised in future.
      ...
      (err, maps) => {
        ...
        transform.map(
          ...
          (gmaps, i, groups, mcb) => {
            ...
            // In case of error in any, mark all requests within the current group as failed.
            fcb(map(gmaps, (gmap) => gmap.call), (err, gres) => {
              return err ? mcb(undefined, zip(gmaps, map(gmaps, (gmap) => [err]))): mcb(undefined, zip(gmaps, gres));
            });
          },
          (err, gres) => {
            if (err) {
