...
const batchifyfn = (fn, delay, max) => {
  ...
  const flush = () => {
    ...
    debug(
      'Calling batch function %s with %d batched calls', name, bcalls.length);
...
// batch_* as well.
const batchify = (fn, delay, max) => extend(isFunction(fn) ?
    batchifyfn(fn, delay, max) : {}, object(pairs(fn)),
      object(map(filter(functions(fn), (k) => /^batch_/.test(k)),
        (k) => [k.substr(6), batchifyfn(nbind(fn, k), delay, max)])));
