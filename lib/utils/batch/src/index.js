  ...
  const wrapper = function() {
    ...
    if (calls.length === 1)
      setTimeout(() => {
        ...
        debug('Calling batch function %s with %d batched calls', name,
          bcalls.length);
        fcb(map(bcalls, (call) => initial(call)), (err, vals) => {
          // Pass each call result to the corresponding callback
          debug('Returning %d batch function results', vals ? vals.length :
            undefined);
          return map(zip(bcalls, vals), (call) => last(call[0]).apply(
            undefined, err ? [err] : call[1]));
        });
      }, timeout);
  };
    ...
    map(calls, (call, i) => {
      fcb.apply(undefined, call.concat([(err, val) => {
        ...
        if (count === calls.length) {
          debug('Calling back with %d batch function results',
            count);
          cb(undefined, vals);
        }
      }]));
...
const batchify = (fn, timeout) => extend(
  ...
  object(pairs(fn)),
  object(map(filter(functions(fn), (k) => /^batch_/.test(k)), (k) => [k.substr(
    6), batchifyfn(nbind(fn, k), timeout)])));
