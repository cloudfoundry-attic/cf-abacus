...
const {
  bind, defaults, each, extend, filter, flatten, functions, groupBy, identity, initial, isFunction, last, map,
  object, pairs, sortBy, values, zip
} = require('underscore');
...
const defaultConfig = () => ({
  delay: process.env.BATCH_DELAY ? parseInt(process.env.BATCH_DELAY) : 20,
  maxSize: parseInt(process.env.BATCH_SIZE) || 10
});
...
const batchifyfn = (fn, delay, max, size) => {
  // Use a default time delay of 20 msec and a default batch size of 10
  const opt = defaults(
    {
      timeout: delay,
      msize: max,
    ...
    {
      timeout: defaultConfig().delay,
      msize: defaultConfig().maxSize,
      csize: () => 1
    }
  );
  ...
  const flush = () => {
    ...
    debug('Calling batch function %s with %d batched calls', name, fcalls.length);
    fcb(map(fcalls, (call) => initial(call)), (err, vals) => {
      if (err) debug('Batch function %s returning %d batch results with error %o', name, vals ? vals.length : 0, err);
      else debug('Batch function %s returning %d batch results', name, vals ? vals.length : 0);

      // Pass each call result to the corresponding callback
      return map(zip(fcalls, vals), (call) => {
  ...
  const wrapper = extend(
    function() {
      ...
      // Pass the list of calls to the batch function when the batch is
      // full or after the configured timeout
      if (bsize >= opt.msize) {
        if (tid)
          if (opt.timeout) clearTimeout(tid);
          else clearImmediate(tid);
        flush();
      }
      if (bcalls.length === 1) tid = opt.timeout ? setTimeout(flush, opt.timeout) : setImmediate(flush);
    },
    {
      // Preserve the original function name
      fname: name
...
const markAllAsFailed = (group, err) => zip(group, map(group, () => [err]));

// Return a function that organizes a batch of calls in groups, applies each
// group of calls and assembles the results.
// Warning: In case there is an error in one of the groups the whole group fails. This behaviour
// is due the fact that with the current implementation of transform.map we could not easily distinguish where
// the error has happened. Issue #832
      ...
      (err, maps) => {
        ...
        transform.map(
          ...
          (gmaps, i, groups, mcb) => {
            ...
            fcb(map(gmaps, (gmap) => gmap.call), (err, gres) => {
              return err ? mcb(undefined, markAllAsFailed(gmaps, err)) : mcb(undefined, zip(gmaps, gres));
            });
          },
          (err, gres) => {
            if (err) {
            ...
            cb(undefined, map(sortBy(flatten(gres, true), (res) => res[0].i), (res) => res[1]));
          }
        );
      }
    );
...
// batch_* as well.
const batchify = (fn, delay, max, size) =>
  extend(
    isFunction(fn) ? batchifyfn(fn, delay, max, size) : {},
    object(pairs(fn)),
    object(
      map(filter(functions(fn), (k) => /^batch_/.test(k)), (k) => [
        k.substr(6),
        batchifyfn(nbind(fn, k), delay, max, size)
      ])
    )
  );
