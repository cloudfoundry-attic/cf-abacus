...
describe('abacus-batch', () => {
  ...
  it('reports batched call errors', (done) => {
    // Create a batch function that returns an error
    const bsum = spy((batch, cb) => cb(undefined, map(batch, (args) => ['err' + (args[0] + args[1]), undefined])));
    const sum = batch.batchify(bsum, 100);
  ...
  it('groups batches of error calls', (done) => {
    const bsum = spy((batch, cb) => cb('err' + (batch[0][0] + batch[0][1])));

    const gsum = batch.groupBy(bsum, (args, cb) => cb(undefined, args[0] % 2));
    const sum = batch.batchify(gsum, 1000, 2);
