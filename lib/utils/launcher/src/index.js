...
const runCLI = () => {
  ...
  if (process.env.VCAP_APPLICATION) {
    ...
    debug('Starting application %s in %s', appname, cwd);
    ...
    each(range(napps), (a) => {
      ...
      debug('Starting application %s.%d with port %s in %s', appname, a, port + a, cwd);
      childProcess = cp.spawn(cmd, args, {
        env: env,
        stdio: 'inherit'
      });
      debug('Started application %s.%d', appname, a);
      childProcess.unref();
    });
  }
