...
const webapp = () => {
  ...
  app.useHealthCheck = (middlware) => {
    if (middlware)
      healthCheckMiddlware = middlware;
    else
      healthCheckMiddlware = defaultHealthCheck;
  };
  ...
  app.listen = wrap(app.listen, (listen, opt, cb) => {
    if (secured())
      app.use('/hystrix.stream', oauth.basicStrategy(
        process.env.API, hystrix.scopes,
    ...
    if (secured())
      app.use('/healthcheck', oauth.basicStrategy(
        process.env.API, healthCheckScopes,
        process.env.JWTKEY, process.env.JWTALGO
      ));
    ...
    if(cluster.isMaster() && eureka())
      eureka.register(
        eureka(), appname, vcapenv.iport() || server.address().port,
        vcapenv.iaddress(), (err, val) => {
          if(err)
