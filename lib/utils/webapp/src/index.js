...
const webapp = () => {
  ...
  app.listen = wrap(app.listen, (listen, opt, cb) => {
    if (secured())
      app.use(
        '/hystrix.stream',
        oauth.basicStrategy(process.env.AUTH_SERVER, hystrix.scopes, process.env.JWTKEY, process.env.JWTALGO)
      );
    ...
    if (secured())
      app.use(
        '/healthcheck',
        oauth.basicStrategy(process.env.AUTH_SERVER, healthCheckScopes, process.env.JWTKEY, process.env.JWTALGO)
      );
