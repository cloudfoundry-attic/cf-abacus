...
const runCLI = () => {
  ...
  // Start a Webapp
  if(commander.start) {
    // Set default port, command line --port has higher priority then
    // the existing env, then rc files then default to 9080
    ...
    // Stop a Webapp
    cp.exec('pkill -f "node ' +
      [vcapenv.appname(), vcapenv.appindex()].join('-') + '"',
      (err, stdout, stderr) => {
