...
describe('cf-abacus-request', () => {
    it('sends HTTP requests', (done) => {
        ...
        const server = http.createServer((req, res) => {
            if (req.url === '/ok/request' && req.method === 'OPTIONS')
                ...
                req.on('end', () => {
                    ...
                    res.end(JSON.stringify({ message: JSON.parse(body) }));
                });
            }
        });
        ...
        request.waitFor('http://localhost::p/ok/request', { p: server.address().port }, (err, val) => {
            ...
            request.get('http://localhost::p/:v/:r', { cache: true, p: server.address().port, v: 'ok', r: 'request' }, (err, val) => {
    ...
    it('batches HTTP requests', (done) => {
                ...
                req.on('data', (chunk) => {
                    ...
                    const bres = map(body, (breq) => {
                        if (breq.uri === '/ok/request' && breq.method === 'GET')
                            // Return an OK response with a body
                            return { statusCode: 200, body: gets++ === 0 ? 'okay' : 'notok' };
                            ...
                            // Return a JSON object with a message field, to test
                            // request's ability to pick up that message
                            return { statusCode: 500, body: { message: breq.body }};
                    });
        ...
        // Wait for the server to become available
        request.waitFor('http://localhost::p/batch', { p: server.address().port }, (err, val) => {
            if(err) throw err;
            ...
            // Send an HTTP request, expecting an OK response
            brequest.get('http://localhost::p/:v/:r', { cache: true, p: server.address().port, v: 'ok', r: 'request' }, (err, val) => {
                expect(err).to.equal(undefined);
                expect(val.statusCode).to.equal(200);
                expect(val.body).to.equal('okay');
                done1();
            ...
            // Send an HTTP request, expecting a 500 status code
            // Here test the option to pass the URI as a field of the options object
            brequest.post({ uri: 'http://localhost::p/:v/:r', p: server.address().port, v: '500', r: 'request' }, (err, val) => {
                expect(err.message).to.equal('HTTP response status code 500');
                expect(val).to.equal(undefined);
                done1();
            });
            ...
            // Send an HTTP request, expecting an error message
            brequest.post('http://localhost::p/:v/:r', { p: server.address().port, v: 'err', r: 'request', body: 'duh' }, (err, val) => {
                expect(err.message).to.equal('duh');
                expect(val).to.equal(undefined);
                done1();
            });
