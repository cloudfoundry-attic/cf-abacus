...
const request = (uri, opts, cb) => {
  ...
  // Send the request
  const send = (t) => {
    debug('Sending %s request %s' + (t.options.body ? ' %o' : '%s'),
        t.options.method, t.uri, t.options.body ? t.options.body : '');
    _request(t.options, (err, res) => {
      ...
      debug('Received %s response %d' + (res.body ? ' %o' : '%s'),
        t.options.method, res.statusCode, res.body ? res.body : '');
      if(t.options.nowait)
        return undefined;
