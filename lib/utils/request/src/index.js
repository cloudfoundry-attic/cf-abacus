...
const route = (template, parms) => {
  return template.replace(/:[a-z_][a-z0-9_]*/ig,
    (name) => parms[name.substr(1)]);
};

// Setup request default options
const drequest = request.defaults({ json: true, rejectUnauthorized: false,
    forever: true, pool: { maxSockets: 1000 } });
...
const target = (m, uri, opt, cb) => {
  ...
  const options = (uri, opt) => {
    if(typeof opt === 'object')
      return extend(clone(opt), { route: uri });

    if(typeof uri === 'string')
      return { route: uri };

    return extend(clone(uri), { route: uri.route || uri.uri });
  };

  // Resolve the route URI
  const resolve = (opts) => extend(clone(opts), { uri: route(opts.route,
    opts) });
...
const cache = memoize(() => {
  ...
  return {
    ...
    put: (t, res, cb) => {
      return db.put(t.uri, JSON.stringify(pick(res, 'statusCode', 'cookies',
        'headers', 'body')), () => {
          debug('Cached %s %s response %d %o', t.options.method, t.uri,
            res.statusCode || 200, res.body ? res.body : '');
...
const xrequest = (uri, opts, cb) => {
  ...
  // Send the request
  const send = (t) => {
    debug('Sending %s request %s %o', t.options.method, t.uri,
      t.options.body || '');
...
const batchOp = (m, opt) => {
  ...
  return (reqs, cb) => {
    ...
    const targets = map(reqs, (args, i, reqs) =>
      ({ i: i, target: target(m, args[0], args[1], args[2]) }));
    ...
    transform.map(targets, (t, i, targets, ccb) => {
      ...
      const cached = map(filter(res, (t) => t.cached), (r) => ({ i: r.i,
        res: r.cached }));
      ...
      transform.map(groups, (group, g, groups, gcb) => {
        ...
        transform.map(fgres, (r, i, fgres, rcb) => {
          ...
          const res = map(sortBy(cached.concat(fgres), (r) => r.i),
            (r) => r.res);
