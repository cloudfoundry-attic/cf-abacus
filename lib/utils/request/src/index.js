...
const route = (template, parms) => {
  return template.replace(/:[a-z_][a-z0-9_]*/ig, (name) => parms[name.substr(
    1)]);
};
...
const target = (m, uri, opt, cb) => {
  ...
  const options = (uri, opt) => {
    if (typeof opt === 'object')
      return extend(defopts(), clone(opt), {
        route: uri
      });
    if (typeof uri === 'string')
      return extend(defopts(), {
        route: uri
      });
    return extend(defopts(), uri, {
...
const cacheable = (t, res) => {
  if (!t.options.cache || t.options.method !== undefined && t.options.method !==
    'GET')
    return false;
  ...
  return {
    ...
    get: (t, cb) => {
      return db.get(t.uri, (err, res) => {
        if (err) {
          debug('Didn\'t find %s %s response in cache', t.options.method,
            t.uri);
          return cb(err);
        }
    ...
    put: (t, res, cb) => {
      return db.put(t.uri, JSON.stringify(pick(res, 'statusCode',
...
const request = (uri, opts, cb) => {
  ...
  // Send the request
  const send = (t) => {
    debug('Sending %s request %s' + (t.options.body ? ' %o' : '%s'), t.options
      .method, t.uri, t.options.body ? t.options.body : '');
    _request(t.options, (err, res) => {
      ...
      debug('Received %s response %d' + (res.body ? ' %o' : '%s'), t.options
        .method, res.statusCode, res.body ? res.body : '');
      if (t.options.nowait)
        return undefined;
  ...
  return (reqs, cb) => {
    ...
    transform.map(targets, (t, i, targets, ccb) => {
      ...
      transform.map(groups, (group, g, groups, gcb) => {
        ...
        }, pick(t.target.options, 'method', 'headers', 'json',
          'body')));
        ...
        post(url.resolve(group[0].target.uri, '/batch'), {
          ...
          if (res)
            gcb(undefined, map(res.body, (r, i) => {
              if (r.statusCode >= 500 && r.statusCode <=
                599)
                return {
                  i: group[i].i,
