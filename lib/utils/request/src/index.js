...
const cacheableTarget = (t) => {
  if (!t.options.cache || !t.callback || (t.options.method !== undefined && t.options.method !== 'GET')) return false;
  return true;
};

// Return true if the given request target and response are cacheable
const cacheable = (t, res) => {
  if (!cacheableTarget(t)) return false;
  if (res && res.statusCode !== undefined && res.statusCode !== 200) return false;
...
const cached = (uri) => {
  ...
  if (!res) debug("Didn't find resource %s in cache", uri);
  else debug('Found resource %s in cache %d %o', uri, res.statusCode || 200, res.body ? res.body : '');
  return res;
};
...
const httpexc = (res) => {
  const exc = new Error(
    ...
    res.body && res.body.message
      ? res.body.message
      : res.body && res.body.error ? res.body.error : 'HTTP response status code ' + res.statusCode
  );
...
const createRequest = () => (uri, opts, cb) => {
  ...
  // Send the request
  const send = (t, scb) => {
    debug('Sending %s request %s %o', t.options.method, t.uri, t.options.body || '');
