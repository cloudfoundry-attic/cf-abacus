...
//   JSON response
// });
const xrequest = extend((uri, opts, cb) => {
  ...
  if(cacheable(t))
    return lock(t.uri, (err, unlock) => {
      if(err) return unlock(t.callback(err));
      const res = cached(t.uri);
      if(res)
        return unlock(t.callback(undefined, res));

      // Send the request
      return send(t, (err, res) => unlock(t.callback(err, res)));
    });

  // If not cacheable then just sent the request
  return send(t, (err, res) => t.callback(err, res));
}, {
  fname: 'request'
});
...
extend(xrequest, {
  ...
  batch_get: extend(batchOp('GET'), {
    fname: 'request.get'
  }),
  batch_head: extend(batchOp('HEAD'), {
    fname: 'request.head'
  }),
  batch_patch: extend(batchOp('PATCH'), {
    fname: 'request.patch'
  }),
  batch_post: extend(batchOp('POST'), {
    fname: 'request.post'
  }),
  batch_put: extend(batchOp('PUT'), {
    fname: 'request.put'
  }),
  batch_del: extend(batchOp('DELETE'), {
    fname: 'request.del'
  }),
  batch_delete: extend(batchOp('DELETE'), {
    fname: 'request.delete'
  })
});
