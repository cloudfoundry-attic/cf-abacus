...
const ping = function(uri, opt) {
  ...
  if(moment.now() - pings[t.uri].t >= 250) {
    ...
    this.options(t.uri, t.options, t.callback);
  }
...
const waitFor = function(uri, opt, time, cb) {
  ...
  const i = setInterval(() => {
    const n = this.ping(t.uri, t.options);
...
const noDefaults = createRequest();
const defaults = (options) => createRequest(options);
