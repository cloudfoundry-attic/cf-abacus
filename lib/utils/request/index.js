...
const lrudown = require('cf-abacus-lrudown');
...
const cacheable = (t, res) => {
    if(!t.options.cache || t.options.method !== undefined && t.options.method !== 'GET')
        return false;
...
const cache = memoize(() => {
    const db = levelup('cf-abacus-request-cache', { db: (loc) => new lrudown(loc) });
    ...
    return {
        ...
        get: (t, cb) => {
            return db.get(t.uri, (err, res) => {
                if(err) {
                    debug('Didn\'t find %s %s response in cache', t.options.method, t.uri);
                    return cb(err);
                }

                // Return the cached response
                debug('Found %s %s response in cache %d %o', t.options.method, t.uri, res.statusCode || 200, res.body ? res.body : '');
        ...
        put: (t, res, cb) => {
            return db.put(t.uri, JSON.stringify(pick(res, 'statusCode', 'cookies', 'headers', 'body')), () => {
                debug('Cached %s %s response %d %o', t.options.method, t.uri, res.statusCode || 200, res.body ? res.body : '');
                cb(undefined, res);
            });
        }
    };
    ...
    const send = (t) => {
        ...
        _request(t.options, (err, res) => {
            ...
            if(err) {
                ...
                if(t.options.nowait)
                    return undefined;
                return t.callback(err, undefined);
            }
...
const batchOp = (m, opt) => {
    ...
    return (reqs, cb) => {
        ...
        const targets = map(reqs, (args, i, reqs) => ({ i: i, target: target(m, args[0], args[1], args[2]) }));
        ...
        transform.map(targets, (t, i, targets, ccb) => {
            if(cacheable(t.target))
                return cache().get(t.target, (err, res) => {
                    ...
                    ccb(undefined, extend(clone(t), { cached: [undefined, res] }));
                });
            ...
            const cached = map(filter(res, (t) => t.cached), (r) => ({ i: r.i, res: r.cached }));
            ...
            transform.map(groups, (group, g, groups, gcb) => {
                ...
                const greq = map(group, (t) => extend({ uri: url.parse(t.target.uri).path }, pick(t.target.options, 'method', 'headers', 'json', 'body')));

                // Send the POST request to the target's host /batch path
                post(url.resolve(group[0].target.uri, '/batch'), { body: greq }, (err, res) => {
                    ...
                    if(res)
                        gcb(undefined, map(res.body, (r, i) => {
                            ...
                            return { i: group[i].i, res: [undefined, { statusCode: r.statusCode || 200, headers: r.headers || [], body: r.body }] };
                        }));
                    else
                ...
                transform.map(fgres, (r, i, fgres, rcb) => {
                    ...
                    const res = map(sortBy(cached.concat(fgres), (r) => r.i), (r) => r.res);
                    cb(undefined, res);
                });
            });
        });
