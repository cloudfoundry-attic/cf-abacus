...
const period = (t) => {
  ...
  const per = d.getUTCFullYear() * 10000 + (d.getUTCMonth() + 1) * 100 +
    d.getUTCDate();
...
const forward = (b, per, rw, cb) => {
  ...
  debug('Forwarded bucket %d, period %d, operation %s to partitions %o', b,
    per, rw, pars);
...
const partitioner = (bucket, period, forward, balance) => {
  return (k, t, rw, cb) => {
    if (Array.isArray(t)) {
      ...
      const periods = range(bounds[0], bounds[1] + 1, bounds[1] < bounds[0] ?
        -1 : 1);
      ...
      transform.map(periods, (per, i, l, cb) => {
        forward(buck, per, rw, (err, par) => err ? cb(err) : cb(
          undefined, par));
      }, (err, fpars) => {
        if (err) {
        ...
        transform.map(upars, (par, i, l, cb) => {
          balance(par, rw, (err, bpar) => err ? cb(err) : cb(
            undefined, bpar));
        }, (err, bpars) => {
          if (err) {
