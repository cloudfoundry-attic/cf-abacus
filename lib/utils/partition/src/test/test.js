...
describe('abacus-partition', () => {
  it('distributes keys over several partitions', () => {
    ...
    partition('Hello', moment(
      '2014-11-06 19:06:54').valueOf(), 'read', (err, val) => {
        expect(err).to.equal(undefined);
        expect(val).to.deep.equal([0, 201411]);
      });
    partition('Hey', moment(
      '2014-10-06 19:06:54').valueOf(), 'read', (err, val) => {
        expect(err).to.equal(undefined);
        expect(val).to.deep.equal([1, 201410]);
      });
    partition('Blah', moment(
      '2014-10-06 19:06:54').valueOf(), 'read', (err, val) => {
        expect(err).to.equal(undefined);
        expect(val).to.deep.equal([3, 201410]);
      });
    partition('Awwww', moment(
      '2014-11-06 19:06:54').valueOf(), 'read', (err, val) => {
        expect(err).to.equal(undefined);
  ...
  it('distributes time ranges over partition ranges', () => {
    ...
    partition('Hey', [
      moment('2014-10-06 19:06:54').valueOf(),
      moment('2014-12-06 19:06:54').valueOf()
    ], 'read', (err, val) => {
      expect(err).to.equal(undefined);
  ...
  it('reports forwarding and balancing errors', () => {
    ...
    epartition('Blah', moment(
      '2014-10-06 19:06:54').valueOf(), 'read', (err, val) => {
        expect(err).to.equal(berr);
      });
    epartition('Awwww', moment(
      '2014-11-06 19:06:54').valueOf(), 'read', (err, val) => {
  ...
  context('distributes sample keys evenly', () => {
    ...
    const timeInMs = moment('2014-11-06 19:06:54').valueOf();

    // Distribute keys into partitions
    const distribute = (fn, timeFn, keys, numPartitions, numBuckets) => {
      const counts = Array.apply(null, { length: numPartitions });
      counts.fill(0);

      const idealDistKeys = keys.length / counts.length;
      ...
      map(fn(keys, keys.length), (k) => {
        ...
        partitionFn(k, time, 'read', (err, p) => {
          ...
          if (reduce(counts, (a, c) => a + c, 0) === keys.length) {
            debug('Key counts %o; max error %d %%; Distribution distance:',
    ...
    const test = (timeFn, keys, numPartitions, numBuckets) => {
      it('first 1000 keys', () => {
        distribute(first, timeFn, keys, numPartitions, numBuckets);
      });

      it('last 1000 keys', () => {
        distribute(last, timeFn, keys);
      });

      it('sample of 1000 keys', () => {
        distribute(sample, timeFn, keys);
      });
    };
    ...
    context('4 partitions with 4000 buckets', () => {
      ...
      context('with wide time-range', () => {
        const low = moment(timeInMs).subtract(50, 'year').valueOf();
        const high = moment(timeInMs).add(50, 'year').valueOf();
        const timeFn = () => Math.random() * (high - low) + low;
      ...
      context('with random keys', () => {
        ...
        it('keys', () => {
          distribute(() => keys, timeFn, keys, 4, 4000);
        });
      });
    });
    ...
    context('6 partitions with 4000 buckets', () => {
      ...
      context('with random keys', () => {
        ...
        it('keys', () => {
          distribute(() => keys, timeFn, keys, 6, 4000);
        });
      });
    });
  });
  ...
  it('do not balance when the key is undefined', () => {
    ...
    part(undefined, moment(
      '2014-11-06 19:06:54').valueOf(), 'write', (err, val) => {
        expect(err).to.be.ok;
      });
