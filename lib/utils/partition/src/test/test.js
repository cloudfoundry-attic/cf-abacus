...
describe('abacus-partition', () => {
  ...
  it('distributes time ranges over partition ranges', () => {
    ...
    partition('Hey', [Date.parse(
      'Mon Oct 06 2014 19:06:54 GMT-0800 (PST)'), Date.parse(
      'Sat Dec 06 2014 19:06:54 GMT-0800 (PST)')], 'read', (err, val) => {
        expect(err).to.equal(undefined);
    ...
    partition('Blah', [Date.parse(
      'Set Dec 06 2014 19:06:54 GMT-0800 (PST)'), Date.parse(
      'Mon Oct 06 2014 19:06:54 GMT-0800 (PST)')], 'read', (err, val) => {
        expect(err).to.equal(undefined);
        expect(val).to.deep.equal([
          [3, 201412],
  ...
  it('reports forwarding and balancing errors', () => {
    ...
    const epartition = partition.partitioner(partition.bucket,
      partition.period, (b, per, rw, cb) => {
