...
describe('abacus-partition', () => {
  ...
  it('distributes time ranges over partition ranges', () => {
    ...
    partition('Hey', [
      moment.utc('2014-10-06 19:06:54').valueOf(),
      moment.utc('2014-12-06 19:06:54').valueOf()
    ], 'read', (err, val) => {
      expect(err).to.equal(undefined);
  ...
  it('reports forwarding and balancing errors', () => {
    ...
    epartition('Blah', moment.utc(
      '2014-10-06 19:06:54').valueOf(), 'read', (err, val) => {
        expect(err).to.equal(berr);
      });
    epartition('Awwww', moment.utc(
      '2014-11-06 19:06:54').valueOf(), 'read', (err, val) => {
  ...
  context('distributes sample keys evenly', () => {
    ...
    context('4 partitions with 4000 buckets', () => {
      ...
      context('with wide time-range', () => {
        const low = moment.utc(timeInMs).subtract(50, 'year').valueOf();
        const high = moment.utc(timeInMs).add(50, 'year').valueOf();
        const timeFn = () => Math.random() * (high - low) + low;
