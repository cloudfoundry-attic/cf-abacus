...
describe('cf-abacus-partition', () => {
    it('distributes keys over several partitions', () => {
        // Get partitions for some keys and times
        partition('Hello', Date.parse('Thu Nov 06 2014 19:06:54 GMT-0800 (PST)'), 'read', (err, val) => {
            expect(err).to.equal(undefined);
            expect(val).to.deep.equal([0, 201411]);
        });
        partition('Hey', Date.parse('Mon Oct 06 2014 19:06:54 GMT-0800 (PST)'), 'read', (err, val) => {
            expect(err).to.equal(undefined);
            expect(val).to.deep.equal([1, 201410]);
        });
        partition('Blah', Date.parse('Mon Oct 06 2014 19:06:54 GMT-0800 (PST)'), 'read', (err, val) => {
            expect(err).to.equal(undefined);
            expect(val).to.deep.equal([3, 201410]);
        });
        partition('Awwww', Date.parse('Thu Nov 06 2014 19:06:54 GMT-0800 (PST)'), 'read', (err, val) => {
            expect(err).to.equal(undefined);
    ...
    it('distributes time ranges over partition ranges', () => {
        ...
        partition('Hey', [Date.parse('Mon Oct 06 2014 19:06:54 GMT-0800 (PST)'), Date.parse('Sat Dec 06 2014 19:06:54 GMT-0800 (PST)')], 'read', (err, val) => {
            expect(err).to.equal(undefined);
            expect(val).to.deep.equal([[1, 201410], [1, 201411], [1, 201412]]);
        });
        partition('Blah', [Date.parse('Set Dec 06 2014 19:06:54 GMT-0800 (PST)'), Date.parse('Mon Oct 06 2014 19:06:54 GMT-0800 (PST)')], 'read', (err, val) => {
            expect(err).to.equal(undefined);
    ...
    it('reports forwarding and balancing errors', () => {
        ...
        const epartition = partition.partitioner(partition.bucket, partition.period, (b, per, rw, cb) => {
        ...
        epartition('Blah', Date.parse('Mon Oct 06 2014 19:06:54 GMT-0800 (PST)'), 'read', (err, val) => {
            expect(err).to.equal(berr);
        });
        epartition('Awwww', Date.parse('Thu Nov 06 2014 19:06:54 GMT-0800 (PST)'), 'read', (err, val) => {
            expect(err).to.equal(ferr);
        });
        epartition('Blah', [Date.parse('Mon Oct 06 2014 19:06:54 GMT-0800 (PST)'), Date.parse('Sat Dec 06 2014 19:06:54 GMT-0800 (PST)')], 'read', (err, val) => {
            expect(err).to.equal(berr);
        });
        epartition('Awwww', [Date.parse('Mon Oct 06 2014 19:06:54 GMT-0800 (PST)'), Date.parse('Sat Dec 06 2014 19:06:54 GMT-0800 (PST)')], 'read', (err, val) => {
            expect(err).to.equal(ferr);
        });
    });
    ...
    it('distributes sample keys evenly', () => {
        ...
        map([first, last, sample], (filter) => {
            ...
            map(filter(keys, 1000), (k) => {
                partition(k, Date.parse('Thu Nov 06 2014 19:06:54 GMT-0800 (PST)'), 'read', (err, p) => {
                    // Count keys in each partition
                    if(err) throw err;
                    counts[p[0]]++;
                    if(reduce(counts, (a, c) => a + c, 0) === 1000) {
                        console.log('Distribution');
                        // Expect 250 +- 20% keys in each partition
                        map(counts, (c) => {
                            const dist = (c - 250) / 250 * 100;
                            console.log(dist.toFixed(2) + '%');
                            expect(Math.abs(dist)).to.be.below(20);
                        });
                    }
                });
            });
