...
const loadPollingFromEnvironment = () => {
  ...
  const allowedOrgs = process.env.ORGS_TO_REPORT ? JSON.parse(process.env.ORGS_TO_REPORT) : undefined;
...
const loadHealthCheckFromEnvironment = () => {
  const thirtyMinutes = 1000 * 60 * 30;
  return {
    threshold: parseInt(process.env.HEALTHCHECK_THRESHOLD) || thirtyMinutes
  };
};
...
const loadFromEnvironment = () => {
  return {
    ...
    healthcheck: loadHealthCheckFromEnvironment()
  };
};
