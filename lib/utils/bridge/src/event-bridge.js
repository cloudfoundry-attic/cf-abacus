...
const create = (opts) => {
  ...
  // const singleUsageDocHandler = function*(usageDoc, event) {
  ...
  // const multipleUsageDocsHandler = function*(usageDocs, event) {
  //   debug('Processing %d usage docs', usageDocs.length ? usageDocs.length : 1);
  //   for (let usageDoc of usageDocs)
  //     yield singleUsageDocHandler(usageDoc, event);
  // };
  ...
  const processEvent = function*(event) {
    ...
    debug('Created usages: %j', usages);
    // yield multipleUsageDocsHandler(usages, event);
