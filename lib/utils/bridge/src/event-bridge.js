...
const isCreatedResponse = (response) => response && response.statusCode === 202;

const isNotSupportedResponse = (response) => response && response.statusCode === 451;
...
const create = (opts) => {
  ...
  const upsertToCarryOver = yieldable((usage, response, guid, state, cb) => {
    carryOver.upsert(usage, response, guid, state, cb);
  });
  ...
  const sendUsage = function*(usage, event) {
    ...
    if (isCreatedResponse(response)) {
      ...
      yield upsertToCarryOver(usage, response, event.metadata.guid, event.entity.state);
    } else if (isConflictingResponse(response)) {
      debug('Conflicting usage document: %o', usage);
      emitter.emit('usage.conflict');
    } else if (isNotSupportedResponse(response)) {
      debug('Account license is not supported: %o', usage);
      emitter.emit('usage.notsupported');
    } else
