...
const create = (opts) => {
  const poll = (callback) => {
    ...
    const onReadPageDocument = (doc, cb) => {
      if (isOldEnough(doc, opts.minAge)) callback(doc, cb);
      else cb();
    };
    ...
    const onReadPageFailure = (err, response) => {
      if (isGuidNotFoundResponse(response)) emitter.emit('finished', createGuidNotFoundError());
      else {
        const msg = util.format('Could not read events due to error "%s" and response "%j".', err, response);
        edebug(msg);
