...
describe('event-reader', () => {
  ...
  beforeEach(() => {
    sandbox.stub(paging, 'pageIterator').yields(undefined);
    itemIteratorStub = sandbox.stub(paging, 'itemIterator');
  ...
  describe('finished event', () => {
    ...
    context('when readPage reports failure', () => {
      beforeEach(() => {
        readPageStub.callsFake((uri, cfToken, perf, statistics, { processResourceFn, success, failure }) => {
