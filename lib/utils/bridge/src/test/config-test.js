  ...
  describe('from environment variables', () => {
    context('environment variables not set', () => {
      before(() => {
        ...
        delete process.env.HEALTHCHECK_THRESHOLD;
      ...
      it('contains default healthcheck values', () => {
        const thirtyMinutes = 1000 * 60 * 30;

        expect(cfg.healthcheck).to.deep.equal({
          threshold: thirtyMinutes
        });
      });
    });
    ...
    context('environment variables set', () => {
      before(() => {
        ...
        process.env.DB_URI = 'https://services.example.org/mongo';
        ...
        process.env.HEALTHCHECK_THRESHOLD = '30000';
        cfg = config.loadFromEnvironment();
      });
      ...
      it('contains specific healthcheck values', () => {
        expect(cfg.healthcheck).to.deep.equal({
          threshold: 30000
        });
      });
