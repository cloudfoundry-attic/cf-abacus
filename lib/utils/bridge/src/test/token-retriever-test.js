...
describe('token-retriever', () => {
  ...
  context('when token start succeeds', () => {
    ...
    context('when token without scopes is retrieved', () => {
      ...
      it('retrieves the token via oauth package', () => {
        ...
        assert.calledWithExactly(oauthCacheStub, authServerURI, clientId, clientSecret, undefined);
      });
    });
    ...
    context('when token with scopes is retrieved', () => {
      ...
      it('retrieves the token via oauth package', () => {
        assert.calledOnce(oauthCacheStub);
        assert.calledWithExactly(oauthCacheStub, authServerURI, clientId, clientSecret, `${firstScope} ${secondScope}`);
      });
    });
  });
  ...
  context('when token start fails', () => {
    beforeEach(() => {
      tokenStartStub
        .onFirstCall()
        .yields(new Error('Failed to start'))
        .onSecondCall()
        .yields();
    });
