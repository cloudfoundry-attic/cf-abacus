...
const create = (config, statistics) => {
  ...
  const db = yieldable(
    throttle(retry(breaker(batch(dbclient(partition.singleton, dbclient.dburi(config.url, 'abacus-cf-bridge'))))))
  );
  ...
  const write = function*(value) {
    ...
    try {
      ...
      edebug('Failed to write value with id "%s": %o. Error: %o', documentId, newDocument, err);
