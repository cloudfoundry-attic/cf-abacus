...
class Application {
  ...
  *createHealthCheckMiddleware(cfg) {
    return (req, res) => {
      const healthy = this.healthMonitor.healthy();
  ...
  *load() {
    ...
    this.bridge.on('usage.notsupported', () => {
      this.statistics.usage.success.notsupported++;
    });
    ...
    this.healthMonitor = createHealthMonitor(this.bridge, this.cfg.healthcheck.threshold, {
