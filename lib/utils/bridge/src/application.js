...
class Application {
  ...
  *load() {
    ...
    const reporter = createReporter(this.statistics, this.errors);
    ...
    this.bridge.on('usage.conflict', (operationStart) => {
      this.statistics.usage.conflicts++;
      perf.report(perfUsageIdentifier, operationStart);
    });
    this.bridge.on('usage.skip', (operationStart) => {
      this.statistics.usage.skips++;
      perf.report(perfUsageIdentifier, operationStart);
    });
