...
const debugStart = (opts) => {
  debug('Getting token from "%s" with client_id "%s" and scopes "%j".',
    opts.authServerURI,
    opts.clientId,
    opts.scopes || []
  );
...
const debugStartSuccess = (opts) => {
  debug('Acquired token from "%s" with client_id "%s" and scopes "%j".',
    opts.authServerURI,
    opts.clientId,
    opts.scopes || []
  );
...
const retrieve = (opts, cb) => {
  ...
  retryStartToken((err) => {
    if (!err)
      debugStartSuccess(opts);
    else
      debugStartFailure(opts, err);
    cb(err, token);
  });
};
