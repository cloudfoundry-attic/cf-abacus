...
describe('abacus-dataflow', () => {
  ...
  describe('dataflow mapper', function() {
    ...
    context('on error storing in the output db', () => {
      ...
      beforeEach((done) => {
        ...
        const opts = {
          input: {
            ...
            schema: Pair,
            post: '/v1/replay',
            get: '/v1/replay/t/:t/k/:kx/:ky',
            dbname: 'abacus-dataflow-replay',
            wscope: iwscope,
            rscope: irscope,
            key: ikey,
            time: itime
          ...
          output: {
            type: 'sum',
            get: '/v1/outplay/t/:t/k/:kx/:ky',
            dbname: 'abacus-dataflow-outplay',
            rscope: orscope,
            keys: okeys,
            times: otimes
          },
        ...
        const logInput = yieldable.functioncb(dataflow.logInput);
        const doc = {
          id: dbclient.tkuri('1/2', moment.now()),
          t: 1443650828617,
          x: 1,
          y: 2,
          processed_id: '0001488815391973-0-0-0-0',
          processed: 1488815391973
        };
        logInput(doc, db, (err) => {
          expect(err).to.equal(null);
          done();
      ...
      it('replays data', (done) => {
        ...
        dataflow.replay(mapper, 1000, (err, replayedDocs) => {
          expect(err).to.equal(null);
          expect(replayedDocs.length).to.equal(1);
  ...
  describe('dataflow reducer', function() {
    ...
    it('detects error when there is no sink', (done) => {
      ...
      // Define a test reduce transform that accumulates the sum of
      // numbers
      let bigSum = function *(accum, udoc, auth) {
        if(udoc.x < 7)
          return [extend({
            t: udoc.t,
            x: udoc.x
      ...
      dataflow.replay(reducer, 1000, (err, vals) => {
        ...
        treduce([6, 6, 7], (accum, ival, i, l, cb) => {
          ...
          // expect the 2 errors to be replayed, although they're duplicates
          // this is due to the reduce function that returns error for them
          dataflow.replay(reducer, 1000, (err, vals) => {
            expect(err).to.equal(null);
            expect(vals.length).to.deep.equal(2);
    ...
    it('propagates error from the sink and does not store to output',
      (done) => {
        ...
        dataflow.replay(reducer, 1000, (err, vals) => {
          expect(err).to.equal(null);
          // expect no docs for replay
          expect(vals.length).to.deep.equal(0);
    ...
    it('does batch post, and gather the errors', (done) => {
      ...
      const reducer = dataflow.reducer(multiDoc, {
        ...
        output: {
          type: 'sum',
          get: '/v1/reductions/t/:t/k/:kg',
          dbname: 'abacus-dataflow-reduktions',
          rscope: orscope,
          keys: okeys,
          times: otimes
        },
        sink: {
          host: 'http://localhost:9081',
          authentication: () => 'Bearer authentication',
          posts: ['/v2/reduktions', '/v2/reduktions', '/v2/reduktions']
        }
      });
    ...
    context('on error storing in the output db', () => {
      ...
      it('replays data', (done) => {
        ...
        dataflow.replay(reducer, 1000, (err, replayedDocs) => {
          expect(err).to.equal(null);
          expect(replayedDocs.length).to.equal(1);
  ...
  describe('dataflow sink', () => {
    ...
    const distribute = (keys, expectFn, done) => {
      ...
      map(keys, (k) => {
        yieldable.functioncb(dataflow.sink)(k, remotesink, numPartitions,
          (err, uri) => {
            expect(err).not.to.ok;
            ...
            if (sum === keys.length) {
              debug('%d keys distributed in %j; max error %d %%; ' +
                'Distribution distance:', keys.length, counts, errorPercentage);
              // Expect keysInPartition +- errorPercentage keys
              // in each partition
              map(counts, (count) => {
                const distance =
                  (count - idealDistKeys) / idealDistKeys * 100;
    ...
    it('distributes random orgs evenly', (done)=> {
      const low = moment.utc().subtract(50, 'year').valueOf();
      const high = moment.utc().add(50, 'year').valueOf();
      const timeFn = () => Math.random() * (high - low) + low;

      const keys = [];
      _(1000).times(() => {
        keys.push(dbclient.kturi(uuid.v4(), timeFn()));
      });
      const expectFn = (count, distance, errorPercentage) => {
        expect(Math.abs(distance)).to.be.below(errorPercentage);
      };
