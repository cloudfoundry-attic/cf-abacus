...
describe('abacus-dataflow', () => {
        ...
        (accum, ival, i, l, cb) => {
          ...
          request.post(
            ...
            (err, pval) => {
              ...
              expect(pval.statusCode).to.be.oneOf([422, 201]);
  ...
  describe('dataflow reducer', function() {
    ...
    context('on business error', () => {
      ...
      const validatePost = (error, statusCode, cb) => request.post(
        'http://localhost::p/v1/nbs',
        {
          p: server.address().port,
          auth: {
        ...
        (err, pval) => {
          expect(pval.body).to.deep.equal(error);
          expect(pval.statusCode).to.equal(statusCode);
          cb();
        }
      );
      ...
      it('returns correct status code when timeout error occurs', (done) => {
        ...
        validatePost(timeoutError, 418, done);
      });
      ...
      it('returns correct status code when plan bussines error occurs', (done) => {
        ...
        validatePost(planError, 422, done);
      });
    });
