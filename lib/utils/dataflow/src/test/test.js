...
describe('abacus-dataflow', () => {
  ...
  beforeEach((done) => {
    ...
    dbclient.drop(process.env.DB_URI, /^abacus-dataflow-/, done);
  });
  ...
  describe('db', () => {
    context('in CF environment', () => {
      const backupDB = process.env.DB_URI;
      const backupVcapEnvApp = vcapenv.app;
      ...
      after(() => {
        process.env.DB_URI = backupDB;
        vcapenv.app = backupVcapEnvApp;
      });

      it('fails creating db', () => {
        const dbFn = () => db('name', () => {}, undefined);
        expect(dbFn).to.throw(/DBALIAS|DB/);
      });
    });
  });
});
