...
describe('abacus-dataflow', () => {
  ...
  describe('dataflow sink', () => {
    ...
    const distribute = (keys, done) => {
      const counts = {};

      const idealDistKeys = keys.length / 6;
      const errorPercentage = 25;

      map(keys, (k) => {
        yieldable.functioncb(dataflow.sink)(k, remotesink, 6, (err, uri) => {
          expect(err).not.to.ok;
