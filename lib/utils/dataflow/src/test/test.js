...
describe('abacus-dataflow', () => {
  ...
  describe('dataflow mapper', function() {
    ...
    it('save to output db when it gets duplicate (409)', (done) => {
      ...
      treduce([1, 2, 3], (accum, ival, i, l, cb) => {
        ...
        dataflow.replay(mapper, 1000, (err, vals) => {
          ...
          expect(vals.replayed.length).to.equal(0);
          expect(vals.failed.length).to.equal(1);
          done();
        });
      });
    });
    ...
    it('detects error when there is no sink', (done) => {
      ...
      treduce([6, 7, 7, 8], (accum, ival, i, l, cb) => {
        ...
        dataflow.replay(mapper, 3600000, (err, vals) => {
          ...
          expect(vals.replayed.length).to.equal(3);
          expect(vals.failed.length).to.equal(0);
    ...
    it('propagates mapper error from the sink, does not store to output',
      (done) => {
        // Create a test Web app
        const app = webapp();
    ...
    context('with documents missing in the output db', () => {
      ...
      beforeEach((done) => {
        ...
        postspy = (reqs, cb) => {
          ...
          cb(undefined, [[undefined, outputDoc.x === 2 ? {
            statusCode: 401
          } : {
            statusCode: 201
          }]]);
        ...
        const logInput = yieldable.functioncb(dataflow.logInput);
        treduce(inputDocs, (accum, ival, i, l, cb) => {
      ...
      it('replays data', (done) => {
        ...
        dataflow.replay(mapper, 3600000, (err, docs) => {
          expect(err).to.equal(null, 'Unexpected error ' + err);
          expect(docs.replayed.length).to.equal(2);
          expect(docs.failed).to.deep.equal([inputDocs[1].id]);
        ...
        request.post('http://localhost::p/v1/nbs', {
          ...
          request.get(pval.headers.location, {}, (err, val) => {
            ...
            expect(val.body.id).to.match(
              new RegExp('t/[0-9].*-0-0-0/k/' + idoc.x));
