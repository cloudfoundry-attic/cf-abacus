...
const poll = (fn, done, timeout, interval) => {
  ...
  const doneCallback = (err) => {
    ...
    if (Date.now() - startTimestamp > timeout)
      ...
      setTimeout(() => fn(doneCallback), interval);
  };

  fn(doneCallback);
};

const eventually = (fn, done = () => {}, timeout = 1000, interval = 100) => {
  const fnWithCallback = (doneCallBack) => {
    try {
      fn();
      doneCallBack();
    }
  ...
  poll(fnWithCallback, done, timeout, interval);
};
            ...
            if (ival < 7) {
              ...
              eventually(() => {
                ...
                request.get('http://localhost::p/v1/enbs/t/:t/get/error', {
                  ...
                  // 6 is submitted twice, but there should only be one in db
                  // No duplicate error doc
                  expect(val.body.length).to.equal(1);
