...
describe('abacus-dataflow', () => {
  ...
  it('dataflow reducer cleans the cache when logoutputs fail', (done) => {
    // Mock the db module to returns db timeout.
    const odb = {
      batch_put: () => [[{ status: 502 }, undefined]]
    };

    const logOutputs = yieldable.functioncb(dataflow.logOutputs);
    ...
    const odocs = [{
      ...
      x: 5,
      val: 9,
      nb_id: 't/0001474089242044-0-0-0-0/k/5',
      id: 'k/odd/t/0001474088400000-0-0-0-0',
      processed_id: '0001474089242044-0-0-0-0',
      processed: 1474089242044,
      rev: '2-f21c67a5497105a5ce22c7913c8e5121'
    }, {
      nb_id: 't/0001474089242044-0-0-0-0/k/5',
      id: 'k/5/t/0000000000000000',
      processed_id: '0001474089242044-0-0-0-0',
      processed: 1474089242044
    }];
    ...
    map(odocs, (odoc) =>
      ocache.set(dbclient.kturi(dbclient.k(odoc.id), 0), odoc));

    logOutputs(odocs, odb, ocache, (err) => {
      expect(err).to.deep.equal({ status: 502 });
      map(odocs, (odoc) =>
        expect(ocache.get(dbclient.kturi(dbclient.k(odoc.id), 0)))
        .to.equal(undefined));
      done();
    });
  });
    ...
    it('runs a map transform and stores its inputs and outputs', (done) => {
      ...
      dataflow.replay(mapper, 1000, (err, vals) => {
        ...
        postspy = (reqs, cb) => {
          ...
          const val = reqs[0][1];
          expect(val.headers).to.include.keys('X-BATCH-ID');
          expect(val.headers.authorization).to.deep.equal(
