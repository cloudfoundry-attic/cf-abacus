...
const replayDoc = (odb, edb, routes) => function *(stats, doc, i, l) {
...
const replayAllDocs = (processingStats, routes) => (idocs, cb) => {
  yieldable.functioncb(function *() {
    ...
    if(err)
      ...
      debug('Replayed input docs %o', stats);
    cb(err, stats);
  });
};
...
const replay = (routes, twindow, cb) => {
  ...
  yieldable.functioncb(function *() {
    yield idb.readAllPages(pagingOptions, replayAllDocs(returnDocs, routes));
  })((err) => {
    cb(err, returnDocs);
  });
};
