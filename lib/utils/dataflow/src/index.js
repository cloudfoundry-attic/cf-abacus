...
const dupInput = (oid) => {
  ...
  throw extend(new Error('Document update conflict'), {
    id: oid,
    status: 409,
    error: 'conflict',
...
const mapper = (mapfn, opt) => {
  ...
  const ddup = dedupe(opt.input.slack, opt.input.dedupe);
  ...
  const play = function *(req, idoc) {
    ...
    const okeys = opt.output.keys(idoc);
    const otimes = map(
      opt.output.times(idoc), (t) => t === undefined ? itime : t);
  ...
  return yieldable(batch(batch.groupBy(function *(calls) {
    ...
    try {
      ...
      yield cacheAccum(
        ...
        last(calls)[0].okey[0], ocache, accum.crev, odb);
