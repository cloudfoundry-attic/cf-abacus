...
const logInput = function *(idoc, ikey, itime, utime, idb) {
  ...
  const uid = dbclient.tkuri(ikey, utime);
...
const mapper = (mapfn, opt) => {
  ...
  routes.post(opt.input.post, throttle(function *(req) {
    ...
    yieldable.functioncb(function *() {
      ...
      debug('Calling map function with input doc %o', idoc);
      const odocs = yield ymapfn(idoc, auth);
  ...
  return yieldable(batch(batch.groupBy(function *(calls) {
    ...
    try {
      ...
      const accum = yield lastAccum(
        calls[0][0].idoc, okey, calls[0][0].itime, ocache, odb);
  ...
  routes.post(opt.input.post, throttle(function *(req) {
    ...
    yieldable.functioncb(function *() {
      ...
      const oids = yield greduce({
        g: (opt.output.group || opt.output.key)(idoc),
        idoc: idoc,
        ilog: ilog,
        itime: itime,
        auth: auth
