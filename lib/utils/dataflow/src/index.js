...
const {
  each, extend, filter, find, last, map, memoize, object, omit, pairs, pick, reduce, rest, uniq, zip
} = require('underscore');
...
const detectDup = function*(oid, ddup, odb) {
  ...
  // Always check db because of the bloom filter bug(#453).
  if (ddoc === undefined || ddoc === false) {
    debug('May be a duplicate output doc %s', oid);
    if (odb) {
      const odoc = yield odb.get(oid);
      if (odoc !== undefined) throw dupError(oid);
    }
  } else if (ddoc === true) throw dupError(oid);
...
const cacheAccum = (itype, idoc, odoc, okey, ocache, odb) => {
  ...
  const ocdoc = extend({}, odoc, idoc.id ? object([[idname(itype), idoc.id]]) : {});

  odebug('Caching accumulated output doc %s, rev %s, cache id %s', odoc.id, odoc.rev, ocid);
  debug('Caching accumulated output doc %s, rev %s, cache id %s', odoc.id, odoc.rev, ocid);
...
const cacheAccums = (itype, idoc, odocs, okeys, ocache, odb) => {
  ...
  if (find(odocs, (odoc) => odoc.error)) return odocs;

  return map(odocs, (odoc, i, l) => cacheAccum(itype, idoc, odoc, okeys[i], ocache, odb));
};
...
const lastAccum = function*(idoc, okey, otime, ocache, odb) {
  ...
  if (cdoc) {
    odebug('Retrieved cached accumulated output doc id %s, rev %s', cdoc.id, cdoc.rev);
    debug('Retrieved cached accumulated output doc id %s, rev %s, %o', cdoc.id, cdoc.rev, cdoc);
...
const db = (dbname, dbh, dburi) => {
  ...
  return !dbname
    ? undefined
    : yieldable(throttle(retry(breaker(batch((dbh || dbhandle)(dburi || uris()[dbalias], dbname), 20, 100, dbcsize)))));
};
...
const errordb = (dbname, dbh) =>
  !dbname
    ? undefined
    : yieldable(throttle(retry(breaker(batch((dbh || edbhandle)(uris()[dbalias], dbname), 20, 100, dbcsize)))));
...
const buildPlayResponse = (req, opt, played) => {
  let statusCode = 201; // success
  if (played.error) {
    const err = played.error;
    statusCode = err.status ? err.status : err.statusCode ? err.statusCode : 422; // defaults to 422
  }
...
const mapper = (mapfn, opt) => {
  ...
  const ddup =
    opt.input.dedupe === false || opt.input.dedupe === 0 ? undefined : dedupe(opt.input.slack, opt.input.dedupe);
  ...
  const play = function*(idoc, auth) {
    ...
    try {
      ...
      debug('Calling map function with input doc %o', playedInputDoc);
      try {
        odocs = yield ymapfn(playedInputDoc, auth);
      } catch (err) {
        edebug('Error while executing mapper function for input doc [%o], error: %o', playedInputDoc, err);
        return {
          doc: playedInputDoc,
          error: err
        };
      ...
      debug('Processed input doc %s, produced output docs %o', playedInputDoc.id, map(podocs, (podoc) => podoc.id));

      error = checkError(podocs);

      // Post the output docs to the configured sink.
      if (opt.sink.host && opt.sink.posts && !error)
        error = yield postOutputs(
          podocs,
          skeys,
          stimes,
          opt.sink.host,
      ...
      if (odb && !error) yield logOutputs(podocs, odb);
...
const groupReduce = (itype, yreducefn, otype, ocache, odb, shost, spartition, sposts, ddup) => {
  return yieldable(
    batch(
      batch.groupBy(
        function*(calls) {
          ...
          try {
            ...
            const accums = yield tmap(zip(calls[0][0].okeys, calls[0][0].otimes), function*(kts, i, l) {
              if (!odb) return {};
              return yield lastAccum(calls[0][0].idoc, kts[0], kts[1], ocache, odb);
            });
            ...
            if (odb && !errors.length) {
              ...
              each(calls, (call, i, l) => {
                cacheAccums(itype, call[0].idoc, pgdocs[i], call[0].okeys, ocache, odb);
              });
            }
...
const replaydb = (dbname, dbh) =>
  !dbname ? undefined : throttle(retry(breaker(batch((dbh || idbhandle)(uris()[dbalias], dbname)))));
