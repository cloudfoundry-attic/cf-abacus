...
const dedupeInput = function *(okeys, otimes, ddup, odb) {
  if(!ddup || !odb)
    return;
...
const cacheAccum = (itype, ilog, odoc, okey, ocache, ocrev, odb) => {
  if(!odb)
    return odoc;

  const ocid = dbclient.kturi(okey, 0);
  const oclog = extend({},
    odoc, ilog.id ? object([[idname(itype), ilog.id]]) : {}, {
      id: ocid,
      _rev: ocrev
    });
  debug('Caching accumulated output doc %s', ocid);
  ocache.set(ocid, oclog);
...
const cacheAccums = (itype, ilog, odocs, okeys, ocache, ocrevs, odb) => {
  if(!odb)
    return odocs;
  const oclogs = map(odocs, (odoc, i, l) => {
    return cacheAccum(itype, ilog, odoc, okeys[i], ocache, ocrevs[i], odb);
  });
  return oclogs;
};
...
const postOutput = function *(
  olog, shost, spartition, spost, authentication) {
  if(!spost || !shost)
    return olog;
  ...
  try {
    ...
    if(res.statusCode !== 201)
      throw postError(olog.id, res);

    debug('Posted %s successfully to sink', olog.id);
    return olog;
  }
  catch(exc) {
    edebug('Exception posting %s to sink, %o', olog.id, exc);
    debug('Exception posting %s to sink, %o', olog.id, exc);
    throw exc;
  }
};

// Build a list of output docs
const buildOutputs = (itype, ilog, otype, odocs, okeys, otimes, now) => {
...
const postOutputs = function *(odocs,
  ...
  // Posts output docs to the configured sink service
...
const logOutputs = function *(odocs, ddup, odb) {
  if(odb)
    yield tmap(odocs.reverse(), function *(odoc, i, l) {
      // Log each doc into the output database
      yield logOutput(odoc, odb);
    });

  if(ddup)
    map(odocs.reverse(), (odoc) => {
      // Add each doc id to duplicate filter
      ddup.add(odoc.id);
    });
};
...
const mapper = (mapfn, opt) => {
  ...
  const play = function *(req, idoc) {
    ...
    // Generate a processed id for the input doc
    const pidoc = extend(idoc, {
      processed_id: seqid.pad16(seqid()),
      processed: Date.now()
    });

    // Check for duplicate input doc
    yield dedupeInput(okeys, otimes, ddup, odb);

    // Log the input doc
    const ilog = yield logInput(pidoc, ikey, itime, idb);
    ...
    const podocs = buildOutputs(
      ...
      now);
    const poids = map(podocs, (podoc) => podoc.id);
    debug('Processed input doc %s, produced output docs %o', ilog.id, poids);
    ...
    // Log the output docs
    yield logOutputs(podocs, ddup, odb);
  ...
  return yieldable(batch(batch.groupBy(function *(calls) {
    ...
    try {
      ...
      const accums = yield tmap(
        zip(calls[0][0].okeys, calls[0][0].otimes),
        function *(kts, i, l) {
          return yield lastAccum(calls[0][0].ilog,
            kts[0], kts[1], ocache, odb);
        });

      // Apply the reduction function to each input doc in the batch
      const idocs = map(calls, (call) => call[0].ilog);
      debug(
        'Calling group reduce function with accums %o and input docs %o',
        accums, idocs);

      const gdocs = yield yreducefn(
        map(accums, (a) => a.doc), idocs, calls[0][0].authorization);
      ...
      const gres = yield tmap(pgdocs, function *(podocs, i, l) {
        ...
        const poids = map(podocs, (podoc) => podoc.id);
        return [undefined, poids];
      });

      // Cache the last accumulated output docs
      cacheAccums(
        itype, last(calls)[0].ilog, last(pgdocs),
        last(calls)[0].okeys,
        ocache, map(accums, (a) => a.crev),
        odb);
...
const reducer = (reducefn, opt) => {
  ...
  const greduce = groupReduce(
    opt.input.type,
    yreducefn,
    opt.output.type, ocache, ddup, odb,
    opt.sink.host, opt.sink.apps, opt.sink.post);
...
const replay = (routes, twindow, cb) => {
  ...
  yieldable.functioncb(function *() {
    ...
    const t = Date.now();
    const sid = dbclient.tkuri('', seqid.pad16(t - tw));
    const eid = dbclient.tkuri('', seqid.pad16(t) + 'ZZZ');
    ...
    const rlogs = yield treduce(ilogs.rows, function *(rlogs, row, i, l) {
      const ilog = row.doc;

      // Look for the ouput doc corresponding to an input doc
      debug('Checking output doc for input %s', ilog.id);
      const ikey = dbclient.k(ilog.id);
      const itime = dbclient.t(ilog.id);
      const okeys = opt.output.keys(ilog, ikey);
      const otimes = opt.output.times(ilog, itime);
      ...
      const rlog = yield routes.play(undefined,
          omit(dbclient.undbify(ilog), 'id', 'processed'));
    ...
    if(err)
      ...
      debug('Replayed input docs %o', rlogs);
    cb(err, rlogs);
  });
};
