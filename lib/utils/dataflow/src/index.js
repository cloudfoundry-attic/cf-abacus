...
const logOutput = function *(itype, ilog, odoc, okey, otime, odb) {
  if(!odb)
    return odoc;
  const oid = dbclient.kturi(okey(odoc), otime(odoc));
  const olog = extend({},
...
// Log an accumulated output doc
const logAccum = function *(
  itype, ilog, odoc, okey, otime, ocache, ocrev, odb) {
  if(!odb)
    return odoc;

  // Store the doc in the output db
  const oid = dbclient.kturi(okey(ilog), otime(ilog));
  const olog = extend({},
    ilog.id ? object([[idname(itype), ilog.id]]) : {},
    odoc, {
      id: oid
  ...
  yield odb.put(olog);
  debug('Logged accumulated output doc %o', olog);

  // Cache it out as well
...
const lastAccum = function *(ilog, okey, otime, ocache, odb) {
  ...
  const cdoc = yield ocache.get(dbclient.kturi(okey(ilog), 0));
  if(cdoc) {
...
const logOutputs = function *(
  ...
  const oids = yield tmap(odocs, function *(odoc, i, l) {
    // Log each output doc
    const olog = i === 0 && ocache ?
      yield logAccum(itype, ilog, odoc, okey, otime, ocache, ocrev, odb) :
      yield logOutput(itype, ilog, odoc, okey, otime, odb);
  ...
  return yieldable(batch(batch.groupBy(function *(calls) {
    ...
    try {
      ...
      const gdocs = yield yreducefn(
        accum.doc, map(calls, (call) => call[0].idoc), calls[0][0].auth);
      debug('Output docs from group reduction %o', gdocs);

      // Log the output docs and build the array of results for
      // the group
      const gres = yield treduce(gdocs, function *(gres, odocs, i, l) {
        const oids = yield logOutputs(
          ...
          shost, spartition, spost,
          calls[i][0].auth);
        return gres.concat([[undefined, oids]]);
      }, []);
...
const reducer = (reducefn, opt) => {
  ...
  routes.post(opt.input.post, throttle(function *(req) {
    ...
    // Forward authorization header field to reducefn and log
    const auth = req.headers && req.headers.authorization ?
      req.headers.authorization : undefined;
