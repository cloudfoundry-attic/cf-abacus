...
// Throttle the number of request in a batch.
const brequest = yieldable(retry(batch(throttle(breaker(request))), retries));
...
const detectDup = function *(oid, ddup, odb) {
  ...
  // Only use the lru of the dedup. Since we're not serializing and
  // deserializing the bloom filter, when the app restart, bloom filter would
  // give incorrect value
  if(!ddoc) {
...
const logInput = function *(idoc, idb) {
  ...
  // we might recieve a 409 when datbase write is successful the previous call
  try {
    yield idb.put(idoc);
  }
  catch(e) {
...
const logOutputs = function *(odocs, odb, ocache) {
  const ids = map(odocs.concat([]).reverse(), (doc) => pick(doc, 'id', 'rev'));
  odebug('Logging output docs %o %o', ids, odocs);
  debug('Logging output docs %o %o', ids, odocs);

  // Find unique docs by id, as we only want the last version of each doc
  const wdocs = uniq(odocs.concat([]).reverse(), (doc) => doc.id);
  map(wdocs, (w) => {
    w._id = w.id;
    if(w.rev)
      w._rev = w.rev;
  ...
  const wids = map(wdocs.concat([]), (doc) => pick(doc, 'id', 'rev'));
  odebug('Logging unique output docs %o %o', wids, wdocs);
  debug('Logging unique output docs %o %o', wids, wdocs);

  // Log all output docs in one batch put
  try {
    const results = yield odb.batch_put(map(wdocs,(w) => [w]));
    debug('Logging results %o for %o', results, wids);
    let errors = false;
    map(results, (r, i) => {
      if(r[0]) {
        errors = true;
        odebug('Error logging output doc %s, rev %s, %o, %o',
          wdocs[i].id, wdocs[i].rev, r[0], wdocs[i]);
        debug('Error logging output doc %s, rev %s, %o',
          wdocs[i].id, wdocs[i].rev, r[0]);
      }
      else if(r[1]) {
        wdocs[i].rev = r[1].rev;
        odebug('Logged new output doc %s, rev %s, %o', wdocs[i].id, 
          wdocs[i].rev, wdocs[i]);
        debug('Logged new output doc %s, rev %s', wdocs[i].id, wdocs[i].rev);
      }
    });
    if(errors)
      map(results, (r) => {
    ...
    // Dataflow batch put to the db failed. Clear the cache, so that the next
    // accumulated doc would be from the db.
    if(ocache)
      map(ids, (id) => {
        const docId = dbclient.kturi(dbclient.k(id.id), 0);
        ocache.del(docId);
      });

    odebug('Error logging output docs %o %o %o', wids, e, wdocs);
    debug('Error logging output docs %o %o %o', wids, e, wdocs);
    throw e;
  }
};
...
const idbpartition = (n) => partition.partitioner(
  partition.bucket, partition.period,
  iforward(n), partition.balance);

// Return a handle to the error db
const edbhandle = (dbserver, name) => dbclient(
  idbpartition(0), dbclient.dburi(dbserver, name));
...
const mapper = (mapfn, opt) => {
  ...
  routes.post(opt.input.post, function *(req) {
  ...
  if(opt.input.get)
    routes.get(opt.input.get, function *(req) {
      const ks = map(filter(
  ...
  if(opt.output.get)
    routes.get(opt.output.get, function *(req) {
      const ks = map(filter(
        pairs(req.params), (p) => /^k/.test(p[0])), (p) => p[1]).join('/');
      const ts = map(filter(
        pairs(req.params), (p) => /^t/.test(p[0])), (p) => p[1]).join('/');
  ...
  if(opt.error && opt.error.get)
    routes.get(opt.error.get, function *(req) {
  ...
  if(opt.error && opt.error.delete)
    routes.delete(opt.error.delete, function *(req) {
      const ks = map(filter(
        pairs(req.params), (p) => /^k/.test(p[0])), (p) => p[1]).join('/');
      const ts = map(filter(
        pairs(req.params), (p) => /^t/.test(p[0])), (p) => p[1]).join('/');
  ...
  return yieldable(batch(batch.groupBy(function *(calls) {
    ...
    try {
      ...
      if(odb && !errors.length) {
        ...
        yield logOutputs(allpodocs, odb, ocache);
...
const idbhandle = (dbserver, name) => dbclient(
  idbpartition(parseInt(vcapenv.appindex())), dbclient.dburi(dbserver, name));
...
module.exports.logOutputs = logOutputs;
