  ...
  if(opt.error && opt.error.delete)
    routes.delete(opt.error.delete, throttle(function *(req) {
      ...
      if (req.headers && req.headers.authorization) {
        ...
        authenticatedUser =
          oauth.getUserInfo(authHeader);
      }
      // Get doc from edb
      const id = dbclient.tkuri(ks, ts);
      debug('Retrieving error doc with id %s by client %o',
      ...
      console.log('Removing error doc %j by client %j ',
        edoc, authenticatedUser);
      const status = yield edb.remove(edoc);
      console.log('Delete status %j by client %j',
