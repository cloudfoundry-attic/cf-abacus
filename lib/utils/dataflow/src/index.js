...
const lastMonth = (t) => moment.utc(parseInt(t))
    .subtract(1, 'months').startOf('month').valueOf();
...
const mapper = (mapfn, opt) => {
  ...
  const play = function *(idoc, auth) {
    ...
    const playedInputDoc = extend({}, idoc, {
    ...
    try {
      ...
      // Log the input doc
      if(idb)
        yield logInput(playedInputDoc, idb);
  ...
  if(opt.error && opt.error.delete)
    routes.delete(opt.error.delete, throttle(function *(req) {
      ...
      console.log('[audit] Removing error doc %j by client %j ', edoc, client);
      const status = yield edb.remove(edoc);
      console.log('[audit] Delete status %j by client %j', status, client);
...
const groupReduce = (itype,
  ...
  return yieldable(batch(batch.groupBy(function *(calls) {
    ...
    try {
      ...
      const ologs = yield treduce(idocs, function *(log, udoc, i, l) {
...
const reducer = (reducefn, opt) => {
  ...
  if(opt.error && opt.error.delete)
    routes.delete(opt.error.delete, throttle(function *(req) {
      ...
      console.log('[audit] Removing error doc %j by client %j ...', edoc,
        client);
      const status = yield edb.remove(edoc);
      console.log('[audit] Delete status %j by client %j', status, client);
...
const replay = (routes, twindow, cb) => {
  ...
  yieldable.functioncb(function *() {
    ...
    const odb = replaydb(odbName, opt.output.dbhandle);
    ...
    debug('Using databases input %s, output %s and error %s',
      idbName, odbName, edbName);

    // Retrieve the last input docs from the input db using a time
    // range db query back to the configured replay time
    const now = moment.now();
    const sid = ['t', seqid.pad16(now - tw)].join('/');
    const eid = ['t', seqid.pad16(now)].join('/');
    ...
    const rdocs = yield treduce(idocs.rows, function *(rdocs, row, i, l) {
      const idoc = dbclient.undbify(row.doc);
      ...
      try {
        debug('No existing output doc for input %s, replaying %o',
          idoc.id, idoc);
        const played = yield routes.play(omit(idoc, 'id', 'processed'),
        ...
        rdocs.replayed = rdocs.replayed.concat([rdoc]);
        return rdocs;
      }
      catch (e) {
        debug('Failed to replay input doc %s', idoc.id);
        rdocs.failed = rdocs.failed.concat([idoc.id]);
        return rdocs;
      }
