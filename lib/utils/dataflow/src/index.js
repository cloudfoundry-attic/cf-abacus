...
const logOutputs = function *(
  itype, ilog,
  otype, odocs, okeys, otimes,
  odb,
  shost, spartition, spost,
  authentication) {

  // Posts output docs to the configured sink service
  const ologs = yield tmap(odocs, function *(odoc, i, l) {
    ...
    return yield postOutput(olog, shost, spartition, spost, authentication);
  });

  // Log output docs to the output database
  const oids = yield treduce(ologs.reverse(), function *(oids, olog, i, l) {

    // Log accumulated output doc
    yield logOutput(olog, odb);
    return oids.concat([olog.id]);
  }, []);
...
const mapper = (mapfn, opt) => {
  ...
  const play = function *(req, idoc) {
    ...
    const ilog = yield logInput(
      pidoc, ikey, itime, idb, okeys, otimes, ddup, odb);
    ...
    try {
      const oids = yield logOutputs(
        opt.input.type, ilog,
        opt.output.type, podocs, okeys, otimes,
        odb,
        opt.sink.host, opt.sink.apps, opt.sink.post,
        opt.sink.authentication);
      debug('Processed input doc %s, produced outputs %o', ilog.id, oids);
    }
    catch(err) {
...
const groupReduce = (
  ...
  return yieldable(batch(batch.groupBy(function *(calls) {
    ...
    try {
      ...
      // Generate sequential processed ids for the output docs
      const now = Date.now();
      const pgdocs = map(gdocs, (odocs, i, l) => {
        return map(odocs, (odoc) => {
          return extend(odoc, {
            processed_id: seqid.pad16(seqid()),
            processed: now
          });
      ...
      // the group
      const gres = yield tmap(pgdocs, function *(odocs, i, l) {
        const oids = yield logOutputs(
          itype, calls[i][0].ilog,
          otype, odocs, calls[i][0].okeys, calls[i][0].otimes,
          odb,
          shost, spartition, spost,
      ...
      // Cache the last accumulated outputs
      cacheAccums(
