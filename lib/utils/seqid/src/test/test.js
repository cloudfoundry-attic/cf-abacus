  ...
  describe('seqid', () => {
    ...
    context('with timestamp', () => {
      it('uses the timestamp for sequential ids', () => {
        expect(seqid(1537596306000)).to.match(/1537596306000-\d{16}-0-0-0-0/);
        expect(seqid(1537596306000)).to.match(/1537596306000-\d{16}-0-0-0-1/);
      });
      ...
      it('throws on too short timestamp', () => {
        const sequenceFn = () => seqid('123');
        expect(sequenceFn).to.throw('Not a timestamp');
      });
