  ...
  const slackBasedWindows = (dimension) => {
    if(slack.scale && slackscale[slack.scale][dimension])
      return map(Array(Math.ceil(
        1 / slackscale[slack.scale][dimension] * slack.width) + 1), () => null);
    return [null];
  };
...
const cellfn = (timeWindow, processed, usageTime) => {
  return (dimension) => {
    const index = timeWindowIndex(timeWindow[dimensions.indexOf(dimension)],
      moment.utc(processed).toDate(), moment.utc(usageTime).toDate(),
