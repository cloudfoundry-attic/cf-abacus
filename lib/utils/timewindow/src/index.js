...
const { each, map, rest } = require('underscore');
...
const shiftWindow = (before, after, twindow, dim) => {
  each(Array(Math.max(0, Math.min(twindow.length, diff(before, after, dim)))), () => {
...
const shift = (updateDoc, doc, end) => {
  if (doc && doc.processed && end - doc.processed >= 0) {
