...
describe('abacus-timewindow', () => {
  ...
  it('adjusts the time window', () => {
    const timewindow = [[null], [null], [null], [5, 4, 3, 2, 1],
      ['Now', 'Last']];
    const twCreation = Date.UTC(2016, 5, 5);

    // Expect the days to get cut down.
    let adjustTo = Date.UTC(2016, 5, 3);
    expect(time.adjustWindows(timewindow, twCreation, adjustTo)[3]).to.
      deep.equal([3, 2, 1]);
    expect(time.adjustWindows(timewindow, twCreation, adjustTo)[4]).to.
      deep.equal(['Now', 'Last']);

    // Expect the days to get cut down.
    adjustTo = Date.UTC(2016, 5, 4);
    expect(time.adjustWindows(timewindow, twCreation, adjustTo)[3]).to.
      deep.equal([4, 3, 2, 1]);
    expect(time.adjustWindows(timewindow, twCreation, adjustTo)[4]).to.
      deep.equal(['Now', 'Last']);
    ...
    // Same day
    adjustTo = Date.UTC(2016, 5, 5);
    expect(time.adjustWindows(timewindow, twCreation, adjustTo)[3]).to.
      deep.equal([5, 4, 3, 2, 1]);
    expect(time.adjustWindows(timewindow, twCreation, adjustTo)[4]).to.
      deep.equal(['Now', 'Last']);

    // Date2 > date1
    adjustTo = Date.UTC(2016, 5, 6);
    expect(time.adjustWindows(timewindow, twCreation, adjustTo)[3]).to.
      deep.equal([5, 4, 3, 2, 1]);
    expect(time.adjustWindows(timewindow, twCreation, adjustTo)[4]).to.
      deep.equal(['Now', 'Last']);
