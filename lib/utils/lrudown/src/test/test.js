...
describe('cf-abacus-lrudown', () => {
    ...
    it('implements the leveldown API', (done) => {
        const db = levelup('test-levelup', { db: (loc) => new lrudown(loc) });

        // Put some values in the db
        const put = (done) => {
            let cbs = 0;
            const cb = () => { if(++cbs === 2) done(); };

            db.put('hello', 'Hello', (err, doc) => cb(expect(err).to.equal(undefined)));
            db.put('hey', 'Hey', (err, doc) => cb(expect(err).to.equal(undefined)));
        };
    ...
    it('can be used as a leveldown db adapter', (done) => {
        const db = new PouchDB('test-store', { db: lrudown });
        ...
        const put = (done) => {
            ...
            db.put({ _id: 'hello', value: 'Hello' }, (err, doc) => cb(expect(err).to.equal(null)));
            db.put({ _id: 'hey', value: { greeting: 'Hey' }}, (err, doc) => cb(expect(err).to.equal(null)));
        };
        ...
        const get = (done) => {
            ...
            db.get('hello', (err, doc) => { hellodoc = doc; cb(expect(doc.value).to.equal('Hello')); });
            db.get('hey', (err, doc) => { heydoc = doc; cb(expect(doc.value.greeting).to.equal('Hey')); });
        };
    ...
    it('evicts cached entries after some time', (done) => {
        const db = new PouchDB('test-evict', { db: lrudown });
        ...
        const get = (done) => {
            ...
            // Expect to get the documents previously put into the db
            db.get('hello', (err, doc) => { cb(expect(doc.value).to.equal('Hello')); });
            db.get('hey', (err, doc) => { cb(expect(doc.value.greeting).to.equal('Hey')); });
        };
    ...
    it('implements db lifecycle functions', (done) => {
        const db = new PouchDB('test-lifecycle', { db: lrudown });
        ...
        const db2 = new PouchDB('test-lifecycle', { db: lrudown });
        const get = (done) => {
            let cbs = 0;
            const cb = () => { if(++cbs === 2) done(); };

            // Expect to get the documents previously put into the db
            db2.get('hello', (err, doc) => { cb(expect(doc.value).to.equal('Hello')); });
            db2.get('hey', (err, doc) => { cb(expect(doc.value.greeting).to.equal('Hey')); });
        };
        ...
        // handle
        const db3 = new PouchDB('test-lifecycle', { db: lrudown });
        const getagain = (done) => {
            let cbs = 0;
            const cb = () => { if(++cbs === 2) done(); };
