...
const store = (cache, key) => /\xffdocument-store\xff/.test(key) ? cache.docs :
  cache.meta;

// Convert a key to a string key we can use in an LRU cache
const toKey = (key) => typeof key === 'string' ?
...
LRUDown.prototype._batch = function(batch, options, cb) {
  ...
  const self = this;
  map(batch, (r) => r.type === 'put' ? self._put(r.key, r.value) : r.type ===
    'del' ? self._del(r.key) : undefined);
  process.nextTick(cb);
};
