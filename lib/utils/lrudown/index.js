...
// Setup debug log
const debug = require('cf-abacus-debug')('cf-abacus-lrudown');
...
const store = (cache, key) => /\xffdocument-store\xff/.test(key) ? cache.docs : cache.meta;

// Convert a key to a string key we can use in an LRU cache
const toKey = (key) => typeof key === 'string' ? '$' + key : JSON.stringify(key);
...
LRUDown.prototype._open = function(options, cb) {
    ...
    const opt = extend(clone({ max: 64 * 1024, length: (n) => n.length, dispose: (key, n) => {}, maxAge: 1000 * 60 * 5 }), options);
    this._cache = caches[this._loc] || (caches[this._loc] = { docs: lru(opt), meta: meta() });
...
LRUDown.prototype._batch = function(batch, options, cb) {
    ...
    const self = this;
    map(batch, (r) => r.type === 'put' ? self._put(r.key, r.value) : r.type === 'del' ? self._del(r.key) : undefined);
    process.nextTick(cb);
};
