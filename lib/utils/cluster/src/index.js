...
const setWorkers = (w) => {
  ...
  debug('Requesting cluster update to %d workers', w);
  emitter.emit('workers', w);
  return workers;
};

// Configure the cluster to run a single worker
const singleton = () => setWorkers(1);
...
const fork = () => {
  const w = workers ? workers :
    process.env.CLUSTER_WORKERS ? parseInt(process.env.CLUSTER_WORKERS) : 1;
  debug('Forking %d cluster workers', w);
  map(range(w), () => monitor(cluster.fork()));
};
...
const master = () => {
  ...
  process.title = 'node ' + (process.env.TITLE ||
    require(path.join(process.cwd(), 'package.json')).name) + ' master';
...
const worker = () => {
  ...
  process.title = 'node ' + (process.env.TITLE ||
    require(path.join(process.cwd(), 'package.json')).name) + ' worker';
...
module.exports.workers = setWorkers;
