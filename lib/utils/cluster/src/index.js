...
const calculateWorkers = (w) => {
  if (w) return w;
  else if (process.env.CLUSTER_WORKERS == 'calculate') return os.cpus().length;
  ...
  map(cluster.workers, (worker) => {
    ...
    if (worker.heartbeat < cutoff)
      if (worker.exitedAfterDisconnect) {
        ...
        edebug("Cluster worker %d didn't report heartbeat, disconnecting it", worker.process.pid);
