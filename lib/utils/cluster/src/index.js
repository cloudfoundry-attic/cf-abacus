...
const heartMonitor = () => {
  ...
  map(cluster.workers, (worker) => {
    ...
    if (worker.heartbeat < cutoff) {
      debug(
        'Cluster worker %d didn\'t report heartbeat, disconnecting it',
        worker.process.pid);
      worker.disconnect();
    }
  });
...
const master = () => {
  ...
  process.title = 'node ' + (process.env.TITLE || require(path.join(
    process.cwd(), 'package.json')).name) + ' master';
  ...
  cluster.on('exit', (worker, code, signal) => {
    debug('Cluster worker %d exited with code %d', worker.process.pid,
      code);
  });
  ...
  emitter.on('message', (msg) => {
    if (msg.server) {
      ...
      if (msg.server.listening) debug('Cluster worker %d listening on %d',
...
const worker = () => {
  ...
  process.title = 'node ' + (process.env.TITLE || require(path.join(
    process.cwd(), 'package.json')).name) + ' worker';
