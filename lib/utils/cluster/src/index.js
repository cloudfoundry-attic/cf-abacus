...
const calculateWorkers = (w) => {
  if (w) 
    return w;
  else if (process.env.CLUSTER_WORKERS == 'calculate')
  ...
  map(cluster.workers, (worker) => {
    ...
    if(worker.heartbeat < cutoff)
      if (worker.exitedAfterDisconnect) {
        edebug('Ignoring report heartbeat for disconnected cluster worker %d',
          worker.process.pid);
        debug('Ignoring report heartbeat for disconnected cluster worker %d',
          worker.process.pid);
      }
      else {
