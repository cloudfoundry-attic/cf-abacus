...
describe('abacus-cluster', () => {
    ...
    it('sets up and monitors a Node cluster master', () => {
        ...
        expect(clust.workers[0].send.args).to.deep.equal([[{ master: process.pid, x: 1 }]]);
    ...
    it('forks a default of 2 workers', () => {
        const listen = spy();
        process.exit = spy();
        ...
        // Expect 2 workers
        expect(clust.workers.length).to.equal(2);
    });
    ...
    it('forks a configured number of workers', () => {
        ...
        // Configure the cluster to use 3 cpus
        ...
        // Expect 3 workers to have been forked
