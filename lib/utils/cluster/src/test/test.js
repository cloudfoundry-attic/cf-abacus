...
describe('abacus-cluster', () => {
  ...
  context('calculate workers', () => {
    process.exit = spy(process, 'exit');
    const listen = spy();
    let server;
    ...
    it('forks a configured number of workers', () => {
      // Configure the cluster to use 3 workers
      process.env.CLUSTER_WORKERS = 'calculate';
  
      cluster.scale(undefined);
      server.listen();
  
      // Expect number of CPUs as number of workers
      const numCPUs = os.cpus().length;
      expect(cluster.size()).to.equal(numCPUs);
      expect(clust.workers.length).to.equal(numCPUs);
    });
    ...
    it('scales with number of CPUs provided to scale', () => {
      const expectedWorkers = 2;
  
      // Configure the cluster to use 3 workers
      process.env.CLUSTER_WORKERS = 'calculate';
  
      cluster.scale(expectedWorkers);
  
      server.listen();
  
      // Expect number of CPUs as provided to scale
      expect(cluster.size()).to.equal(expectedWorkers);
      expect(clust.workers.length).to.equal(expectedWorkers);
    });
  });
