...
// Setup debug log
const debuglog = require('cf-abacus-debug');
const debug = debuglog('cf-abacus-cluster');
const xdebug = debuglog('@x-cf-abacus/cluster');
...
const emitter = events.emitter('cf-abacus-cluster/emitter');
const on = (e, l) => {
    emitter.on(e, l);
};
...
// Fork one worker per CPU, in a local test environment limit to 2 workers
const fork = () => {
    map(usedCPUs ? usedCPUs : process.env.VCAP_APP_HOST ? os.cpus() : first(os.cpus(), 2), (cpu) => monitor(cluster.fork()));
};
