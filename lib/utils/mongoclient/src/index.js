...
const listDatabases = (server, db, cb) => {
  ...
  db.admin().listDatabases((err, dbs) => {
    if (err) {
      edebug('Failed to list databases on %s because of %o',
        purify(server), err);
      return cb(err);
    }
...
const listCollections = (server, db, cb) => {
  ...
  db.listCollections().toArray((err, collections) => {
    if (err) {
      edebug('Failed to list collections on %s because of %o',
        purify(server), err);
      return cb(err);
    }
...
const findServer = (servers, partition, cb) => {
  ...
  transform.filter(servers, (server, i, servers, cb) => {
    ...
    if(server)
      debug('Partition %s found on server %s', partition, purify(server));
    else
...
const dburi = (server, name) => {
  ...
  const composeDBUri = (server, partition, cb) => {
    ...
    cb(null, queryOpts ? [path, partition.join('-')].join('-') + queryOpts :
...
const mongoclient = (partitionFn, uriFn, connectionFn, errorDBFn) => {
  ...
  return extend(dbClient, {
    readAllPages: (opts, processingFn, cb) =>
      dbops.readAllPages(dbClient, opts, processingFn, cb)
  });
};
