...
const batchOp = (op, rw, docs, opt, part, pool, cb) => {
  ...
  // Dedupe the documents in case or read operation
  let documents = docs;
  if (rw === 'read') {
    const ids = map(docs, (doc) => doc._id);
    const uniqueIds = uniq(ids);
    documents = map(uniqueIds, (id) => ({ _id: id }));
    if (documents.length !== docs.length)
      debug('De-duped documents to %o', documents);
  }

  // Build a map of requested docs to target dbs
  transform.map(documents, (doc, i, docs, mcb) => {
    ...
    transform.map(groups, (gmaps, i, groups, mcb) => {
      ...
      if (mergedRows.length === docs.length)
        return lcb(null, mergedRows);

      // Duplicate results
      debug('Duplicating %d db op results', docs.length - mergedRows.length);
      const duplicatedRows = map(docs, (doc) =>
        find(mergedRows, (row) => row.doc._id === doc._id)
      );
      debug('Duplicated results %o', duplicatedRows);
      lcb(null, duplicatedRows);
    });
  });
};
...
const dbclient = (part, uri, cons) => {
  ...
  return {
    ...
    put: (doc, cb) => {
      ...
      singleOp((db, doc, cb) => {
        const collection = db.collection(getCollectionName(db));
        if (db.dbconsOptions && db.dbconsOptions.db)
          // Mongo 2.x cannot handle upsert with the same _id
          // claiming that we "change" it. Therefore we need additional
          // db call to check if we need insert or update. Newest 3.x can
          // just do upsert instead.
          collection.find({ _id: doc._id }).limit(1).next((err, res) => {
            if (err)
              cb(error(err), undefined);
            if (res)
              return collection.updateOne({ _id: doc._id },
                { $set: undbify(doc) }, { upsert: true },
                (err, val) => cb(error(err), doc));
          });
    ...
    remove: (doc, cb) => {
      ...
      singleOp((db, doc, cb) => {
        const collection = db.collection(getCollectionName(db));
        collection.deleteOne({ _id: doc._id, _rev: doc._rev },
          (err, val) => cb(error(err), val));
      }, 'write', dbify(doc), dbopt.partition, pool, (err, val) => {
      ...
      if(opt.startkey && opt.endkey)
        rangeOp((db, docs, opt, cb) => {
          ...
          collection.find({ _id: query },
            ...
            return cb(null, map(res, (result) => ({ doc: result })));
          });
        }, 'read', [{
          _id: opt.startkey
        }, {
    ...
    bulkDocs: (docs, opt, cb) => {
      ...
      bulkOp((db, docs, opt, cb) => {
        ...
        bulk.execute((err, res) => err ? cb(error(err)) : cb(null, docs));
      }, 'write', map(docs, dbify),
      ...
      batchOp((db, docs, opt, cb) => {
        ...
        if (db.dbconsOptions && db.dbconsOptions.db) {
          ...
          each(docs, (doc) => {
            ...
            collection.find({ _id: doc._id }).limit(1).next((err, res) => {
              if (err)
                cb(error(err), undefined);
              if (res)
                bulk.find({ _id: doc._id }).upsert().updateOne(undbify(doc));
              else
                bulk.insertOne(doc);
            });
          });
          ...
          collection.insertMany(docs, opt,
            (err, res) => err ? cb(error(err)) : cb(null, docs));
    ...
    batch_remove: (batch, cb) => {
      ...
      batchOp((db, docs, opt, cb) => {
        ...
        collection.deleteMany({
          _id: { $in: pluck(docs, '_id') }
        }, opt, (err, res) => err ? cb(error(err)) : cb(null, docs));
      }, 'write', map(batch, (args) => dbify(args[0], {
        _deleted: true
      })), {}, dbopt.partition, pool,
        (err, rows) => {
