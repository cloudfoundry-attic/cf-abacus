...
const removeCollectionFromUrl = (uri) => {
  ...
  // Mutate parsed URL
  const removed = parts.splice(2, parts.length - 2);
  debug('Removed %s from db uri %s', removed, uri);

  // Mutate the pathname intentionally
...
const getCollectionName = (db) => {
  ...
  // in the form /dbname
  if (parts.length < 3) {
    debug('Using documents collection for uri %s', puri(uri));
    return 'documents';
  }
  // Return the specified collection with path /dbname/collection
  debug('Using %s collection for uri %s', parts[2], puri(uri));
  return parts[2];
};
...
const mongoclient = (part, uri, cons) => {
  ...
  return {
    ...
    batch_get: extend((batch, cb) => {
      ...
      fname: 'mongoclient.get'
    }),

    // Batch version of put
    batch_put: extend((batch, cb) => {
      ...
      fname: 'mongoclient.put'
    }),

    // Batch version of remove
    batch_remove: extend((batch, cb) => {
