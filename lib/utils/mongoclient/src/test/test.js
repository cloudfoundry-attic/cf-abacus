...
describe('abacus-mongoclient', () => {
  ...
  const helloAgainKey = dbclient.kturi('Hello', moment.utc(
    '2014-11-06 19:07:54').valueOf());
  const heyKey = dbclient.tkuri('Hey', moment.utc(
    '2014-10-06 19:06:54').valueOf());
  const blahKey = dbclient.kturi('Blah', moment.utc(
    '2014-10-06 19:06:54').valueOf());
  const awwwwKey = dbclient.kturi('Awwww', moment.utc(
    '2014-11-06 19:06:54').valueOf());
  ...
  it('distributes bulk operations over 4 db partitions', (done) => {
    ...
    const getrangeerr = (done) => {
      ...
      db.allDocs({
        include_docs: true,
        startkey: dbclient.kturi('Blah', moment.utc(
          '2014-10-05 19:06:54').valueOf()),
        endkey: dbclient.kturi('Blah', moment.utc(
  ...
  it('gets proper collection name', () => {
    ...
    // simplest case
    expect(dbclient.getCollectionName({
      dbconsUrl: 'mongodb://10.11.25.240:27017/db'
    })).to.equal(defaultCollectionName);

    // credentials and no collection
    expect(dbclient.getCollectionName({
      dbconsUrl: 'mongodb://user:password@10.11.25.240:27017/db'
    })).to.equal(defaultCollectionName);

    // request parameters and no collection
    expect(dbclient.getCollectionName({
      dbconsUrl: 'mongodb://user:password@10.11.25.240:27017/db?ssl=true'
    })).to.equal(defaultCollectionName);

    // credentials and collection
    expect(dbclient.getCollectionName({
      dbconsUrl: 'mongodb://user:password@10.11.25.240:27017/' +
      'db/' + collectionName
    ...
    // replica set
    expect(dbclient.getCollectionName({
      dbconsUrl: 'mongodb://user:password@' +
      '10.11.25.240:27017,' +
      '10.11.25.241:27017,' +
      '10.11.25.242:27017' +
      '/db/' + collectionName
  ...
  context('when DBs are dropped', () => {
    ...
    // sure we are starting clean
    const clean = (db, done) => {
      debug('drop: starting clean ...');
      let cbs = 0;
      const cb = () => {
        if(++cbs === 3) {
          debug('drop: clean finished');
          done();
        }
      };
    ...
    // Drop and reuse the database
    const dropAndResue = (db, done) => {
      debug('drop: starting dropAndResue ...');
      dbclient.drop(dbserver(), /^abacus-mongoclient-/, (err) => {
        expect(err).to.equal(undefined);
        db.resetPool();
        debug('drop: dropAndResue finished');
        done();
      });
    };
    ...
    // Drop the database
    const drop = (db, done) => {
      debug('drop: starting drop ...');
      dbclient.drop(dbserver(), /^abacus-mongoclient-/, (err) => {
        expect(err).to.equal(undefined);
        debug('drop: drop finished');
        done();
      });
    };

    // Verify document content
    const missingCheck = (db, done) => {
      debug('drop: starting check for missing doc ...');
      let cbs = 0;
      const cb = (err, doc) => {
        expect(err).to.equal(null);
        expect(doc).to.equal(undefined);
        if (++cbs === 3) {
          debug('drop: missing check finished');
          done();
        }
      };
    ...
    it('can reuse the DB', (done) => {
      const db = batch(dbclient(undefined,
        dbclient.dburi(dbserver(), 'abacus-mongoclient-test-drop')));

      clean(db, () => put(db, () => get(db, () => dropAndResue(db, () =>
        missingCheck(db, done)))));
    });

    it('can reuse the DB without removing partition pooling', (done) => {
      const db = batch(dbclient(undefined,
        dbclient.dburi(dbserver(), 'abacus-mongoclient-test-drop')));

      clean(db, () => put(db, () => get(db, () => drop(db, () =>
        missingCheck(db, done)))));
    });

    it('can work with new DB', (done) => {
      const db = batch(dbclient(undefined,
        dbclient.dburi(dbserver(), 'abacus-mongoclient-test-drop')));

      clean(db, () => put(db, () => get(db, () => drop(db, () => {
        const newdb = batch(dbclient(undefined,
          dbclient.dburi(dbserver(), 'abacus-mongoclient-test-drop')));
        missingCheck(newdb, done);
      }))));
    });
  });
