  ...
  it('calls back with the documents', (done) => {
    ...
    const remove = (db, done) => {
      ...
      db.remove(hellodoc, (err, doc) => {
        expect(err).to.equal(null);
        cb(expect(doc).to.deep.equal(extend(hellodoc, { _deleted: true })));
      });
      db.remove(hello2doc, (err, doc) => {
        expect(err).to.equal(null);
        cb(expect(doc).to.deep.equal(extend(hello2doc, { _deleted: true })));
      });
      db.remove(heydoc, (err, doc) => {
        expect(err).to.equal(null);
        cb(expect(doc).to.deep.equal(extend(heydoc, { _deleted: true })));
      });
    };
