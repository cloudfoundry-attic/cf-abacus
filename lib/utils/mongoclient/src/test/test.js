...
describe('abacus-mongoclient', () => {
  ...
  it('reads custom Mongo options from environment', (done) => {
    ...
    const checkOptions = (err, db) => {
      ...
      expect(db.dbconsOptions).to.deep.equal(
        // check if we added the "sslValidate" internal default value
        extend(JSON.parse(process.env.MONGO_OPTS), { sslValidate: false })
      );
  ...
  it('supports replica set DB URIs', () => {
    ...
    // Verify that mongodb.connect will NOT use collections
    ...
    verifyCollectionRemove(replicaSetCollectionFunc, 'mongodb://' + replicaSet +
      '/dbname?replicaSet=foo&ssl=true');
  });
  ...
  it('gets documents in the exact order', (done) => {
    ...
    const numberOfDocuments = process.env.CI_TEST ? 20 : 100;
    ...
    // Offload MongoDB on Travis
    const coolDown = (cb) => {
      const timeout = process.env.CI_TEST ? 5000 : 1;
      setTimeout(() => cb, timeout);
    };

    // Run the above steps
    clean(db, () => put(db, () => get(db, () => coolDown(done))));
  });
