...
describe('abacus-mongoclient', () => {
  ...
  before(function(done) {
    if (!process.env.DBCLIENT || process.env.DBCLIENT !== 'abacus-mongoclient') {
      console.log('  Skipping mongoclient tests. DBCLIENT set to', process.env.DBCLIENT || 'default');
      this.skip();
      done();
    }
  ...
  it('distributes db operations over several db partitions', (done) => {
    ...
    const db = dbclient(
      ...
      dbclient.dburi(dbservers(), 'abacus-mongoclient-test'),
      (uri, opt, cb) => {
        // Cause DB handler errors on DB partition 2-201411
        return /mongoclient-test-2-201411/.test(uri) ? cb(perr) : dbclient.dbcons(uri, opt, cb);
      }
    );
  ...
  it('distributes bulk operations over 4 db partitions', (done) => {
    ...
    const db = dbclient(
      ...
      dbclient.dburi(dbservers(), 'abacus-mongoclient-testbulk'),
      (uri, opt, cb) => {
        // Cause DB handler errors on DB partition 2-201411
        return /mongoclient-testbulk-2-201411/.test(uri) ? cb(perr) : dbclient.dbcons(uri, opt, cb);
      }
    );
  ...
  it('works with different collection', (done) => {
    ...
    const fullCollection = (cb) => {
      ...
      const db = dbclient(
        undefined,
        dbclient.dburi(dbservers(), 'abacus-mongoclient-collections-test/full-collection')
      );
      expect(db).to.not.equal(undefined);
      clean(db, () =>
        put(db, () => {
    ...
    const emptyCollection = (cb) => {
      ...
      const db = dbclient(
        undefined,
        dbclient.dburi(dbservers(), 'abacus-mongoclient-collections-test/empty-collection')
      );
      expect(db).to.not.equal(undefined);
      clean(db, () => {
  ...
  it('does not retry on missing documents', (done) => {
    const db = dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-missing-docs'));
    const batchDB = batch(
      dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-missing-docs-batch'))
    );
  ...
  it('does not retry on conflicting documents', (done) => {
    const db = dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-conflicting-docs'));
    const batchDB = batch(
      dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-conflicting-docs-batch'))
    );
  ...
  it('calls back with the document ids and revision', (done) => {
    ...
    const db = dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-correct-docs'));
    const batchDB = batch(
      dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-correct-docs-batch'))
    );
  ...
  it('stores revision fields', (done) => {
    ...
    const db = dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-revisions-docs'));
    const batchDB = batch(
      dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-revisions-docs-batch'))
    );
  ...
  it('handles documents with error field', (done) => {
    ...
    const db = dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-docs-with-error'));
    const batchDB = batch(
      dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-docs-with-error-batch'))
    );
  ...
  it('reads custom Mongo options from environment', (done) => {
    ...
    const db = dbclient(
      undefined,
      dbclient.dburi(dbservers(), 'abacus-mongoclient-mongooptions-test/collection'),
  ...
  it('with many servers, returns the correct server', (done) => {
    ...
    const multiServerFunc = dbclient.dburi(
      [dbserver() + '/abacus-mongoclient-0', dbserver() + '/abacus-mongoclient-1'],
      'abacus'
    );
    multiServerFunc(['0', '201703'], (err, u) =>
      cb(expect(u).to.equal(dbserver() + '/abacus-mongoclient-0/abacus-0-201703'))
    );
    multiServerFunc(['1', '201703'], (err, u) =>
      cb(expect(u).to.equal(dbserver() + '/abacus-mongoclient-1/abacus-1-201703'))
    );
    multiServerFunc(['2', '201703'], (err, u) =>
      cb(expect(u).to.equal(dbserver() + '/abacus-mongoclient-0/abacus-2-201703'))
    );
    multiServerFunc(['3', '201703'], (err, u) =>
      cb(expect(u).to.equal(dbserver() + '/abacus-mongoclient-1/abacus-3-201703'))
    );
  });
  ...
  it('with one server, returns this server', (done) => {
    ...
    const singleServerFunc = dbclient.dburi([dbserver() + '/abacus-mongoclient-0'], 'abacus');
    singleServerFunc(['4', '201703'], (err, u) =>
      cb(expect(u).to.equal(dbserver() + '/abacus-mongoclient-0/abacus-4-201703'))
    );
    singleServerFunc(['5', '201703'], (err, u) =>
      cb(expect(u).to.equal(dbserver() + '/abacus-mongoclient-0/abacus-5-201703'))
    );
  });
  ...
  it('with many servers, memoizes the correct server', (done) => {
    ...
    multiServerFunc(['6', '201703'], (err, u) =>
      cb(expect(u).to.equal(dbserver() + '/abacus-mongoclient-0/abacus-6-201703'))
    );
    multiServerFunc(['6', '201703'], (err, u) =>
      cb(expect(u).to.equal(dbserver() + '/abacus-mongoclient-0/abacus-6-201703'))
    );
  ...
  it('if the partition exits, returns the correct server', (done) => {
    const db = dbclient(undefined, dbclient.dburi(dbserver() + '/abacus-mongoclient-1', 'abacus'));
    ...
    db.put(
      ...
      (err, doc) => {
        ...
        multiServerFunc(['0', '201411'], (err, u) => {
          expect(u).to.equal(dbserver() + '/abacus-mongoclient-1/abacus-0-201411');
  ...
  it('does not overwrite document _id', (done) => {
    const unbatchedDb = dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-document-ids'));
  ...
  it('gets documents in the exact order', function(done) {
    const db = batch(dbclient(undefined, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-document-order')));
  ...
  it('bulk and batch ops distribute over partitions in collections', (done) => {
    ...
    const db = batch(
      dbclient(
        ...
        dbclient.dburi(dbservers(), 'abacus-mongoclient-partitions-test/collection'),
        (uri, opt, cb) => {
          // Cause DB handler errors on DB partition 2-201411
  ...
  context('range test', () => {
    beforeEach(function() {
      if (!process.env.DBCLIENT || process.env.DBCLIENT !== 'abacus-mongoclient') this.skip();
    });

    const part = partition.partitioner(partition.bucket, partition.period, partition.forward, partition.balance, true);
    const db = batch(dbclient(part, dbclient.dburi(dbservers(), 'abacus-mongoclient-range-test')));
  ...
  it('supports readAllPages', function(done) {
    const part = partition.partitioner(partition.bucket, partition.period, partition.forward, partition.balance, true);
    const db = dbclient(part, dbclient.dburi(dbservers(), 'abacus-mongoclient-test-read-all-pages'));
