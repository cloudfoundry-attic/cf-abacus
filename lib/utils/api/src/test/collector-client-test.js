...
describe('CollectorClient', () => {
  const authorizationHeader = 'Authorization: value';
  ...
  beforeEach(async () => {
    ...
    const authHeaderStub = sinon.stub().returns(authorizationHeader);
    client = new CollectorClient(`http://localhost:${port}`, authHeaderStub);
  });
  ...
  describe('#postUsage', () => {
    ...
    context('when server accepts usage', () => {
      ...
      it('sends usage to server', async () => {
        ...
        expect(argReq.headers.authorization).to.equal(authorizationHeader);
      });
    });
