...
const connectCouch = (connect) => {
  ...
  class CouchStore extends Store {
    constructor(options) {
      super();
      let dbHandle = couchclient(partition.singleton, couchclient.dburi(options.dbUri, options.dbName));
      this.db = dbHandle;
    ...
    getExpiryTime(session) {
      ...
      if (session && session.cookie && session.cookie.expires) return moment.utc(session.cookie.expires).valueOf();
    ...
    touch(sid, session, callback) {
      ...
      this.db.get(sid, (err, doc) => {
        ...
        if (session && session.cookie && session.cookie.expires) {
          debug('touch session from %s to %s', doc.expires, session.cookie.expires);
    ...
    setDesignDocument() {
      const createDesignDoc = () => {
        ...
        this.db.put(doc, (err) => {
          if (err) debug('failed to update %o', err);
          else debug('successfully posted design document');
        });
      };

      this.db.get('_design/couchdb-session', (err, doc) => {
        // Delete the design docs if it exists & create the new one
        if (err || _.isEmpty(doc)) createDesignDoc();
      });
    }
