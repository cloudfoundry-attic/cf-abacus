...
describe('abacus-couchStore', () => {
  ...
  const isNotCouchClient = process.env.DBCLIENT && process.env.DBCLIENT !== 'abacus-couchclient';
  ...
  before(function(done) {
    if (isNotCouchClient) {
      console.log('  Skipping couchstore tests. DBCLIENT set to', process.env.DBCLIENT);
      this.skip();
    }

    if (!/:/.test(server()))
      dbclient.drop(undefined, /^abacus-test-dashboard-0-0-mrview/, () => {
        dbclient.drop(undefined, /^abacus-test-dashboard/, () => setupStore(done));
      });
    else dbclient.drop(server(), /^abacus-test-dashboard/, () => setupStore(done));
  });
  ...
  it('get session cookie', (done) => {
    store.get('test-cookie', (err, doc) => {
      expect(err).to.be.equal(null);
      expect(doc).to.deep.equal({ name: 'test-cookie', cookie: { maxAge: 5000 } });
      done();
    });
  });
  ...
  it('touch session cookie with expire time', (done) => {
    let updateCookie = {
      name: 'test-cookie',
      cookie: {
        expires: moment.utc('2014-11-06 19:07:54')
      }
    };
  ...
  it('remove expired session', (done) => {
    let updateCookie = {
      name: 'test-cookie',
      cookie: {
        expires: moment.utc('2017-08-08 19:07:54')
      }
    };
