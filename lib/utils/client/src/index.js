...
const reporter = (statistics, errors, errorFn) => {
  ...
  const processBusinessErrorReport = (response, usage, t0, options, cb) => {
    ...
    if (response.statusCode === 409) {
      ...
      if (! isEmpty(options) && ! options.reportBusinessError) {
        debug('Changing status code to 201 since usage is present');
  ...
  const reportUsage = (usage, token, options = {}, cb = () => {}) => {
    ...
    reliableRequest.post(':collector/v1/metering/collected/usage', {
      ...
      if (response.body && response.body.error) {
        processBusinessErrorReport(response, usage, t0, options, cb);
...
const poll = (fn, opts, done) => {
  ...
  const doneCallback = (err) => {
    ...
    if (moment.now() - startTimestamp > options.totalTimeout) {
      ...
      setTimeout(() => {
        debug('Calling %s after >= %d ms...', fn.name,
...
const waitForStartAndPoll = (url, fn, opts, done) => {
