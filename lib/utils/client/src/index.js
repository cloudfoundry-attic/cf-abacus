...
const poll = (fn, checkFn, done, timeout = 5000, interval = 1000) => {
...
const waitForStartAndPoll = (url, token, opts, done) => {
  ...
  request.waitFor(url, options, options.startTimeout, (err, uri) => {
    ...
    request.get(uri, options, (err, response) => {
      ...
      poll(options.fn, options.checkFn, (error) => {
        done(error);
      }, t, options.pollInterval);
    });
  });
};
