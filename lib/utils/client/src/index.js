...
const reporter = (errors, errorFn) => {
  const registerError = (message, error, response, name, time) => {
    const errorMessage =
      error || response ? util.format('%s; error: %s; response: %j', message, error, response) : message;
    ...
    if (name && time) perf.report(name, time, undefined, new Error(errorMessage));
  };
  ...
  const processSuccessfulReport = (response, usage, t0, cb) => {
    if (!response.headers || !response.headers.location) {
      const message = util.format('No Location header found in ' + 'response %j for usage %j', response, usage);
      edebug(message);
      cb(new Error(message));
      return;
    }

    debug('Successfully reported usage %j with headers %j', usage, response.headers);
  ...
  const processBusinessErrorReport = (response, usage, t0, cb) => {
    const message = util.format('Business error for usage %j. Response: %j', usage, response);
    debug(message);
  ...
  const processFailedReport = (error, response, t0, cb) => {
    ...
    const message = util.format('Failed reporting usage. Consecutive failures: %d', errors.consecutiveReportFailures);
    errorCb(message, error, undefined, 'report', t0);
  ...
  const reportUsage = (usage, token, cb = () => {}) => {
    ...
    reliableRequest.post(
      ...
      (error, response) => {
        ...
        // Response
        if (response.statusCode === 201) processSuccessfulReport(response, usage, t0, cb);
        else processFailedReport(error, response, t0, cb);
      }
    );
  };
...
const poll = (fn, opts, done) => {
  ...
  const doneCallback = (err) => {
    if (!err) {
      ++successCount;
      debug('Expectation in %s met (%d/%d)', fn.name, successCount, options.successCount);
    ...
    if (moment.now() - startTimestamp > options.totalTimeout) {
      debug('Expectation not met for %d ms. Error: %o', options.totalTimeout, err);
      done(new Error(err));
    } else {
      debug('Scheduling new execution of %s after %d', fn.name, options.pollInterval);
      setTimeout(() => {
        debug('Calling %s ...', fn.name);
        if (options.checkFn) fn(options.checkFn, doneCallback);
        else fn(doneCallback);
