...
const reporter = (errors, errorFn) => {
  ...
  const processFailedReport = (response, usage, t0, cb) => {
    debug('Failed reporting usage: %j. Response: %j', usage, response);
    ...
    const message = util.format('Failed reporting usage: %j. Response: %j, Consecutive failures: %d',
      usage, response, errors.consecutiveReportFailures);
    const body = response.body;
    const err = body && body.error ? extend(new Error(), body) : new Error(message);
    errorCb(message, err, undefined, 'report', t0);
  ...
  const reportUsage = (usage, token, cb = () => {}) => {
    ...
    reliableRequest.post(
      ...
      (error, response) => {
        ...
        const bodyError = response.body && response.body.error ? response.body.error : undefined;
        if (response.statusCode === 202 && !bodyError)
          processSuccessfulReport(response, usage, t0, cb);
        else
          processFailedReport(response, usage, t0, cb);
      }
    );
  };
