...
const tests = (secured) => {
  ...
  context('success', () => {
    beforeEach((done) => {
      responseError = undefined;
      responseBody = generateResponse(202, { location: 'some location' });
    ...
    context('when location header is missing', () => {
      beforeEach(() => {
        responseError = undefined;
        responseBody = generateResponse(202);
      });
  ...
  context('failure', () => {
    context('on error during reporting', () => {
      ...
      it('raises correct reporting errors', () => {
        ...
        expect(moment.utc(errors.lastErrorTimestamp).valueOf()).to.be.above(testStartTime);
      });
    ...
    context('on business error', () => {
      context('on 409 response code', () => {
        ...
        context('with retry', () => {
          ...
          context('and no pre-existing errors', () => testConflict(errorResponseBody, 0));

          context('and no pre-existing errors', () => testConflict(errorResponseBody, 17));
        });
      ...
      context('on 451 response code', () => {
        const errorResponseBody = {
          status: 451,
          error: 'license',
          reason: 'Unsupported license type TRIAL',
          noretry: true,
          nobreaker: true
        };
        ...
        beforeEach((done) => {
          responseError = undefined;
          responseBody = generateResponse(451, {}, errorResponseBody);

          errors.consecutiveReportFailures = 1;
      ...
      context('on 202 response code', () => {
        ...
        beforeEach((done) => {
          responseError = undefined;
          responseBody = generateResponse(202, {}, errorResponseBody);

          reporter.reportUsage(appUsage, token, (error, res) => {
            expect(error).to.be.an('Error');
            expect(error).to.have.property('error', errorResponseBody.error);
            expect(error).to.have.property('reason', errorResponseBody.reason);
            expect(error).to.have.property('cause', errorResponseBody.cause);
            expect(res).to.equal(responseBody);
            done();
          });
        });
        ...
        it('raises correct reporting errors', () => {
          ...
          expect(errors.lastError)
            .to.match(/^Failed reporting usage: (.*?). Response: (.*?), Consecutive failures: 1/);
          expect(moment.utc(errors.lastErrorTimestamp).valueOf()).to.be.above(testStartTime);
        });
