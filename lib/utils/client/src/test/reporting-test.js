...
const tests = (secured) => {
  ...
  beforeEach(() => {
    ...
    requestPost = spy((uri, opts, cb) => {
      cb(responseError, responseBody);
    });
    stub(request, 'post', requestPost);
    ...
    reporter = require('..')(statistics, errors);
  });
  ...
  context('failure', () => {
    context('on error during reporting', () => {
      ...
      it('calls back error function', (done) => {
        ...
        reporter = require('..')(statistics, errors, callbackFn);
    ...
    context('on business error', () => {
      context('on 409 response code', () => {
        const checkResponse = (error, response, expectedResponseBody, done) => {
          expect(error).to.equal(undefined);
          expect(response).to.deep.equal(expectedResponseBody);
          done();
        };

        const testConflict = (errorResponseBody, failures, options) => {
          beforeEach((done) => {
            responseError = undefined;
            ...
            if(isEmpty(options))
              reporter.reportUsage(appUsage, token, (error, res) => {
                checkResponse(error, res, responseBody, done);
              });
            else
              reporter.reportUsage(appUsage, token, options, (error, res) => {
                checkResponse(error, res, generateResponse(201, {}, {
                  original: '409 Conflict'
                }), done);
              });
          });
        ...
        context('with retry', () => {
          ...
          context('and 17 pre-existing errors', () =>
        ...
        context('with returnBusinessErrors = false', () => {
          const errorResponseBody = {
            error: 'conflict',
            reason: 'Conflict. Please retry'
          };

          const options = { returnBusinessErrors: false };

          context('and no pre-existing errors', () =>
            testConflict(errorResponseBody, 0, options));

          context('and 17 pre-existing errors', () =>
            testConflict(errorResponseBody, 17, options));
        });
      });
