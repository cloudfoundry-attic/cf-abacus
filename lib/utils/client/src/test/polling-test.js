  ...
  context('waiting on an endpoint', () => {
    ...
    beforeEach(() => {
      ...
      routes.get('/test', (request, response) => {
        const index = requestNumber >= requests.length ? requests.length - 1 : requestNumber++;
    ...
    const pollWithCheckFn = (checkFn, cb) => {
      request.get('http://localhost::p/test', { p: serverPort }, (error, response) => {
        try {
          if (checkFn) checkFn(error, response);
        } catch (e) {
          cb(e);
          return;
        }
    ...
    const pollFn = (cb) => request.get('http://localhost::p/test', { p: serverPort }, (error, response) => cb(error));
    ...
    context('when check is successful', () => {
      ...
      it('succeeds', (done) => {
        client.waitForStartAndPoll('http://localhost::p/test', pollWithCheckFn, buildOptions(), done);
      });
    });
    ...
    context('when check fails', () => {
      ...
      it('errors after timeout', (done) => {
        client.waitForStartAndPoll(
          ...
          pollWithCheckFn,
          buildOptions({ totalTimeout: 3000 }),
          (error) => {
    ...
    context('without check function', () => {
      ...
      it('succeeds', (done) => {
        ...
        client.waitForStartAndPoll('http://localhost::p/test', pollFn, options, done);
      });
    });
    ...
    context('with no options', () => {
      ...
      it('succeeds', (done) => {
        client.waitForStartAndPoll(`http://localhost:${serverPort}/test`, pollFn, {}, done);
      });
    ...
    context('when initial start fail', () => {
      ...
      it('time outs', (done) => {
        client.waitForStartAndPoll('http://localhost::p/test', pollWithCheckFn, { startTimeout: 1000 }, (error) => {
    ...
    context('with token', () => {
      ...
      it('succeeds', (done) => {
        ...
        client.waitForStartAndPoll('http://localhost::p/test', pollWithCheckFn, options, (error) => {
          expect(error).to.equal(undefined);
          expect(reqmock.get.firstCall.args[1].headers.authorization).to.equal('token');
          done();
        });
      });
    });
