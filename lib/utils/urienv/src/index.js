...
const domain = (uris) => {
  let primaryDomain = process.env.PRIMARY_DOMAIN;

  let domain = uris[0].split('.').slice(1).join('.');
  if (primaryDomain && validDomain(primaryDomain, uris))
    domain = primaryDomain;

  debug('Using domain: %s', primaryDomain);
  return domain;
};
