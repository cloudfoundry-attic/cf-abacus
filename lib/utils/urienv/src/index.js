...
// CloudFoundry URIs are resolved using the route URI from the VCAP_APPLICATION env variable.

const { keys, map, object, some } = require('underscore');
...
const hosted = (alias, uris, options) => {
  ...
  if (options && options.skipMissing && !env(alias)) {
    debug('Skipping missing app route %s', alias);
    return undefined;
  }
...
const url = (alias, def, options) => {
  // In a Cloud Foundry env, compute the URL using the application URI
  const uris = (vcapenv.app() || {}).application_uris;
  if (uris !== undefined && uris.length) return hosted(alias, uris, options);

  // In a local env, compute the URL using defaults
  return local(alias, def);
};

// Compute the URLs of a collection of apps, use like this:
//   resolve({ abc: 'http://localhost:1234', def: 4567, ghi: undefined })
//
// The above call will return
//  { abc: 'http://abc.bluemix.net', def: 'http://def.bluemix.net', ghi: 'http://ghi.bluemix.net' }
// in an app mapped to 'xyz.bluemix.net' and
//  { abc: 'http://localhost:1234', def: 'http://localhost:4567', ghi: 'http://localhost:8080' }
// otherwise
//
const resolve = (apps, options) => object(map(keys(apps), (k) => [k, url(k, apps[k], options)]));
