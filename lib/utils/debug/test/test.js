...
describe('cf-abacus-debug', () => {
    ...
    it('logs messages', () => {
        ...
        expect(console.log.args[0][0]).to.match(new RegExp(new Date(now).toISOString()));
    ...
    it('formats objects and errors', () => {
        ...
        expect(console.log.args[0][0]).to.match(/test %s Huh { x: 123, y: \'abc\' } { message: \'Something broke\' } - Error: Something broke at /);
        expect(console.log.args[0][1]).to.equal(process.pid);
    });
    ...
    it('truncates long strings', () => {
        ...
        expect(console.log.args[0][0]).to.match(new RegExp('test %s Truncate \'' + s.substring(0, 1023) + '\.\.\.'));
        expect(console.log.args[0][1]).to.equal(process.pid);
    });
    ...
    it('provides an Express configuration middleware', () => {
        ...
        config({ path: '/else' }, res, next);
        expect(next.called).to.equal(true);

        // Expect middleware to apply dynamic configuration
        config({ path: '/log', query: { config: 'disabled' }}, res, next);
        expect(d.enabled()).to.equal(false);
        expect(res.status.args[0]).to.deep.equal([200]);
        expect(res.send.args[0]).to.deep.equal([{ config: '' }]);

        config({ path: '/log', query: { config: 'enabled' }}, res, next);
        expect(d.enabled()).to.equal(true);
        expect(res.status.args[1]).to.deep.equal([200]);
        expect(res.send.args[1]).to.deep.equal([{ config: '*' }]);

        config({ path: '/log', query: { config: 'disabled' }}, res, next);
        expect(d.enabled()).to.equal(false);
        expect(res.status.args[2]).to.deep.equal([200]);
        expect(res.send.args[2]).to.deep.equal([{ config: '' }]);

        config({ path: '/log', query: { config: 'test' }}, res, next);
        expect(d.enabled()).to.equal(true);
        expect(res.status.args[3]).to.deep.equal([200]);
        expect(res.send.args[3]).to.deep.equal([{ config: 'test' }]);
    });
});
