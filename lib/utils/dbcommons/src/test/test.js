...
describe('abacus-dbcommons', () => {
  ...
  describe('puri', () => {
    ...
    it('purifies the URL', () => {
      expect(dbCommons.puri('https://user:password@example.com')).
        to.equal('https://***:***@example.com');
      expect(dbCommons.puri('mongodb://user:password@server.host')).
  ...
  describe('pad16', () => {
    ...
    it('pads to 16 digits', () => {
      expect(dbCommons.pad16('1')).to.equal(
        '0000000000000001'
      );
    });

    it('pads string with leading zeros to 16 digits', () => {
      expect(dbCommons.pad16('0201')).to.equal(
        '0000000000000201'
      );
    });

    it('pads string bigger than 16 digits to zeros', () => {
      expect(dbCommons.pad16('3540000000000000001')).to.equal(
        '0000000000000000'
      );
    });
  });
  ...
  describe('kturi', () => {
    ...
    it('converts key and time to URI', () => {
      expect(dbCommons.kturi('one', 10)).to.equal(
        'k/one/t/0000000000000010'
      );
    });
  ...
  describe('tkuri', () => {
    ...
    it('converts time and key to URI', () => {
      expect(dbCommons.tkuri('one', 10)).to.equal(
        't/0000000000000010/k/one'
      );
    });

    it('converts time to URI', () => {
      expect(dbCommons.tkuri(undefined, 10)).to.equal(
        't/0000000000000010'
      );
    });
  });
  ...
  describe('t', () => {
    ...
    it('gets time from key-based URI', () => {
      expect(dbCommons.t('k/one/t/0000000000000010')).to.equal(
        '0000000000000010'
      );
    });

    it('gets time with segments from key-based URI', () => {
      expect(dbCommons.t('k/one/two/t/0000000000000010/123')).to.equal(
        '0000000000000010/123'
      );
    });

    it('gets time from time-based URI', () => {
      expect(dbCommons.t('t/0000000000000010/k/one')).to.equal(
        '0000000000000010'
      );
    });

    it('gets time with segments from time-based URI', () => {
      expect(dbCommons.t('t/0000000000000010/123/k/one/two')).to.equal(
        '0000000000000010/123'
      );
    });
  ...
  describe('k', () => {
    ...
    it('gets key from key-based URI', () => {
      expect(dbCommons.k('k/one/t/0000000000000010')).to.equal(
        'one'
      );
    });

    it('gets key with segments from key-based URI', () => {
      expect(dbCommons.k('k/one/two/t/0000000000000010/123')).to.equal(
        'one/two'
      );
    });

    it('gets key from time-based URI', () => {
      expect(dbCommons.k('t/0000000000000010/k/one')).to.equal(
        'one'
      );
    });

    it('gets key with segments from time-based URI', () => {
      expect(dbCommons.k('t/0000000000000010/123/k/one/two')).to.equal(
        'one/two'
      );
    });
  ...
  describe('readAllPages', () => {
    ...
    const pagingOpts = {
      ...
      pageSize: 2,
      skip: 0,
      processingFn: processingFnStub
    };

    const getDBOpts = (opts) => extend({
      startkey: 't/0/start',
      endkey: 't/10/end',
      include_docs: true,
      limit: 2,
    ...
    context('on success', () => {
      ...
      it('reads all pages from db', (done) => {
        dbCommons.readAllPages(dbClient, pagingOpts, (error) => {
          expect(error).to.equal(undefined);

          assert.calledThrice(allDocsStub);
          assert.calledWith(allDocsStub.firstCall, getDBOpts());
          assert.calledWith(allDocsStub.secondCall, getDBOpts({ skip: 2 }));
          assert.calledWith(allDocsStub.thirdCall, getDBOpts({ skip: 4 }));
    ...
    context('on failure reading a page', () => {
      ...
      it('tries to read two pages from db', (done) => {
        dbCommons.readAllPages(dbClient, pagingOpts, () => {
