...
const { defaults, extend, first, flatten, groupBy, map, omit, rest, sortBy, values, zip } = require('underscore');

// Setup debug log
const debug = require('abacus-debug')('abacus-dbcommons');
const xdebug = require('abacus-debug')('x-abacus-dbcommons');
...
const singleOp = (op, rw, doc, part, pool, dbopt, cb) => {
  const lcb = (err, res) => {
    if (err) xdebug('Single db op failed, error %o', err);
    cb(err, res);
  };
  dbPartition(
    key(doc._id),
...
const batchOp = (op, rw, docs, opt, part, pool, dbopt, cb) => {
  const lcb = (err, res) => {
    if (err) xdebug('Batch db op failed, error %o', err);
    cb(err, res);
  };
...
const bulkOp = (op, rw, docs, opt, part, pool, dbopt, cb) => {
  const lcb = (err, res) => {
    if (err) xdebug('Bulk db op failed, error %o', err);
    cb(err, res);
  };
...
const rangeOp = (op, rw, docs, opt, part, pool, dbopt, cb) => {
  ...
  const lcb = (err, res) => {
    if (err) {
      xdebug('Range db op failed, error %o', err);
      cb(err, res);
      return;
    }
