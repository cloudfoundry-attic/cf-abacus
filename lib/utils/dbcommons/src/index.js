...
const dbPartitions = (k, t, rw, part, pool, dbopt, cb) => {
  part(k, t, rw, (err, pars) => (err ? cb(err) : transform.map(pars, (p, i, pars, pcb) => pool(p, rw, pcb), cb)));
};
...
const singleOp = (op, rw, doc, part, pool, dbopt, cb) => {
  ...
  dbPartition(key(doc._id), time(doc._id), rw, part, pool, dbopt, (err, db) => (err ? lcb(err) : op(db, doc, lcb)));
};
...
const rangeOp = (op, rw, docs, opt, part, pool, dbopt, cb) => {
  ...
  const k = key(docs[0]._id) ? (key(docs[0]._id) === key(docs[1]._id) ? key(docs[0]._id) : undefined) : undefined;
