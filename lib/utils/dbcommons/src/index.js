...
// Setup debug log
const debug = require('abacus-debug')('abacus-dbops');
const edebug = require('abacus-debug')('e-abacus-dbops');
...
const readAllPages = (db, opt, cb) => {
  ...
  db.allDocs({
    ...
    opt.processingFn(docs, (processingError) => {
      if (processingError)
        cb(processingError);
      else
        readAllPages(db, extend({}, opt, { skip: opt.skip + docs.length }), cb);
    });
  });
};
