...
// Setup debug log
const debug = require('cf-abacus-debug')('cf-abacus-hystrix');
...
const percentiles = (values) => {
    return object(map([0, 25, 50, 75, 90, 95, 99, 99.5, 100], (percent) => [ percent.toString(), percentile(values, percent) ]));
};
...
const command = (s) => {
    ...
    return extend({
        type: 'HystrixCommand', name: s.name, group: s.name,

        // Current command state
        currentTime: s.time, isCircuitBreakerOpen: s.circuit === 'open' || s.circuit === 'half-open',
    ...
    (() => {
        ...
        return h ? {
            requestCount: h.ok + h.errors, errorCount: h.errors, errorPercentage: h.errors / (h.ok + h.errors || 1) * 100
        } : {
            requestCount: 0, errorCount: 0, errorPercentage: 0
        };
    })(),

    // Sum the counts accumulated in the rolling count buckets
    (() => {
        const c = reduce(s.counts,
            (a, c) => ({ ok: a.ok + c.ok, errors: a.errors + c.errors, timeouts: a.timeouts + c.timeouts, rejects: a.rejects + c.rejects }),
            { ok: 0, errors: 0, timeouts: 0, rejects: 0 });
        return {
    ...
    (() => {
        const l = sortBy(reduce(s.latencies, (a, l) => a.concat(l.latencies), []), (v) => v);
        const p = percentiles(l);
        const m = mean(l);
        return {
            latencyExecute: p, latencyExecute_mean: m,
            latencyTotal: p, latencyTotal_mean: m
        };
    })(),
...
const stream = () => {
    return (req, res, next) => {
        if(req.path === '/hystrix.stream') {
            ...
            req.on('close', () => { clearInterval(writer); });
        }
        else next();
    };
};
