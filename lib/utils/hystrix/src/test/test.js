...
const perf = require('cf-abacus-perf');

describe('cf-abacus-hystrix', () => {
    let clock;
    beforeEach(() => {
        // Setup fake timers
    ...
    it('serves a stream of hystrix stats', () => {
        ...
        const next = spy();
        const res = {};
        const socket = { setTimeout: stub().returns(res) };
        ...
        stream({ path: '/else' }, res, next);
        expect(next.args.length).to.equal(1);

        // Expect middleware to return a Hystrix stats stream
        stream({ path: '/hystrix.stream', query: { delay : 1000 }, socket: socket, on: on }, res, next);
        ...
        expect(socket.setTimeout.args).to.deep.equal([[0]]);
        ...
        const m = { name: 'foo', time: t,
            counts: [{ i: i(1000), ok: 1, errors: 1, timeouts: 1, rejects: 1 }],
            latencies: [{ i: i(10000), latencies: [10] }],
            health: [{ i: i(500), ok: 1, errors: 3 }],
            circuit: 'open' };
        ...
        expect(res.write.args[2]).to.deep.equal(['data: {"type":"HystrixCommand","name":"foo","group":"foo","currentTime":' + (t + 900) + ',"isCircuitBreakerOpen":true,"currentConcurrentExecutionCount":0,"requestCount":4,"errorCount":3,"errorPercentage":75,"rollingCountCollapsedRequests":0,"rollingCountExceptionsThrown":1,"rollingCountFailure":1,"rollingCountFallbackFailure":0,"rollingCountFallbackRejection":0,"rollingCountFallbackSuccess":0,"rollingCountResponsesFromCache":0,"rollingCountSemaphoreRejected":0,"rollingCountShortCircuited":1,"rollingCountSuccess":1,"rollingCountThreadPoolRejected":0,"rollingCountTimeout":1,"latencyExecute":{"0":10,"25":10,"50":10,"75":10,"90":10,"95":10,"99":10,"100":10,"99.5":10},"latencyExecute_mean":10,"latencyTotal":{"0":10,"25":10,"50":10,"75":10,"90":10,"95":10,"99":10,"100":10,"99.5":10},"latencyTotal_mean":10,"propertyValue_circuitBreakerRequestVolumeThreshold":20,"propertyValue_circuitBreakerSleepWindowInMilliseconds":5000,"propertyValue_circuitBreakerErrorThresholdPercentage":50,"propertyValue_circuitBreakerForceOpen":false,"propertyValue_circuitBreakerForceClosed":false,"propertyValue_circuitBreakerEnabled":true,"propertyValue_executionIsolationStrategy":"THREAD","propertyValue_executionIsolationThreadTimeoutInMilliseconds":800,"propertyValue_executionIsolationThreadInterruptOnTimeout":true,"propertyValue_executionIsolationThreadPoolKeyOverride":null,"propertyValue_executionIsolationSemaphoreMaxConcurrentRequests":20,"propertyValue_fallbackIsolationSemaphoreMaxConcurrentRequests":10,"propertyValue_metricsRollingStatisticalWindowInMilliseconds":10000,"propertyValue_requestCacheEnabled":true,"propertyValue_requestLogEnabled":true,"reportingHosts":1}\n\n']);
