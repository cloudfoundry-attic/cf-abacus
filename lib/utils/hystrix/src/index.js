...
const { each, extend, last, map, object, reduce, sortBy, values } = require('underscore');
...
const percentile = (values, percent) => {
  ...
  return values.length === 0
    ...
    : high >= values.length
      ...
      : low === high ? values[low] : Math.round(values[low] + (rank - low) * (values[high] - values[low]));
};
...
const percentiles = (values) => {
  return object(
    map([0, 25, 50, 75, 90, 95, 99, 99.5, 100], (percent) => [percent.toString(), percentile(values, percent)])
  );
};
  ...
  return extend(
    {
      ...
      currentTime: s.time,
      isCircuitBreakerOpen: s.circuit === 'open' || s.circuit === 'half-open',
...
const scopes = 'abacus.system.read';
...
const stream = () => (req, res, next) => {
  ...
  write();
  const writer = setInterval(write, Math.max(req.query.delay || 2000, 200));
