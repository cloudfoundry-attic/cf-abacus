...
const inflight = (maxClientInflight, maxRemainingInflight, secured) => {
  const createCounter = (req) => {
    const server = getServer(req);
    const compositeCounter = new CompositeCounter(server);

    if (!secured || (isOAuthRequest(req) && isClientRequest(req)))
  ...
  return (req, res, next) => {
    ...
    if (counter.isFull()) {
      xdebug('Too many %s requests', counter.getKey());
