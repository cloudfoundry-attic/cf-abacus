...
const inflight = require('./lib/middleware/inflight');
const createRateLimiter = require('./lib/middleware/rate-limiting/rate-limiter');
...
const maxInternalInflight = process.env.MAX_SYSTEM_INFLIGHT || 4;
...
const useRateLimiting = () => process.env.RATE_LIMIT_WINDOW !== undefined;
const getDbUri = () => uris[dbalias][0];
...
const listen = function(opt, cb) {
  ...
  process.title = 'node ' + (process.env.TITLE || [vcapenv.appname(), vcapenv.appindex()].join('-')) + ' express';
...
const registerRateLimiter = (app) => {
  const rateLimiter = createRateLimiter(getDbUri(), process.env.RATE_LIMIT_WINDOW);
  app.use(rateLimiter.middleware);
  process.on('exit', rateLimiter.closeDbConnection);
};
...
// Return an Express app configured with a basic minimum selection of middleware.
const basicApp = () => {
  const app = express();
  app.basic = true;
...
const runCLI = () => {
  ...
  if (commander.start) {
    ...
    // Stop the Webapp
    cp.exec('pkill -f "node ' + vcapenv.appname() + '.* express"', (err, stdout, stderr) => {
