...
const inflight = () => {
  ...
  return (req, res, next) => {
    ...
    xdebug('%d inflight requests', server.inflight);
    ...
    const done = () => {
      ...
      if (server.inflight === 0) server.emit('quiet');
    };
    res.on('finish', done);
    res.on('close', done);

    if (server.inflight > maxInflight) {
      res.status(429).send('Too many requests in parallel');
      return;
    }
...
const lconf = (opt) => {
  if (opt !== undefined) debug('Listen options %o', opt);
