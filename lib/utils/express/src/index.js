  ...
  return (req, res, next) => {
    ...
    if(value.redirect)
      if(value.redirect === 'object')
        if(value.redirect.status)
          res.redirect(value.redirect.status, value.redirect.url);
        else
          res.redirect(value.redirect.url);
      else
...
const listen = function(opt) {
  ...
  process.title = 'node ' + (process.env.TITLE ||
    require(path.join(process.cwd(), 'package.json')).name) + ' express';
