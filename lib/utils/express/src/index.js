...
const sendErrorToClient = (err, response) => {
  if (response.headersSent) {
    edebug('Cannot send error to client: response already sent');
    return;
  }
...
const runCLI = () => {
  ...
  if (commander.start) {
    ...
    // Stop the Webapp
    cp.exec(`pkill -f "node ${vcapenv.appname()}-[0-9]+ express"`, (err, stdout, stderr) => {
