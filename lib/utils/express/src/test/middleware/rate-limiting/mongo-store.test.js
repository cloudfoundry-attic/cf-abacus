    ...
    context('when value is successfully increased', () => {
      ...
      context('when db counter is outdated', () => {
        ...
        context('when db counter is successfully reset', () => {
          beforeEach((done) => {
            collection.updateOne.yields(undefined, undefined);
          ...
          it('db counter is reset in db', () => {
            assert.calledOnce(collection.updateOne);
            assert.calledWith(collection.updateOne, {
              _id: key
            },{
              $set: {
                requestsCount: 1,
        ...
        context('when db counter is not successfully reset', () => {
          ...
          beforeEach(() => {
            collection.updateOne.yields(resetError, undefined);
          });

          it('error is not propagated', (done) => {
            mongoStore.incr(key, (err, result) => {
