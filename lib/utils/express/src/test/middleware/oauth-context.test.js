...
describe('oauth-context', () => {
  ...
  context('when request contains bearer authorization', () => {
    beforeEach(() => {
      const token = signToken([], ['abacus.usage.write', 'abacus.usage.resource.read']);
      authorization = {
        bearer: token
      };
    });
    ...
    it('should inject oauth data in request object for downstream middlewares', (done) => {
      performGet((err, res) => {
        ...
        const scopes = argReq.context.oauth.scopes;
        ...
        expect(scopes.readResourceScopes).to.deep.equal(['resource']);
        expect(scopes.writeResourceScopes).to.deep.equal([]);
