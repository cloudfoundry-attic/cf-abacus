...
describe('abacus-express', () => {
  ...
  context('provides a more complete Express app setup', () => {
    ...
    context('An HTTP request returns a specific status code', () => {
      ...
      it('should send the status code as statusCode field in the response',
        (done) => {
          test({ statusCode: 304 }, done);
        });

      it('should send the status code as status field in the response',
        (done) => {
    ...
    context('An HTTP request with origin header', () => {
      ...
      it('should receive status CORS headers', (done) => {
        request.get({
          ...
          expect(val.statusCode).to.equal(200);
          expect(val.headers['access-control-allow-credentials'])
            .to.not.equal(undefined);
          expect(val.headers['access-control-allow-origin'])
            .to.not.equal(undefined);
          expect(val.headers['access-control-allow-methods'])
            .to.not.equal(undefined);
          expect(val.headers['access-control-allow-headers'])
            .to.not.equal(undefined);
          expect(val.headers['access-control-max-age'])
            .to.not.equal(undefined);
