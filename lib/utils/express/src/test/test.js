...
describe('abacus-express', () => {
  ...
  it('quiesces before bailing out', (done) => {
    ...
    // Send an HTTP get request, expecting an error and the server to bailout after quiescing
    const bailout = () => {
      request.get(
    ...
    // Send an HTTP request to a route that will never respond, to help test server quiescing
    request.get(
      'http://localhost::p/:v/:r',
      {
        p: server.address().port,
