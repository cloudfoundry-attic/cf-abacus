...
const debug = require('cf-abacus-debug');
const request = require('cf-abacus-request');
const express = require('..');

/* jshint undef: false */

describe('cf-abacus-express', () => {
    let clock;
    let exit;
    beforeEach(() => {
    ...
    it('quiesces before bailing out', (done) => {
        ...
        request.get('http://localhost::p/:v/:r', { p: server.address().port, v: 'inflight', r: 'request' }, spy());

        // Send an HTTP get request, expecting an error and the server
        // to bailout after quiescing
        request.get('http://localhost::p/:v/:r', { p: server.address().port, v: 'bailout', r: 'request' }, (err, val) => {
    ...
    it('provides a more complete Express app setup', (done) => {
        ...
        // Turn on debugging
        debug.enable('cf-abacus-express');
        ...
        app.use((req, res, next) => {
            ...
            if(req.path === '/ok/value')
                res.value = { type: 'txt', body: 'value', header: { Foo: 'bar' },
                    cookie: { Cook: 'blah', Cook2: { value: 'duh' }, Cook3: { value: 'hey', options: 'boo' }},
                    locals: { a: 'b' }, props: { c: 'd' }, location: 'http://bar.com', links: { x: 'http://bar.com' }};
            next();
        });
        app.use((req, res, next) => {
            // Return a status 304 response
            if(req.path === '/304/statuscode')
                res.value = { statusCode: 304 };
        ...
        app.use((req, res, next) => {
            ...
            if(req.path === '/ok/redirect')
                res.value = { redirect: 'http://localhost:' + server.address().port + '/ok/request' };
            next();
        });
        ...
        app.get('/err/request/:message', (req, res) => {
            ...
            res.status(500).send({ message: req.params.message });
        });
        ...
        request.get('http://localhost::p/:v/:r', { p: server.address().port, v: 'ok', r: 'value' }, (err, val) => {
            expect(err).to.equal(undefined);
            expect(val.statusCode).to.equal(200);
        ...
        request.get('http://localhost::p/:v/:r', { p: server.address().port, v: '304', r: 'statuscode' }, (err, val) => {
            expect(err).to.equal(undefined);
            expect(val.statusCode).to.equal(304);
            done1();
        });
        request.get('http://localhost::p/:v/:r', { p: server.address().port, v: '304', r: 'status' }, (err, val) => {
            expect(err).to.equal(undefined);
            expect(val.statusCode).to.equal(304);
            done1();
        });
        ...
        request.get('http://localhost::p/:v/:r', { p: server.address().port, v: 'ok', r: 'redirect' }, (err, val) => {
            expect(err).to.equal(undefined);
            expect(val.statusCode).to.equal(200);
            expect(val.body).to.equal('okay');
            done1();
        ...
        request.post('http://localhost::p/:v/:r', { p: server.address().port, v: 'post', r: 'request', body: { value: 'value' }}, (err, val) => {
            expect(err).to.equal(undefined);
            expect(val.statusCode).to.equal(200);
            done1();
        });
        ...
        request.get({ uri: 'http://localhost::p/:v/:r', p: server.address().port, v: '500', r: 'request' }, (err, val) => {
            expect(err.code).to.equal(500);
            expect(err.message).to.equal('HTTP response status code 500');
            expect(val).to.equal(undefined);
            done1();
        ...
        request.get('http://localhost::p/:v/:r/:m', { p: server.address().port, v: 'err', r: 'request', m: 'duh' }, (err, val) => {
        ...
        request.options('http://localhost::p/:v/:r', { p: server.address().port, v: 'options', r: 'request', headers: { origin: 'http://foo.com' }}, (err, val) => {
            expect(err).to.equal(undefined);
            expect(val.statusCode).to.equal(200);
            expect(val.headers['access-control-allow-origin']).to.equal('http://foo.com');
            done1();
        });

        // Send an HTTP get request, expecting a 404 status code
        request.get('http://localhost::p/:v/:r', { p: server.address().port, v: '404', r: 'request' }, (err, val) => {
            expect(err).to.equal(undefined);
        ...
        request.get('http://localhost::p/:v/:r', { p: server.address().port, v: 'exception', r: 'request' }, (err, val) => {
            expect(err.code).to.equal(500);
            expect(err.message).to.equal('HTTP response status code 500');
            expect(val).to.equal(undefined);
            done1();
    ...
    it('handles server events', () => {
        ...
        server.emit('upgrade', { url: 'http://foo.com' });
