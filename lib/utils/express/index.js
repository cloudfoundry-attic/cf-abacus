...
// Setup debug log
const csdebug = require('cf-abacus-debug');
const debug = require('cf-abacus-debug')('cf-abacus-express');

// Load optional dev dependency
/* eslint no-extra-parens: 1 */
const inspector = (() => {
    try { return require('express-inspector'); }
...
const cors = () => {
    return (req, res, next) => {
        if(req.headers.origin) {
            ...
            res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, Content-Length, apiKey, X-Requested-With');
...
const accumulate = (accum, length, chunk, encoding) => {
    return chunk && accum.length < length ? accum + chunk.toString(encoding ? encoding : 'utf8', 0, length - accum.length) : accum;
};
...
const isText = (mime) => {
    return /css|form-data|html|javascript|json|perl|postscript|rdf|rtf|sgml|svg|text|urlencoded|xml/.test(mime);
};
        ...
        return (req, res, next) => {
            ...
            res.send = function() {
                ...
                const body = arguments.length === 2 ?
                    typeof arguments[0] !== 'number' && typeof arguments[1] === 'number' ? arguments[0] : arguments[1] : arguments[0];
...
const beforeLogger = () => {
    const morg = morgan(':remote-addr - - :method :url HTTP/:http-version :status :res[content-length] :referrer :user-agent - :response-time ms', {
        immediate: true,
        stream: {
            write: (msg, encoding) => {
...
const sendValue = () => {
    return (req, res, next) => {
        ...
        map(keys(value.cookie), (k) => {
            ...
            if(typeof c === 'object')
                if(c.options) res.cookie(k, c.value, c.options); else res.cookie(k, c.value);
            else
                res.cookie(k, c);
        });
        ...
        if(value.redirect)
            if(value.redirect === 'object')
                if(value.redirect.status) res.redirect(value.redirect.status, value.redirect.url); else res.redirect(value.redirect.url);
            else
...
const inflight = () => {
    ...
    return (req, res, next) => {
        const server = req.client.server ? req.client.server : req.client.socket.server;
...
// catch any errors not caught in async callbacks
const catchall = () => {
    return (req, res, next) => {
...
const uncaught = (bailcb) => {
    ...
    return (err, req, res, next) => {
        ...
        try {
            ...
            res.status(err.status || err.statusCode || 500).send({ error: debug.externalize(err) });
        }
        catch(exc) {
...
const bailout = (err, server, quiesce) => {
    ...
    emitter.emit('message', { server: { exiting: err }});
...
const listen = function(opt) {
    ...
    const o = typeof opt === 'object' ?
        { port: opt.port !== undefined ? opt.port : defaultPort(), key: pem(opt.key ? opt.key : defaultKey()), cert: pem(opt.cert ? opt.cert : defaultCert()) }
        : { port: opt !== undefined ? opt : defaultPort(), key: pem(defaultKey()), cert: pem(defaultCert()) };

    // Set the process title
    process.title = 'node ' + (process.env.TITLE || require(path.join(process.cwd(), 'package.json')).name) + ' express';
    ...
    var server = o.key && o.cert ? https.createServer({ key: o.key, cert: o.cert }, app) : http.createServer(app);
    ...
    server.on('error', (err) => {
        ...
        emitter.emit('message', { server: { noretry: err }});
    ...
    server.listen(o.port, () => {
        ...
        emitter.emit('message', { server: { listening: server.address().port }});
    });
...
const emitter = events.emitter('cf-abacus-express/emitter');
const on = (e, l) => {
    emitter.on(e, l);
};
...
const runCLI = () => {
    ...
    // Stop a Webapp
        ...
        cp.exec('module=`cat package.json | grep \'"name"\' | awk -F \'"\' \'{ print $4 }\'`; pkill -f "node $module$"; pkill -f "node $module "', function(err, stdout, stderr) {
