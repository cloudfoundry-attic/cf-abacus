...
describe('abacus-dbclient', () => {
  ...
  const revisionPattern = /\d*-\d-\d-\d-\d/;
  ...
  it('calls back with the document ids and revision', (done) => {
    ...
    const put = (db, done) => {
      ...
      const cb = (err, doc) => {
        ...
        expect(doc.rev).to.match(revisionPattern);
        if (++cbs === 3) {
    ...
    const get = (db, done) => {
      ...
      const cb = (err, doc) => {
        ...
        expect(doc).to.not.equal(undefined);
        expect(doc.id).to.not.equal(undefined);
        expect(doc._rev).to.match(revisionPattern);
        if (++cbs === 3) {
    ...
    const putlist = (db, done) => {
      ...
      db.bulkDocs(
        ...
        (err, docs) => {
          ...
          expect(docs[0].rev).to.match(revisionPattern);
          expect(docs[1].id).to.equal(helloAgainKey);
          expect(docs[1].rev).to.match(revisionPattern);
          expect(docs[2].id).to.equal(heyKey);
          expect(docs[2].rev).to.match(revisionPattern);
    ...
    const getrange = (db, done) => {
      ...
      db.allDocs(
        ...
        (err, docs) => {
          ...
          expect(docs.rows.length).to.equal(2);
          expect(docs.rows[0].doc.value).to.equal('Hello');
          expect(docs.rows[0].value.rev).to.match(revisionPattern);
          expect(docs.rows[1].doc.value).to.equal('Hello again');
          expect(docs.rows[1].value.rev).to.match(revisionPattern);
  ...
  it('manages revision fields', (done) => {
    const document = {
      processed: 1446418800000,
      processed_id: '0001446418800000-0-0-0-0'
    };
  ...
  it('gets proper collection name', () => {
    ...
    expect(
      dbclient.getCollectionName({
        name: 'mongodb://user:password@10.11.25.240:27017/' + 'db/' + collectionName
      })
    ...
    expect(
      dbclient.getCollectionName({
        name: 'mongodb://user:password@10.11.25.240:27017/' + 'db/' + collectionName + '?ssl=true'
      })
    ).to.equal(collectionName);
  ...
  context('range test', () => {
    ...
    const db = batch(dbclient(part, dbclient.dburi(dbservers(), 'abacus-dbclient-range-test')));
