...
const dbcons = (uri, opt, cb) => {
  ...
  mongoDB.connect(url, defaults(opt || {}, {
    // TODO: check for mongo alternative
    ajax: {
...
const errdb = (name, err) => ({
  ...
  collection: (id) => ({
    ...
    insertMany: (docs, opt, cb) => setImmediate(() => cb(err)),
...
const dbclient = (part, uri, cons) => {
  ...
  return {
    ...
    get: (id, cb) => {
      ...
      singleOp((db, doc, cb) => {
        ...
        collection.find({ _id: doc._id }).limit(1).next((err, val) =>
          val === null ? cb(error(err), undefined) : cb(error(err), val)
        );
      }, 'read', {
        _id: id
    ...
    put: (doc, cb) => {
      ...
      singleOp((db, doc, cb) => {
        const collection = db.collection('documents');
        collection.insertOne(doc, (err, val) => cb(error(err), val));
    ...
    allDocs: (opt, cb) => {
      ...
      if(opt.startkey && opt.endkey)
        ...
        rangeOp((db, docs, opt, cb) => {
          ...
          const collection = db.collection('documents');
          collection.find({
            _id: { $gt: opt.startkey, $lt: opt.endkey }
    ...
    bulkDocs: (docs, opt, cb) => {
      ...
      bulkOp((db, docs, opt, cb) => {
        ...
        bulk.execute((err, res) => err ? cb(error(err)) : cb(null, res));
    ...
    batch_put: (batch, cb) => {
      ...
      batchOp((db, docs, opt, cb) => {
        ...
        collection.insertMany(docs, opt,
          (err, res) => err ? cb(error(err)) : cb(null, res.ops));
