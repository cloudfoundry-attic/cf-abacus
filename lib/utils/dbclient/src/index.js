...
const removeCollectionFromUrl = (u) => {
  ...
  debug('Removing collection from db uri %s. Result: %s', puri(u), result);
  return result;
};
...
const dbConnect = (server, options, cb) => {
  ...
  mongoDB.connect(driverUrl, options, (err, client) => {
    if (err) {
      edebug('Failed to connect to %s because of %o', puri(server), err);
      return cb(err);
    }
    ...
    debug('Connected to mongodb uri %s, database %s', puri(server), dbName);
