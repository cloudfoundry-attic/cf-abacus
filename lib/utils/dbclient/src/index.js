...
const { contains, defaults, extend, filter, find, map, omit, pluck } = require('underscore');
...
const listCollections = (server, db, cb) => {
  debug('Listing collections on %s', puri(server));
...
const dropDatabase = (server, db, database, cb) => {
  debug('Dropping database %s on %s', database, puri(server));
  db.db(database).dropDatabase((err) => {
    if (err) {
      edebug('Failed to drop database %s on %s because of %o', database, puri(server), err);
      return cb(err);
    }
...
const dropCollection = (server, db, collection, cb) => {
  debug('Dropping collection %s on %s', collection, puri(server));
...
const dbcons = (uri, opt, cb) => {
  ...
  mongoDB.connect(driverUrl, options, (err, db) => {
    if (err) edebug('Failed to connect to mongodb uri %s because of %o', puri(url), err);
    else {
...
const getCollectionName = (db) => {
  ...
  if (db.errorDB) {
    debug('Using documents collection for error db %s', puri(db.name));
...
const mongoclient = (partitionFn, uriFn, connectionFn, errorDBFn) => {
  ...
  const updateRevision = (doc) => extend({}, doc, { _rev: seqid() });
        ...
        (db, doc, cb) => {
          ...
          collection
            ...
            .next((err, val) => {
              ...
              cb(error(err), val ? updateRevision(val) : val);
            });
        },
        'read',
        {
    ...
    put: (doc, cb) => {
      ...
      dbops.singleOp(
        (db, doc, cb) => {
          const collection = db.collection(getCollectionName(db));
          if (doc._rev) {
            const usageDoc = updateRevision(doc);
            ...
            const usageDoc = updateRevision(doc);
            collection.insertOne(usageDoc, (err, val) => {
              if (err && err.code && err.code === 11000) {
                // Warning: mutating variable err, but that's intentional
                err.status = 409;
                cb(error(err), undefined);
    ...
    bulkDocs: (docs, opt, cb) => {
      ...
      dbops.bulkOp(
        (db, docs, opt, cb) => {
          ...
          const usageDocs = map(docs, (doc) => {
            const newRevisionDoc = updateRevision(doc);
            if (doc._rev)
              bulk
                .find({ _id: doc._id, _rev: doc._rev })
                .upsert()
                .updateOne(newRevisionDoc);
            else
              bulk
                .insert(newRevisionDoc);
            return newRevisionDoc;
          });
          bulk.execute(
            (err, res) =>
              err
                ? cb(error(err))
                : cb(null, map(usageDocs, (doc) => ({
                  ok: true,
                  id: doc._id,
                  rev: doc._rev
                })))
            ...
            .toArray((err, res) => {
              ...
              return cb(
                ...
                map(map(docs, (doc) => ({ doc: find(res, (r) => doc._id === r._id) })), (row) => ({
                  doc: row.doc ? updateRevision(row.doc) : row.doc
                }))
              );
            });
        },
