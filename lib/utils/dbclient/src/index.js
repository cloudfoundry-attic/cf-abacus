...
const dbpartition = (k, t, rw, part, pool, cb) => {
  part(k, t, rw, (err, p) => err ?
    cb(null, errdb(dburi('partition-err')([k, t ]), err)) : pool(p, rw, cb));
};
...
const dbclient = (part, uri, cons) => {
  ...
  const pool = (p, rw, cb) => {
    ...
    const u = dbopt.uri(p);
    debug('Using db %s in %s mode', u, rw);
    ...
    const dbk = [u, rw].join('-');
    return partitions[dbk] ?
      cb(null, partitions[dbk]) : dbopt.cons(u, {
        // Skip db setup in read mode, as we don't need the db to be
        // created if it doesn't exist
        skip_setup: rw === 'read'
      }, (err, db) => {
        debug('Constructing db handle for db %s in %s mode', u, rw);
