...
// Provides a subset of the MongoDB API over a set of DB partitions

const { contains, defaults, each, extend, filter, find, map, omit, pluck } = require('underscore');

const dbCommons = require('abacus-dbcommons');
const { tkuri, kturi, k, t, dbify, undbify, pad16, puri } = dbCommons({});
...
// Setup debug log
const debug = require('abacus-debug')('abacus-dbclient');
const edebug = require('abacus-debug')('e-abacus-dbclient');
const pdebug = require('abacus-debug')('p-abacus-dbclient');
...
const getCollectionName = (u) => {
  ...
  let collection = defaultCollectionName;
  ...
  debug('Using collection %s for URI %s', collection, puri(uri));
  return collection;
};
...
const removeCollectionFromUrl = (u) => {
  ...
  debug('Removing collection from db uri %s. Result: %s', puri(u), puri(result));
  return result;
};

const dbConnect = (server, options, cb) => {
  debug('Connecting to %s with options %o', puri(server), options);
  const driverUrl = removeCollectionFromUrl(server);
  mongoDB.connect(driverUrl, options, (err, client) => {
    if (err) {
      edebug('Failed to connect to %s because of %o', puri(driverUrl), err);
      return cb(err);
    }

    const dbName = (url.parse(driverUrl).pathname || '/db').substring(1);
    const db = client.db(dbName);

    // Warning: mutating variable db, but that's intentional
    db.name = dbName;
    db.errorDB = false;
    db.collectionName = getCollectionName(server);
    db.url = driverUrl;
    db.dbconsOptions = options;

    debug('Connected to mongodb uri %s, database %s, collection %s', puri(db.url), db.name, db.collectionName);

    return cb(err, db, client);
  });
};

// List all databases on the specified server
const listDatabases = (server, client, dbName, cb) => {
  debug('Listing databases on %s', puri(server));
  client.db(dbName).admin().listDatabases((err, dbs) => {
    if (err) {
      edebug('Failed to list databases on %s because of %o', puri(server), err);
      return cb(err);
    }

    const dbNames = map(dbs.databases, (d) => d.name);
    debug('Listed %s databases', dbNames);
    return cb(null, dbNames);
  });
};

// List all collections on the specified server
const listCollections = (server, client, dbName, cb) => {
  debug('Listing collections in %s n %s', dbName, puri(server));
  client.db(dbName).listCollections().toArray((err, collections) => {
    if (err) {
      edebug('Failed to list collections on %s because of %o', puri(server), err);
      return cb(err);
    }
...
const listPartitions = (server, client, dbName, mode, cb) => {
  if (mode === 'db') listDatabases(server, client, dbName, cb);
  else listCollections(server, client, dbName, cb);
};

// Drop the specified database on the specified server
const dropDatabase = (server, client, dbName, cb) => {
  debug('Dropping database %s on %s', dbName, puri(server));
  client.db(dbName).dropDatabase((err) => {
    if (err) {
      edebug('Failed to drop database %s on %s because of %o', dbName, puri(server), err);
      return cb(err);
    }
...
const dropCollection = (server, client, dbName, collection, cb) => {
  debug('Dropping collection %s in db %s on %s', collection, dbName, puri(server));
  client.db(dbName).collection(collection).drop((err) => {
    if (err) {
      edebug('Failed to drop collection %s on %s because of %o', collection, puri(server), err);
      return cb(err);
    }
...
const dropPartition = (server, client, dbName, partition, mode, cb) => {
  if (mode === 'db')
    dropDatabase(server, client, partition, cb);
  else
    dropCollection(server, client, dbName, partition, cb);
};
...
const dropServerPartitions = (server, regex, cb) => {
  ...
  dbConnect(server, options, (err, db, client) => {
    if (err) return cb(err);

    // List all partitions on the server
    const mode = getPartitioningMode(server);
    return listPartitions(server, client, db.name, mode, (err, partitions) => {
      if (err) {
        client.close();
        return cb(err);
      }
      ...
      debug('Matching partitions for regexp %s: %s', regex, matchingPartitions);
      return transform.map(
        matchingPartitions,
        (partition, i, l, cb) => dropPartition(server, client, db.name, partition, mode, cb),
        (err) => {
          if (err) {
            client.close();
            return cb(err);
...
const dropServerDatabases = (server, regex, cb) => {
  ...
  dbConnect(server, options, (err, db, client) => {
    ...
    // List all databases
    return listDatabases(server, client, db.name, (err, dbs) => {
      if (err) {
        client.close();
        return cb(err);
      }
      ...
      return transform.map(
        matchingDatabases,
        (dbName, i, l, cb) => dropDatabase(server, client, dbName, cb),
        (err) => {
          if (err) {
            client.close();
            return cb(err);
...
const dropDatabases = (servers, regex, cb) => {
  debug('Deleting databases matching %s on servers %s', regex, servers);

  transform.map(servers, (server, i, servers, cb) => {
    if (isLocalServer(server)) dropServerDatabases(server, regex, cb);
    else {
      debug('Skipping non-local server %s', puri(server));
      cb();
    }
...
const findServer = (servers, partition, cb) => {
  ...
  transform.filter(servers, (server, i, servers, cb) => {
    ...
    if (server) debug('Partition %s found on server %s', partition, puri(server));
...
const dburi = (server, name) => {
  ...
  const composeDBUri = (server, partition, cb) => {
    ...
    const path = /:name/.test(srv)
      ? request.route(srv, { name: name })
      : [srv, name].join('/');

    cb(
      null,
      queryOpts
        ? [path, partition.join('-')].join('-') + queryOpts
        : [path, partition.join('-')].join('-')
    );
  };
...
// Post-process db errors and mark them such that they nicely flow through circuit breakers and retries
const error = (err) => {
  if (!err) return err;
...
const mongoclient = (partitionFn, uriFn, connectionFn, errorDBFn) => {
  ...
  const dbops = dbCommons(dbOptions);
    ...
    const pooldb = (p, cb) => {
      ...
      dbOptions.uri(p, (err, u) => {
        ...
        return lock(u, (err, unlock) => {
          ...
          return dbOptions.cons(u, {}, (err, db) => {
            if (err) return unlock(cb(null, dbOptions.errdb('dbcons-err-' + u, err)));
  ...
  return {
    ...
    get: (id, cb) => {
      ...
      dbops.singleOp(
        (db, doc, cb) => {
          const collection = db.collection(db.collectionName);
          collection
            .find({ _id: doc._id })
            .limit(1)
            .next((err, val) => {
    ...
    put: (doc, cb) => {
      ...
      dbops.singleOp(
        (db, doc, cb) => {
          const collection = db.collection(db.collectionName);
          if (doc._rev)
            collection.updateOne({ _id: doc._id }, { $set: doc }, { upsert: true }, (err, val) => {
              ...
              const modifiedOk = val.modifiedCount === 1 || val.upsertedCount === 1;
              cb(error(err), {
                ok: modifiedOk,
                id: modifiedOk ? doc._id : undefined,
                rev: modifiedOk ? doc._rev : undefined
              });
            });
          else
    ...
    bulkDocs: (docs, opt, cb) => {
      ...
      dbops.bulkOp(
        (db, docs, opt, cb) => {
          // Update the proper subset of the list of docs on each selected db partition
          const collection = db.collection(db.collectionName);
