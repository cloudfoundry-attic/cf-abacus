...
const dbpartition = (k, t, rw, part, pool, cb) => {
  part(k, t, rw, (err, p) => err ? cb(null, errdb(dburi('partition-err')([k,
    t
  ]), err)) : pool(p, cb));
};
...
const singleOp = (op, rw, doc, part, pool, cb) => {
  dbpartition(key(doc._id), time(doc._id), rw, part, pool, (err, db) => err ?
    cb(err) : op(db, doc, cb));
};
...
const batchOp = (op, rw, docs, opt, part, pool, cb) => {
  ...
  transform.map(docs, (doc, i, docs, mcb) => {
    ...
    dbpartition(key(doc._id), time(doc._id), rw, part, pool, (err, db) =>
      err ? mcb(err) : mcb(null, {
        i: i,
        doc: doc,
        db: db
      }));
    ...
    transform.map(groups, (gmaps, i, groups, mcb) => {
      ...
      cb(null, map(sortBy(flatten(grows, true), (row) => row[0].i), (
        row) => row[1]));
    });
  });
};
...
const bulkOp = (op, rw, docs, opt, part, pool, cb) => {
  ...
  transform.map(docs, (doc, i, docs, mcb) => {
    ...
    transform.map(groups, (gmaps, i, groups, mcb) => {
      // Return a zip of the requests and corresponding results lists
      op(gmaps[0].db, map(gmaps, (map) => map.doc), opt, (err, rows) =>
        err ? mcb(err) : mcb(null, zip(gmaps, rows)));
...
const rangeOp = (op, rw, docs, opt, part, pool, cb) => {
  ...
  dbpartitions(key(docs[0]._id), [time(docs[0]._id), time(docs[1]._id)], rw,
    part, pool, (err, dbs) => {
      if (err) return cb(err);
      ...
      transform.reduce(dbs, (accum, db, i, dbs, rcb) => {
        ...
        if (opt.limit && accum.length === opt.limit) return rcb(
          undefined, accum);

        // Apply the db operation, with an adjusted row limit if needed
        op(db, docs, opt.limit ? extend(clone(opt), {
          limit: opt.limit - accum.length
        }) : opt, (err, rows) => err ? rcb(err) : rcb(undefined,
          accum.concat(rows)));
      }, [], cb);
    });
};
...
const dbclient = (part, uri, cons) => {
  const dbopt = defaults(
  {
    ...
    partition: partition,
    uri: dburi('db'),
    cons: (uri, cb) => dbcons(uri, undefined, cb)
  });
  ...
  const pool = (p, cb) => {
    ...
    return partitions[u] ? cb(null, partitions[u]) : dbopt.cons(u,
      (err, db) => {
  ...
  return {
    ...
    get: (id, cb) => {
      ...
      singleOp((db, doc, cb) => {
        db.get(doc._id, (err, val) => err && err.name === 'not_found' ?
          cb(null, undefined) : cb(err, val));
      }, 'read', {
        _id: id
      }, dbopt.partition, pool, cb);
    },
    ...
    allDocs: (opt, cb) => {
      ...
      // Search for docs with keys in the given range
        rangeOp((db, docs, opt, cb) => {
          ...
          _id: opt.endkey
        }], opt, dbopt.partition, pool, (err, rows) => err ? cb(err) : cb(
          null, {
            rows: rows
          }));
      else
      // Search for docs with the given keys
        bulkOp((db, docs, opt, cb) => {
          ...
          db.allDocs(extend(clone(opt), {
            keys: map(docs, (doc) => doc._id)
          }), (err, res) => err ? cb(err) : cb(null, res.rows));

        }, 'read', map(opt.keys, (id) => ({
          _id: id
        })), opt, dbopt.partition, pool, (err, rows) => err ? cb(err) :
        cb(null, {
          rows: rows
        }));
    },
    ...
    bulkDocs: (docs, opt, cb) => {
      ...
      bulkOp((db, docs, opt, cb) => {
        ...
        // selected db partition
        db.bulkDocs(docs, opt, (err, res) => err ? cb(err) : cb(null,
          res));
    ...
    batch_get: (batch, cb) => {
      ...
      batchOp((db, docs, opt, cb) => {
        ...
        return err ? cb(err) : cb(null, map(rows, (row) => {
          if (row.error)
            return row.error === 'not_found' ? [null, undefined] : [
              row.error, undefined
            ];
          if (row.doc === null)
    ...
    batch_put: (batch, cb) => {
      ...
      }, 'write', map(batch, (args) => args[0]), {}, dbopt.partition, pool,
        (err, rows) => {
          return err ? cb(err) : cb(null, map(rows, (row) => {
            return row.error === true ? [row, undefined] : row.error ? [
              row.error, undefined
            ] : [null, row];
          }));
        });
    ...
    batch_remove: (batch, cb) => {
      ...
      }, 'write', map(batch, (args) => extend(clone(args[0]), {
        _deleted: true
