...
const time = (uri) => {
    debug('Extracting time from %s', uri);
    const tk = /^t-(.*)-k-/.exec(uri);
    const t = tk ? tk[1] : /-t-(.*)/.exec(uri)[1];
    debug('Extracted time %s', t);
    return t;
};
...
// multiple segments separated by / as well
const key = (uri) => {
    debug('Extracting key from %s', uri);
    const kt = /^k-(.*)-t-/.exec(uri);
    const k = kt ? kt[1] : /-k-(.*)/.exec(uri)[1];
    debug('Extracted key %s', k);
    return k;
};
...
// db partitions
const dbclient = (ppart, puri, pcons) => {
    const part = ppart ? ppart : partition;
    const uri = puri ? puri : dburi('db');
    const cons = pcons ? pcons : dbcons;
    ...
    const pool = (p, cb) => {
        ...
        return partitions[u] ? cb(null, partitions[u]) : cons(u, (err, db) => {
    ...
    return {
        ...
        allDocs: (opt, cb) => {
            ...
            if(opt.startkey && opt.endkey)
                ...
                }, 'read', [{ _id: opt.startkey }, { _id: opt.endkey }], opt, part, pool, (err, rows) => err ? cb(err) : cb(null, { rows: rows }));
            else
                ...
                }, 'read', map(opt.keys, (id) => ({ _id: id })), opt, part, pool, (err, rows) => err ? cb(err) : cb(null, { rows: rows }));
        },
        ...
        batch_get: (batch, cb) => {
            ...
            }, 'read', map(batch, (args) => ({ _id: args[0] })), opt, part, pool, (err, rows) => {
        ...
        batch_put: (batch, cb) => {
            ...
            }, 'write', map(batch, (args) => args[0]), {}, part, pool, (err, rows) => {
                return err ? cb(err) : cb(null, map(rows, (row) => {
        ...
        batch_remove: (batch, cb) => {
            ...
            }, 'write', map(batch, (args) => extend(clone(args[0]), { _deleted: true })), {}, part, pool, (err, rows) => {
                return err ? cb(err) : cb(null, map(rows, (row) => {
...
const dbcons = (uri, cb) => cb(undefined, /:/.test(uri) ? new PouchDB(uri, { ajax: { rejectUnauthorized: false }}) : new PouchDB(uri, { db: memdown }));
