...
describe('abacus-oauth', () => {
  ...
  describe('authorize', () => {
    ...
    const authorities = ['resource.write', 'resource.read', 'system.write', 'system.read'];
    ...
    it('fails when system scope is missing and resource scopes are empty', () => {
      assertFails(
        ...
        (err) => {
          expect(err)
            .to.be.an('Error')
            .that.has.property('header').that.deep.equals({
              'WWW-Authenticate': 'Bearer realm="cf", error="insufficient_scope", error_description=""'
            });
        }
      );
    });
