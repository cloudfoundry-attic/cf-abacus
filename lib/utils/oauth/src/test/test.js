...
describe('abacus-oauth', () => {
  ...
  it('Get bearer access token with valid scope', (done) => {
    const app = express();
    ...
    oauth.getBearerToken('http://localhost:' + server.address().port,
      'abacus', 'HS256', 'abacus.system.read',
      (err, token) => {
        expect(err).to.equal(undefined);
  ...
  it('Get bearer access token with invalid scope', (done) => {
    const app = express();
