...
const { each, extend } = require('underscore');
...
describe('abacus-oauth', () => {
  ...
  describe('parseTokenScope', () => {
    it('should extract the resource from the scope', () => {
      const token = signToken(
        [],
        ['abacus.usage.test-resource.read', 'abacus.usage.test-resource.write', 'some.other.scope']
      );
    ...
    it('should extract the system scope', () => {
      const token = signToken([], ['abacus.usage.read', 'abacus.usage.write']);
      const scopes = oauth.parseTokenScope(token);
      expect(scopes).not.to.equal(undefined);
    ...
    it('should fail with incorrect scope', () => {
      const token = signToken([], ['some.other.scope']);
      const scopes = oauth.parseTokenScope(token);
      expect(scopes).not.to.equal(undefined);
      expect(scopes.hasSystemReadScope).to.equal(false);
      expect(scopes.hasSystemWriteScope).to.equal(false);
    ...
    it('should extract system read and resource write scopes', () => {
      const token = signToken([], ['abacus.usage.read', 'abacus.usage.test-resource.write']);
      const scopes = oauth.parseTokenScope(token);
      expect(scopes).not.to.equal(undefined);
      expect(scopes.hasSystemReadScope).to.equal(true);
      expect(scopes.hasSystemWriteScope).to.equal(false);
  ...
  describe('validator', () => {
    ...
    it('rejects calls with a missing token', (done) => {
      request.get(
        'http://localhost::port/protected/resource',
        {
      ...
      request.get(
        ...
        (err, val) => {
          ...
          expect(val.headers['www-authenticate']).to.equal(
            'Bearer realm="cf",' + ' error="invalid_token",' + ' error_description="invalid signature"'
          );
          done();
        }
      );
  ...
  describe('authorizer', () => {
    ...
    beforeEach((done) => {
      const app = express();
      app.use(oauth.authorizer(tokenSecret, tokenAlgorithm, ['read', 'write']));
      app.get('/protected/resource', (req, res) => {
        res.send('okay');
      });
      server = app.listen(0, done);
    ...
    it('rejects calls with a token with insufficient scopes', (done) => {
      ...
      request.get(
        ...
        (err, val) => {
          ...
          expect(val.headers['www-authenticate']).to.equal(
            'Bearer realm="cf",' + ' error="insufficient_scope",' + ' error_description="read,write"'
          );
          done();
        }
      );
    ...
    it('allows calls with a token with required scopes', (done) => {
      const token = signToken([], ['read', 'write', 'other']);
      request.get(
        'http://localhost::port/protected/resource',
        {
          port: server.address().port,
  ...
  describe('authorize', () => {
    ...
    const authorities = ['resource.write', 'resource.read', 
    ...
    const createError = (scopes) => ({
      statusCode: 403,
      header: {
        'WWW-Authenticate':
          'Bearer realm="cf", error="insufficient_scope",' +
          ` error_description="${scopes.join(',')}"`
      }
    });
    ...
    it('passes when all requested (single) internal-resource scopes contained', () => {
      oauth.authorize(authHeader, {
        internalResource: ['internal.write']
      });
    });

    it('passes when all requested (multiple) internal-resource scopes contained', () => {
      oauth.authorize(authHeader, {
        internalResource: ['internal.read', 'internal.write']
      });
    });
    ...
    it('fails when malformed token is passed', () => {
      assertFails(
        ...
        (err) => {
          ...
          expect(err).to.have.property('header').that.deep.equals({
    ...
    it('fails when one of resource scopes is missing', () => {
      assertFails(
        ...
        (err) => {
          expect(err).to.be.an('Error');
          const expectedError = createError(['resource.write', 'resource.read','resource.missing']);
          expect(err).to.have.property('statusCode', expectedError.statusCode);
          expect(err).to.have.property('header').that.deep.equals(expectedError.header);
        }
      );
    });

    it('fails when one of internal-resource scopes is missing', () => {
      assertFails(
        () => {
          oauth.authorize(authHeader, {
            internalResource: ['internal.write', 'internal.read', 'internal.missing']
          });
        },
        (err) => {
          expect(err).to.be.an('Error');
          const expectedError = createError(['internal.write', 'internal.read','internal.missing']);
          expect(err).to.have.property('statusCode', expectedError.statusCode);
          expect(err).to.have.property('header').that.deep.equals(expectedError.header);
        }
      );
    ...
    it('fails when system scope is missing and resource scopes are empty', () => {
      assertFails(
        ...
        (err) => {
          ...
          expect(err)
            .to.be.an('Error')
            .that.has.property('header').that.deep.equals(expectedError.header);
        }
      );
    });
  ...
  const checkTokens = (invalidToken, validToken, callsCount, expectedCallsCount, done) => {
    invalidToken.start((err) => {
      expect(err)
        .to.be.an('Error')
        .that.has.property('message', 'Unable to get OAuth token');
      ...
      validToken.start((err) => {
        ...
        if (callsCount) expect(callsCount()).to.equal(expectedCallsCount);
  ...
  const cacheIt = process.env.CACHE === 'false' ? it.skip : it;

  cacheIt('get OAuth bearer access token using cache', (done) => {
    ...
    const invalidToken = oauth.cache('http://localhost:' + server.address().port, 'valid', 'secret', 'invalid');

    const validToken = oauth.cache('http://localhost:' + server.address().port, 'valid', 'secret', 'scopes');
  ...
  it('get OAuth bearer access token using cache directly', (done) => {
    ...
    // Unable to get a token from auth server's token endpoint
    const invalidToken = oauth.cache(
  ...
  it('validate token is started only once', (done) => {
    const app = express();
  ...
  it('get OAuth bearer access token without expires_in', (done) => {
    // Create a test Express application
    const app = express();
  ...
  it('authorization server responds with errors', (done) => {
    ...
    app.get('/v2/info', (req, res) => {
      ...
      if (queries === 1) res.status(404).end();
      else if (queries === 2) res.send({ message: 'returning invalid data' });
      else if (queries === 3)
        res.send({
          token_endpoint: [req.protocol, '://', req.headers.host].join('')
        });
    ...
    each([1, 2, 3, 4], () => {
      const token = oauth.cache('http://localhost:' + server.address().port, 'user', 'secret', 'scope');
    ...
    oauth.getBearerToken(
      ...
      (err, token) => {
        ...
        oauth.getBearerToken(
          ...
          'invalid',
          (err, token) => {
            expect(err).to.be.an('Error').that.has.property('message', 'Unable to get OAuth token');
            expect(token).to.equal(undefined);
            done();
          }
        );
  ...
  context('when using basic authentication strategy', () => {
    ...
    let oauthTokenResponse = {
      access_token: signToken(['abacus.system.read'], ['abacus.system.read']),
      token_type: 'bearer',
      expires_in: 1
    };
    ...
    beforeEach(() => {
      ...
      api.post('/oauth/token', (req, res) => {
        if (typeof oauthTokenResponse === 'number') res.sendStatus(oauthTokenResponse);
        else res.send(oauthTokenResponse);
      });
    ...
    it('should reject when token is malformed', (done) => {
      request.get(
        ...
        (err, val) => {
          ...
          expect(val.statusCode).to.equal(401);
          expect(val.headers['www-authenticate']).to.equal(
            'Basic realm="cf",' + ' error="invalid_token",' + ' error_description="malformed"'
          );
          done();
        }
      );
    ...
    it('should reject when no auth header is provided', (done) => {
      request.get('http://localhost::port/protected/resource', requestOpts(server.address().port), (err, val) => {
        expect(err).to.equal(undefined);
        expect(val.statusCode).to.equal(401);
    ...
    it('should return body with valid auth', (done) => {
      request.get(
        'http://localhost::port/protected/resource',
        requestOpts(server.address().port, authHeader(`Basic ${encodedCredentials}`)),
        (err, val) => {
          expect(err).to.equal(undefined);
          expect(val.statusCode).to.equal(200);
          expect(val.body).to.equal('okay');
    ...
    it('should fail when unauthorized', (done) => {
      ...
      request.get(
        ...
        (err, val) => {
          ...
          expect(val.statusCode).to.equal(401);
          expect(val.headers['www-authenticate']).to.equal(
            'Bearer realm="cf", error="invalid_token",' + ' error_description="jwt malformed"'
          );
          done();
        }
      );
