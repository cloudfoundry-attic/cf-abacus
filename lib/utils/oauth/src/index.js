...
const systemReadScope = 'abacus.system.read';

const requiredSystemScope = {
  system: [systemReadScope]
};

// Return an Express middleware that verifies basic authentication
const basicStrategy = (authServer) => {
  return (req, res, next) => {
    ...
    if (req.headers && req.headers.authorization &&
      ...
      try {
        ...
        getBearerToken(authServer, credentials[0], credentials[1],
          systemReadScope, (err, token) => {
            if(err) {
              edebug('Error while retrieving bearer token due to %o', err);
              res.status(500).end();
              return;
            }
            // FIXME need authorize? we already supply scopes to getBearer?
            try {
              authorize(token, requiredSystemScope);
              next();
            }
            catch (e) {
              edebug('Unable to authorize given access token due to %o', e);
