  ...
  if (cachedEndpoint) {
    ...
    get(apiHostName + '/v2/info', (err, val) => {
      ...
      if (val.statusCode >= 400) {
        debug('%s - %s - has returned an error - response %d' + (val.body ? ' %o' : '%s'),
          val.request.method,
          val.request.path,
          val.statusCode,
          val.body || ''
...
const newToken = (endpoint, path, clientId, secret, scopes, cb) => {
  ...
  post(`${endpoint}/${path}?grant_type=client_credentials` + (scopes ? '&scope=' + encodeURIComponent(scopes) : ''), {
    headers: {
...
const containsRequiredScopes = (expectedScope, scopes) => {
  ...
  if (!scopesMissing(expectedScope.internalResource) && allScopesContained(expectedScope.internalResource, scopes))
    return true;
...
const authorize = (auth, expectedScope) => {
  ...
  if (!containsRequiredScopes(expectedScope, authScopes)) {
    ...
    throw buildError(403,{
      'WWW-Authenticate':
        'Bearer realm="cf", error="insufficient_scope", error_description="'
        + `${expectedScope.resource || expectedScope.internalResource || ''}"`
    });
  }
...
const cache = (authServer, id, secret, scopes, path = 'oauth/token', isAuthServer = false) => {
  ...
  wrapper.start = (cb) => {
    ...
    const getToken = (endpoint) => {
      debug('Getting OAuth bearer access token from endpoint %s for client %s and scopes %s', endpoint, id, scopes);
