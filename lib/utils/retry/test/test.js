...
describe('cf-abacus-retry', () => {
    ...
    it('retries failed calls to functions with callback', () => {
        ...
        let calls = 0;
        const div = function div(x, y, cb) { calls++; return calls < 3 ? cb(err, undefined) : cb(undefined, x / y); };
        const r = retry(div);
        r(3, 2, cb);
        ...
        expect(cb.args).to.deep.equal([[undefined, 1.5]]);
    });

    it('retries failed calls to generators', () => {
    ...
    it('returns an error after 5 retries', () => {
        ...
        const err = new Error('div');
        let calls = 0;
        const div = function div(x, y, cb) { calls++; cb(err, undefined); };
        const r = retry(div);
        r(3, 2, cb);
