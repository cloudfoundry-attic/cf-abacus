...
const options = (retries, min, max, factor, random) => {
  ...
  return {
    retries: def(retries, parseInt(process.env.RETRIES) || 5),
...
const retryfn = (fn, retries, min, max, factor, random) => {
  ...
  const wrapper = extend(
    function() {
      ...
      if (retries === foreverRetries) opt.forever = true;
...
module.exports.forever = foreverRetries;
