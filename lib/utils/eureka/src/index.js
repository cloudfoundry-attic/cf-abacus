...
const enabled = () => process.env.EUREKA;

const server = () => `${uris.eureka}/eureka/v2`;
    ...
    request.post(
      ...
      (err, response) => {
        if (err || (response && response.statusCode !== 204)) {
          edebug(`Couldn't register app ${app} port ${port} uri ${uri} , %o %o`, err, sanitizeResponse(response));
          return;
        }
...
const deregister = (server, app, uri, cb) => {
  ...
  request.delete(
    ...
    (err, val) => {
      if (err || (val && val.statusCode !== 200)) edebug(`Couldn't deregister app ${app} uri ${uri} %o`, err);
      else debug(`Deregistered app ${app} uri ${uri}`);
      cb(err, val);
    }
  );
};
