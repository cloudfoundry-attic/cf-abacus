  ...
  it('registers services in a Eureka registry', (done) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      // Handle the Eureka REST operations
        ...
        req.url === '/eureka/v2/apps/TEST/test.0' && req.method === 'DELETE') {
        res.statusCode = 200;
        res.end('OK');
      }
      else if(
        req.url === '/eureka/v2/apps/TEST/test.0' && req.method === 'GET') {
    ...
    eureka.register(s, 'test', '0', '127.0.0.1', 1234, (err, val) => {
      expect(err).to.equal(undefined);
      cb();
    });

    // Lookup an instance
    eureka.instance(s, 'test', '0', (err, val) => {
      expect(err).to.equal(undefined);
    ...
    eureka.deregister(s, 'test', '0', (err, val) => {
      expect(err).to.equal(undefined);
      cb();
    });
  });
