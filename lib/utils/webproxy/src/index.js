...
const proxy = (targets) => {
  ...
  const urls = map(
    targets ? targets : process.env.TARGETS ? parse(process.env.TARGETS) : ['http://localhost:9080'],
    (t) => url.parse(t)
  );
  const paths = uniq(map(urls, (u) => u.path));
  const routes = map(paths, (p) => ({
    path: p,
    targets: map(filter(urls, (u) => u.path === p), (u) => u.protocol + '//' + u.host)
  }));
  ...
  app.use((req, res, next) => {
    const route = filter(routes, (r) => r.path === req.path.substring(0, r.path.length))[0];
    if (!route) {
    ...
    debug('Proxying request %s %s to %s', req.method, req.url, route.targets[target]);
    proxy.web(req, res, {
      xfwd: true,
...
const runCLI = () => {
  ...
  if (commander.start) {
    ...
    // Stop the Web proxy
    cp.exec('pkill -f "node abacus-webproxy"', (err, stdout, stderr) => {
