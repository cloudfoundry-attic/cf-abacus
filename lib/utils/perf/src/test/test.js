...
describe('abacus-perf', () => {
  ...
  it('performs healthcheck on healthy app', () => {
    ...
    expect(perf.isHealthy(5)).to.equal(true);
  });

  it('performs healthcheck to sick app', () => {
    ...
    expect(perf.isHealthy()).to.equal(false);
  });

  it('performes healthy additional healthcheck provided', () => {
    ...
    // the additional healthChecks failed
    expect(perf.isHealthy(undefined, healthcheck)).to.equal(true);
  });

  it('catch additional healthcheck error', () => {
    ...
    // the additional healthChecks failed
    expect(perf.isHealthy(undefined, healthcheck)).to.equal(false);
  });
});
