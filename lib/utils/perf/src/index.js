...
const countSample = process.env.PERF_COUNT_SAMPLE ? 
  JSON.parse(process.env.PERF_COUNT_SAMPLE) : 
  { granularity: 'seconds', size: 10 };

const getWindow = (sample) => {
  const seconds = 1000;
  ...
  if (!samples[sample.granularity])
    throw new Error('Count sample granularity is not supported: %s', 
      sample.granularity);

  if (!Number.isInteger(sample.size))
    throw new Error('Count sample size is not a valid integer: %s', 
      sample.size);
...
const countWindow = getWindow(countSample);
...
const rollCounts = (buckets, time) => {
  return roll(buckets, time, countWindow.win, countWindow.max, () => ({
    i: 0,
    ok: 0,
    errors: 0,
    timeouts: 0,
