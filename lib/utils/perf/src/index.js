...
const roll = (buckets, time, win, max, proto) => {
  ...
  const all = current.length !== 0 && last(current).i === i ? current :
    current.concat([extend(clone(proto()), {
      i: i
    })]);
  return last(all, max);
};
...
const stats = (name, time, roll) => {
  ...
  return roll !== false ? (() => {
    ...
    return {
      ...
      time: time,
      counts: counts,
      latencies: rollLatencies(s.latencies, time),
      health: rollHealth(s.health, time, counts),
      circuit: s.circuit
    };
  })() : {
    name: name,
...
const accumulateStats =
  (name, time, latency, err, timeout, reject, circuit) => {
    debug('Accumulating stats for function %s', name);
    debug('latency %d, err %s, timeout %d, reject %s, circuit %s', latency, err,
      timeout, reject, circuit);
    ...
    debug('%d ok, %d errors, %d timeouts, %d rejects, %d count buckets', c.ok,
      c.errors, c.timeouts, c.rejects, counts.length);
    ...
    if (!err && !timeout && !reject) {
      ...
      l.latencies = l.latencies.length < 100 ? l.latencies.concat([latency]) :
        l.latencies;
      debug('%d latencies, %d latencies buckets', l.latencies.length, latencies
        .length);
    }
...
const onMessage = (msg) => {
  if (msg.metrics) {
    ...
    if (msg.metrics.call) {
      ...
      accumulateStats(c.name, c.time, c.latency, c.error, c.timeout, c.reject,
        c.circuit);
    }
