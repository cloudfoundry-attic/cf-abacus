...
const countSample = (() => {
  ...
  const sample = process.env.PERF_COUNT_SAMPLE
    ? JSON.parse(process.env.PERF_COUNT_SAMPLE)
    : { granularity: 'seconds', size: 10 };

  if (!samples[sample.granularity])
    throw new Error('Count sample granularity is not supported: %s', sample.granularity);

  if (!Number.isInteger(sample.size)) throw new Error('Count sample size is not a valid integer: %s', sample.size);

  return {
    win: samples[sample.granularity] * sample.size,
    max: sample.size
  };
})();
...
const report = (name, time, latency, err, timeout, reject, circuit) => {
  ...
  const call = () => ({
    name: name,
    time: t,
    latency: latency === undefined ? moment.now() - t : latency,
    error: err || undefined,
    timeout: timeout || 0,
    reject: reject || false,
...
const roll = (buckets, time, win, max, proto) => {
  ...
  const all =
    current.length !== 0 && last(current).i === i
      ? current
      : current.concat([
        extend({}, proto(), { i: i })
      ]);
...
const rollCounts = (buckets, time) => {
  return roll(buckets, time, countSample.win, countSample.max, () => ({
    i: 0,
    ok: 0,
    errors: 0,
    timeouts: 0,
...
const accumulateStats = (name, time, latency, err, timeout, reject, circuit) => {
  ...
  const updateCount = (c) => {
    ...
    debug(
      '%d ok, %d errors, %d timeouts, %d rejects, %d count buckets',
      c.ok,
      c.errors,
      c.timeouts,
      c.rejects,
...
const healthy = (threshold) => {
  ...
  return !find(all(moment.now(), true), (stat) => {
    if(stat.circuit.includes('open')) {
      debug('Found open circuit breaker for stat %s', stat.name);
      return true;
    }
