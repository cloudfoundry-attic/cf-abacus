...
const isHealthy = (threshold, nextCheck) => {
  const window = Date.now();
  const bucket = all(window, false);
  let healthy = true;

  // go through each function call performance
  const culprit = find(bucket, (stat) => {
    let requests, errors, percent;
    requests = errors = percent = 0;
    // go through its health status
    each(stat.health, (health) => {
      requests += health.ok + health.errors;
      errors += health.errors 
    });
    debug('%s: has %d requests and %d errors', stat.name, requests, errors);
    percent = 100 * (errors / requests || 1);
    // if one function call is unhealthy, the app is not healthy.
    return percent > (threshold || 5);
  });

  if (culprit) {
    debug('%s exceeds the acceptable failure threshold', culprit.name);
    healthy = false;
  }
  else if (nextCheck)
    // performs additional healthcheck unrelated to metrics performance
    try {
      healthy = nextCheck(); // expects boolean method
      debug('additional healthchecks return ', healthy);
    }
    catch (error) {
      healthy = false;
      debug('error encountered when performing additional healthchecks');
    }
