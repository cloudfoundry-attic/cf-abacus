...
describe('cf-abacus-perf', () => {
    it('accumulates function call stats', () => {
        ...
        expect(on.args[3]).to.deep.equal([{ metrics: { stats: m }}]);
    });
    ...
    it('stores received function call stats', () => {
        ...
        const i = (w) => Math.ceil(t / w);
        const m = { name: 'foo', time: t,
            counts: [{ i: i(1000), ok: 10, errors: 10, timeouts: 10, rejects: 10 }],
            latencies: [{ i: i(10000), latencies: [100] }],
            health: [{ i: i(500), ok: 10, errors: 30 }],
            circuit: 'closed' };
        e.emit('message', { metrics: { stats: m }});
    ...
    it('resets function call stats', () => {
        ...
        const i = (w) => Math.ceil(t / w);
        const m = { name: 'foo', time: t,
            counts: [{ i: i(1000), ok: 0, errors: 0, timeouts: 0, rejects: 0 }],
            latencies: [{ i: i(10000), latencies: [100, 10] }],
            health: [{ i: i(500), ok: 0, errors: 0 }],
            circuit: 'closed' };
        expect(perf.stats('foo', t)).to.deep.equal(m);
    });
});
