...
// Small utility that computes real time function call performance and
// reliability metrics. The stats are computed using rolling windows of call
// successes, failures, timeouts, circuit breaker rejections and latencies.
...
// Setup debug log
const debug = require('cf-abacus-debug')('cf-abacus-perf');
...
const onMessage = (msg) => {
    if(msg.metrics) {
        ...
        if(msg.metrics.call) {
            ...
            accumulateStats(c.name, c.time, c.latency, c.error, c.timeout, c.reject, c.circuit);
        }
...
const calls = events.emitter('cf-abacus-perf/calls');
calls.on('message', onMessage);

// Report function call metrics
const report = (name, time, latency, err, timeout, reject, circuit) => {
    calls.emit('message', { metrics: { call: { name: name, time: time, latency: latency, error: err, timeout: timeout, reject: reject, circuit: circuit }}});
};
...
const roll = (buckets, time, win, max, proto) => {
    ...
    const all = current.length !== 0 && last(current).i === i ? current : current.concat([extend(clone(proto()), { i: i })]);
    return last(all, max);
};

// Return a rolling window of 10 secs of counts
const rollCounts = (buckets, time) => {
    return roll(buckets, time, 10000, 10, () => ({ i: 0, ok: 0, errors: 0, timeouts: 0, rejects: 0 }));
};

// Return a rolling window of 60 secs of latencies
const rollLatencies = (buckets, time) => {
    return roll(buckets, time, 60000, 6, () => ({ i: 0, latencies: [] }));
};
...
const rollHealth = (buckets, time, counts) => {
    ...
    const b = reduce(counts, (a, c) => ({ ok: a.ok + c.ok, errors: a.errors + c.errors + c.timeouts + c.rejects }), { ok: 0, errors: 0 });

    // Roll the window and store the computed health in the last bucket
    const health = roll(buckets, time, 2000, 4, () => ({ i: 0, ok: 0, errors: 0 }));
...
const stats = (name, time, roll) => {
    const s = accumulatedStats[name] || { name: name, time: time, counts: [], latencies: [], health: [], circuit: 'closed' };
    /* eslint no-extra-parens: 1 */
    return roll !== false ? (() => {
        const counts = rollCounts(s.counts, time);
        return { name: name, time: time, counts: counts, latencies: rollLatencies(s.latencies, time), health: rollHealth(s.health, time, counts), circuit: s.circuit };
    })() : {
        name: name, time: time, counts: s.counts, latencies: s.latencies, health: s.health, circuit: s.circuit
    };
};
...
const reset = (name, time) => {
    ...
    /* eslint no-extra-parens: 1 */
    const astats = ((s) => {
        return s ? { name: name, time: time, counts: [], latencies: s.latencies, health: [], circuit: 'closed' } :
            { name: name, time: time, counts: [], latencies: [], health: [], circuit: 'closed' };
    ...
    emitter.emit('message', { metrics: { stats: astats }});
    return astats;
};

// Return all the accumulated stats
const all = (time, roll) => {
    return map(keys(accumulatedStats), (k) => stats(k, time, roll));
};

// Process function call metrics and update accumulated call stats
const accumulateStats = (name, time, latency, err, timeout, reject, circuit) => {
    debug('Accumulating stats for function %s', name);
    debug('latency %d, err %s, timeout %d, reject %s, circuit %s', latency, err, timeout, reject, circuit);

    // Retrieve the current call stats for the given function
    const s = stats(name, time, false);

    // Compute up to date counts window and increment counts in last bucket
    ...
    debug('%d ok, %d errors, %d timeouts, %d rejects, %d count buckets', c.ok, c.errors, c.timeouts, c.rejects, counts.length);

    // Compute up to date latencies window and add latency to last bucket, up to
    // max bucket size
    const latencies = rollLatencies(s ? s.latencies : [], time);
    if(!err && !timeout && !reject) {
        const l = last(latencies);
        l.latencies = l.latencies.length < 100 ? l.latencies.concat([latency]) : l.latencies;
        debug('%d latencies, %d latencies buckets', l.latencies.length, latencies.length);
    }
    ...
    // Warning: mutating variable accumulatedStats
    const astats = { name: name, time: time, counts: counts, latencies: latencies, health: health, circuit: circuit };
    accumulatedStats[name] = astats;
