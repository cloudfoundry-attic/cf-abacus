...
const callback = (name, err, val, time, timeout, reject, circuit, cb) => {
  if (err) edebug('Error in breaker: %j, for metric: %s, with err.nobreaker: %j', err, name, err.nobreaker);

  // Report function call metrics
  const now = moment.now();
  perf.report(name, now, now - time, err && err.nobreaker ? undefined : err, timeout, reject, circuit);

  // Call back
  return timeout
    ? cb(new TimeoutError(name, timeout), undefined)
    : reject ? cb(new CircuitBreakerError(name), undefined) : cb(err, val);
};
    ...
    return fn.apply(
      ...
      args.concat([
        (err, val) => {
          ...
          } else debug('Function %s calling back with success', name);
...
// well.
const breaker = (fn, timeout, threshold, errors, reset) =>
  extend(
