...
const TimeoutError = function(name, timeout) {
  ...
  this.message = util.format('Function %s timed out after %d ms', name,
    timeout);
};
...
const call = (name, fn, args, time, opt, circuit, cb) => {
  ...
  const tid = setTimeout(() => {
    ...
    return callback(name, undefined, undefined, time, opt.timeout, false,
      circuit, cb);
  }, opt.timeout);
  ...
  try {
    return fn.apply(undefined, args.concat([(err, val) => {
      ...
      // Call back
      debug('Function %s calling back with %s', name, err ? 'error' :
        'success');
...
const breakerfn = (fn, timeout, threshold, errors, reset) => {
  ...
  const wrapper = function() {
    ...
    if (circuit === 'open' && time - optime > opt.reset) {
      ...
      circuit = 'half-open';
      return call(name, fcb, initial(arguments), time, opt, circuit,
        (err, val) => {
...
const breaker = (fn, timeout, threshold, errors, reset) => extend(
  ...
  object(pairs(fn)),
  object(map(functions(fn), (k) => [k, breakerfn(nbind(fn, k), timeout,
    threshold, errors, reset)])));
