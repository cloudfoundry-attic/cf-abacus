...
describe('cf-abacus-breaker', () => {
    ...
    it('can call a regular function taking a callback', () => {
        ...
        const div = function div(x, y, cb) { return x ? y ? cb(undefined, x / y) : cb(err, undefined) : (() => { throw err; })(); };
        const b = breaker(div);
        b(3, 2, cb);
        b(3, 0, cb);
        b(0, 0, cb);
        ...
        expect(cb.args).to.deep.equal([[undefined, 1.5], [err, undefined], [err, undefined]]);
    });

    it('can call a generator', () => {
        // Call a generator
        const cb = spy();
        const err = new Error('div');
        const div = function div(x, y, cb) { return x ? y ? cb(undefined, x / y) : cb(err, undefined) : (() => { throw err; })(); };
        ...
        // Expect the callback to be passed the normal result or error
        expect(cb.args.length).to.equal(3);
        expect(cb.args).to.deep.equal([[null, 1.5], [err, undefined], [err, undefined]]);
    });
    ...
    it('can converts all functions in a module', () => {
        ...
        // Expect the breaker function to contain the name of the wrapped function
        expect(b.div.fname).to.equal('div');
    ...
    it('handles function call timeouts', () => {
        ...
        const cb = spy();
        const sum = function sum(x, y, cb) { setTimeout(() => { cb(undefined, x + y); }, 20000); };
        const b = breaker(sum);
        b(3, 2, cb);
    ...
    it('fails fast after several errors', () => {
        ...
        const div2 = function div2(x, y, cb) { return y ? cb(undefined, x / y) : cb(err, undefined); };
        ...
        expect(cb.args).to.deep.equal([[err, undefined], [err, undefined], [err, undefined], [cerr, undefined], [cerr, undefined]]);
    });
    ...
    it('resets after some time then closes', () => {
        ...
        const div3 = function div3(x, y, cb) { return y ? cb(undefined, x / y) : cb(err, undefined); };
        ...
        expect(cb.args).to.deep.equal([[err, undefined], [err, undefined], [err, undefined], [cerr, undefined], [undefined, 1], [undefined, 1]]);
    });
    ...
    it('resets after some time then reopens', () => {
        ...
        const div4 = function div4(x, y, cb) { return y ? cb(undefined, x / y) : cb(err, undefined); };
        ...
        expect(cb.args).to.deep.equal([[err, undefined], [err, undefined], [err, undefined], [cerr, undefined], [err, undefined], [cerr, undefined]]);
    });
});
