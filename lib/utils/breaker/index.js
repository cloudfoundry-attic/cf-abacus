...
// Setup debug log
const debug = require('cf-abacus-debug')('cf-abacus-breaker');
...
const options = (callTimeout, callThreshold, errorPercentage, resetTimeout) => {
    return { timeout: callTimeout || 5000, threshold: callThreshold || 10, errors: errorPercentage || 50, reset: resetTimeout || 5000 };
};

// An error used to report a timeout
const TimeoutError = function(name, timeout) {
    Error.captureStackTrace(this);
    this.message = util.format('Function %s timed out after %d ms', name, timeout);
};
...
const CircuitBreakerError = function(name) {
    Error.captureStackTrace(this);
    this.message = util.format('Function %s failed-fast due to open circuit-breaker', name);
};
...
const callback = (name, err, val, time, timeout, reject, circuit, cb) => {
    ...
    const now = Date.now();
    perf.report(name, now, now - time, err, timeout, reject, circuit);

    // Call back
    return timeout ? cb(new TimeoutError(name, timeout), undefined) : reject ? cb(new CircuitBreakerError(name), undefined) : cb(err, val);
};
...
const call = (name, fn, args, time, opt, circuit, cb) => {
    ...
    const tid = setTimeout(() => {
        ...
        return callback(name, undefined, undefined, time, opt.timeout, false, circuit, cb);
    }, opt.timeout);
    ...
    try {
        ...
        debug('Function %s calling back with exception', name);
    ...
    const wrapper = function() {
        ...
        if(circuit === 'closed') {
            ...
            if(calls < opt.threshold || errors < opt.errors)
                return call(name, fn, initial(arguments), time, opt, circuit, cb);

            // Open the circuit when we reach the circuit max error rate
            // (we are above the volume threshold and error percentage)
            debug('Tripping circuit breaker for function %s, after %d calls and %d\% errors', name, calls, errors);
            circuit = 'open';
            optime = time;
        }
        if(circuit === 'open' && time - optime > opt.reset) {
            ...
            circuit = 'half-open';
            return call(name, fn, initial(arguments), time, opt, circuit, (err, val) => {
        ...
        return callback(name, undefined, undefined, time, 0, true, circuit, cb);
    };
...
const breaker = (fn, timeout, threshold, errors, reset) => extend(
    ...
    object(pairs(fn)),
    object(map(functions(fn), (k) => [k, breakerfn(nbind(fn, k), timeout, threshold, errors, reset)])));
