...
describe('abacus-schema', () => {
    ...
    it('validate a data object that does not match a schema', () => {
        ...
        expect(res.send.args[0]).to.deep.equal([[
            { field: 'data.a', message: 'is required', value: { b: 1.23, c: 'Z', d: [1, 'x'], y: 1 } },
            { field: 'data', message: 'has additional properties', value: 'data.y' },
            { field: 'data.b', message: 'is the wrong type', value: 1.23 },
            { field: 'data.c', message: 'must be an enum value', value: 'Z' },
            { field: 'data.d.1', message: 'is the wrong type', value: 'x' }
        ]]);
    });
});
