...
describe('abacus-schema', () => {
  ...
  it('converts a schema to a GraphQL schema', () => {
    const gql = schema.graphql(objectType('doc', {
      a: required(string()),
      b: required(time()),
      c: enumType('xy', ['X', 'Y'], 'Y'),
      d: required(arrayOf(number()))
    ...
    const obj = new _g.GraphQLObjectType({
      name: 'doc',
      description: undefined,
      fields: () => ({
        a: {
          type: new _g.GraphQLNonNull(_g.GraphQLString)
        },
        b: {
          type: new _g.GraphQLNonNull(_g.GraphQLInt)
        },
        c: {
          type: new _g.GraphQLEnumType({
            name: 'xy',
            description: undefined,
            values: {
              X: {
        ...
        d: {
          type: new _g.GraphQLNonNull(new _g.GraphQLList(_g.GraphQLFloat))
        }
      })
    });

    expect(gql.name).to.equal(obj.name);
    expect(gql.getFields()).to.deep.equal(obj.getFields());
  });
});
