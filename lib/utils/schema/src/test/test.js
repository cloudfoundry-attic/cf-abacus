...
describe('abacus-schema', () => {
  ...
  it('converts a schema to a GraphQL schema', () => {
    ...
    const ugraph = schema.graph(unionType('udoc', [
      objectType('type1', { desc: string() }),
      objectType('type2', { desc2: string() })
    ]));
    ...
    const uobj = new graphql.GraphQLUnionType({
      ...
      types: [
        new graphql.GraphQLObjectType({
          name: 'type1',
          fields: () => ({
            desc: graphql.GraphQLString
          })
        }),
        new graphql.GraphQLObjectType({
          name: 'type2',
          fields: () => ({
            desc2: graphql.GraphQLString
          })
        })
      ]
    });
