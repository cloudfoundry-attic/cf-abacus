...
const schema = (t) => {
  return {
    ...
    objectType: (t) => ({
      ...
      description: t.description,
      type: 'object',
      properties: object(map(pairs(t.properties), (p) => [p[0], schema(p[1])])),
      required: map(filter(pairs(t.properties), (p) => p[1].required), (p) => p[0]),
...
const validateFunction = (code, eslintConfigFile, errors) => {
  if (process.env.VALIDATE_FUNCTIONS !== 'true') return true;
  const result = lint(code, eslintConfigFile);
  if (!result.ok) result.errors.forEach((error) => errors.push(error));
  return result.ok;
};
