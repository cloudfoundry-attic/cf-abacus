...
const dbpartition = (n) => partition.partitioner(partition.bucket, period,
  forward(n), partition.balance);
...
const db = (dbserver, name) => dbclient(dbpartition(4), dbclient.dburi(dburi(
  dbserver, name)));

// Return a handle to a LRU cache
const cache = (name) => dbclient(dbpartition(1), dbclient.dburi(name),
  (uri, cb) => dbclient.dbcons(uri, {
    db: lrudown
  }, cb));
