...
describe('abacus-aggregation-db', () => {
  ...
  it('distributes keys over several partitions', () => {
    ...
    p('Hello', Date.parse('Thu Nov 06 2014 19:06:54 GMT-0800 (PST)'),
      'read', (err, val) => {
    ...
    p('Blah', Date.parse('Mon Oct 06 2014 19:06:54 GMT-0800 (PST)'),
      'read', (err, val) => {
        expect(err).to.equal(undefined);
        expect(val).to.deep.equal([3, 201410]);
      });
    p('Awwww', Date.parse('Thu Nov 06 2014 19:06:54 GMT-0800 (PST)'),
      'read', (err, val) => {
        expect(err).to.equal(undefined);
  ...
  it('distributes db ops over several db partitions', (done) => {
    ...
    const clean = (done) => {
      ...
      aggrdb.get(dbclient.kturi('Hello', Date.parse(
        ...
        doc ? aggrdb.remove(doc, cb) : cb(err, doc));
      aggrdb.get(dbclient.kturi('Hello', Date.parse(
    ...
    const remove = (done) => {
      ...
      // Expect no errors
      aggrdb.remove(hellodoc, (err, doc) => cb(expect(err).to.equal(
        null)));
      aggrdb.remove(hello2doc, (err, doc) => cb(expect(err).to.equal(
        null)));
      aggrdb.remove(heydoc, (err, doc) => cb(expect(err).to.equal(
        null)));
    };
