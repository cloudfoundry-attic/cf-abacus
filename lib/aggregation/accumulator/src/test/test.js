...
describe('abacus-usage-accumulator', () => {
  ...
  it('usage older than the slack is rejected', (done) => {
    ...
    usage[0].metered_usage = [
      { metric: 'memory', quantity: { consuming: 3, since: 1451952000000 } }];
    usage[1].metered_usage = [
      { metric: 'memory', quantity: { consuming: 2, since: 1453593600000 } }];
    usage[2].metered_usage = [
      { metric: 'memory', quantity: { consuming: 1, since: 1453680000000 } }];

    const first = [null,
      {
        quantity: {
          current: { consumed: 0, consuming: 2, since: 1453593600000 }
        },
        cost: { burned: 0, burning: 0.00028, since: 1453593600000 }
      },
      null
    ];
    const last = [null, null, null, null,
      {
        quantity: {
          current: { consumed: 0, consuming: 1, since: 1453680000000 }
        },
        cost: { burned: 0, burning: 0.00014, since: 1453680000000 }
      }
    ];
    ...
    post(usage[0], null, () => post(usage[1], null,
      () => {
