  ...
  it('accumulates usage over time', function(done) {
    ...
    const usage = [
      ...
      },
      {
        ...
        start: Date.UTC(now.getUTCFullYear(),
          ...
          now.getUTCMonth(), now.getUTCDate(), 2),
        plan_id: 'basic',
        region: 'us',
        organization_id: 'a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf27',
        space_id: 'aaeae239-f3f8-483c-9dd0-de5d41c38b6a',
      ...
      },
      {
        ...
        start: Date.UTC(now.getUTCFullYear(),
          ...
          now.getUTCMonth(), now.getUTCDate(), 3),
        plan_id: 'standard',
        region: 'us',
        organization_id: 'a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf27',
        space_id: 'aaeae239-f3f8-483c-9dd0-de5d41c38b6a',
      ...
      },
      {
        ...
        resource_instance_id: '0b39fa70-a65f-4183-bae8-385633ca5c87',
        start: Date.UTC(now.getUTCFullYear(),
          now.getUTCMonth(), now.getUTCDate() - 1, 2),
        end: Date.UTC(now.getUTCFullYear(),
          now.getUTCMonth(), now.getUTCDate() - 1, 3),
        plan_id: 'standard',
        region: 'us',
        organization_id: 'a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf27',
        space_id: 'aaeae239-f3f8-483c-9dd0-de5d41c38b6a',
    ...
    postspy = (reqs, cb) => {
      ...
      if(val.body.plan_id === 'standard')
        try {
          const sexpecta = [[0], [0], [0],
            [{ current: 0.01 }, { current: 0.01 }, 0], []
          ];
          const sexpectb = [[0], [0], [0],
            [
          ...
          if(now.getUTCDate() === 1) {
            sexpecta[4].push({ current: 0.01 });
            sexpecta[4].push({ current: 0.01 });
            sexpectb[4].push({ current:
              { consumed: 0, consuming: 8, since: 1420246800000 } });
            sexpectb[4].push({ current:
              { consumed: 0, consuming: 4, since: 1420246500000 } });
          }
          else {
            sexpecta[4].push({ current: 0.02, previous: 0.01 });
            sexpecta[4].push(0);
            sexpectb[4].push({ current:
          ...
          expect(val.body.accumulated_usage[1].quantity)
            .to.deep.equal(sexpecta);
          expect(val.body.accumulated_usage[3].quantity)
            .to.deep.equal(sexpectb);
