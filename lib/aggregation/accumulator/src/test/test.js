...
describe('abacus-usage-accumulator', () => {
  it('accumulates usage over time', function(done) {
    ...
    const clean = (done) => {
      ...
      map(usage, (u) => {
        logdb.get(dbclient.kturi([u.organization_guid, u.service_instance_id]
          .join('-'), [day(u.end), u.start, u.end].join('-')), (
          err, doc) => doc ? logdb.remove(doc, cb) : cb(err,
          doc));
        accumdb.get(dbclient.kturi([u.organization_guid, u.service_instance_id]
          .join('-'), day(u.end)), (err, doc) => {
            if (doc) accumdb.get(doc.last_accumulated_usage_id, (
                err, doc) => doc ? accumdb.remove(doc, cb) :
              cb(err, doc));
            else cb(err, doc);
    ...
    const checkaggr = (done) => {
      setTimeout(() => {
        ...
        expect(reqmock.noWaitPost.args[0][0]).to.equal(
          'http://localhost:9202/v1/metering/accumulated/usage'
        );
    ...
    const get = (done) => {
      ...
      map(usage, (u) => request.get(locations[u.id], {}, (err, val) => {
        ...
        if (val.body.usage_id === '332') {
          expect(val.body.accumulated_usage[1].quantity).to.equal(
            0.01);
          expect(val.body.accumulated_usage[1].delta).to.equal(
            0.01);
        }
