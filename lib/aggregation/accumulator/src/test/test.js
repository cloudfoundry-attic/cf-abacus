...
const usageTemplate = (u, s, e, p, mp, rp, pp, st, lapi, hapi, mem, pmem) => ({
  ...
  prices: {
    metrics: [
      { name: 'storage', price: selectprice(pp) ? 1 : 0.5 },
      {
...
describe('abacus-usage-accumulator', () => {
  ...
  it('accumulates usage over time', (done) => {
    ...
    const usage = [
      ...
      // 332
      // usage 332 comes as a duplicate of usage 330 and should be skipped
      usageTemplate(
        2,
        time(0),
        time(0),
      ...
      // 333
      // usage 333 and 334 represent usage for two consecutive days for a different plan than the first set
      usageTemplate(
        3,
        time(2),
        time(2),
      ...
      // 335
      // usage 335 represents usage for a different consumer, but otherwise same as usage 333;
      // should not be skipped as duplicate but accumulated separately
      extend(
        usageTemplate(
          5,
          time(2),
      ...
      // 336
      // the same as 335 but with different resource_id; should not be detected as duplicate and accumulated
      extend(
        usageTemplate(
          6,
          time(2),
    ...
    // 3 & 4: accumulated differently. Summary: 4 * 24 ~> 96
    // 5: accumulated differently. Summary: 40 * 0 ~> 0
    const expected = [
      {
    ...
    const verify = (secured, done) => {
      ...
      const app = accumulator(systemToken);

      // Listen on an ephemeral port
      const server = app.listen(0);

      debug('accumulate over time: verify -> accumulator started on port %d', server.address().port);
      ...
      const check = () => {
        if (++checks === 7) {
          ...
          expect(unmatched).to.equal(3); // Only check final accumulated value
      ...
      postspy = (reqs, cb) => {
        debug('accumulate over time: verify -> Posted new accumulated usage %o', reqs);

        map(reqs, (req) => {
          // Expect accumulated usage to be posted to the aggregator service
          expect(req[0]).to.equal('http://localhost:9300/v1/metering/accumulated/usage');

          // Expect accumulated values
          const val = req[1];
          try {
            debug('accumulate over time: verify -> ' + 'verify accumulated usage[1] %o', val.body.accumulated_usage[1]);
            expect(expected).to.deep.include(val.body.accumulated_usage[1]);

            debug('accumulate over time: verify -> ' + 'verify accumulated usage[3] %o', val.body.accumulated_usage[3]);
            expect(expected).to.deep.include(val.body.accumulated_usage[3]);

            debug('accumulate over time: verify -> ' + 'verified accumulated usage');
            matched++;
          } catch (e) {
            unmatched++;
            debug('accumulate over time: verify -> ' + 'unable to verify accumulated usage %o', e);
          }
      ...
      transform.reduce(
        ...
        (a, u, i, l, cb) => {
          ...
          const uval = extend({}, u, {
            resource_instance_id: ['0b39fa70-a65f-4183-bae8-385633ca5c87', secured ? 1 : 0].join('-')
          });
          ...
          request.post(
            ...
            (err, val) => {
              debug('accumulate over time: verify -> post response error %o, response %o', err, val);
              ...
              if (i === 2) {
                ...
                debug('accumulate over time: verify -> get metered usage from %s', val.headers.location);
                brequest.get(val.headers.location, {}, (err, val) => {
                  ...
                  expect(omit(val.body, 'id', 'processed', 'processed_id')).to.deep.equal(uval);
                  debug('accumulate over time: verify finished');
                  done();
                });
              }
  ...
  it('does not perform time-based accumulation across a window boundary', (done) => {
    debug('window boundary: starting test ...');
    let records = 0;
    // Set the clock to 2015-10-31:02:00:00
    clock = sinon.useFakeTimers({
      now: Date.UTC(2015, 9, 31, 2),
      toFake: ['Date']
    });

    // Set up usage for the time-based metric
    // Start at October 30 - 17:00 until Nov 1 - 14:00
    // Running summary for October should give 23 * 2 = 46
    // Summary for November should give 24
    const usage = [
    ...
    debug('window boundary: accumulator started on port %d', server.address().port);

    const postNov = () => {
      // Set the clock to 2015-11-01:23:00:00
      clock.restore();
      clock = sinon.useFakeTimers({
        now: Date.UTC(2015, 10, 1, 23),
        toFake: ['Date']
      });
    ...
    postspy = (reqs, cb) => {
      ...
      if (records === 1) {
        map(reqs, (req) => {
          ...
          expect(req[1].body.accumulated_usage[0].windows).to.deep.equal(expectedOctober);
        });
        debug('window boundary: checked November usage');
        // Post the November usage
        postNov();
      }
      if (records === 3) {
        map(reqs, (req) => {
          debug('window boundary: verifying accumulated usage %o', req);
          expect(req[1].body.accumulated_usage[0].windows).to.deep.equal(expectedNovember);
        });
  ...
  it('usage out of the slack is rejected', (done) => {
    ...
    // 1 gb running after. 168 until the end of month.
    ...
    usage[0].metered_usage = [
      {
        ...
        quantity: {
          consuming: 10,
          previous_consuming: 0,
          since: 1451520000000 // December 31, 2015 12:00:00 AM
        }
      }
    ];
    usage[1].metered_usage = [
      {
        ...
        quantity: {
          consuming: 2,
          previous_consuming: 0,
          since: 1453593600000 // January 24, 2016 12:00:00 AM
        }
      }
    ];
    usage[2].metered_usage = [
      {
        ...
        quantity: {
          consuming: 1,
          previous_consuming: -2,
          since: 1453680000000 // January 25, 2016 12:00:00 AM
        }
      }
    ];
    ...
    postspy = (reqs, cb) => {
      records++;
      if (records === 1) expect(reqs[0][1].body.accumulated_usage[0].windows[3]).to.deep.equal(first);
      if (records === 2) {
        expect(reqs[0][1].body.accumulated_usage[0].windows[3]).to.deep.equal(last);
        done();
      }
      cb(undefined, [[undefined, { statusCode: 201 }]]);
    };
    ...
    const post = (usage, slack, scode, done) => {
      ...
      if (slack) process.env.SLACK = slack;
      ...
      debug('slack rejection: accumulator started on port %d', server.address().port);
    ...
    post(usage[0], null, 409, () =>
      post(usage[1], null, 201, () => {
        clock = sinon.useFakeTimers({
          now: Date.UTC(2016, 0, 29, 2), // 2016-01-29
          toFake: ['Date']
        });
        post(usage[2], '5D', 201);
      })
  ...
  it('attaches error and reason when there is a business error', (done) => {
    ...
    // Returns 2015-01-03 with the given hour for testing
    const time = (h) => Date.UTC(2016, 0, 25, h);
    ...
    const usage = {
      ...
      resource_instance_id: '0b39fa70-a65f-4183-bae8-notfoundc875',
      start: time(0),
      end: time(0),
      plan_id: 'error',
      organization_id: 'a3d7fe4d-3cb1-4cc3-a831-notfoundcf27',
      space_id: 'aaeae239-f3f8-483c-9dd0-notfoundb6a',
      consumer_id: 'external:bbeae239-f3f8-483c-9dd0-notfoundbab',
    ...
    postspy = (reqs, cb) => cb(undefined, [[undefined, { statusCode: 201 }]]);
  ...
  it('takes error and reason from the sink and returns it', (done) => {
    ...
    postspy = (reqs, cb) => {
      expect(reqs[0][0]).to.equal('http://localhost:9300/v1/metering/accumulated/usage');
      expect(
        omit(reqs[0][1].body, 'id', 'processed', 'processed_id', 'accumulated_usage', 'metered_usage_id')
      ).to.deep.equal(omit(usage, 'measured_usage', 'metered_usage'));
