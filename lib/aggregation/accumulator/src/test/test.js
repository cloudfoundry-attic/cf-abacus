...
describe('abacus-usage-accumulator', () => {
  ...
  it('usage older than the slack is rejected', (done) => {
    ...
    // Set up usage for the time-based metric
    const usage = usageTemplate(20, 1445299200000, 1445299200000, 'basic',
      'basic-test-metering-plan', 'basic-test-rating-plan',
      'test-pricing-basic', 0, 0, 0, 2);
    usage.metered_usage = [
      { metric: 'memory', quantity: { consuming: 2, since: 1446249600000 } }];
    ...
    postspy = (reqs, cb) => {
      throw new Error('postspy should not have been called');
    };

    // Post the old usage
    debug('slack rejection: starting usage POST requests ...');
    request.post('http://localhost::p/v1/metering/metered/usage', {
      p: server.address().port,
      body: usage
    }, (err, val) => {
      ...
      // Wait 3 seconds for the postspy to be called or not
      setTimeout(() => {
        done()
      }, 3000);
    });
