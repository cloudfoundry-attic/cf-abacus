...
describe('abacus-usage-accumulator', () => {
    ...
    const expected = [
      {
        ...
        windows: [
          ...
          [null],
          [null],
          [{ quantity: { previous: 0.03, current: 0.06 }, cost: 0.0018 }, null, null],
          [{ quantity: { previous: 0.03, current: 0.06 }, cost: 0.0018 }, null]
        ]
      },
      {
        metric: 'memory',
      ...
      },
      {
        ...
        windows: [
          ...
          [null],
          [null],
          [{ quantity: { current: 0.01 }, cost: 0.0004 }, { quantity: { current: 0.01 }, cost: 0.0004 }, null],
          [{ quantity: { previous: 0.01, current: 0.02 }, cost: 0.0008 }, null]
        ]
      },
      {
        metric: 'memory',
      ...
      },
      {
        ...
        windows: [
          ...
          [null],
          [null],
          [{ quantity: { current: 1 }, cost: 0.04 }, null, null],
          [{ quantity: { current: 1 }, cost: 0.04 }, null]
        ]
      },
      {
        metric: 'memory',
  ...
  it('performs time-based accumulation across a window boundary', (done) => {
    ...
    const expected = [[null], [null], [null], [currDay, prevDay, null], [currMonth, prevMonth]];
  ...
  it('attaches error and reason when there is a business error', (done) => {
    ...
    request.post(
      ...
      (err, val) => {
        ...
        const sanitizedError = omit(err, 'headers');
        expect(sanitizedError).to.deep.equal(extend(new Error('emplannotfound'), {
          code: 500,
          statusCode: 500,
          error: 'emplannotfound',
          reason: 'Metering plan with id invalidMeteringPlan not found',
          cause: {
            statusCode: 404
          }
        }));
  ...
  it('takes error and reason from the sink and returns it', (done) => {
    ...
    request.post(
      ...
      (err, val) => {
        expect(val).to.equal(undefined);
        const sanitizedError = omit(err, 'headers');
        expect(sanitizedError).to.deep.equal(extend(new Error('esink'), {
          code: 500,
          statusCode: 500,
          error: 'esink',
          reason: [
