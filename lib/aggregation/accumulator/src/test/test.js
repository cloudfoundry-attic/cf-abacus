...
describe('abacus-usage-accumulator', () => {
  it('accumulates usage over time', function(done) {
    ...
    const checkaggr = (done) => {
      setTimeout(() => {
        // Expect usage docs to have been posted to the aggregator service
    ...
    const get = (done) => {
      ...
      transform.map(usage, (u, i, l, cb) =>
        request.get(locations[u.id], {}, (err, val) => {
          ...
          expect(['332', '333', '334', '335']).to.include(
            val.body.normalized_usage_id);
          ...
          // Expect to have seen accumulated values for the test plans
          expect(basic).to.equal(true);
          expect(standard).to.equal(true);
          done();
        });
    };
