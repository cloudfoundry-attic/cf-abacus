...
const accumulate = function*(accums, usage) {
  ...
  const slackCutoff = moment.utc().startOf('month').add(slackLimit, 'milliseconds');
  if (usage.end <= lastMonth.valueOf() && now > slackCutoff.valueOf()) {
  ...
  const newa = extend({}, amerge, {
    accumulated_usage: map(usage.metered_usage, (meteredUsage) => {
      ...
      let accumulateWindow = accum(amerge.accumulated_usage, meteredUsage.metric);
