...
const { getReducerConfig } = require('abacus-dataflow-reducer-config-factory');
...
const accumulator = (token) => {
  ...
  const reducer = dataflow.reducer(accumulate, getReducerConfig('accumulator', secured, sampling, token));
