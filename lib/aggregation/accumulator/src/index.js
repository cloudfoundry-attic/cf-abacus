...
const ratingcb = require('abacus-rating-config');
...
const rconfig = yieldable(ratingcb);
...
const accumulate = function *(a, u) {
  ...
  const mconf = yield config(u.metering_plan_id, systemToken && systemToken());

  // Retrieve the rating config
  const rconf = yield rconfig(u.rating_plan_id,
    systemToken && systemToken());

  const plan = filter(mconf.plans, (p) => p.plan_id === u.plan_id)[0];
