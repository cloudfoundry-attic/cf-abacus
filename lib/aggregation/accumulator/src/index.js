...
const accumdb = yieldable(retry(breaker(db(uris.couchdb,
  'abacus-accumulated-usage'))));
...
const accumulators = (sid) => (config(sid) || {
  aggregations: []
}).aggregations;
...
const accumulate = (a, u) => {
  ...
  const newa = extend(clone(amerge), {
    accumulated_usage: map(zip(accumulators(u.service_id),
      amerge.accumulated_usage,
      u.metered_usage), (r) => {
...
const accumulateUsage = function *(u) {
  ...
  yield post((yield aggreguri(alogdoc.organization_guid, alogdoc.start)) +
