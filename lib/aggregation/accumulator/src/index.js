...
const { createDataflowReducerConfig } = require('./lib/accumulator-config');
...
const slackCutoff = () => {
  ...
  return moment.utc().startOf('month').add(slackLimit, 'milliseconds');
};

const isOutOfSlack = (usageEnd, now) => {
  const lastMonth = moment.utc().subtract(1, 'month').endOf('month');
  return usageEnd <= lastMonth.valueOf() && now > slackCutoff();
};
...
const accumulate = function*(accums, usage) {
  ...
  // Accumulate new usage into the first of the set of output docs produced at each reduction iteration
  const a = accums[0];

  const now = moment.now().valueOf();
  if (isOutOfSlack(usage.end, now)) {
    const msg = `Last month slack window ended on ${slackCutoff().toString()}`;
  ...
  const getCurrentQuantity = (timeWindow) =>{
    return timeWindow && timeWindow.quantity && timeWindow.quantity.current
      ? timeWindow.quantity.current
      : 0;
  };
  ...
  const newa = extend({}, amerge, {
    accumulated_usage: map(usage.metered_usage, (meteredUsage) => {
      ...
      // a function that gives the value of the submitted usage in the dimension window.
      const getCell = timewindow.cellfn(accumulateWindow, amerge.processed, usage.end);
      ...
      return {
        ...
        windows: map(accumulateWindow, (window, i) => {
          ...
          const twi = timewindow.timeWindowIndex(window, amerge.processed, usage.end, dimensions[i], true);
          ...
          return map(window, (timeWindow, j) => {
            const bounds = timewindow.timeWindowBounds(amerge.processed, dimensions[i], -j);
            const current = getCurrentQuantity(timeWindow);
...
const accumulator = (token) => {
  ...
  const reducer = dataflow.reducer(accumulate, createDataflowReducerConfig(secured(), sampling, token));
