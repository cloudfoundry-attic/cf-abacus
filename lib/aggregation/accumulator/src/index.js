...
const accumdb = yieldable(
  retry(breaker(db.statedb(uris.couchdb, 'abacus-accumulated-usage'))));
const logdb = yieldable(
  retry(breaker(batch(db.logdb(uris.couchdb,
  'abacus-accumulated-usage-log')))));
...
const updateAccumulatedUsage =
  yieldable(batch(batch.groupBy(function *(calls) {
    ...
    try {
      ...
      yield storeAccumulatedUsage(newa, aid, lcall[0].alogid, lcall[0].ulogid);
