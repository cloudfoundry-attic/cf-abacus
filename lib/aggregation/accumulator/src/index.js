...
const windowsSizes = process.env.TIME_WINDOWS_SIZES ?
  JSON.parse(process.env.TIME_WINDOWS_SIZES) : undefined;
...
const accumulate = function *(accums, u) {
  ...
  const amerge = a ? extend({}, a, umerge) : extend({
    accumulated_usage: []
  }, umerge);
    ...
    accumulated_usage: map(u.metered_usage, (mu) => {
      ...
      return {
        ...
        windows: map(aw, (w, i) => {
          ...
          const twi = timewindow.timeWindowIndex(w, now, u.end,
            dimensions[i], true);
          ...
          if(a && now > processed)
            timewindow.shiftWindow(a.processed, now, w, dimensions[i]);
...
const startApp = (token) => {
  ...
  if (!cluster.isWorker() || cluster.isDisabled()) {
    ...
    dataflow.replay(app.reducer, 0, (err) => {
      if (err)
        edebug('Replay failed with error %o', err);
      else
...
const runCLI = () => {
  if (secured()) {
    ...
    systemToken.start((err) => {
      if (err)
        edebug('Unable to obtain oAuth token due to %o', err);
      else
        startApp(systemToken);      
    });
  }
  else
    startApp();
