            ...
            if(twi === j) {
              if(tw && tw.quantity && tw.quantity.current) {
                debug('Accumulating window %d index %d' +
                  ' new quantity %o old quantity %o from %d to %d'
                  , i, twi, tw, mu, bounds.from, bounds.to);

                return {
                  quantity: {
                    previous: tw.quantity.current,
                    current: accumfn(mplan.metrics, mu.metric)
                      (tw.quantity.current, mu.quantity, u.start, u.end,
                      bounds.from, bounds.to)
                  }
                };
              }
              debug('New window %d index %d new quantity %o from %d to %d',
                i, twi, tw, bounds.from, bounds.to);

              return {
                quantity: {
                  current: accumfn(mplan.metrics, mu.metric)
                    (0, mu.quantity, u.start, u.end, bounds.from, bounds.to)
                }
              };
            }
