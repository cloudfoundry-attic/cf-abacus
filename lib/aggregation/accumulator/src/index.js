  ...
  const newa = extend({}, amerge, {
    accumulated_usage: map(u.metered_usage, (mu) => {
      ...
      return {
        metric: mu.metric,
        quantity: map(aq, (q, i) => {
          const timediff = timescale[i] ? Math.floor(newend / timescale[i])
            - Math.floor(oldend / timescale[i]) : 0;
...
const storeAccumulatedUsage = function *(a, aid, alogid, ulogid) {
  ...
  yield accumcache.put(extend({}, adoc, { dbrev: arev.rev }));
  ...
  yieldable(batch(batch.groupBy(function *(calls) {
    ...
    try {
      ...
      const rres = yield treduce(calls, function *(rres, call) {
        ...
        newa: omit(a, 'last_accumulated_usage_id'),
        ares: []
      });
      ...
      yield storeAccumulatedUsage(
        rres.newa, aid, lcall[0].alogid, lcall[0].ulogid);
