...
const dimensions = [dimension.sec, dimension.min, dimension.hour, dimension.day, dimension.month];
...
const accumulate = function*(accums, u) {
  debug('Accumulating usage %o from %d and new usage %o from %d', accums[0], accums[0] ? accums[0].end : 0, u, u.end);
    ...
    accumulated_usage: map(u.metered_usage, (mu) => {
      ...
      return {
        ...
        windows: map(aw, (w, i) => {
          ...
          const twi = timewindow.timeWindowIndex(w, now, u.end, dimensions[i], true);

          // Expand the timewindows if necessary
          if (i >= indexOf(dimensions, slack().scale) && w.length - 1 < twi)
            times(twi - w.length + 1, () => w.push(null));

          // Shift the windows if now is part of a new time window
          if (a && now > processed) timewindow.shiftWindow(a.processed, now, w, dimensions[i]);

          /* eslint complexity: [1, 6] */
          const quantities = map(w, (tw, j) => {
            const bounds = timewindow.timeWindowBounds(now, dimensions[i], -j);
            const current = tw && tw.quantity && tw.quantity.current ? tw.quantity.current : 0;
            ...
            if (tw && tw.quantity && tw.quantity.current) {
              debug('Accumulating window %d index %d' + ' new quantity %o old quantity %o', i, j, tw, mu);
              return {
                quantity: {
          ...
          return map(
            ...
            (q) =>
              q
                ? extend(q, { cost: q.quantity.current ? rfn(rp, q.quantity.current) : 0 })
                : null
          );
        })
      };
