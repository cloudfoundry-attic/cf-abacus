...
// Return metrics accumulation transforms for a given resource id
const accumulators = (rid) => (config(rid) || { transforms: [] }).transforms;
...
const accumfn = (t) => {
  ...
  // If there is an accumulate function then use it, else parse the configured
  // formula to get the corresponding accumulate function for SUM, MAX and AVG
  return t.accumulate ? eval('(' + t.accumulate + ')') : {
    SUM: (a, c) => a ? a + c : c,
...
const accumulate = (a, u) => {
  ...
  const amerge = !a ? extend({ start: day(u.end), end: eod(u.end),
    accumulated_usage: [] }, umerge) : extend(clone(a), umerge);
  ...
  const newa = extend(clone(amerge), {
    accumulated_usage: map(zip(accumulators(u.resource_id),
      amerge.accumulated_usage, u.metered_usage), (z) => {
        const q = z[1] ? z[1].quantity : undefined;
        const newq = accumfn(z[0])(q, z[2].quantity);
        return { unit: z[0].id, quantity: newq, delta: newq - (q || 0) };
      })
  });
...
const logUsage = function *(u, alogid, ulogid) {
  ...
  try {
    ...
    if(dupdoc) {
      debug('Found existing usage %o in %s', dupdoc,
        dupdoc.dbrev ? 'cache' : 'db');
      return dupdoc;
    }

    // Log the new usage
    debug('Logging new usage %s', ulogid);
    const logdoc = extend(clone(u), { id: ulogid, metered_usage_id: u.id,
...
const storeAccumulatedUsage = function *(a, aid, alogid, ulogid) {
  ...
  const adoc = extend(clone(omit(a, 'dbrev')), { id: aid,
    last_accumulated_usage_id: alogid, processed_usage_id: ulogid });
  const arev = yield accumdb.put(extend(clone(adoc), { _id: aid },
    a.dbrev ? { _rev: a.dbrev } : {}));
...
const accumulateUsage = function *(u) {
  ...
  const alogdoc = extend(clone(omit(newa, 'dbrev')), { id: alogid,
    processed_usage_id: ulogid });
  yield post((yield aggreguri(alogdoc.organization_id, alogdoc.start)) +
    '/v1/metering/accumulated/usage', { body: alogdoc });
