...
const accumulate = function *(a, u) {
  ...
  const pconf = yield prices(u.resource_id, u.end,
    systemToken && systemToken());
  const plan = filter(rconf.plans, (p) => p.plan_id === u.plan_id)[0];
  ...
  const newa = extend({}, amerge, {
    accumulated_usage: map(u.metered_usage, (mu) => {
      ...
      const rfn = ratefn(plan.metrics, mu.metric);
        ...
        windows: map(aw, (w, i) => {
          ...
          const quantities = map(w, (wh, j) => {
            if(si === j) {
              if(wh && wh.quantity && wh.quantity.current) {
                ...
                return {
                  quantity: {
                    ...
                    current: accumfn(plan.metrics, mu.metric)
              ...
              return {
                quantity: {
                  current: accumfn(plan.metrics, mu.metric)(0, mu.quantity)
                }
              };
            }
...
const okeysV2 = (udoc) =>
  [[udoc.organization_id, udoc.resource_instance_id,
    udoc.consumer_id || 'UNKNOWN', udoc.metering_plan_id,
    udoc.rating_plan_id, udoc.price_id].join('/')];

// Return the configured price for the given plan, metric, and country
const priceV2 = (pconf, rid, metric, country) => {
  if(pconf) {
    // Find the specified plan
    const plan = filter(pconf.rating_plans, (p) => p.rating_plan_id
      === rid);
    if(plan.length) {
      // Find the specified metric price
      const metrics = filter(plan[0].metrics, (m) => m.name === metric);
...
// Accumulate usage and return new accumulated value
const accumulateV2 = function *(a, u) {
  debug(
    'Accumulating usage %o from %d and new usage %o from %d',
    a, a ? a.end : 0, u, u.end);
  ...
  const mconf = yield meterConfig(u.resource_id, u.end,
    systemToken && systemToken());
  const mplan = filter(mconf.metering_plans, (m) => m.metering_plan_id ===
    u.metering_plan_id)[0];

  // Retrieve the configured price for the resource
  const pconf = yield pricesV2(u.resource_id, u.end, u.price_id,
    systemToken && systemToken());

  // Retrieve the configured rate for the resource
  const rconf = yield rateConfig(u.resource_id, u.end,
    systemToken && systemToken());
  const rplan = filter(rconf.rating_plans, (r) => r.rating_plan_id ===
    u.rating_plan_id)[0];
...
// Process a group of usage docs and compute the corresponding
// accumulated usage
const accumulateUsageV2 = function *(accum, udocs) {
  const ologs = yield treduce(udocs, function *(log, udoc, i, l) {
    const res = yield accumulateV2(last(log)[0], udoc);
    return log.concat([[res]]);
  }, [[
    accum
  ]]);
...
const accumulator = () => {
  ...
  const reducerV2 = dataflow.reducer(accumulateUsageV2, {
    input: {
      ...
      dbname: 'abacus-accumulator-metered-usage-v2',
      wscope: iwscope,
      rscope: rscope,
      key: ikeyV2,
      group: igroupV2,
    ...
    output: {
      ...
      get: '/v2/metering/accumulated/usage' +
        '/k/:korganization_id/:kresource_instance_id' +
        '/:kconsumer_id/:kmetering_plan_id' +
        '/t/:tend/:tstart',
      dbname: 'abacus-accumulator-accumulated-usage-v2',
      rscope: rscope,
      keys: okeysV2,
      times: otimes
    },
