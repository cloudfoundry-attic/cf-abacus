...
const accumulated = function *(orgid, siid, time) {
    ...
    const doc = yield accumdb.get(id);
    if(doc)
...
const store = function *(a, u) {
    ...
    yield logdb.put(extend(clone(ulogdoc), { _id: ulogid }));
    ...
    const alogdoc = extend(clone(a), { id: alogid, processed_usage_id: ulogid });
    ...
    const adoc = extend(clone(a), { id: aid, last_accumulated_usage_id: alogid, processed_usage_id: ulogid });
    yield accumdb.put(extend(clone(adoc), { _id: aid }));
