...
const reqmock = clone(request);
require.cache[require.resolve('cf-abacus-request')].exports = extend(reqmock, { norespost: spy((uri, req, cb) => cb()) });
    ...
    it('accumulates usage over time', function(done) {
        ...
        const get = (done) => {
            ...
            map(usage, (u) => request.get(locations[u.id], {}, (err, val) => {
                ...
                expect(reqmock.norespost.args[0][0]).to.equal('http://localhost:9202/v1/metering/accumulated/usage');
                expect(reqmock.norespost.args[1][0]).to.equal('http://localhost:9202/v1/metering/accumulated/usage');
