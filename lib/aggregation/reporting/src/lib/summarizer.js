...
const summarizeWindowElement = (metric, element, summaryFn, time, bounds) => {
  ...
  // Not needed if DB does not contain cost data. Here we're trying to reduce the report size
    ...
    return extend(
      { metric: metric },
      {
        windows: map(planMetrics[0].windows, (w, i) => {
          return map(w, (s, j) => {
            const aggregatedResult = reduce(planMetrics, (a, u) => {
              // Only add the plan usage window if it is defined.
              if (u && u.windows[i][j] && u.windows[i][j] !== null)
                return a !== null
                  ? { quantity: a.quantity + u.windows[i][j].quantity, summary: a.summary + u.windows[i][j].summary }
                  : { quantity: u.windows[i][j].quantity, summary: u.windows[i][j].summary };
              return a;
            }, null);
