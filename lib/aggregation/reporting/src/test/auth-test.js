...
describe('abacus-usage-report-auth', () => {
  ...
  const prepareRatedUsage = (done) => {
    ...
    const rated = helpers.ratedTemplate(id, oid2, sid, 
      testResourceId, 1420502400000, 1420502500000, 1420502500000,
      helpers.buildAggregatedUsage(21, 300, 3300, {
        consumed: 1108800000,
        consuming: 14
      }, {
        ...
        helpers.consumerReferenceTemplate(oid2, sid, 'basic', 1420502500000,
          'UNKNOWN')
      ]);
    const consumer1 = helpers.ratedConsumerTemplate(oid2, sid,
    ...
    helpers.storeRatedUsage(rated, 
      () => helpers.storeRatedUsage(consumer1, done));
  };

  const prepareAccumulatedUsage = (done) => {
    const accumulated = helpers.accumulatedTemplate(oid2, rid, sid,
    testResourceId, helpers.buildAccumulatedUsage(
