...
const storeRatedUsage = (ratedUsage, cb = () => {}) => {
  ...
  yieldable.functioncb(ratedb.put)(extend({}, ratedUsage, {
    _id: ratedUsage.id
  }), (err, val) => {
...
describe('abacus-usage-report', () => {
  ...
  it('retrieves rated usage with 16 significant digits', (done) => {
    ...
    const verify = (secured, done) => {
      ...
      request.get(
        ...
        ':organization_id/aggregated/usage', {
          p: server.address().port,
          organization_id: bigNumberRated.organization_id
        }, (err, val) => {
          expect(err).to.equal(undefined);
    ...
    // Add rated usage with BigNumber
    storeRatedUsage(bigNumberRated);
