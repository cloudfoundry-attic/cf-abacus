...
const storeAccumulatedUsage = (accumulatedUsage, cb = () => {}) => {
  const accumulatordb = dataflow.db('abacus-accumulator-accumulated-usage');
  yieldable.functioncb(accumulatordb.put)(extend({}, accumulatedUsage, {
    _id: accumulatedUsage.id
  }), (err, val) => {
    expect(err).to.equal(null);
    cb();
  });
...
describe('abacus-usage-report', () => {
    ...
    before((done) => {
      const rated = {
        ...
        resources: [{
          ...
          plans: [{
            plan_id: 'basic/basic-test-metering-plan/' +
              'basic-test-rating-plan/test-pricing-basic',
            ...
            {
              plan_id: 'standard/basic-test-metering-plan/' +
                'standard-test-rating-plan/test-pricing-standard',
              metering_plan_id: 'basic-test-metering-plan',
  ...
  context('when rated usage contains a slack', () => {
    before((done) => {
      ...
      const rated = {
        id: 'k/a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf29/t/0001446418800000',
        ...
        spaces: [{
          ...
          consumers: ['UNKNOWN/t/0001446418800000',
            'UNKNOWN2/t/0001446163200000']
        }]
      };
      const consumer = {
        id: 'k/a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf29/' +
      ...
      const consumer2 = {
        id: 'k/a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf29/' +
          'aaeae239-f3f8-483c-9dd0-de5d41c38b6a/UNKNOWN2/t/0001446163200000',
        consumer_id: 'UNKNOWN2',
        processed: 1446163200000,
        resources: [{
          resource_id: 'test-resource',
          aggregated_usage: [{
            metric: 'memory',
  ...
  context('when accumulated usage has small numbers', () => {
    before((done) => {
      const accumulated = {
        id: 'k/a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf27/UNKNOWN/' +
          '0b39fa70-a65f-4183-bae8-385633ca5c87/basic/' +
    ...
    it('Retrieve accumulated usage using a GraphQL query', (done) => {
      const verify = (done) => {
        ...
        const query = '{ resource_instance(organization_id: ' +
          ...
          'resource_instance_id: "0b39fa70-a65f-4183-bae8-385633ca5c87", ' +
          'plan_id: "basic", metering_plan_id: "basic-test-metering-plan", ' +
          'rating_plan_id: "basic-test-rating-plan", pricing_plan_id: ' +
          '"test-pricing-basic", time: 1446508800000) { organization_id, ' +
          'consumer_id, resource_instance_id, plan_id, ' +
          'accumulated_usage { metric, windows { cost, charge, summary } }, ' +
          'windows { charge }}}';
  ...
  context('when querying complex usage with graphql', () => {
            ...
            [{
              quantity: {
                current: { consuming: 0, consumed: 3628800000,
                  since: 1456099200000 },
                previous: { consuming: 2, consumed: 0, since: 1454284800000 }
              },
              cost: 50803200
            }]
          ]
    ...
    xit('Retrieve accumulated usage using a GraphQL query', (done) => {
      const verify = (done) => {
        // Create a test report app
        const app = report();
        ...
        // Query with sub selections in quantity
        const query2 = '{ resource_instance(organization_id: ' +
          '"org", consumer_id: "con", resource_instance_id: "ins", ' +
          'plan_id: "basic", metering_plan_id: "basic-test-metering-plan", ' +
          'rating_plan_id: "basic-test-rating-plan", pricing_plan_id: ' +
          '"test-pricing-basic", time: 1456185600000) { organization_id, ' +
          'consumer_id, resource_instance_id, plan_id, ' +
          'accumulated_usage { metric, windows { quantity { consumed }}}}}';
          ...
          'http://localhost::p/v1/metering/aggregated/usage/graph/:query', {
            ...
            request.get(
              'http://localhost::p/v1/metering/aggregated/usage/graph/:query', {
                ...
                // With subselections, the code should actually throw an error
                // This expects that the graphql errors are returned in the body
                // May subject to change in the future
                expect(val.statusCode).to.equal(400);
                expect(val.body).to.deep.equal([{
                  message: 'Field "quantity" of type "Float!" ' +
                    'must not have a sub selection.'
                } ]);
                done();
              });
          });
