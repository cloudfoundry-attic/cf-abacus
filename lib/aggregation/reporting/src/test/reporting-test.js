...
describe('abacus-usage-report', () => {
          ...
          storage.aggregator.put(require('./json/consumer-2.json'), () =>
            storage.aggregator.put(require('./json/consumer-3.json'), () =>
              storage.aggregator.put(require('./json/consumer-4.json'), () =>
                storage.aggregator.put(require('./json/consumer-5.json'),
                  done))))));
    });
  ...
  context('when requesting non-existing usage', () => {
    ...
    context('on missing consumer documents', () => {
      ...
      const buildRatedUsageFromTemplate = (processedTimeConsumer1,
        processedTimeConsumer2) => builder.ratedTemplate(id, orgid,
        sid, testResourceId, 1420502400000, 1420502500000, 1420502500000,
        builder.buildAggregatedUsage(21, 300, 3300, {
          ...
          builder.consumerReferenceTemplate(processedTimeConsumer1,
            'consumer_1'),
          builder.consumerReferenceTemplate(processedTimeConsumer2,
            'consumer_2')
        ]);
      ...
      beforeEach((done) => {
        const consumer1 = builder.ratedConsumerTemplate(
          ...
          consumerProcessedTime, 'consumer_1');
      ...
      it('should fail when missing consumer time is in the current month',
        (done) => {
          storage.aggregator.put(
            buildRatedUsageFromTemplate(consumerProcessedTime,
            ...
            () => {
              request.get(
                ...
                ':organization_id/aggregated/usage/:time', {
                  ...
                  expect(err.message).to.equal('Failed to find ' +
