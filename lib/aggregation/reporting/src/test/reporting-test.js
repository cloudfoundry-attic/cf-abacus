...
describe('abacus-usage-report', () => {
  before((done) => {
    dbclient.drop(process.env.DB_URI, /^abacus-aggregator|^abacus-accumulator/, done);
  });
  ...
  context('when shifting resource windows', () => {
    ...
    const requestAndValidateUsage = (organizationId, windows, cb) => {
      requestUsage(organizationId, (body) => {
        ...
        expect(body.spaces[0].resources.length).to.equal(1);
        expect(body.spaces[0].resources[0].plans.length).to.equal(1);
        const au = body.spaces[0].resources[0].plans[0].aggregated_usage[0];
        expect(au.windows).to.deep.equal(windows);
        expect(body.spaces[0].consumers[0].resources.length).to.equal(1);
