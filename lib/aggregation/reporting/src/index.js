...
const chargeUsageCb = (time, aggregatedUsage, auth, counter, cb) => {
  ...
  const chargePlan = (plan, i, l, cb) => {
    ...
    getRatingPlanCb(planId, auth, (err, rplan) => {
      ...
      const chargeMetric = (metric, i, l, cb) => setImmediate(() => {
        ...
        cb(undefined,
          chargePlanMetric(metric, time, aggregatedUsage.processed,
  ...
  const resourceMetrics = (resource) => {
    ...
    each(resource.plans, (plan) =>
      each(plan.aggregated_usage, (metric) =>
  ...
  const chargeResource = (resource, i, l, cb) => {
    ...
    tmap(resource.plans, chargePlan, (err, chargedPlans) => {
      ...
      transformMapCb(err, resource, {
        aggregated_usage: map(resourceMetrics(resource),
  ...
  tmap(aggregatedUsage.resources, chargeResource, (err, consumerResources) => {
    debug('Charging aggregated usage for organization, %s',
...
const summarizeInstanceUsage = function *(time, aggregaredUsage,
  ...
  const summarizedUsage = extend({}, aggregaredUsage, {
    accumulated_usage: map(aggregaredUsage.accumulated_usage, (m) => {
      ...
      return summarizePlanMetric(m, time, aggregaredUsage.processed,
...
const consumerUsage = function *(usage, resourceId, startOfMonthKey, counter) {
  ...
  if (counter)
    counter.set('consumerDBDocs', consumers.length);

  const consumersDocMap = buildConsumersMap(consumers, ids,
    startOfMonthKey, resourceId);
  if (counter)
    counter.set('consumerProcessingDocs', consumersDocMap.size);
...
const orgUsage = (orgId, time = moment.now(), auth, resourceId, cb) => {
  ...
  consumerUsageCb(orgId, time, auth, resourceId, counter,
    (err, consumerUsage) => {
      if(err) {
