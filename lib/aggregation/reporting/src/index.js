...
const summarizeUsageCb = (t, a, auth, cb) => {
  ...
  const summarizePlan = (p, i, l, cb) => {
    ...
    getMeteringPlanCb(p.metering_plan_id, auth, (err, mplan) => {
      ...
      const mapFn = (m, i, l, cb) => process.nextTick(() =>
  ...
  const summarizeResource = (rs, i, l, cb) => {
    debug('Summirizing resuorce %s', rs.resource_id);
  ...
  const summarizeConsumer = (c, i, l, cb) => {
    debug('Summirizing consumer %s', c.consumer_id);
  ...
  tmap(a.resources, summarizeResource, (err, resourcesAccum) => {
    debug('Summirizing aggregated usage for organization, %s',
      a.organization_id);
    if (err) {
...
const consumerUsage = function *(u) {
  ...
  for (let s of doc.spaces) {
    const updatedConsumers = yield updateConsumers(u,
      s.consumers, s.space_id, consumersDocMap);
    s.consumers = updatedConsumers;
  }
...
const orgUsage = function(orgid, t = moment.now(), auth, cb) {
  ...
  consumerUsageCb(orgid, t, auth, (err, consumerUsage) => {
    ...
    summarizeUsageCb(t, consumerUsage, tokenAuth, (err, summarizedUsage) => {
      if(err) {
        debug('Could not summarize usage for org %s due to: %o', orgid, err);
        cb(err);
        return;
      }
      ...
      chargeUsageCb(t, summarizedUsage, tokenAuth, (err, chargedUsage) => {
        if(err) {
          debug('Could not charge usage for org %s due to: %o', orgid, err);
          cb(err);
          return;
        }
...
const retrieveUsage = (req, res) => {
  ...
  orgUsage(req.params.organization_id,
    ...
    req.headers && req.headers.authorization, (err, usage) => {
      ...
      res.send(omit(dbclient.undbify(usage),
