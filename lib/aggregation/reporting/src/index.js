...
const chargeUsageCb = (t, r, auth, cb) => {
  ...
  const chargeResource = (rs, i, l, cb) => {
    ...
    transform.map(rs.plans, mapFn, (err, plans) => {
      if (err) {
        cb(err);
        return;
      }
  ...
  transform.map(r.resources, chargeResource, (err, cr) => {
    if (err) {
      cb(err);
      return;
    }
    ...
    const consumerMapFn = (consumer, i, l, cb) => {
      transform.map(consumer.resources, chargeResource,
        (err, cr) => {
          if (err) {
            cb(err);
            return;
    ...
    const spacesMapFn = (space, i, l, cb) => {
      transform.map(space.resources, chargeResource, (err, cr) => {
        if (err) {
          cb(err);
          return;
        }

        transform.map(space.consumers, consumerMapFn, (err, accumConsumers) => {
          if (err) {
            cb(err);
            return;
          }
    ...
    transform.map(r.spaces, spacesMapFn, (err, accumSpaces) => {
      if (err) {
        cb(err);
        return;
      }
...
const summarizeUsageCb = (t, a, auth, cb) => {
  ...
  const summarizePlan = (p, i, l, cb) => {
    ...
    getMeteringPlanCb(p.metering_plan_id, auth, (err, mplan) => {
      ...
      transform.map(p.aggregated_usage, mapFn, (err, planAccum) => {
  ...
  const summarizeResource = (rs, i, l, cb) =>
    transform.map(rs.plans, summarizePlan, (err, resAccum) => {
      transformMapCb(err, rs, { plans: resAccum }, cb);
    });


  const summarizeConsumer = (c, i, l, cb) =>
    transform.map(c.resources, summarizeResource, (err, consumerAccum) => {
      transformMapCb(err, c, { resources: consumerAccum }, cb);
    });

  const summarizeSpace = (s, i, l, cb) =>
    transform.map(s.resources, summarizeResource, (err, resourceAccum) => {
      if (err) {
        cb(err);
        return;
      }

      transform.map(s.consumers, summarizeConsumer, (err, consumerAccum) => {
  ...
  transform.map(a.resources, summarizeResource, (err, resourcesAccum) => {
    if (err) {
      cb(err);
      return;
    }

    transform.map(a.spaces, summarizeSpace, (err, spacesAccum) => {
...
const summarizeUsage = function *(t, a, auth) {
  const ySummarizeUsage = yieldable(summarizeUsageCb);
  return yield ySummarizeUsage(t, a, auth);
};
...
const updateConsumers = function *(u, consumers, spaceId, consumersDocMap) {
  ...
  for (let c of consumers) {
    ...
    if (processedConsumers === maxConsumers) {
      yield setImmediateEx();
      debug('Callback from timeout');
      processedConsumers = 0;
    }
