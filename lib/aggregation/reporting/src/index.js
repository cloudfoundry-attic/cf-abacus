...
const updateConsumers = function *(u, consumers, spaceId, consumerDocs) {
  const ret = [];
  let processedConsumers = 0;
  for (let c of consumers) {
    debug('Processing consumer %s', c.id);
    const t = moment.utc().valueOf();

    if (processedConsumers === 100) {
      yield setTimeoutEx(10);
      debug('Callback from 10 ms timeout');
      processedConsumers = 0;
    }

    // Retrieve the consumer doc with the matching doc_id
    const consumer = filter(consumerDocs,
      (con) => dbclient.k(con.doc._id) ===
        [u.organization_id, spaceId, c.id].join('/'))[0].doc;
    ...
    processedConsumers ++;
    debug('Processing time (ms) %d', moment.utc().valueOf() - t);
  }
  return ret;
};
...
const consumerUsage = function *(u) {
  ...
  debug('DB retrieval time (ms) %d', moment.utc().valueOf() - t);

  const doc = extend({}, u);
  for (let s of doc.spaces) {
    const updatedConsumers = yield updateConsumers(u, s.consumers, s.space_id,
      consumers);
