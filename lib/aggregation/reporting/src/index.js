...
const aggregatordb = dataflow.db('abacus-aggregator-aggregated-usage', undefined, uris[dbaliasAggregator]);
// Configure accumulated usage db
const accumulatordb = dataflow.db('abacus-accumulator-accumulated-usage', undefined, uris[dbaliasAccumulator]);
...
const traverseWindows = (metric, processed, processFn) =>
  extend({}, metric, {
    windows: map(metric.windows, (window, windowIndex) =>
      map(window, (windowElement, windowElementIndex) => {
        ...
        const bounds = timewindow.timeWindowBounds(processed, dimensions[windowIndex], -windowElementIndex);
...
const buildWindows = (entity) =>
  map(zip.apply(_, map(entity, (element) => element.windows)), (zippedUsage) =>
...
const chargeUsageCb = (time, aggregatedUsage, auth, counter, cb) => {
  ...
  const chargePlan = (plan, i, l, cb) => {
    ...
    getRatingPlanCb(planId, auth, (err, rplan) => {
      if (err) {
        debug('Could not obtain rating plan id %s due to: %o', planId, auth, err);
        cb(err);
        return;
      }

      const chargeMetric = (metric, i, l, cb) =>
        setImmediate(() => {
          const chargeFunction = chargeFunctionMemoizer(
            planId + metric.metric,
            chargefn(planId, rplan.metrics, metric.metric)
          );

          cb(undefined, chargePlanMetric(metric, time, aggregatedUsage.processed, chargeFunction, counter));
        });

      // Calculate the charges for all the metrics under a plan
      tmap(plan.aggregated_usage, chargeMetric, (err, calculatedUsage) => {
        transformMapCb(
          err,
          plan,
          {
  ...
  const chargeResource = (resource, i, l, cb) => {
    ...
    tmap(resource.plans, chargePlan, (err, chargedPlans) => {
      ...
      transformMapCb(
        ...
        resource,
        {
          aggregated_usage: map(resourceMetrics(resource), (metric) => chargeAggregatedMetric(metric, chargedPlans)),
          plans: chargedPlans,
  ...
  tmap(aggregatedUsage.resources, chargeResource, (err, consumerResources) => {
    ...
    if (err) {
      debug('Could not charge resources due to: %o', err);
      cb(err);
      return;
    }
...
const chargeInstanceUsage = function*(time, ratedUsage, auth, counter) {
  debug('Charging instance usage for time %o and rated usage %o', time, ratedUsage);
...
// metric object, query time, usage processed time, charge function
const summarizePlanMetric = (metric, time, processed, sfn, counter) => {
  ...
  const summarizeWindowElement = (windowElement, bounds) => {
    ...
    return windowElement
      ? sfn
        ? extend({}, windowElement, { summary: sfn(time, windowElement.quantity, bounds.from, bounds.to) })
        : extend({}, windowElement)
      : null;
  };

  const summarizedMetric = traverseWindows(metric, processed, summarizeWindowElement);
...
const summarizeUsageCb = (time, aggregatedUsage, auth, counter, cb) => {
  ...
  const summarizePlan = (plan, i, l, cb) => {
    ...
    const planId = plan.metering_plan_id;
    if (pdebug.enabled) {
      uniquePlanIds.add(planId);
    ...
    // Find the metrics configured for the given resource
    getMeteringPlanCb(planId, auth, (err, mplan) => {
      if (err) {
        debug('Could not obtain metering plan id %s due to: %o', planId, auth, err);
        cb(err);
        return;
      }

      const summarizeMetric = (metric, i, l, cb) =>
        setImmediate(() => {
          const summarizeFunction = summarizeFunctionMemoizer(
            planId + metric.metric,
            summarizefn(planId, mplan.metrics, metric.metric)
          );

          cb(undefined, summarizePlanMetric(metric, time, aggregatedUsage.processed, summarizeFunction, counter));
        });

      tmap(plan.aggregated_usage, summarizeMetric, (err, planAccum) => {
...
const summarizeInstanceUsage = function*(time, aggregaredUsage, auth, counter) {
  debug('Summarizing instance usage for time %o and aggregated usage %o', time, aggregaredUsage);
    ...
    each(consumer.resources, (resource) => {
      each(resource.plans, (plan) => {
        each(plan.aggregated_usage, (au) => {
          each(au.windows, (w, i) => {
            timewindow.shiftWindow(consumer.processed, usage.processed, w, dimensions[i]);
          });
        });
      });
    });
...
const buildConsumersMap = (consumers, ids, startOfMonthKey, resourceId) => {
  ...
  for (let i = 0; i < consumers.length; i++) {
    if (skipMissingConsumer(consumers[i], ids[i], startOfMonthKey)) continue;
...
const consumerUsage = function*(usage, resourceId, startOfMonthKey, counter) {
  ...
  if (counter) counter.set('consumerDBDocs', consumers.length);

  const consumersDocMap = buildConsumersMap(consumers, ids, startOfMonthKey, resourceId);
  if (counter) counter.set('consumerProcessingDocs', consumersDocMap.size);

  // Return a copy of the usage data with consumer objects populated
  const doc = extend({}, usage);
  for (let space of doc.spaces)
    space.consumers = yield updateConsumers(usage, space.consumers, space.space_id, consumersDocMap);
  debug('Report generation completed');
  return doc;
};

const getConsumerUsage = function*(orgId, time, auth, resourceId, counter) {
  ...
  return yield consumerUsage(usageDoc, resourceId, eid, counter);
};
...
const orgUsage = (orgId, time = moment.now(), auth, resourceId, cb) => {
  ...
  consumerUsageCb(orgId, time, auth, resourceId, counter, (err, consumerUsage) => {
    if (err) {
    ...
    summarizeUsageCb(time, consumerUsage, tokenAuth, counter, (err, summarizedUsage) => {
      if (err) {
      ...
      chargeUsageCb(time, summarizedUsage, tokenAuth, counter, (err, chargedUsage) => {
        if (err) {
          edebug('Could not charge usage for org %s due to: %o', orgId, err);
          cb(err);
          return;
        }

        pdebug('Organization report for %s has counters: %o', orgId, counter.list());
...
const orgsUsageCb = (orgIds, time, auth, cb) => {
  tmap(orgIds, (orgid, i, l, cb) => orgUsage(orgid, time, auth, undefined, cb), (err, result) => cb(err, result));
};
...
const resourceInstanceUsage = function*(
  ...
  pdebug('Resource instance report for %s has counters: %o', id, counter.list());
...
// Retrieve a usage report summary for a given org and time in msec
routes.get('/v1/metering/organizations/:organization_id/aggregated/usage/:time', throttle(retrieveUsage));

routes.get('/v1/metering/organizations/:organization_id/aggregated/usage', throttle(retrieveUsage));
