...
const setTimeoutEx = yieldable((delay, cb) => {
  setTimeout(cb, delay);
});
...
const summarizeUsage = function *(t, a, auth) {
  ...
  const summarizePlan = function *(p) {
  ...
  const summarizeResource = function *(rs) {
    ...
    return extend({}, rs, {
      plans: yield tmap(rs.plans, summarizePlan)
    });
  };

  const summarizeConsumer = function *(c) {
    return extend({}, c, {
      resources: yield tmap(c.resources, summarizeResource)
    });
  };

  const summarizeSpace = function *(s) {
    return extend({}, s, {
      resources: yield tmap(s.resources, summarizeResource),
      consumers: yield tmap(s.consumers, summarizeConsumer)
    });
  };
  ...
  const s = extend({}, a, {
    ...
    spaces: yield tmap(a.spaces, summarizeSpace)
  });
...
const updateConsumers = function *(u, consumers, spaceId, consumersDocMap) {
  ...
  for (let c of consumers) {
    ...
    // fix for OOM: sleep to release the event loop for orgs with many consumers
    if (processedConsumers === maxConsumers) {
      yield setTimeoutEx(eventLoopTimeOut);
      debug('Callback from %d ms timeout', eventLoopTimeOut);
