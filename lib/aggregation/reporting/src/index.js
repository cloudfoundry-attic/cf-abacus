...
const aggrdb = yieldable(
  retry(breaker(batch(db(uris.couchdb, 'abacus-aggregated-usage')))));
...
const queryType = new GraphQLObjectType({
  ...
  fields: () => ({
      organization: {
        ...
        resolve: (root, args) => {
          return yieldable.promise(orgUsage)(args.organization_guid, args.date);
        }
      },
      organizations: {
        ...
        resolve: (root, args) => {
          return yieldable.promise(orgsUsage)(
            args.organization_guids, args.date);
        }
      },
      account: {
        type: new GraphQLList(organizationType),
...
routes.get(
  '/v1/organizations/:organization_guid/usage/:day', throttle(function *(req) {
    ...
    return {
      body: omit(doc,
        '_id', '_rev', 'last_aggregated_usage_id', 'accumulated_usage_id')
    };
  }));
