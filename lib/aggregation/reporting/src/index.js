...
// Given the metric object, query time, usage processed time, charge function
const chargePlanMetric = (metric, time, processed, cfn) => {
  ...
  return traverseWindows(metric, processed, chargeWindowElement);
};
...
const chargeUsageCb = (time, ratedUsage, auth, cb) => {
      ...
      getRatingPlanCb(plan.rating_plan_id, auth, (err, rplan) => {
        ...
        const calculatedUsage = map(plan.aggregated_usage, (metric) => {
          ...
          return chargePlanMetric(metric, time, ratedUsage.processed,
            chargeFunctionMemoizer(plan.rating_plan_id + metric.metric,
  ...
  tmap(ratedUsage.resources, chargeResource, (err, consumerResources) => {
    ...
    const spacesMapFn = (space, i, l, cb) => {
      tmap(space.resources, chargeResource, (err, consumerResources) => {
        ...
        tmap(space.consumers, consumerMapFn, (err, accumulatedConsumers) => {
          if (err) {
            cb(err);
            return;
          }
    ...
    tmap(ratedUsage.spaces, spacesMapFn, (err, accumulatedSpaces) => {
      if (err) {
        cb(err);
        return;
      }
...
const chargeUsage = function *(time, ratedUsage, auth) {
  ...
  return yield yChargeUsage(time, ratedUsage, auth);
};

const chargeInstanceUsage = function *(time, ratedUsage, auth) {
...
// Summarize a metric
const summarizeMetric = (metric, time, processed, sfn) => {
  ...
  return traverseWindows(metric, processed, summarizeWindowElement);
};
...
const summarizeUsageCb = (time, aggregatedUsage, auth, cb) => {
  ...
  const summarizePlan = (plan, i, l, cb) => {
    ...
    getMeteringPlanCb(plan.metering_plan_id, auth, (err, mplan) => {
      ...
      const mapFn = (metric, i, l, cb) => setImmediate(() => {
        cb(undefined, summarizeMetric(metric, time, aggregatedUsage.processed,
          summarizeFunctionMemoizer(plan.metering_plan_id + metric.metric,
  ...
  const summarizeSpace = (space, i, l, cb) => {
    debug('Summirizing space %s', space.space_id);
...
const summarizeInstanceUsage = function *(t, a, auth) {
  ...
  const summarizedUsage = extend({}, a, {
