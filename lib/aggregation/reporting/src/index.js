...
const aggregatordbV2 = dataflow.db('abacus-aggregator-aggregated-usage-v2');

// Determines the boundary of a window based upon the document's time,
// the current time scale of the window, and the slack index
const windowBounds = (d, wi, si) => {
  const args = [d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate(),
    d.getUTCHours(), d.getUTCMinutes(), d.getUTCSeconds()].slice(0, 6 - wi);
  args[args.length - 1] -= si;
  const from = Date.UTC.apply(null, args);
  args[args.length - 1] += 1;
  const to = Date.UTC.apply(null, args);
  return {
    from: from,
    to: to
  };
...
const chargefn = (plans, plan, metric) => {
  return filter(filter(plans, (p) => p.plan_id === plan)[0].metrics,
    (m) => m.name === metric)[0].chargefn;
};

// Return the summarize function for a given metric
const summarizefn = (plans, plan, metric) => {
  return filter(filter(plans, (p) => p.plan_id === plan)[0].metrics,
    (m) => m.name === metric)[0].summarizefn;
};
  ...
  const chargeResource = function *(rs) {
    ...
    const plans = map(rs.plans, (p) => {
      ...
      const cu = map(p.aggregated_usage, (m) => {
        ...
        const cfn = chargefn(conf.plans, p.plan_id, m.metric);

        // Clone the metric and return it along with the calculated charge
        return extend({}, m, {
          windows: map(m.windows, (w, i) => {
            return map(w, (wi, j) => {
              const bounds = windowBounds(new Date(r.processed), i, j);
              return wi ? extend({}, wi, {
                charge: cfn(t, wi.cost, bounds.from, bounds.to)
              }) : null;
            });
          })
        });
    ...
    const chargeMetric = (metric) => {
      return extend({}, metric, {
        windows: map(metric.windows, (w, i) => {
          return map(w, (wi, j) => {
            // Do not try to calculate the charge if the window is null
            return wi === null ? null : extend({}, wi, {
              charge: reduce(plans, (a, p) => {
                // reduce the match metric's plan aggregated usage
                const rw = reduce(filter(p.aggregated_usage,
                  (m) => m.metric === metric.metric), (a, w) => {
                  // Take the sum of a and the window charge unless a is null
                  if(w.windows[i][j] && w.windows[i][j].charge !== null)
                    return a !== null ? a + w.windows[i][j].charge
                      : w.windows[i][j].charge;
                  return a;
                }, null);
                return a !== null ? a + rw : rw;
              }, null)
            });
          });
        })
...
const summarizeUsage = function *(t, a, auth) {
  ...
  const summarizeResource = function *(rs) {
    ...
    // Summarize a metric
    const summarizeMetric = (m, sfn) => {
      // Clone the metric and extend with a usae summary
      return extend({}, m, {
        windows: map(m.windows, (w, i) => {
          return map(w, (wi, j) => {
            const bounds = windowBounds(new Date(a.processed), i, j);
    ...
    return extend({}, rs, {
      aggregated_usage: map(rs.aggregated_usage, (m) => {
        return summarizeMetric(m);
      }),
      plans: map(rs.plans, (p) => {
        return extend({}, p, {
          aggregated_usage: map(p.aggregated_usage, (m) => {
            return summarizeMetric(m,
              summarizefn(conf.plans, p.plan_id, m.metric));
          })
        });
      })
    });
...
const consumerUsage = function *(u) {
  ...
  map(u.spaces, (s) => {
    map(s.consumers, (c) => {
      // Build the full doc id
      ids.push(['k', u.organization_id, s.space_id, c].join('/'));
    })
  });
    ...
    spaces: map(u.spaces, (s) => {
      return extend({}, s, {
        consumers: map(s.consumers, (c) => {
          return omit(filter(consumers, (con) => con.doc._id ===
            ['k', u.organization_id, s.space_id, c].join('/'))[0].doc,
            ['_id', 'id', '_rev', 'accumulated_usage_id', 'processed']);
        })
      });
    })
  });
...
// Return the charge function for a given plan and metric
const chargefnV2 = (plans, plan, metric) => {
  return filter(filter(plans, (p) => p.rating_plan_id === plan)[0].metrics,
    (m) => m.name === metric)[0].chargefn;
};

// Return the summarize function for a given metric
const summarizefnV2 = (plans, plan, metric) => {
  return filter(filter(plans, (p) => p.metering_plan_id === plan)[0].metrics,
    (m) => m.name === metric)[0].summarizefn;
};

// Compute the charges for the given rated usage
const chargeUsageV2 = function *(t, r, auth) {
  // Charge the rated usage under a resource
  const chargeResource = function *(rs) {

    // Find the metrics configured for the given resource
    const conf = yield rateConfig(rs.resource_id, r.end,
      systemToken && systemToken());

    // Compute the charge of each metric under the resource's metering_plans
    const meteringPlans = map(rs.metering_plans, (mp) => {
      // Calculate the charge of each metric under the meter_plan's rate_plans
      const ratingPlans = map(mp.rating_plans, (rp) => {
        // Calculate the charge of each metric under the rate plan's prices
        const prices = map(rp.prices, (p) => {
          // Calculate the charges for all the metrics under a plan
          const cu = map(p.aggregated_usage, (m) => {
            // Find the charge function configured for each metric
            const cfn = chargefnV2(conf.rating_plans, rp.rating_plan_id,
              m.metric);
        ...
        // Calculate the charge for a metric across the rate plan's prices
        const chargeMetric = (metric) => {
          return extend({}, metric, {
            windows: map(metric.windows, (w, i) => {
              return map(w, (wi, j) => {
                // Do not try to calculate the charge if the window is null
                return wi === null ? null : extend({}, wi, {
                  charge: reduce(prices, (a, p) => {
                    // reduce the match metric's plan aggregated usage
                    const rw = reduce(filter(p.aggregated_usage,
                      (m) => m.metric === metric.metric), (a, w) => {
                      // Take the sum of a and the window charge 
                      // unless a is null
                      if(w.windows[i][j] && w.windows[i][j].charge !== null)
                        return a !== null ? a + w.windows[i][j].charge
                          : w.windows[i][j].charge;
        ...
        // Return the rating plan's prices including the calculated charges
        // and the aggregated charge for the rating plan
        return extend({}, rp, {
          aggregated_usage: map(rp.aggregated_usage, chargeMetric),
          prices: prices,
          windows: map(zip.apply(_, map(prices, (p) => {
            return p.windows
          })),
            (zu) => {
              return map(unzip(zu), (uu) => {
      ...
      // Calculate the charge for a metric accross the meter plan's
      // rating_plans
      const chargeMetric = (metric) => {
        return extend({}, metric, {
          windows: map(metric.windows, (w, i) => {
      ...
      // Return the metering plan's rating plans including the calculated
      // charges and the aggregated charge for the metering plan
      return extend({}, mp, {
        aggregated_usage: map(mp.aggregated_usage, chargeMetric),
        rating_plans: ratingPlans,
        windows: map(zip.apply(_, map(ratingPlans, (p) => {
          return p.windows
        })),
          (zu) => {
            return map(unzip(zu), (uu) => {
    ...
    // Calculate the charge for a metric across the resource's metereing plans
    const chargeMetric = (metric) => {
      return extend({}, metric, {
        windows: map(metric.windows, (w, i) => {
          return map(w, (wi, j) => {
            // Do not try to calculate the charge if the window is null
            return wi === null ? null : extend({}, wi, {
              charge: reduce(meteringPlans, (a, p) => {
                // reduce the match metric's plan aggregated usage
                const rw = reduce(filter(p.aggregated_usage,
                  (m) => m.metric === metric.metric), (a, w) => {
                  // Take the sum of a and the window charge unless a is null
    ...
    // Return the resource's metering plans including the calculated charges
    // and the aggregated charge for the resource
    return extend({}, rs, {
      aggregated_usage: map(rs.aggregated_usage, chargeMetric),
      metering_plans: meteringPlans,
      windows: map(zip.apply(_, map(meteringPlans, (p) => {
        return p.windows
      })),
        (zu) => {
          return map(unzip(zu), (uu) => {
...
// Compute usage summaries for the given aggregated usage
const summarizeUsageV2 = function *(t, a, auth) {
  // Summarize the aggregated usage under a resource
  const summarizeResource = function *(rs) {

    // Find the metrics configured for the given resource
    const conf = yield meterConfig(rs.resource_id, a.end,
      systemToken && systemToken());
    ...
    return extend({}, rs, {
      ...
      metering_plans: map(rs.metering_plans, (p) => {
        return extend({}, p, {
          aggregated_usage: map(p.aggregated_usage, (m) => {
            return summarizeMetric(m,
              summarizefnV2(conf.metering_plans, p.metering_plan_id, m.metric));
          })
        });
      })
    });
...
// Return the usage for an org in a given time period
const orgUsageV2 = function *(orgid, time, auth) {
  // Forward authorization header field to account to authorize
  const o = auth ? { headers: { authorization: auth } } : {};
  ...
  debug('Retrieving latest rated usage between %s and %s', eid, sid);
  const doc = yield aggregatordbV2.allDocs({
    endkey: eid,
    startkey: sid,
    descending: true,
    limit: 1,
  ...
  debug('Found rated usage %o', doc.rows[0].doc);
  return yield chargeUsageV2(t,
    yield summarizeUsageV2(t, doc.rows[0].doc, auth), auth);
};

// Retrieve a V2 usage report summary for a given org and time in msec
const retrieveUsageV2 = function *(req) {
  debug('Retrieving rated usage for organization %s on %s',
    req.params.organization_id, req.params.time);

  // Retrieve and return the rated usage for the given org and time
  const doc = yield orgUsageV2(req.params.organization_id,
    req.params.time ? parseInt(req.params.time) : undefined,
    req.headers && req.headers.authorization);

  return {
    body: omit(dbclient.undbify(doc),
      ['last_rated_usage_v2_id', 'aggregated_usage_v2_id',
      'accumulated_usage_v2_id'])
  };
};
...
// Retrieve a v2 usage report summary for a given org and time in msec
routes.get(
  '/v2/metering/organizations/:organization_id/aggregated/usage/:time',
  throttle(retrieveUsageV2));

routes.get(
  '/v2/metering/organizations/:organization_id/aggregated/usage',
  throttle(retrieveUsageV2));
