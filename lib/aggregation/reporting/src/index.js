...
const orgUsageHelper = function *(orgid, time, auth) {
...
const orgUsage = function(orgid, t = moment.now(), auth, cb) {
  const orgUsageCb = yieldable.functioncb(orgUsageHelper);
  orgUsageCb(orgid, t, auth, (err, consumerUsage) => {
    if(err) {
      cb(err);
      return;
    }
    ...
    summarizeUsageCb(t, consumerUsage, tokenAuth, (err, su) => {
      if(err) {
        cb(err);
        return;
      }

      chargeUsageCb(t, su, tokenAuth, (err, cu) => {
        if(err) {
          cb(err);
          return;
        }
