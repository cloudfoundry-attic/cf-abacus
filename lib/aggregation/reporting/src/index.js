...
const aggrdb = yieldable(retry(breaker(batch(db(uris.couchdb,
...
const consumerType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    services: {
      ...
      description: 'The list of metered services.'
    }
  })
});
...
const queryType = new GraphQLObjectType({
  ...
  fields: () => ({
    organization: {
      ...
      resolve: (root, args) => {
        return yieldable.promise(orgUsage)(args.organization_guid,
          args.date);
      }
    },
    organizations: {
      ...
      resolve: (root, args) => {
        return yieldable.promise(orgsUsage)(args.organization_guids,
          args.date);
      }
    },
    account: {
...
routes.get('/v1/organizations/:organization_guid/usage/:day',
  throttle(function *(req) {
    ...
    const doc = a ? a : {
      id: req.params.organization_guid,
      services: [],
      spaces: []
    };
    return {
      body: omit(doc, '_id', '_rev', 'last_aggregated_usage_id',
        'accumulated_usage_id')
    };
  }));

// Retrieve aggregated usage using a GraphQL query
routes.get('/v1/metering/aggregated/usage/graph/:query',
  throttle(function *(req) {
    debug('Retrieving aggregated usage using graphql query %s',
      req.params.query);
    ...
    return {
      body: omit(doc.data, '_id', '_rev', 'last_aggregated_usage_id',
        'accumulated_usage_id')
    };
  }));
