...
const consumerUsage = function *(u, resourceId) {
  ...
  // Error on missing consumer document
  const errorIndex = findIndex(consumers, (c) => c.error);
  if (errorIndex !== -1) {
    const errorMsg = `Failed to find a consumer with id ${ids[errorIndex]}`;
    edebug(errorMsg);
    throw extend(new Error(errorMsg), {
      statusCode: 500,
      error: errorMsg,
      reason: consumers[errorIndex].error,
      noretry: true,
      nobreaker: true
    });
  }
  ...
  for (let c of consumers) {
    const consumerKey = dbclient.k(c.doc._id);
    ...
    consumersDocMap.set(consumerKey, c.doc);
  }
