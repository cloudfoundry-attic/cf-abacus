...
const chargeUsageCb = (t, r, auth, cb) => {
  ...
  tmap(r.resources, chargeResource, (err, cr) => {
    if (err) {
      cb(err);
      return;
    }

    const consumerMapFn = (consumer, i, l, cb) => {
      tmap(consumer.resources, chargeResource,
        (err, cr) => {
          if (err) {
            cb(err);
            return;
    ...
    const spacesMapFn = (space, i, l, cb) => {
      tmap(space.resources, chargeResource, (err, cr) => {
        if (err) {
          cb(err);
          return;
        }

        tmap(space.consumers, consumerMapFn, (err, accumConsumers) => {
          if (err) {
            cb(err);
            return;
          }
    ...
    tmap(r.spaces, spacesMapFn, (err, accumSpaces) => {
      if (err) {
        cb(err);
        return;
      }
...
const chargeUsage = function *(t, r, auth) {
  ...
  return yield yChargeUsage(t, r, auth);
};
...
const transformMapCb = (err, resource, accumResult, cb) => {
  if (err) {
    cb(err);
    return;
  }
...
const summarizeUsageCb = (t, a, auth, cb) => {
  ...
  const summarizePlan = (p, i, l, cb) => {
    ...
    getMeteringPlanCb(p.metering_plan_id, auth, (err, mplan) => {
      ...
      tmap(p.aggregated_usage, mapFn, (err, planAccum) => {
        transformMapCb(err, p, { aggregated_usage: planAccum }, cb);
      });
    });
  };
  ...
  const summarizeConsumer = (c, i, l, cb) => {
    debug('Summarizing consumer %s', c.consumer_id);
    tmap(c.resources, summarizeResource, (err, consumerAccum) => {
      transformMapCb(err, c, { resources: consumerAccum }, cb);
    });
  };

  const summarizeSpace = (s, i, l, cb) => {
    debug('Summirizing space %s', s.space_id);
    tmap(s.resources, summarizeResource, (err, resourceAccum) => {
      if (err) {
      ...
      tmap(s.consumers, summarizeConsumer, (err, consumerAccum) => {
        transformMapCb(err, s, {
  ...
  tmap(a.resources, summarizeResource, (err, resourcesAccum) => {
    ...
    tmap(a.spaces, summarizeSpace, (err, spacesAccum) => {
      transformMapCb(err, a, {
...
const summarizeUsage = function *(t, a, auth) {
  ...
  return yield summarizeUsageEx(t, a, auth);
};
...
const consumerUsage = function *(u, resourceId, startOfMonthKey) {
  ...
  for (let i = 0; i < consumers.length; i++) {
    if (!consumers[i].doc) {
      const errorMsg = `Failed to find consumer with id ${ids[i]}`;
      edebug(errorMsg);
      // throw an error in case the consumer proccessed time is in the current
      // month or skip it otherwise
      if(isConsumerProcessedThisMonth(ids[i], startOfMonthKey))
        throw extend(new Error(errorMsg), {
          statusCode: 500,
          error: errorMsg,
          reason: consumers[i].error,
          noretry: true,
          nobreaker: true
        });
      else
    ...
    if (resourceId) {
      ...
      consumers[i].doc.resources = filter(consumers[i].doc.resources, (res) =>
  ...
  for (let s of doc.spaces)
    s.consumers = yield updateConsumers(u,
      s.consumers, s.space_id, consumersDocMap);
...
const orgUsage = function(orgid, t = moment.now(), auth, resourceId, cb) {
  ...
  consumerUsageCb(orgid, t, auth, resourceId, (err, consumerUsage) => {
    ...
    summarizeUsageCb(t, consumerUsage, tokenAuth, (err, summarizedUsage) => {
      if(err) {
      ...
      chargeUsageCb(t, summarizedUsage, tokenAuth, (err, chargedUsage) => {
        if(err) {
...
const retrieveUsage = (req, res, cb) => {
  ...
  orgUsage(req.params.organization_id,
    ...
    req.headers && req.headers.authorization, resourceId, (err, usage) => {
      ...
      const result = omit(dbclient.undbify(usage),
