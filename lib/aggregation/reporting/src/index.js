    ...
    each(consumer.resources, (resource) => {
      each(resource.plans, (plan) => {
        each(plan.aggregated_usage, (au) => {
          each(au.windows, (w, i) => {
            timewindow.shiftWindow(consumer.processed, usage.processed, w, timewindow.dimensions[i]);
          });
        });
      });
    });
...
const runCLI = () => {
  ...
  const server = reporting().listen();
  
  if (process.env.CONNECTION_TIMEOUT)
    server.setTimeout(process.env.CONNECTION_TIMEOUT);
};
