...
const { extend, each, filter, first, map, omit } = require('underscore');
...
const createCharger = require('./lib/charger.js');
...
const orgUsage = (orgId, time = moment.now(), auth, resourceId, cb) => {
  ...
  consumerUsageCb(orgId, time, auth, resourceId, counter, (err, consumerUsage) => {
    ...
    const summarizer = createSummarizer(time, consumerUsage, tokenAuth, counter);
    summarizer.summarizeUsage((err, summarizedUsage) => {
      ...
      const charger = createCharger(time, summarizedUsage, tokenAuth, counter);
      charger.chargeUsage((err, chargedUsage) => {
        if (err) {
...
const resourceInstanceUsage = function*(
  ...
  const summarizer = createSummarizer(doc.processed, doc, auth, counter);
  const summarizedInstanceReport = yield summarizer.summarizeInstanceUsage();

  const charger = createCharger(doc.processed, summarizedInstanceReport, auth, counter);
  const resourceInstanceReport = yield charger.chargeInstanceUsage();
