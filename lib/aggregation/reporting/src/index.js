  ...
  const summarizeResource = function *(rs) {
    ...
    return extend({}, rs, {
      ...
      plans: yield tmap(rs.plans, function *(p) {
        ...
        const mplan = yield mconfig(p.metering_plan_id,
...
const summarizeInstanceUsage = function *(t, a, auth) {
  ...
  // Find the metrics configured for the given metering plan
  const mplan = yield mconfig(a.metering_plan_id,
    systemToken && systemToken());
  const s = extend({}, a, {
    accumulated_usage: map(a.accumulated_usage, (m) => {
...
const consumerUsage = function *(u) {
  ...
  const shift = (o, n, w, i) => {
    if(w.length < 1)
      return;
    const diffs = [timewindow.secondsWindowDiff, timewindow.minutesWindowDiff,
      timewindow.hoursWindowDiff, timewindow.dateWindowDiff,
      timewindow.monthWindowDiff
    ];

    // Shift by the number of positions counted
    map(Array(Math.max(0, Math.min(w.length, diffs[i](o, n)))), () => {
      w.unshift(null);
      w.pop();
    });
  };
...
const resourceInstanceUsage = function *(
  orgid, conid, resid, planid, mplanid, rplanid, pplanid, time, auth) {
  // Forward authorization header field to account to authorize
  const o = auth ? { headers: { authorization: auth } } : {};
  ...
  const d = new Date(t);
  const mt = Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), 1);
  const k = [orgid, conid, resid, planid, mplanid, rplanid, pplanid].join('/');
