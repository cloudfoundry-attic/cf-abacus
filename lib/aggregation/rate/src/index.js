...
// Return the price for the given resource, plan, metric, and country
...
const caggrfn = (metrics, metric) => {
  return filter(metrics, (m) => m.name === metric)[0].costaggregatefn;
};
...
const rate = (u, pc, r) => {
  ...
  const rateResource = (resource) => {
    ...
    const metrics = config(resource.resource_id).metrics;

    // Rate the new aggregated plan usage
    ratedResource.plans = map(resource.plans, (plan) => {
      const ratedPlan = extend(clone(plan), { cost: 0 });
      ratedPlan.aggregated_usage = map(plan.aggregated_usage, (a) => {

        // Find the rate function configured for each metric
        const rfn = ratefn(metrics, a.metric);

        // Find the cost aggregate function configured for each metric
        const caggr = caggrfn(metrics, a.metric);

        // Clone the metric quantity and return it along with the cost
        const ratedMetric = extend(clone(a), {
          cost: rfn(price(
            resource.resource_id, plan.plan_id, a.metric, pc), a.quantity)
        });

        // Aggregate the metric costs to the overall resource metric costs
        const resourceMetric = filter(ratedResource.aggregated_usage,
          (m) => { return m.metric === a.metric; })[0];
        resourceMetric.cost = caggr(resourceMetric.cost, ratedMetric.cost);

        // Aggregate the cost of the metric to the cost of the plan
        ratedPlan.cost = caggr(ratedPlan.cost, ratedMetric.cost);
        return ratedMetric;
      });
