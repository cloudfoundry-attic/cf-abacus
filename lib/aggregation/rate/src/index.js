...
const getprice = (rid, pid, uid, country) => {
  ...
  if(resource) {
    ...
    if(plan.length) {
      ...
      const metrics = filter(plan[0].metrics, (m) => m.id === uid);
...
// Return metrics aggregation transforms for a given resource id
...
const rate = (u, pc, r) => {
  ...
  const rateResource = (resource) => {
    ...
    // Find the transforms configured for the given resource
    const tforms = transforms(resource.resource_id);
    ...
    ratedResource.plans = map(resource.plans, (plan) => {
      const ratedPlan = extend(clone(plan), {
        aggregated_usage: map(plan.aggregated_usage, (a) => {
          ...
          // Find the rate function configured for the given unit
          const t = filter(tforms, (t) => t.id === a.unit);
          const ratefn = t.length ? t[0].ratefn : (p, q) => 0;

          // Clone the metric quantity and return it along with the cost
          const ratedMetric = extend(clone(a), {
            cost: ratefn(getprice(
              resource.resource_id, plan.plan_id, a.unit, pc), a.quantity)
          });
