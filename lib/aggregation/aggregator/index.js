...
const store = function *(a, u) {
    ...
    const alogdoc = omit(extend(clone(a), { _id: alogid, id: alogid, accumulated_usage_id: u.id }), '_rev');
    yield aggrdb.put(alogdoc);
    debug('Logged aggregated usage %o', alogdoc);

    // Update the aggregated usage
    const aid = dbclient.kturi(k, [a.start].join('-'));
    const adoc = extend(clone(a), { _id: aid, id: aid, last_aggregated_usage_id: alogid, accumulated_usage_id: u.id });
    yield aggrdb.put(adoc);
