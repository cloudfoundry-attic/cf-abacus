...
describe('abacus-usage-aggregator', () => {
  ...
  it('constructs aggregated usage for an organization', () => {
      ...
      spaces: [
        {
          ...
          consumers: [{
            ...
            space_id: 'aaeae239-f3f8-483c-9dd0-de5d41c38b6a',
            processed: 1420286400000,
            doc_id: 'k/a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf28/' +
              'aaeae239-f3f8-483c-9dd0-de5d41c38b6a/' +
              'external:bbeae239-f3f8-483c-9dd0-de6781c38bab/basic/' +
              'test-metering-plan/test-rating-plan/test-pricing-plan/t/' +
    ...
    agg[0].space(
      'aaeae239-f3f8-483c-9dd0-de5d41c38b6a').consumer(
        ...
        'aaeae239-f3f8-483c-9dd0-de5d41c38b6a', 0,
        'k/a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf28/' +
        'aaeae239-f3f8-483c-9dd0-de5d41c38b6a/' +
        'external:bbeae239-f3f8-483c-9dd0-de6781c38bab/basic/' +
        'test-metering-plan/test-rating-plan/test-pricing-plan/t/' +
        '0001420286400000/0001420286400000');
    agg[0].space(
      'aaeae239-f3f8-483c-9dd0-de5d41c38b6a').consumer(
        'external:bbeae239-f3f8-483c-9dd0-de6781c38bab',
        'aaeae239-f3f8-483c-9dd0-de5d41c38b6a', 1420286400000,
        'k/a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf28/' +
        'aaeae239-f3f8-483c-9dd0-de5d41c38b6a/' +
        'external:bbeae239-f3f8-483c-9dd0-de6781c38bab/basic/' +
        'test-metering-plan/test-rating-plan/test-pricing-plan/t/' +
  ...
  it('aggregates usage for an organization', (done) => {
    ...
    const verify = (secured, done) => {
      ...
      const consumer = (u, seq) => ({
        ...
        space_id: u.space_id,
        processed: now.getTime(),
        doc_id: ['k', [u.organization_id, secured ? 1 : 0].join('-'),
          u.space_id, u.consumer_id, 't',
          seqid.pad16(now.getTime()) + seq].join('/')
      });
        ...
        transform.reduce(usage, (a, u, i, l, cb) => {
          ...
          request.post('http://localhost::p/v1/metering/accumulated/usage', {
            ...
            brequest.get(val.headers.location, {}, (err, val) => {
              ...
              .to.deep.equal(omit(uval, 'processed', 'processed_id'));
  ...
  it('aggregator shifts all windows successfully', (done) => {
    clock.restore();
    ...
    postspy = (reqs, cb) => {
      ...
      if(records === 3) {
        ...
        expect(con.resources[0].plans[1].aggregated_usage[0])
          .to.deep.equal(expectedStandard);
        // Check the id doc
        expect(iddoc.organization_doc_id).to.equal(org.id);
        expect(iddoc.consumer_doc_id).to.equal(con.id);
        done();
      }
