...
require.cache[require.resolve('abacus-service-config')].exports =
  extend((sid) => ({
    id: '1234',
    aggregations: [{
      id: 'calls',
...
describe('abacus-usage-aggregator', () => {
  it('constructs aggregated usage for an organization', () => {
    ...
    agg[0].space('space_567').service('1234').resource('calls').quantity =
      12;
    agg[0].space('space_567').consumer('123').service('1234').resource(
      'calls').quantity = 12;
    ...
    expect(JSON.parse(JSON.stringify(agg[0]))).to.deep.equal(aggregated[
      0]);
    ...
    agg[1].space('space_567').service('1234').resource('calls').quantity =
      22;
    agg[1].space('space_567').consumer('123').service('1234').resource(
      'calls').quantity = 22;
    // Serialize to JSON to simulate db storage and retrieval, and expect
    // the object tree to match the above aggregated usage for the org
    expect(JSON.parse(JSON.stringify(agg[1]))).to.deep.equal(aggregated[
      1]);
  });
  ...
  it('aggregates usage for an organization', function(done) {
    ...
    const get = (done) => {
      ...
      map(usage, (u) => request.get(locations[u.id], {}, (err, val) => {
        ...
        if (val.body.usage_id === '222') expect(omit(val.body,
          'id')).to.deep.equal(aggregated[0]);
        if (val.body.usage_id === '223') expect(omit(val.body,
          'id')).to.deep.equal(aggregated[1]);
