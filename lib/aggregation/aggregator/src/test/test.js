...
const { aggregatesInitialUsageFixture, aggregatesWithExisitingUsageFixture, 
  shiftsMonthWindowsFixture, aggregatesWithZeroFixture, aggregatesWithNaNUsage } = require('./fixtures/fixtures'); 
...
describe('abacus-usage-aggregator', () => {
  ...
  context('when secured is not set', () => {
    ...
    context('when aggregates with NaN, null or undefined', () => {
      ...
      it.only('throws error if aggregate function return NaN', async() => {
        startAggregator();
        let error;
        try {
          await sendUsage(aggregatesWithNaNUsage);
        }catch (err) {
          error = err;
        }
    ...
    it('month and day windows successfully', async() => {
      startAggregator();
      await sendUsage(shiftsMonthWindowsFixture.usage[0]);
      await sendUsage(shiftsMonthWindowsFixture.usage[1]);
    ...
    it('aggregates with zero', async() => {
      startAggregator();
      await sendUsage(aggregatesWithZeroFixture.usage[0]);
      await sendUsage(aggregatesWithZeroFixture.usage[1]);
