...
const { correctWindowsTestFixtures, shiftWindowsTestFixtures, aggregateUsageTestFixtures } = require('./fixtures'); 
...
describe('abacus-usage-aggregator', () => {
  ...
  context('when secured is set', () => {
    ...
    aggregateUsageTest('aggregates usage for an organization', () => {
      expect(validatorspy.callCount).to.equal(12);
      expect(authorizespy.args[0][0]).to.equal(undefined);
      expect(authorizespy.args[1][0]).to.equal(undefined);
      expect(authorizespy.args[0][1]).to.deep.equal({ system: ['abacus.usage.write'] });
      expect(authorizespy.args[1][1]).to.deep.equal({ system: ['abacus.usage.read'] });
    });
  ...
  context('when secured is not set', () => {
    ...
    aggregateUsageTest('aggregates usage for an organization', () => {
      ...
      expect(authorizespy.args[0][0]).to.equal(undefined);
      expect(authorizespy.args[1][0]).to.equal(undefined);
      expect(authorizespy.args[0][1]).to.deep.equal(undefined);
      expect(authorizespy.args[1][1]).to.deep.equal(undefined);
    });
