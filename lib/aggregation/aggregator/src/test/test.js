...
require.cache[require.resolve('abacus-cluster')].exports = extend((app) => app, cluster, { single: spy() });

// Mock the service config module
require.cache[require.resolve('abacus-service-config')].exports = extend((sid) => ({ id: '1234', aggregations: [{ id: 'calls', aggregate: (a, qty) => a ? a + qty : qty }]}), config);
