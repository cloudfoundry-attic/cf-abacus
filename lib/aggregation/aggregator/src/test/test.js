...
describe('abacus-usage-aggregator', () => {
  ...
  const cleanUpAggregatorDependancies = () => {
    ...
    delete require.cache[require.resolve('./aggregatorDAO.js')];
  ...
  context('when secured is not set', () => {
    ...
    context('when usage for previous month is sent', () => {
      it('input and output is stored in previous month DB', async() => {
        startAggregator();
        await sendUsage(previousMonthFixture.usage[0]);
        setUpClock(times.startOfNovemberTenPM);
        await sendUsage(previousMonthFixture.usage[1]);

        validateAggregatorOutput(previousMonthFixture.expected.october.firstDoc, 
          await findAggregatorDocs(previousMonthFixture.usage[0]));
        validateAggregatorOutput(previousMonthFixture.expected.november, 
          await findAggregatorDocs(previousMonthFixture.usage[1]));
      });

      it('previous gets aggregated properly', async() => {
        startAggregator();
        await sendUsage(previousMonthFixture.usage[0]);
        setUpClock(Date.UTC(2015, 10, 1, 22));
        await sendUsage(previousMonthFixture.usage[2]);

        const result = await findAggregatorDocs(previousMonthFixture.usage[0]);
        validateAggregatorOutput(previousMonthFixture.expected.october.secondDoc, result);
      });
    });
    ...
    it('aggregates with zero', async() => {
      ...
      validateAggregatorOutput(aggregatesWithZeroFixture.expected, result);
    });
  });
