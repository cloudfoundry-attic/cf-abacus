...
describe('abacus-usage-aggregator', () => {
  ...
  it('aggregates usage for an organization', (done) => {
    ...
    const verify = (secured, done) => {
      ...
      const twindows = (dq, mq, dc, mc, pdq, pmq) => {
        ...
        win[3][0] = dc ? { quantity: dq, previous_quantity: pdq, cost: dc } : { quantity: dq, previous_quantity: pdq };
        win[4][0] = mc ? { quantity: mq, previous_quantity: pmq, cost: mc } : { quantity: mq, previous_quantity: pmq };
        return win;
      };

      // Helper function for creating org resources
      const resource = (apiDQ, apiMQ, memDQ, memMQ, apiDC, apiMC, memDC, memMC, papiDQ, papiMQ, pmemDQ, pmemMQ) => {
        return [
          {
            resource_id: 'test-resource',
            plans: [
              {
                ...
                aggregated_usage: [
                  {
                    metric: 'heavy_api_calls',
                    windows: twindows(apiDQ, apiMQ, apiDC, apiMC, papiDQ, papiMQ)
                  },
                  {
                    metric: 'memory',
                    windows: twindows(memDQ, memMQ, memDC, memMC, pmemDQ, pmemMQ)
                  }
                ]
              }
            ]
  ...
  it('does not fail when aggregating to zero', functioncb(function*() {
    ...
    const usage = extend({}, accumulatedUsage(
      ...
      endT,
      processed,
      { quantity: { current: 0, previous: 80 }, cost: 24 },
      {
        quantity: {
          current: {
            consumed: 518400000,
  ...
  it('trows error if aggregatorFn returns null, undefined or NaN', functioncb(function*() {
    const processed = moment.now();
    const startT = processed - 10000;
    const endT = startT + 5000;
    const orgUsage = require('./test-org')(startT - 10000, endT - 10000, processed - 10000);
  ...
  it('aggregates only to its relevant windows', (done) => {
    ...
    postspy = (reqs, cb) => {
      ...
      if (records === 3) {
        expect(reqs[0][1].body.resources[0].plans[0].aggregated_usage[0].windows[4][1].quantity).to.deep.equal(
          expectedMonth
        );
        expect(reqs[0][1].body.resources[0].plans[0].aggregated_usage[0].windows[3][1].quantity).to.deep.equal(
          expectedDay
        );
        done();
      }
  ...
  it('shifts all windows successfully', (done) => {
    ...
    const expectedBasic = {
      ...
      windows: [
        ...
        [null],
        [null],
        [{ quantity: 500, previous_quantity: null, cost: 75 }, { quantity: 500, cost: 75 }, null],
        [{ quantity: 500, previous_quantity: null, cost: 75 }, { quantity: 500, cost: 75 }]
      ]
    };
    const expectedStandard = {
      metric: 'heavy_api_calls',
      windows: [[null], [null], [null], [null, { quantity: 500, cost: 75 }, null], [null, { quantity: 500, cost: 75 }]]
    };
  ...
  it('attaches error and reasons when there is a business error', (done) => {
    ...
    getspy = (reqs, cb) => {
      ...
      expect(reqs[0][0]).to.equal('http://localhost:9880/v1/rating/plans/:rating_plan_id');
    ...
    request.post(
      ...
      (err, val) => {
        ...
        expect(err).to.have.property('error', 'erplannotfound');
        expect(err).to.have.property('reason', 'Rating plan with id invalidRatingPlan not found');
  ...
  it('processed time falls in prev day/month', (done) => {
    ...
    usage[0].accumulated_usage = [
      {
        ...
        windows: [
          ...
          [null],
          [null],
          [{ quantity: { current: 100 }, cost: 15 }, null],
          [{ quantity: { current: 100 }, cost: 15 }, null]
        ]
      }
    ];
    usage[1].accumulated_usage = [
      {
        ...
        windows: [
          ...
          [null],
          [null],
          [{ quantity: { current: 200 }, cost: 30 }, { quantity: { current: 100 }, cost: 15 }],
          [{ quantity: { previous: 100, current: 300 }, cost: 45 }, null]
        ]
      }
    ];
    ...
    usage[2].accumulated_usage = [
      {
        ...
        windows: [
          ...
          [null],
          [null],
          [{ quantity: { current: 300 }, cost: 45 }, { quantity: { current: 200 }, cost: 30 }],
          [{ quantity: { current: 300 }, cost: 45 }, { quantity: { previous: 100, current: 300 }, cost: 45 }]
        ]
      }
    ];
    ...
    usage[3].accumulated_usage = [
      {
        ...
        windows: [
          ...
          [null],
          [null],
          [{ quantity: { current: 400 }, cost: 60 }, { quantity: { current: 300 }, cost: 45 }],
          [
    ...
    const expectedBasic = {
      ...
      windows: [
        ...
        [null],
        [null],
        [null, { quantity: 400, previous_quantity: null, cost: 60 }],
        [{ quantity: 700, previous_quantity: 300, cost: 105 }, { quantity: 300, cost: 45 }]
      ]
    };
