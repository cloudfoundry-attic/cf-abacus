  ...
  it('aggregates usage for an organization', function(done) {
    ...
    const get = (done) => {
      ...
      map(usage, (u) => request.get(locations[u.id], {}, (err, val) => {
        ...
        // Expect the accumulated usage id to be the same as the usage id
        expect(val.body.accumulated_usage_id).to.deep.equal(u.id);
        expect(val.body.accumulated_usage_id === '222' ||
          val.body.accumulated_usage_id === '223' ||
          val.body.accumulated_usage_id === '224' ||
          val.body.accumulated_usage_id === '225').to.equal(true);
        ...
        const checkaggr = () => {
          ...
          if(val.body.accumulated_usage_id === '224')
            expect(omit(val.body, 'id')).to.deep.equal(aggregated[2]);
          if(val.body.accumulated_usage_id === '225')
            expect(omit(val.body, 'id')).to.deep.equal(aggregated[3]);
        };
        checkaggr();
