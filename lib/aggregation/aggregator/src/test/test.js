...
describe('abacus-usage-aggregator', () => {
  ...
  context('when secured is not set', () => {
    ...
    it('shifts month and day windows successfully', async() => {
      ...
      const result = await findAggregatorDocs(shiftsMonthWindowsFixture.usage.november.fourAM);
    ...
    it('aggregates with zero', async() => {
      ...
      const result = await findAggregatorDocs(aggregatesWithZeroFixture.usage.october.oneAM);
