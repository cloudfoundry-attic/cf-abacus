...
const { buildAccumulatedUsage } = require('./fixtures');  
...
const testResourceID = '0b39fa70-a65f-4183-bae8-385633ca5c87';
...
describe('abacus-usage-aggregator', () => {
  ...
  //   });
  it.only('aggregates usage for an organization', (done) => {
      ...
      const aggregated = [
        {
          ...
          resource_instance_id: testResourceID,
          resource_id: 'test-resource',
          plan_id: 'basic',
          pricing_country: 'USA',
          prices: {
  ...
  it('aggregates only to its relevant windows', (done) => {
    ...
    const usage = [
      buildAccumulatedUsage('bounds', 1446249600000, 1446249600000, 1446256800000),
      buildAccumulatedUsage('bounds', 1446336000000, 1446336000000, 1446418800000),
      buildAccumulatedUsage('bounds', 1446422400000, 1446422400000, 1446418800000)
    ];
    ...
    postspy = (reqs, cb) => {
      ...
      if (records === 3) {
        expect(previousMonthWindow(reqs[0][1])).to.be.null;
        expect(previousDayWindow(reqs[0][1])).to.be.null;

        expect(currentMonthWindow(reqs[0][1])).to.not.be.null;
        expect(currentDayWindow(reqs[0][1])).to.not.be.null;
        done();
      }
