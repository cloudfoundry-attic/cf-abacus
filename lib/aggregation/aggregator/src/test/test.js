...
process.env.RATING = 'http://localhost:9400';
...
describe('abacus-usage-aggregator', () => {
  ...
  it('aggregates usage for an organization', (done) => {
    ...
    const accumulatedUsage = (rid, s, e, hour, api, mem) => ({
      ...
      accumulated_usage: [{
        metric: 'heavy_api_calls',
        quantity: [[0], [0], [0], [api, 0, 0], [api, 0]]
      },
        {
          metric: 'memory',
          quantity: [[0], [0], [0], [mem, 0, 0], [mem, 0]]
        }]
    });
    ...
    const usage = [
      ...
      accumulatedUsage('1b39fa70-a65f-4183-bae8-385633ca5c88', 1420247000000,
        1420249000000, 2, { current: 8 }, {
          current: { consumed: 0, consuming: 3, since: 1420247000000 }
        }),
      accumulatedUsage('1b39fa70-a65f-4183-bae8-385633ca5c88', 1420249000000,
        1420251000000, 3, { previous: 8, current: 10 }, {
    ...
    const verify = (secured, done) => {
      ...
      postspy = (reqs, cb) => {
        ...
        expect(reqs[0][0])
          .to.equal('http://localhost:9400/v1/rating/aggregated/usage');
      ...
      const post = () => {
        ...
        transform.reduce(usage, (a, u, i, l, cb) => {
          ...
          expect(validatorspy.callCount).to.equal(secured ? 8 : 0);
