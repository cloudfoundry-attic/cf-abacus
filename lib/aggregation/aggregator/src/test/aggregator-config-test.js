...
describe('aggregator reducer config test', () => {
  ...
  context('when secured', () => {
    beforeEach(() => {
      reducerConfig = createDataflowReductionConfig(true, sandbox.any, sandbox.any);
    });
  ...
  context('when not secured', () => {
    beforeEach(() => {
      reducerConfig = createDataflowReductionConfig(false, sandbox.any, sandbox.any);
    });
  ...
  context('for usage doc with', () => {
    beforeEach(() => {
      reducerConfig = createDataflowReductionConfig(sandbox.any, sandbox.any, sandbox.any);
    });
    ...
    context('consumer id missing', () => {
      ...
      context('usage doc contains dedup id', () => {
        const udocWithDedupId = extend({}, udocWithoutConsumer, { dedup_id: dedupId });
    ...
    context('consumer id provided', () => {
      ...
      context('usage doc contains dedup id', () => {
        const udocWithDedupId = extend({}, udoc, { dedup_id: dedupId });
