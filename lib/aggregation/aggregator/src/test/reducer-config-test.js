...
const { createReducerConfig } = require('../lib/reducer-config');
...
describe('aggregator reducer config test', () => {
  ...
  const validateReducerConfig = (msg, isSecured) => context(msg, () => {
    beforeEach(() => {
      reducerConfig = createReducerConfig(sandbox.stub().returns(isSecured), sandbox.any, sandbox.any);
    });
    ...
    context('consumer id is missing', () => {
      ...
      it('correct input groups is returned', () => {
        expect(reducerConfig.input.groups(udocWithoutConsumer)[0]).to.equal(
          'org-id');
        expect(reducerConfig.input.groups(udocWithoutConsumer)[1]).to.equal(
          'org-id/space-id/UNKNOWN');
        expect(reducerConfig.input.groups(udocWithoutConsumer)[2]).to.equal(
          'org-id/space-id');
        expect(reducerConfig.input.groups(udocWithoutConsumer)[3]).to.equal(
          'org-id/res-instance-id/UNKNOWN/plan-id/metering-plan-id/rating-plan-id/pricing-plan-id');
      });

      it('output key is correct', () => {
        expect(reducerConfig.output.keys(udocWithoutConsumer)[0])
          .to.equal('org-id');

        expect(reducerConfig.output.keys(udocWithoutConsumer)[1])
          .to.equal('org-id/space-id/UNKNOWN');

        expect(reducerConfig.output.keys(udocWithoutConsumer)[2])
          .to.equal('org-id/space-id');

        expect(reducerConfig.output.keys(udocWithoutConsumer)[3])
          .to.equal('org-id/res-instance-id/UNKNOWN/plan-id/metering-plan-id/rating-plan-id/pricing-plan-id');
      });

      context('usage doc contains dedup id', () => {
        let udocWithDedupId;
        beforeEach(() => {
          reducerConfig = createReducerConfig(sandbox.stub().returns(false), sandbox.any, sandbox.any);
          udocWithDedupId = extend({}, udocWithoutConsumer, {
            dedup_id: dedupId
          });
        });

        it('output key is correct', () => {
          expect(reducerConfig.output.keys(udocWithDedupId)[3]).to.equal(
    ...
    context('consumer id is provided', () => {
      ...
      it('correct input groups is returned', () => {
        expect(reducerConfig.input.groups(udoc)[0]).to.equal(
          'org-id');
        expect(reducerConfig.input.groups(udoc)[1]).to.equal(
          'org-id/space-id/consumer-id');
        expect(reducerConfig.input.groups(udoc)[2]).to.equal(
          'org-id/space-id');
        expect(reducerConfig.input.groups(udoc)[3]).to.equal(
          'org-id/res-instance-id/consumer-id/plan-id/metering-plan-id/rating-plan-id/pricing-plan-id');
      });

      it('output key is correct', () => {
        expect(reducerConfig.output.keys(udoc)[0])
          .to.equal('org-id');

        expect(reducerConfig.output.keys(udoc)[1])
          .to.equal('org-id/space-id/consumer-id');

        expect(reducerConfig.output.keys(udoc)[2])
          .to.equal('org-id/space-id');

        expect(reducerConfig.output.keys(udoc)[3])
          .to.equal('org-id/res-instance-id/consumer-id/plan-id/metering-plan-id/rating-plan-id/pricing-plan-id');
      });
      ...
      context('usage doc contains dedup id', () => {
        ...
        it('output key is correct', () => {
          expect(reducerConfig.output.keys(udocWithDedupId)[3]).to.include(dedupId);
          expect(reducerConfig.output.keys(udocWithDedupId)[3]).to.equal(
