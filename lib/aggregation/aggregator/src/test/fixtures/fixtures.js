...
const aggregatesWithNaNUsage = buildAccumulatedUsage(
  withAccumulatedUsage(usageForAggregatesInitialUsage),
  withAccumulatedUsage([buildAccumulatedUsage(
    withMetricName('heavy_api_calls'),
    withCurrentDayQuantity({ current: undefined }),
    withCurrentMonthQuantity({ current: undefined })
  )])
);

module.exports = { 
  aggregatesInitialUsageFixture, aggregatesWithExisitingUsageFixture, shiftsMonthWindowsFixture, 
  aggregatesWithZeroFixture, aggregatesWithNaNUsage
};
