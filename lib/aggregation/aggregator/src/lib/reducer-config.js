...
const okeys = (udoc, ikey) => {
  const firstKey = [udoc.organization_id, udoc.space_id, udoc.consumer_id || 'UNKNOWN'];
  const secondKey = [udoc.organization_id, udoc.space_id];
  const thirdKey = [
    udoc.organization_id,
    udoc.resource_instance_id,
    udoc.consumer_id || 'UNKNOWN',
...
const createReducerConfig = (secured, sampling, token) => {
  return {
    input: {
