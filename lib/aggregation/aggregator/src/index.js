...
Org.prototype.service = function(id) {
  return lazyCons(this.services, id, Org.Service);
};
...
Org.Space.prototype.service = function(id) {
  return lazyCons(this.services, id, Org.Service);
};
...
Org.Consumer.prototype.service = function(id) {
  return lazyCons(this.services, id, Org.Service);
};

// Represent a service and its aggregated resource usage
Org.Service = function(id) {
  extend(this, {
    id: id,
    plans: [],
    aggregated_usage: []
...
Org.Service.prototype.plan = function(id) {
  return lazyCons(this.plans, id, Org.Plan);
};
Org.Service.prototype.resource = function(unit) {
  return lazyCons(this.aggregated_usage, unit, Org.Resource, 'unit');
};

// Represent a plan and its aggregated resource usage
...
Org.Plan.prototype.resource = function(unit) {
  return lazyCons(this.aggregated_usage, unit, Org.Resource, 'unit');
};

// Represent a resource aggregated usage
Org.Resource = function(unit) {
  extend(this, {
    unit: unit,
    quantity: 0
  });
...
const reviveOrg = (org) => {
  ...
  map(org.services, (s) => {
    s.plan = Org.Service.prototype.plan;
    s.resource = Org.Service.prototype.resource;
    map(s.plans, (s) => {
      s.resource = Org.Plan.prototype.resource;
    });
  });
  map(org.spaces, (s) => {
...
// Return a function to get an aggregate function from a service for an
// aggregation unit
const saggr = (sid) => {
  ...
  return (u) => {
    ...
    // If aggregate function is defined use it, else use the default sum
    // function
    return f.length && f[0].aggregate ? f[0].aggregate : (a, qty) => a ? a +
      qty : qty;
  };
};
...
const aggregate = (a, u) => {
  ...
  map(u.accumulated_usage, (ua) => {
    ...
    aggrfn(newa.service(u.service_id).plan(u.plan_id).resource(unit), delta);
    aggrfn(
      newa.space(u.space_guid).service(u.service_id).resource(unit), delta);
    aggrfn(
      newa.space(u.space_guid).service(u.service_id).plan(u.plan_id)
      .resource(unit), delta);
    aggrfn(
      newa.space(u.space_guid).consumer(u.consumer ? u.consumer.value : 'all')
      .service(u.service_id).resource(unit), delta);
    aggrfn(
      newa.space(u.space_guid).consumer(u.consumer ? u.consumer.value : 'all')
      .service(u.service_id).plan(u.plan_id).resource(unit), delta);
  });
...
const aggregateUsage = function *(u) {
  ...
  const alogid = dbclient.kturi(u.organization_guid,
  ...
  try {
    ...
    // Aggregate usage
    newa = aggregate(a ? a : extend(newOrg(u.organization_guid), {
      start: day(u.end),
      end: eod(u.end)
    }), u);
