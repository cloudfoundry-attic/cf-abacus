...
const aggrdb = yieldable(retry(breaker(db(uris.couchdb,
  'abacus-aggregated-usage'))));
const logdb = yieldable(retry(breaker(batch(db(uris.couchdb,
  'abacus-aggregated-usage-log')))));
...
const aggregate = (a, u) => {
  ...
  map(u.accumulated_usage, (ua) => {
    ...
    aggrfn(newa.space(u.space_guid).service(u.service_id).resource(unit),
      delta);
    aggrfn(newa.space(u.space_guid).consumer(u.consumer ? u.consumer.value :
      'all').service(u.service_id).resource(unit), delta);
  });
