...
const aggregate = (a, u) => {
  ...
  map(u.accumulated_usage, (ua) => {
    ...
    const aggr = (a, delta) => {
      ...
      a.quantity = afn(a.quantity, delta);
    };

    // Apply the aggregate function to the aggregated usage tree
    aggr(
      newa.resource(u.resource_id).metric(ua.metric), ua.delta);
    aggr(
      newa.resource(u.resource_id).plan(u.plan_id)
      .metric(ua.metric), ua.delta);
    aggr(
    ...
    aggr(newa.space(u.space_id).consumer(
        ...
        .resource(u.resource_id).metric(ua.metric), ua.delta);
    aggr(newa.space(u.space_id).consumer(
      u.consumer ? u.consumer.consumer_id : 'ALL')
      .resource(u.resource_id).plan(u.plan_id).metric(ua.metric), ua.delta);
  });
