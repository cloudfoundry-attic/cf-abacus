...
const aggrdb = yieldable(
  retry(breaker(db.logdb(uris.couchdb, 'abacus-aggregated-usage'))));
const logdb = yieldable(
  retry(breaker(batch(db.logdb(uris.couchdb, 'abacus-aggregated-usage-log')))));
...
const aggregate = (a, u) => {
  ...
  map(u.accumulated_usage, (ua) => {
    ...
    const aggr = (a, qty) => {
      ...
      a.quantity = afn(a.quantity, qty[6].previous || 0, qty[6].current);
    };
