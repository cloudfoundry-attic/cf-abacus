...
const calculateUsageEnd = (usage) => {
  let newEnd = usage.processed;

  const startOfMonth = moment.utc().startOf('month');

  const usageEnd = moment.utc(usage.end);
  if (usageEnd.isBefore(startOfMonth))
    newEnd = usageEnd.endOf('month').valueOf();
...
const aggregate = function*(aggrs, usage) {
  ...
  const newEnd = calculateUsageEnd(usage);
  ...
  timewindow.shift(newa, a, Math.max(usage.end, newEnd));
  timewindow.shift(newc, c, Math.max(usage.end, newEnd));
  timewindow.shift(news, s, Math.max(usage.end, newEnd));
