...
// Return a function to get an aggregate function from a resource for an
// aggregation unit
const saggr = (sid) => {
...
const aggregate = (a, u) => {
  ...
  map(u.accumulated_usage, (ua) => {
    ...
    aggrfn(newa.resource(u.resource_id).metric(unit), delta);
    aggrfn(newa.resource(u.resource_id).plan(u.plan_id).metric(unit), delta);
    aggrfn(
