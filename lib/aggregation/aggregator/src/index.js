...
const aggregate = function*(aggrs, u) {
  ...
  each(u.accumulated_usage, (ua) => {
    const metricName = ua.metric;

    const aggregateHashFunction = (currentQuantity, previous, current, aggCell, accCell, previousQuantity) =>
      `${meteringPlanId}${metricName}${currentQuantity}` +
      `${JSON.stringify(previous)}${JSON.stringify(current)}${previousQuantity}`;
    const aggregationFn = memoize(aggrfn(mplan.metering_plan.metrics, metricName), aggregateHashFunction, lruOpts);

    const metricPrice = price(u.prices.metrics, ua.metric);

    const rateHashFunction = (metricPrice, quantity) =>
      `${meteringPlanId}${metricName}${metricPrice}${JSON.stringify(quantity)}`;
    const rateFn = memoize(ratefn(rplan.rating_plan.metrics, metricName), rateHashFunction, lruOpts);
    ...
    const aggr = (am) => {
      ...
      am.windows = map(am.windows, (w, i) => {
        ...
        if (w.length < ua.windows[i].length) each(Array(ua.windows[i].length - w.length), () => w.push(null));
