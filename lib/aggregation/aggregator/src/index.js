...
Org.Resource.prototype.metric = function(unit) {
  return lazyCons(this.aggregated_usage, 'unit', unit, Org.Metric);
};
...
const aggregate = (a, u) => {
  ...
  map(u.accumulated_usage, (ua) => {
    ...
    // Find the aggregate function configured for the given unit
    const t = filter(transforms(u.resource_id), (t) => t.id === ua.unit);
    const aggrfn = t.length ? t[0].aggregatefn : (a, q) => a ? a + q : q;
    const update = (a, delta) => {
      // We're mutating the input quantity here but it's really the simplest
      // way to apply the aggregation formula
      a.quantity = aggrfn(a.quantity, delta);
    };

    // Apply the aggregate function to the aggregated usage tree
    update(newa.resource(u.resource_id).metric(ua.unit), ua.delta);
    update(
      newa.resource(u.resource_id).plan(u.plan_id).metric(ua.unit), ua.delta);
    update(
      newa.space(u.space_id).resource(u.resource_id).metric(ua.unit), ua.delta);
    update(newa.space(u.space_id).resource(u.resource_id).plan(u.plan_id)
      .metric(ua.unit), ua.delta);
    update(newa.space(u.space_id).consumer(
        u.consumer ? u.consumer.consumer_id : 'ALL')
        .resource(u.resource_id).metric(ua.unit), ua.delta);
    update(newa.space(u.space_id).consumer(
      u.consumer ? u.consumer.consumer_id : 'ALL')
      .resource(u.resource_id).plan(u.plan_id).metric(ua.unit), ua.delta);
  });
