  ...
  yield tmap(u.accumulated_usage, function *(ua) {
    ...
    const aggr = (a, qty) => {
      ...
      a.quantity = map(a.quantity, (q, i) => {
        if(Math.floor(n / timescale[i]) - Math.floor(o / timescale[i]) >= 1)
...
const updateAggregatedUsage =
  yieldable(batch(batch.groupBy(function *(calls) {
    ...
    debug(
      'Update aggregated usage for a group of %d usage docs with id %s',
      calls.length, aid);
    ...
    try {
      ...
      const rres = yield treduce(calls, function *(rres, call) {
        ...
        const oldend = dateUTCNumbify(rres.newa.end) || 0;
        ...
        newa: a ? omit(a,'last_aggregated_usage_id') :
          newOrg(calls[0][0].u.organization_id),
        ares: []
      });

      // Store the final new aggregated usage
      const lcall = last(calls);
      yield storeAggregatedUsage(
        rres.newa, aid, lcall[0].alogid, lcall[0].uid);
