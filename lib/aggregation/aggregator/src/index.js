...
const otimes = (udoc, itime) =>
  [itime, itime, map([udoc.start, udoc.end], seqid.pad16).join('/')];
...
Org.Space.prototype.consumer = function(id, space_id, processed, doc_id) {
  // Construct or retrieve the consumer object
  const consumer = lazyCons(this.consumers, 'consumer_id', id, Org.Consumer);
  consumer.processed = processed;
  consumer.doc_id = doc_id;
  consumer.space_id = space_id;

  // Filter out any pruneable consumers
  this.consumers = filter(this.consumers, (c) => shouldNotPrune(c.processed));
};
...
// Gather reasons of error
const compileErrors = (info) => {
  let e;

  // Go through each information and create list of error reasons
  const reasons = reduce(info, (a, i) => {
    e = e || i.error;
    return i.error ? a.concat(i.reason) : a;
  }, []);
...
const aggregate = function *(aggrs, u) {
  ...
  // Compile errors
  const e = compileErrors([mplan, rplan]);
  if(e.error) {
    debug('The usage submitted has business errors ' + e.reasons);
    return [extend({}, u, e)];
  }
  ...
  const iddoc = extend({}, u, {
    accumulated_usage: map(u.accumulated_usage, (au) => {
      return extend({}, {
        metric: au.metric,
        windows: map(au.windows, (w) => {
          return map(w, (tw) => {
            return tw ? extend({}, tw) : tw;
          });
        })
      });
    }) }, {
      processed: u.processed,
      organization_doc_id: dbclient.kturi(okeys(u)[0], dbclient.t(u.id)),
      consumer_doc_id: dbclient.kturi(okeys(u)[1], dbclient.t(u.id))
    });
  ...
  map(u.accumulated_usage, (ua) => {
    ...
    newa.space(u.space_id).consumer(u.consumer_id || 'UNKNOWN',
      u.space_id, u.processed, dbclient.kturi(okeys(u)[1], dbclient.t(u.id)));
...
const aggregateUsage = function *(aggrs, udocs) {
  const ologs = yield treduce(udocs, function *(log, udoc, i, l) {
    ...
    // There is error, returns error without aggregating further
