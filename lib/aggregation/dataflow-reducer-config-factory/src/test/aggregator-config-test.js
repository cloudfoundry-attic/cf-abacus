...
describe('aggregator reducer config test', () => {
  ...
  const validateReducerConfig = (msg, isSecured) => context(msg, () => {
    beforeEach(() => {
      reducerConfig = createAggregatorConfig(sandbox.stub().returns(isSecured), sandbox.any, sandbox.any);
    });
    ...
    context('consumer id is missing', () => {
      ...
      context('usage doc contains dedup id', () => {
        let udocWithDedupId;
        beforeEach(() => {
          reducerConfig = createAggregatorConfig(sandbox.stub().returns(false), sandbox.any, sandbox.any);
