...
// Setup debug log
const debug = require('cf-abacus-debug')('cf-abacus-instance-launcher');
...
const runCLI = () => {
    ...
    if(process.env.VCAP_APPLICATION) {
        ...
        // Add listeners to stdout, stderr and exit messsage and forward the messages to debug logs
        ...
        // instances
        map(range(n), (i) => {
            // Clone the process env and set values for PORT and VCAP_APPLICATION variables
            const env = clone(process.env);
            env.VCAP_APPLICATION = JSON.stringify({ instance_index: i });

            debug('Starting application %s instance %s with port %s', app, i, port + i);
            const c = cp.spawn('npm', ['run', app, port + i], { env: env });
