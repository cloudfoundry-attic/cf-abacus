...
// Minimalistic stub for a Eureka compatible service registry.
...
// Setup debug log
const debug = require('abacus-debug')('abacus-eureka-stub');
...
const db = yieldable(retry(breaker(batch(
  ...
  dbclient.dburi(uris.couchdb, 'abacus-app-instances'))))));
...
const eureka = () => {
  ...
  routes.use(router.batch(routes));
  app.use(routes);
  return app;
};
