...
describe('abacus-provisioning-stub', () => {
  ...
  it('returns the resource_type', (done) => {
    // Create a test provisioning app
    const app = provisioning();
  ...
  it('validates creation of new metering plans', (done) => {
    ...
    const validGetRequest = function(meteringPlan) {
      request.get(
        'http://localhost::p/v1/metering/plans/:metering_plan_id/config', {
          p: server.address().port,
          metering_plan_id: meteringPlan.plan_id
        }, (err, val) => {
          expect(err).to.equal(undefined);
          expect(val.statusCode).to.equal(200);
          expect(val.body).to.deep.equal(meteringPlan);
          checkDone();
        });
    };

    const getFromCache = function(meteringPlan) {
      request.get(
        'http://localhost::p/v1/metering/plans/:metering_plan_id/config', {
          p: server.address().port,
          metering_plan_id: meteringPlan.plan_id
    ...
    const postRequest = function(meteringPlan) {
      request.post(
        'http://localhost::p/v1/metering/plans/:metering_plan_id/config', {
          p: server.address().port,
          metering_plan_id: meteringPlan.plan_id,
          ...
          checkDone();
          validGetRequest(meteringPlan);
          getFromCache(meteringPlan);
        });
    };
    postRequest(meteringPlan);
    request.post(
      'http://localhost::p/v1/metering/plans/:metering_plan_id/config', {
        p: server.address().port,
