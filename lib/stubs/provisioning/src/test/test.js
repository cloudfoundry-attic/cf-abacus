...
describe('abacus-provisioning-stub', () => {
  ...
  it('validates creation of new resource configurations', (done) => {
    ...
    const validGetRequest = function(meteringConfig) {
      request.get(
        'http://localhost::p/v1/metering/plans/:metering_plan_id/config', {
          p: server.address().port,
          metering_plan_id: meteringConfig.plan_id,
          time: 1420070400000
        }, (err, val) => {
          expect(err).to.equal(undefined);
          expect(val.statusCode).to.equal(200);
          expect(val.body).to.deep.equal(meteringConfig);
          checkDone();
        });
    };

    const getFromCache = function(meteringConfig) {
      request.get(
        'http://localhost::p/v1/metering/plans/:metering_plan_id/config', {
          p: server.address().port,
          metering_plan_id: meteringConfig.plan_id,
    ...
    const postRequest = function(meteringConfig) {
      request.post(
        'http://localhost::p/v1/metering/plans/:metering_plan_id/config', {
          p: server.address().port,
          metering_plan_id: meteringConfig.plan_id,
          ...
          checkDone();
          validGetRequest(meteringConfig);
          getFromCache(meteringConfig);
        });
    };
    postRequest(meteringConfig);
    request.post(
      'http://localhost::p/v1/metering/plans/:metering_plan_id/config', {
        p: server.address().port,
