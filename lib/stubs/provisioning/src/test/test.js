...
describe('abacus-provisioning-stub', () => {
  it('returns resource instance info by ' +
    ...
    const verify = (secured, done) => {
      ...
      // Expected return value from provisioned resource instance
      const validated = extend({}, valid, {
      ...
      request.get(
        'http://localhost::p/v2/provisioning/orgs/:org_id/' +
        'spaces/:space_id/consumers/:consumer_id/resources/:resource_id/' +
        'plans/:plan_id/instances/:resource_instance_id/:time', extend({
          p: server.address().port
        }, valid), (err, val) => {
          expect(err).to.equal(undefined);
          expect(val.statusCode).to.equal(200);
          expect(val.body).to.deep.equal(validated);
          cb();
        });
  ...
  it('returns available metering configs', (done) => {
    // Create a test provisioning app
    const app = provisioning();
    ...
    request.get(
      'http://localhost::p/v2/provisioning/metering/resources' +
      '/:resource_id/config/:time', {
        p: server.address().port,
        resource_id: 'object-storage',
        time: 1420070400000
        ...
        expect(val.statusCode).to.equal(200);
        expect(val.body).to.deep.equal(
          require('../metering-configs/object-storage'));
        done();
      });
  });

  it('returns available rating configs', (done) => {
    // Create a test provisioning app
    const app = provisioning();
    ...
    request.get(
      'http://localhost::p/v2/provisioning/rating/resources' +
      '/:resource_id/config/:time', {
        p: server.address().port,
        resource_id: 'object-storage',
        time: 1420070400000
        ...
        expect(val.body).to.deep.equal(
          require('../rating-configs/object-storage'));
        done();
      });
  });
  ...
  it('validates all test metering configurations', () => {
    map([
      'analytics',
      'object-storage',
      'linux-container',
      ...
      console.log('    validating', name, ' resource');
      const conf = require('../metering-configs/' + name);
      expect(schemas.meteringConfig.validate(conf)).to.deep.equal(conf);
      console.log('        validated', name, ' resource');
    });
  });

  it('validates all test rating configurations', () => {
    map([
      'analytics',
      'object-storage',
      'linux-container',
