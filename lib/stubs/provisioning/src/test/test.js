...
describe('abacus-provisioning-stub', () => {
  ...
  it('validates creation of new resource configurations', (done) => {
    ...
    // Validate a valid provisioned resource definition
    const resourceDefinitionV1 = {
    ...
    const validGetRequest = function(resourceDefinition) {
      request.get(
        'http://localhost::p/v1/provisioning/resources/:resource_id/' +
        'config/:time', {
          p: server.address().port,
          resource_id: resourceDefinition.resource_id,
          time: new Date().getTime()
        }, (err, val) => {
          expect(err).to.equal(undefined);
          expect(val.statusCode).to.equal(200);
          expect(val.body).to.deep.equal(resourceDefinition);
          checkDone();
        });
    };

    const invalidGetRequest = function(resourceDefinition) {
      request.get(
        'http://localhost::p/v1/provisioning/resources/:resource_id/' +
        'config/:time', {
          p: server.address().port,
    ...
    const postRequest = function(resourceDefinition) {
      request.post(
        'http://localhost::p/v1/provisioning/resources/:resource_id', {
          p: server.address().port,
          resource_id: resourceDefinition.resource_id,
          ...
          checkDone();
          validGetRequest(resourceDefinition);
          invalidGetRequest(resourceDefinition);
        });
    };
    postRequest(resourceDefinitionV1);
    request.post(
      'http://localhost::p/v1/provisioning/resources/:resource_id', {
        p: server.address().port,
        resource_id: 'test',
