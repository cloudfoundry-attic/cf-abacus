...
const mcache = (k, resource) => {
  mconfigs.set(k, resource);
  return resource;
};
...
const rcache = (k, resource) => {
  rconfigs.set(k, resource);
  return resource;
};

// Return a rating config from the cache
const rcached = (k) => {
  return rconfigs.get(k);
};

// Search for a metering config effective at given time. Search in a local
...
// Search for a metering config effective at given time. Search in a local
// metering-configs directory.
const mconfig = function * (rid, time) {
  // Round time to a 10 min boundary
  const t = Math.floor(time / 600000) * 600000;
  const k = [rid, t].join('/');
  ...
  const unlock = yield lock(k);
  try {
    debug('Retrieving metering config for resource %s effective %d', rid, t);

    // Look in our cache first
    const cc = mcached(k);
    if(cc) {
      debug('Metering config %s effective from %s found in cache',
        rid, time);
      return cc;
    }
    ...
    // Look in the local resources dir
    try {
      const c = require('./metering-configs/' + rid);
      if(c && c.effective <= time)
        return mcache(k, c);
    }
    catch (e) {
    }
    debug('No metering config %s effective from %s found in database',
      rid, time);
...
// Search for a rating config effective at given time. Search in a local
// rating-configs directory.
const rconfig = function * (rid, time) {
  // Round time to a 10 min boundary
  const t = Math.floor(time / 600000) * 600000;
  const k = [rid, t].join('/');
  ...
  try {
    ...
    const cc = rcached(k);
    if(cc) {
      debug('Rating config %s effective from %s found in cache',
        rid, time);
      return cc;
    }
    ...
    // Look in the local resources dir
    try {
      const c = require('./rating-configs/' + rid);
      if(c && c.effective <= time)
        return rcache(k, c);
    }
    catch (e) {
    ...
    debug(
      'No rating config %s effective from %s found in local resources',
      rid, time);
...
// a resource instance (for example that the given app is or was bound at some
// point to that particular instance) and return provisioning information
// for that resource instance. Also returns the org's information:
// account_id, metering_plan_id, rating_plan_id, resource_type, and price_id
routes.get(
  '/v2/provisioning/orgs/:org_id/spaces/:space_id/consumers/' +
  ':consumer_id/resources/:resource_id/plans/:plan_id/instances/' +
  ':resource_instance_id/:time', function *(req) {
  const path = extend(pick(req.params,
    'org_id', 'space_id', 'consumer_id',
  ...
  // This is a stub here so we only validate the resource_id. A complete
  // implementation of this service should validate all the other
  // parameters. Validate if the resource has the metering plan.
  if(!(yield mconfig(req.params.resource_id, parseInt(req.params.time))))
    return {
      status: 404
    };
  // Validate if the resource has the rating plan
  if(!(yield rconfig(req.params.resource_id, parseInt(req.params.time))))
    return {
      status: 404
    };
  ...
  return {
    status: 200,
    body: extend(fake, path, {
      metering_plan_id: req.params.plan_id,
      rating_plan_id: req.params.plan_id
    })
  };
});
...
// Return the metering configuration for a particular resource id and
// effective time (allowing evolution of configurations over time, where
// different versions of a resource configuration will be used at different
// points in time)
routes.get(
  '/v2/provisioning/metering/resources/:resource_id/config/:time',
  function *(req) {
    debug('Retrieving metering resource config %s at time %d',
      req.params.resource_id, req.params.time);

    const conf = yield mconfig(
      req.params.resource_id, parseInt(req.params.time));
    if(!conf)
      return {
        status: 404
...
// Return the rating configuration for a particular resource id and
// effective time (allowing evolution of configurations over time, where
// different versions of a resource configuration will be used at different
// points in time)
routes.get(
  '/v2/provisioning/rating/resources/:resource_id/config/:time',
  function *(req) {
    debug('Retrieving rating resource config %s at time %d',
      req.params.resource_id, req.params.time);

    const conf = yield rconfig(
      req.params.resource_id, parseInt(req.params.time));
    if(!conf)
      return {
        status: 404
...
const provisioning = () => {
  ...
  // bearer access token
  if (secured())
    app.use(/^\/v[1-2]\/provisioning|^\/batch$/,
