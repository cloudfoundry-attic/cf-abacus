...
routes.get(
  ...
  ':app_id/services/:service_id/plans/:plan_id/instances/' +
  ':service_instance_id', function *(req) {
    const path = pick(req.params, 'region', 'org_id', 'space_id',
      'app_id', 'service_id', 'plan_id', 'service_instance_id');
...
routes.get(
  '/v1/provisioning/regions/:region/orgs/:org_id/spaces/:space_id/consumers/' +
  ':app_id/runtimes/:runtime_id/plans/:plan_id', function *(req) {
    const path = pick(req.params,
      'region', 'org_id', 'space_id', 'app_id', 'runtime_id', 'plan_id');
