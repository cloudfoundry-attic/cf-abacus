...
describe('abacus-account-stub', () => {
  ...
  it('returns a resource price config in V2', (done) => {
    process.env.SECURED = 'false';
    oauthspy.reset();
    ...
    request.get(
      'http://localhost::p/v2/pricing/resources' +
      '/:resource_id/prices/:price_id/config/:time', {
        p: server.address().port,
        resource_id: 'object-storage',
        price_id: 'common',
        time: 1420070400000
        ...
        expect(val.statusCode).to.equal(200);
        expect(val.body).to.deep.equal(
          require('../resources/object-storage-common'));
  ...
  it('Run a secured account management stub', (done) => {
    ...
    // Get Pricing config V2 for a resource
    brequest.get(
      'http://localhost::p/v2/pricing/resources' +
      '/:resource_id/prices/:price_id/config/:time', {
        p: server.address().port,
  ...
  it('validates sample price configurations', () => {
    ...
    map(['object-storage-common', 'linux-container-common'], (name) => {
      console.log('    validating', name, ' pricing');
      const conf = require('../resources/' + name);
      expect(schemas.priceConfigV2.validate(conf)).to.deep.equal(conf);
      console.log('        validated', name, ' pricing');
    })
  });
});
