...
// Load and return a resource price config
const configV2 = (rid, pid, time) => {
  try {
    return require('./resources/' + rid + '-' + pid);
  }
  catch(e) {
    return undefined;
  }
...
routes.get(
  ...
  function *(req) {
    ...
    // This is a stub here so we just return our sample resource price
    // configs
    const conf = configV2(req.params.resource_id, req.params.price_id,
    ...
    // Validate and return the resource price config
    schemas.priceConfigV2.validate(conf);
    return {
      status: 200,
      body: require('./resources/' + req.params.resource_id + '-' +
        req.params.price_id)
...
const accounts = () => {
  ...
  // bearer access token
  if (secured())
    app.use(/^\/v[1-2]\/(accounts|orgs|pricing)|^\/batch$/,
