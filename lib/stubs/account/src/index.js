...
// Return the rating configuration given the organization_id,
// resource_id, plan_id, resource_type, and time.
routes.get(
  ...
  // Need further discussion on rating_config schema. For now we're just
  // returning an object holding a rating_plan_id. The implementation could
  // also returns the rating_plan_id along with the whole rating_plan_config
  return {
    status: 200,
    body: {
      rating_plan_id: 'basic-rating-plan'
    }
  };
});

// Return the pricing configuration given the organization_id,
// resource_id, plan_id, resource_type, and time.
routes.get(
  ...
  // Need further discussion on the prices schema that is going to be attached
  // to the usage doc. This is just the placeholder.
  const prices = [{
    metric: 'sample',
    price: 0
  }];
  ...
  return {
    ...
    body: {
      prices: prices,
      pricing_plan_id: 'basic-pricing-plan'
    }
  };
})
