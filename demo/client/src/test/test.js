...
describe('abacus-demo-client v2', () => {
  it('submits usage for a sample resource and retrieves an aggregated ' +
    'usage report V2', (done) => {
    // Configure the test timeout
    const giveup = now.getTime() + 80000;
    ...
    const report = {
      ...
      resources: [{
        ...
        metering_plans: [{
          ...
          aggregated_usage: [{
            metric: 'storage',
            windows: buildWindow(1, 1, 1, undefined)
          }, {
            metric: 'thousand_light_api_calls',
            windows: buildWindow(0.09, 3, 3, undefined)
          }, {
            metric: 'heavy_api_calls',
            windows: buildWindow(45, 300, 300, undefined)
          }],
          rating_plans: [{
            ...
            aggregated_usage: [{
              metric: 'storage',
              windows: buildWindow(1, undefined, 1, undefined)
            }, {
              metric: 'thousand_light_api_calls',
              windows: buildWindow(0.09, undefined, 3, undefined)
            }, {
              metric: 'heavy_api_calls',
              windows: buildWindow(45, undefined, 300, undefined)
            }],
            prices: [{
              price_id: 'common',
              windows: buildWindow(46.09),
              aggregated_usage: [{
                metric: 'storage',
                windows: buildWindow(1, undefined, 1, 1)
              }, {
                metric: 'thousand_light_api_calls',
                windows: buildWindow(0.09, undefined, 3, 0.09)
              }, {
                metric: 'heavy_api_calls',
                windows: buildWindow(45, undefined, 300, 45)
              }]
            }]
          }]
        }]
    ...
    const post = (u, done) => {
      ...
      request.post(collector + '/v2/metering/collected/usage', {
        body: u.usage
      }, (err, val) => {
        expect(err).to.equal(undefined);
    ...
    // Get a usage report for the test organization
    const get = (done) => {
      request.get(reporting + '/v2/metering/organizations' +
        '/:organization_id/aggregated/usage', {
          organization_id: 'us-south:a3d7fe4d-3cb1-4cc3-a831-ffe98e20cf27'
        }, (err, val) => {
          expect(err).to.equal(undefined);
