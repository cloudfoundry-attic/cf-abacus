...
const test = (secured) => {
  ...
  const checkAllTimeWindows = (usage) => {
    ...
    const currentWindowEntry = windowUsage[0];
    expect(currentWindowEntry.quantity.consuming).to.equal(expectedConsuming);
    expect(currentWindowEntry.charge).to.be.above(0);
  };
