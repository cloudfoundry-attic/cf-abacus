...
// Setup the debug log
const debug =
  require('abacus-debug')('abacus-cf-renewer-services-itest');
const responseDebug =
  require('abacus-debug')('abacus-cf-renewer-services-itest-response');
const resultDebug =
  require('abacus-debug')('abacus-cf-renewer-services-itest-result');
const oAuthDebug =
  require('abacus-debug')('abacus-cf-renewer-services-itest-oauth');
...
const test = (secured) => {
  ...
  beforeEach((done) => {
    ...
    routes.get('/v2/service_usage_events', (request, response) => {
      ...
      responseDebug('Returning events %j', serviceUsageEvents);
      response.status(200).send({
  ...
  context('start service in current month', () => {
    beforeEach(() => {
  ...
  context('with service started 3 months ago outside slack window', () => {
    beforeEach(() => {
...
describe.skip('abacus-cf-renewer irrelevant-services-usage-test with oAuth',
  () => test(true));
