...
// Setup the debug log
const debug = require('abacus-debug')('abacus-cf-acceptance-test');
const responseDebug = require('abacus-debug')('abacus-cf-acceptance-test-response');
const resultDebug = require('abacus-debug')('abacus-cf-acceptance-test-result');
...
commander
  .option('-r, --reporting-app <reporting-app>', 'abacus reporting app name', process.env.REPORTING_APP)
  .option('-d, --cf-domain <cf-domain>', 'cf domain', process.env.CF_DOMAIN)
  .option('-o, --organization-guid <guid>', 'abacus organization guid', process.env.ORG_GUID)
...
describe('abacus-acceptance test', () => {
  ...
  const checkAllTimeWindows = (usage, level) => {
    for (const windowType in timeWindows) {
      ...
      if (level !== 'resource') expect(windowUsage[0].quantity.consuming).to.equal(expectedConsuming);
  ...
  const checkReport = (cb) => {
    request.get(
      'https://:reportingApp.:cfDomain/v1/metering/organizations' + '/:organization_id/aggregated/usage',
      {
      ...
      (error, response) => {
        try {
          ...
          expect(resources[0]).to.contain.all.keys('plans', 'aggregated_usage');
          ...
          resultDebug('All usage report checks are successful for: %s', JSON.stringify(response.body, null, 2));
  ...
  const waitForStartAndPoll = (done) => {
    ...
    request.waitFor(
      'https://:reportingApp.:cfDomain/batch',
      {
        reportingApp: commander.reportingApp,
        cfDomain: commander.cfDomain
      },
  ...
  context('with stream of CF events', () => {
    ...
    it('get expected report back', function(done) {
      if (!commander.organizationGuid) done(new Error('Organization GUID missing'));
