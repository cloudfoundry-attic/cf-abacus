...
/*
describe('abacus-usage-meter-itest V2', () => {
  before(() => {
    const start = (module) => {
      const c = cp.spawn('npm', ['run', 'start'],
        { cwd: moduleDir(module), env: clone(process.env) });
  ...
  it('meter normalized usage submissions V2', function(done) {
    // Configure the test timeout based on the number of usage docs, with
    // a minimum of 60 secs
    const timeout = Math.max(60000,
      100 * orgs * resourceInstances * usage);
    ...
    const app = express();
    const routes = router();
    routes.post('/v2/metering/metered/usage', accumulate);
    app.use(routes);
    app.use(router.batch(routes));
    app.listen(9200);
    ...
    const normalizedTemplate = (o, ri, u) => ({
      ...
      account_id: '1234',
      resource_type: '1.0',
      metering_plan_id: pid(ri, u),
      rating_plan_id: pid(ri, u),
      plan_id: pid(ri, u),
      consumer_id: cid(o, ri),
      measured_usage: [
        { measure: 'storage', quantity: 1073741824 },
    ...
    const post = throttle((o, ri, u, cb) => {
      ...
      brequest.post('http://localhost::p/v2/metering/normalized/usage',
        { p: 9100, body: normalizedTemplate(o, ri, u) }, (err, val) => {
          expect(err).to.equal(undefined);
          expect(val.statusCode).to.equal(201);
          expect(val.headers.location).to.not.equal(undefined);
