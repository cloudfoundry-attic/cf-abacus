...
describe('abacus-usage-meter-itest', () => {
  before(() => {
    ...
    if (!process.env.DB)
      lifecycleManager.startModules([lifecycleManager.modules.pouchserver, lifecycleManager.modules.meter]);
    else
      // Delete test dbs on the configured db server
      dbclient.drop(process.env.DB, /^abacus-meter-/, () => {
        lifecycleManager.startModules([lifecycleManager.modules.meter]);
      });
  });
  ...
  it('meter normalized usage submissions', function(done) {
    ...
    */
    ...
    const post = throttle((o, ri, u, cb) => {
      debug('Submit normalized usage for org%d instance%d usage%d', o + 1, ri + 1, u + 1);

      brequest.post(
        'http://localhost::p/v1/metering/normalized/usage',
        { p: 9100, body: normalizedTemplate(o, ri, u) },
        (err, val) => {
          expect(err).to.equal(undefined);
          expect(val.statusCode).to.equal(201);
          expect(val.headers.location).to.not.equal(undefined);

          debug('Normalized usage for org%d instance%d' + 'usage%d, verifying it...', o + 1, ri + 1, u + 1);

          brequest.get(val.headers.location, undefined, (err, val) => {
            debug('Verify usage for org%d instance%d usage%d', o + 1, ri + 1, u + 1);

            expect(err).to.equal(undefined);
            expect(val.statusCode).to.equal(200);

            expect(omit(val.body, 'id', 'processed', 'processed_id')).to.deep.equal(normalizedTemplate(o, ri, u));

            debug('Verified usage for org%d instance%d usage%d', o + 1, ri + 1, u + 1);
    ...
    const verifyAccumulator = (done) => {
      try {
        debug('Verifying accumulator calls %d to equal to %d', accumulate.callCount, orgs * resourceInstances * usage);
    ...
    request.waitFor('http://localhost::p/batch', { p: 9100 }, startTimeout, (err, value) => {
      // Failed to ping usage meter before timing out
      if (err) throw err;

      // Submit normalized usage and verify
      submit(() => {
        const i = setInterval(() => verifyAccumulator(() => done(clearInterval(i))), 250);
      });
    });
  });
});
