...
describe('abacus-perf-test', () => {
  it('measures performance of concurrent usage submissions', function(done) {
    ...
    const report = (o, nri, n) => ({
      ...
      resources: [{
        ...
        metering_plans: [{
          ...
          aggregated_usage: [{
            metric: 'storage',
            windows: metwindow(nri, n, nri, 1, storageCost)
          }, {
            metric: 'thousand_light_api_calls',
            windows: metwindow(nri, n, nri * n, 1, lightCost)
          },
            {
              metric: 'heavy_api_calls',
              windows: metwindow(nri, n, nri * n, 100, heavyCost)
            }],
          rating_plans: [{
            ...
            prices: [{
              ...
              aggregated_usage: [{
                metric: 'storage',
                windows: priwindow(nri, n, nri, 1, storageCost)
              }, {
                metric: 'thousand_light_api_calls',
                windows: priwindow(nri, n, nri * n, 1, lightCost)
              }, {
                metric: 'heavy_api_calls',
                windows: priwindow(nri, n, nri * n, 100, heavyCost)
              }]
            }]
          }]
        }]
    ...
    const post = throttle((o, ri, i, cb) => {
      debug('Submitting org%d instance%d usage%d',
        o + 1, ri + 1, i + 1);
      brequest.post('http://localhost:9080/v2/metering/collected/usage',
        extend({}, opt, { body: usageTemplate(o, ri, i) }), (err, val) => {
          expect(err).to.equal(undefined);
          expect(val.statusCode).to.equal(201);
          debug('Completed submission org%d instance%d usage%d',
    ...
    // Get a usage report for the test organization
    const get = (o, done) => {
      brequest.get('http://localhost:9088' + '/v2/metering/organizations' +
        '/:organization_id/aggregated/usage', extend({}, opt, {
          organization_id: orgid(o)
        }), (err, val) => {
          expect(err).to.equal(undefined);
