...
UAA exposes an endpoint called check_token, but it's use increases the traffic to the server. The barer token generated by UAA is a self validating JWT token. UAA supports both symmetric & asymmetric cryptographic algorithms for token signature and verification.
