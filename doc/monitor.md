...
* Dashboard cannot run with HTTPs without several minor changes
...
Luckilly the `/hystrix.stream` endpoints of the Abacus applications are still usable by following the security recomendations in the [security document](https://github.com/cloudfoundry-incubator/cf-abacus/blob/master/doc/security.md#cloud-foundry). The system integrator can use modified (secured) Hystrix Dashboard or the metrics can be sent to Graphite using the idea in this [DZone article](https://dzone.com/articles/storing-months-historical).
